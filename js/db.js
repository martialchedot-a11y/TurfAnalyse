...
function openDB() {
  ...
  dbPromise = new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = () => {
      ...
      // Ajouté en DB_VERSION 5 : snapshot des fichiers "Statistiques Ecarts
      // Jockeys/Entraineurs | Turf BZH" (cf.
      // js/engine/ecartsJockeyEntraineurParser.js) - 2 entrées au maximum
      // (id='jockeys', id='entraineurs'), chacune remplacée en entier a
      // chaque reimport (photo de l'instant present, pas un historique -
      // voir le commentaire d'en-tete du parser).
      if (!db.objectStoreNames.contains('ecartsJockeyEntraineur')) {
        db.createObjectStore('ecartsJockeyEntraineur', { keyPath: 'id' });
      }
    };
...
// -------------------------------------------------------------------
// Ecarts jockeys/entraineurs (snapshot, cf.
// js/engine/ecartsJockeyEntraineurParser.js). `type` = 'jockeys' ou
// 'entraineurs'. `data` = objet simple { nomNormalise: stats } (pas une Map,
// pour rester structured-clone/JSON-safe côté export/import).
// -------------------------------------------------------------------
export async function saveEcartsJockeyEntraineur(type, data) {
  const t = await tx(['ecartsJockeyEntraineur'], 'readwrite');
  t.objectStore('ecartsJockeyEntraineur').put({ id: type, type, data, importedAt: new Date().toISOString() });
  return new Promise((resolve, reject) => {
    t.oncomplete = () => resolve();
    t.onerror = () => reject(t.error);
  });
...
export async function getEcartsJockeyEntraineur(type) {
  const t = await tx(['ecartsJockeyEntraineur'], 'readonly');
  return promisify(t.objectStore('ecartsJockeyEntraineur').get(type));
}

export async function getAllEcartsJockeyEntraineur() {
  const t = await tx(['ecartsJockeyEntraineur'], 'readonly');
  return promisify(t.objectStore('ecartsJockeyEntraineur').getAll());
}

export async function resetEcartsJockeyEntraineur() {
  const t = await tx(['ecartsJockeyEntraineur'], 'readwrite');
  t.objectStore('ecartsJockeyEntraineur').clear();
  return new Promise((resolve, reject) => {
    t.oncomplete = () => resolve();
    t.onerror = () => reject(t.error);
  });
...
export async function exportAll() {
  const [performances, meetings, races, horses, journal, predictionsExternes, bilansJournaliersSimpleGagnant, bilansJournaliersCroisement, ecartsJockeyEntraineur] = await Promise.all([
    getAllPerformances(),
    getAllMeetings(),
    ...
    getAllEcartsJockeyEntraineur()
  ]);
  return { version: DB_VERSION, exportedAt: new Date().toISOString(), performances, meetings, races, horses, journal, predictionsExternes, bilansJournaliersSimpleGagnant, bilansJournaliersCroisement, ecartsJockeyEntraineur };
}

export async function importAll(data) {
  const t = await tx(['performances', 'meetings', 'races', 'horses', 'journal', 'predictionsExternes', 'bilansJournaliersSimpleGagnant', 'bilansJournaliersCroisement', 'ecartsJockeyEntraineur'], 'readwrite');
  ...
  for (const e of data.ecartsJockeyEntraineur || []) t.objectStore('ecartsJockeyEntraineur').put(e);
  return new Promise((resolve, reject) => {
    t.oncomplete = () => resolve();
    t.onerror = () => reject(t.error);
  });
