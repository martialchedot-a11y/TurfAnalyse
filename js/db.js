...
// Sept magasins : performances (historique, jamais vidé), meetings, races,
// horses (chevaux engagés par course), journal (prédictions journalisées),
// bilansJournaliersSimpleGagnant (historique manuel des bilans quotidiens du
// Jeu Simple Gagnant, cf. page "Bilan Global Simple Gagnant" dans js/app.js),
// bilansJournaliersCroisement (idem pour le Jeu Croisement Couplé/Trio, cf.
// page "Bilan Global Croisement" dans js/app.js), topIAPlace (snapshot
// quotidien "Top 5 IA Placé" Turf BZH, cf. js/engine/topIAPlaceParser.js).
//
// *** sept. 2026 (DB_VERSION 6, a la demande de l'utilisateur) *** : les
// magasins predictionsExternes et ecartsJockeyEntraineur (fonctionnalités
// "Prédictions externes" et "Ecarts Jockeys/Entraineurs", toutes deux
// retirées de l'appli) sont supprimés et remplacés par topIAPlace.
// =============================================================================
...
function openDB() {
  ...
  dbPromise = new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = () => {
      ...
      // DB_VERSION 6 : suppression des magasins predictionsExternes et
      // ecartsJockeyEntraineur (fonctionnalités retirées), remplacés par
      // topIAPlace - snapshot quotidien "Top 5 IA Placé" (cf.
      // js/engine/topIAPlaceParser.js), une seule entrée (id='jour'),
      // remplacée en entier a chaque reimport (photo du jour, pas un
      // historique).
      if (db.objectStoreNames.contains('predictionsExternes')) {
        db.deleteObjectStore('predictionsExternes');
      }
      if (db.objectStoreNames.contains('ecartsJockeyEntraineur')) {
        db.deleteObjectStore('ecartsJockeyEntraineur');
      }
      if (!db.objectStoreNames.contains('topIAPlace')) {
        db.createObjectStore('topIAPlace', { keyPath: 'id' });
      }
      // DB_VERSION 7 (sept. 2026, a la demande de l'utilisateur) :
      // snapshot quotidien "Supplementation" (fichier Excel
      // "Supplementation_PMU_AAAAMMJJ.xlsx", cf.
      // js/engine/supplementationParser.js) - meme principe que
      // topIAPlace (une seule entree id='jour', remplacee en entier a
      // chaque reimport).
      if (!db.objectStoreNames.contains('supplementation')) {
        db.createObjectStore('supplementation', { keyPath: 'id' });
      }
    };
...
//
// *** aout 2026 : normalisation du LIEU dans la cle (a la demande de
// l'utilisateur) ***
// Constat (fiche cheval HENRIQUE) : le fichier musiques designe le meme
// hippodrome sous 2 formes differentes selon la source/date d'export
// ("PARIS VINCENNES" / "VINCENNES", "PARISLONGCHAMP" / "LONGCHAMP" - cf.
// `normaliserHippodrome`, `scoringEngine.js`). Avant ce correctif, la cle
// utilisait `p.lieu` brut (juste majuscule/trim) : la MEME course
// re-exportee un autre jour sous l'autre forme de nom generait une cle
// DIFFERENTE, donc une DEUXIEME entree jamais reconnue comme doublon par
// `dedupePerformances()` (qui compare par cle) - meme apres nettoyage. Cas
// verifie : la victoire d'HENRIQUE du 02/05/2026 et sa place du 13/05/2026
// a Vincennes existaient CHACUNE en double (une fois sous "VINCENNES", une
// fois sous "PARIS VINCENNES"), ce qui gonflait a tort son nombre de
// victoires/courses recentes ET poussait ses 2 performances les plus
// anciennes hors de la fenetre des 10 dernieres utilisee par
// `scoreAptitude()` (score gonfle de 71 a 87 sur la course VINCENNES du
// 24/08/2026 etudiee). Desormais, `p.lieu` est normalise via
// `normaliserHippodrome()` avant d'entrer dans la cle : les 2 formes
// produisent la MEME cle, donc la MEME entree (upsert), et
// `dedupePerformances()` fusionne correctement les doublons deja
// accumules des qu'il est relance.
// *** Fix doublons historique (sept 2026, a la demande de l'utilisateur) ***
// Constat (fiche ENGHIEN vs BLAIN BOUVRON LE GAVRE / BLAIN, meme date
// 06/04/2026) : la cle precedente incluait hippodrome/distance/place/
// discipline en plus de cheval+date. Meme apres normalisation du lieu
// (`normaliserHippodrome`), certaines variantes de nom d'hippodrome ne sont
// pas couvertes par la table de correspondance (ex. "BLAIN BOUVRON LE
// GAVRE" vs "BLAIN" - visiblement la meme reunion sous 2 libelles distincts
// selon la source/date d'export) : la cle produite differait donc encore, et
// la 2e entree n'etait jamais reconnue comme doublon. Un cheval ne pouvant
// courir qu'une seule fois par jour, la cle est desormais basee uniquement
// sur cheval+date : toute 2e ligne pour la meme date (quel que soit le
// libelle d'hippodrome, la distance ou la discipline rapportes) est traitee
// comme le MEME evenement et remplace la precedente au lieu de s'accumuler.
export function clePerformance(p) {
  const norm = (v) => String(v ?? '').trim().toUpperCase();
  return ['perf', norm(p.nomCheval), norm(p.datePerf)].join('|');
}
...
// -------------------------------------------------------------------
// Top IA Placé (snapshot quotidien, cf. js/engine/topIAPlaceParser.js) -
// remplace Predictions externes et Ecarts Jockeys/Entraineurs (sept. 2026).
// Une seule entrée (id='jour'), remplacée en entier a chaque reimport (photo
// du jour, pas un historique cumulatif).
// -------------------------------------------------------------------
export async function saveTopIAPlace(dateVal, rows) {
  const t = await tx(['topIAPlace'], 'readwrite');
  t.objectStore('topIAPlace').put({ id: 'jour', dateVal, rows, importedAt: new Date().toISOString() });
  return new Promise((resolve, reject) => {
    t.oncomplete = () => resolve();
    t.onerror = () => reject(t.error);
  });
...
export async function getTopIAPlace() {
  const t = await tx(['topIAPlace'], 'readonly');
  return promisify(t.objectStore('topIAPlace').get('jour'));
}

export async function resetTopIAPlace() {
  const t = await tx(['topIAPlace'], 'readwrite');
  t.objectStore('topIAPlace').clear();
  return new Promise((resolve, reject) => {
    t.oncomplete = () => resolve();
    t.onerror = () => reject(t.error);
  });
...
// -------------------------------------------------------------------
// Supplementation (snapshot quotidien, cf. js/engine/supplementationParser.js)
// - meme principe que Top IA Place ci-dessus.
// -------------------------------------------------------------------
export async function saveSupplementation(dateVal, rows) {
  const t = await tx(['supplementation'], 'readwrite');
  t.objectStore('supplementation').put({ id: 'jour', dateVal, rows, importedAt: new Date().toISOString() });
  return new Promise((resolve, reject) => {
    t.oncomplete = () => resolve();
    t.onerror = () => reject(t.error);
  });
...
export async function getSupplementation() {
  const t = await tx(['supplementation'], 'readonly');
  return promisify(t.objectStore('supplementation').get('jour'));
}

export async function resetSupplementation() {
  const t = await tx(['supplementation'], 'readwrite');
  t.objectStore('supplementation').clear();
  return new Promise((resolve, reject) => {
    t.oncomplete = () => resolve();
    t.onerror = () => reject(t.error);
  });
...
export async function exportAll() {
  const [performances, meetings, races, horses, journal, bilansJournaliersSimpleGagnant, bilansJournaliersCroisement, topIAPlace, supplementation] = await Promise.all([
    getAllPerformances(),
    getAllMeetings(),
    ...
    (async () => { const t = await tx(['topIAPlace'], 'readonly'); return promisify(t.objectStore('topIAPlace').getAll()); })(),
    (async () => { const t = await tx(['supplementation'], 'readonly'); return promisify(t.objectStore('supplementation').getAll()); })()
  ]);
  return { version: DB_VERSION, exportedAt: new Date().toISOString(), performances, meetings, races, horses, journal, bilansJournaliersSimpleGagnant, bilansJournaliersCroisement, topIAPlace, supplementation };
}

export async function importAll(data) {
  const t = await tx(['performances', 'meetings', 'races', 'horses', 'journal', 'bilansJournaliersSimpleGagnant', 'bilansJournaliersCroisement', 'topIAPlace', 'supplementation'], 'readwrite');
  ...
  for (const e of data.topIAPlace || []) t.objectStore('topIAPlace').put(e);
  for (const s of data.supplementation || []) t.objectStore('supplementation').put(s);
  return new Promise((resolve, reject) => {
    t.oncomplete = () => resolve();
    t.onerror = () => reject(t.error);
  });
