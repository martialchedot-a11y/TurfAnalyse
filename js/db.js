...
import { normaliserHippodrome } from './engine/scoringEngine.js';
...
export function clePerformance(p) {
  ...
  return ['perf', norm(p.nomCheval), norm(p.datePerf), normaliserHippodrome(p.lieu), norm(p.distance), norm(p.place), norm(p.discipline)].join('|');
}
