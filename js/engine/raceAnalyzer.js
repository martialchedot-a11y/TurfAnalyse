...
export function analyser(horses, context, useCote8hPourValue) {
  ...
  const results = horses.map((h) => {
    ...
    // commentaire). Alimente probVictoireIntrinseque plus bas (PASSE 3),
  ...
  // *** Nouveau *** : meme moteur de probabilites (Plackett-Luce), reutilise
  // tel quel, mais applique a scoreGlobalSansCote au lieu de scoreGlobal ->
  // une probabilite de victoire "intrinseque" (sans l'avis du marche),
  // purement informative. N'affecte ni le tri ci-dessous ni aucun autre
  // calcul (Value, classement...).
  const scoresChampSansCote = results.map((r) => r.scoreGlobalSansCote);
  const probasSansCote = ProbabilityEngine.probabilites(scoresChampSansCote);
  results.forEach((r, i) => {
    r.probVictoireIntrinseque = probasSansCote[i].probVictoire;
  });
