...
export function analyser(horses, context, useCote8hPourValue) {
  ...
  const results = horses.map((h) => {
    ...
    const conditions = ScoringEngine.scoreConditions(
      h.entry.reussiteJockey, h.entry.reussiteEntraineur, h.entry.ferrage, disciplinePlat, h.historique
    );
  ...
  // *** Supprime *** : "Force relative (sans cote)" (scoreGlobalSansCote /
  // meilleur du champ x100) a ete retire suite a un backtest reel (4067
  // courses, 3 variantes testees : scoreGlobalSansCote, scoreForme seul,
  // forme+aptitude+similaire) qui a montre qu'un ecart plus grand entre le
  // 1er et le 2e du champ par historique ne predit PAS un taux de victoire
  // plus eleve (taux plat/bruite selon la taille de l'ecart), et que
  // classer par historique seul est meme moins precis que le modele actuel
  // (scoreGlobal avec cote). Voir HEBERGEMENT.md pour le detail. La feature
  // (et scoreGlobalSansCote dans scoringEngine.js) a ete supprimee.

  // TRI (mise à jour sept. 2026, à la demande de l'utilisateur, suite au fix
  // historiquePour qui a débiaisé les backtests) : le seuil de groupe passe
  // de Value<0 à Value<=-30 (voir version précédente de ce commentaire pour
  // le détail du constat - Value<=-30 capte 59,1% de tous les vainqueurs de
  // l'archive avec 21,2% de réussite, contre 4,4% pour Value>-30).
  //
  // *** Second changement (même mise à jour) : logique d'ÉLIMINATION au
  // sein du groupe Value<=-30, plutôt qu'un simple score. ***
  // Constat (script `backtestValueMoins30Elimination.mjs`, 25386 courses
  // ayant au moins 1 cheval Value<=-30, SANS restriction top2/cote) :
  //   - 13,0% des courses n'ont qu'UN SEUL cheval Value<=-30 : ce candidat
  //     unique gagne 44,3% du temps (rendement 90,6%) - la sélection est
  //     déjà faite par construction.
  //   - 87,0% des courses ont PLUSIEURS candidats Value<=-30 en concurrence
  //     (jusqu'à 9 dans un même champ) ; dans ce cas, UN des candidats
  //     Value<=-30 gagne 61,6% du temps - le bon signal existe mais il faut
  //     départager. Plusieurs critères de départage testés (scoreAptitude,
  //     scoreForme, scoreGlobal, ProbVictoire, écart, joursAbsence,
  //     scoreRubriques, scoreCroisement, Value la plus extrême...) : le
  //     critère "cote la plus basse parmi les candidats Value<=-30" domine
  //     nettement (30,1% de réussite sur ce sous-ensemble, contre 26,3%
  //     pour ProbVictoire+ProbTop3 utilisé jusqu'ici, et 28,2% pour Value la
  //     plus extrême). Robuste : stable sur les deux moitiés chronologiques
  //     (29,5%/30,7%), stable après retrait des 50 plus gros gains (29,9%).
  //   - Stratégie complète (candidat unique si n=1, sinon cote la plus
  //     basse) sur les 25386 courses : 31,8% de réussite, 85,3% de
  //     rendement - contre 28,6%/86,1% avec l'ancien tri secondaire
  //     (ProbVictoire+ProbTop3). Robuste (31,5%/32,0% sur les deux moitiés,
  //     31,6% après retrait des 50 plus gros gains).
  // Le tri secondaire au sein du groupe Value<=-30 passe donc de
  // "ProbVictoire+ProbTop3 décroissant" à "cote la plus basse" (cote
  // croissante). Le groupe Value>-30 ("le reste", très rarement en tête de
  // classement - moins de 0,3% des courses n'ont AUCUN cheval Value<=-30)
  // garde le tri ProbVictoire+ProbTop3 décroissant, inchangé.
  results.sort((a, b) => {
    const aNeg = a.value <= -30;
    const bNeg = b.value <= -30;
    if (aNeg !== bNeg) return aNeg ? -1 : 1;
    if (aNeg) {
      // Groupe Value<=-30 : cote la plus basse (favori du marché) en tête.
      const coteA = a.cotePourAffichage > 0 ? a.cotePourAffichage : Infinity;
      const coteB = b.cotePourAffichage > 0 ? b.cotePourAffichage : Infinity;
      return coteA - coteB;
    }
    // Groupe Value>-30 ("le reste") : inchangé, ProbVictoire+ProbTop3 décroissant.
