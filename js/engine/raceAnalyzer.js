...
export function analyser(horses, context, useCote8hPourValue) {
  ...
  const results = horses.map((h) => {
    ...
    // *** Nouveau *** (a la demande de l'utilisateur : "un pourcentage de
    // chance de gagner... sans tenir compte des cotes, purement
    // intrinseque") : meme principe que global ci-dessus, mais a partir de
    // scoreGlobalSansCote (memes composantes SAUF la cote, poids
    // renormalises - voir scoringEngine.js), avec le meme bonus rubriques
    // (deja intrinseque, base sur des criteres techniques - cf. son propre
    // commentaire). Alimente forceRelativeSansCote plus bas (PASSE 3),
    // sans jamais toucher a `global`/`scoreGlobal` (Score Global/Value/
    // classement existants restent strictement inchanges).
    const globalSansCote = ScoringEngine.scoreGlobalSansCote(forme.score, aptitude, conditions, similaire) + bonusRubriques;
    ...
    return {
      ...
      scoreGlobal: global,
      scoreGlobalSansCote: globalSansCote,
      cotePourAffichage,
      coteProbable: 0,
      value: 0,
      probVictoire: 0,
  ...
  // *** Corrige *** (a la demande de l'utilisateur, apres retour sur la
  // premiere version : "ce que je souhaite c'est un % intra-chevaux de la
  // course, comparer les forces en presence") : une VRAIE probabilite de
  // victoire (Plackett-Luce) doit obligatoirement sommer a ~100% sur tout
  // le champ - sans la cote (qui concentre justement la confiance sur un
  // favori), cette somme se repartit plus uniformement sur un grand champ,
  // ce qui donnait des pourcentages jugés "tres faibles" et peu lisibles.
  // Ce n'est PAS ce que l'utilisateur voulait : il veut comparer les
  // chevaux ENTRE EUX (le meilleur du champ = 100%, les autres leur niveau
  // relatif) - une FORCE RELATIVE, pas une probabilite (ne somme donc plus
  // a 100%, et peut afficher plusieurs chevaux avec un pourcentage eleve
  // s'ils sont proches du meilleur). Calcul simple, sans moteur de
  // probabilites : scoreGlobalSansCote de chaque cheval / le plus eleve du
  // champ. N'affecte ni le tri ci-dessous ni aucun autre calcul (Value,
  // classement, probVictoire habituel...).
  const meilleurScoreSansCote = Math.max(...results.map((r) => r.scoreGlobalSansCote));
  results.forEach((r) => {
    r.forceRelativeSansCote = meilleurScoreSansCote > 0
      ? arrondiVBA((r.scoreGlobalSansCote / meilleurScoreSansCote) * 100, 1)
      : 0;
  });
