...
export function analyser(horses, context, useCote8hPourValue) {
  ...
  // de Value<0 à Value<=-30. Constat empirique (archive complète débiaisée,
  // 307836 chevaux, aucune restriction de classement/cote) : Value<=-30
  // capte 59,1% de TOUS les vainqueurs de l'archive avec 21,2% de réussite,
  // contre seulement 4,4% pour Value>-30 - un signal bien plus net qu'un
  // simple Value<0. Deux variantes de tri secondaire au sein de chaque
  // groupe ont été testées par backtest complet (raceAnalyzer.analyser sur
  // 307836 chevaux, seuil de groupe -30 dans les deux cas) avant de choisir :
  //   - "proximité à -30" (Value la plus proche de -30 en tête) : REJETÉ -
  //     19,8% de réussite au rang 1 (moins bon que l'ancien système à
  //     25,5%), car ce critère perd toute l'information du modèle
  //     Plackett-Luce (ProbVictoire+ProbTop3, qui intègre déjà la cote).
  //   - ProbVictoire+ProbTop3 décroissant (INCHANGÉ, comme avant) au sein de
  //     chaque groupe : RETENU - 28,6% de réussite au rang 1 (contre 25,5%
  //     pour l'ancien seuil Value<0), 86,1% de rendement (contre 85,6%),
  //     48,3% des vainqueurs captés au top2 (contre 44,8%). Robuste : stable
  //     sur les deux moitiés chronologiques de l'archive (29,0%/28,3% de
  //     réussite), stable après retrait des 50 plus gros gains (28,5%), et
  //     cote moyenne du rang 1 plus basse (3,72 contre 4,48) - donc plus de
  //     réussite avec des picks pourtant moins "longshot", pas un artefact
  //     de quelques gros gains.
  // Seul le SEUIL de groupe change (0 -> -30) ; le tri secondaire par
  // ProbVictoire+ProbTop3 décroissant est conservé à l'identique.
  results.sort((a, b) => {
