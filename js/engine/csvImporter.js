...
/**
 * Parse le CSV "Réunion complète" (76 colonnes, ou variante "journée" a 77
 * colonnes - voir ci-dessous) et regroupe les lignes par réunion + n° de
 * course, comme `IdentifierCoursesDepuisCSV` + `ChargerDonneesChevauxCourse`
 * en VBA.
 *
 * *** Compatibilite format "journee" (export multi-reunions) *** : certains
 * exports quotidiens ("journee complete", plusieurs reunions dans un seul
 * fichier) inserent une colonne supplementaire "Pedigree Faible" juste avant
 * "Cote Calc" (77 colonnes au lieu de 76), ce qui decale tous les champs a
 * partir de "Cote Calc" (cotePredictive) inclus : numero de reunion, de
 * course, lieu, heure, discipline, allocation, distance, nb partants,
 * arrivee. Le decalage est detecte automatiquement a partir du nombre de
 * colonnes de la premiere ligne de donnees (`decalage` ci-dessous, 0 pour le
 * format standard a 76 colonnes, 1 pour la variante "journee" a 77
 * colonnes) et applique a tous les champs situes a partir de la colonne 57.
 * Un tel fichier "journee" contenant plusieurs reunions, le regroupement se
 * fait desormais sur la paire (numero de reunion, numero de course) et non
 * plus sur le seul numero de course, afin de ne pas fusionner a tort deux
 * courses de meme numero mais de reunions differentes.
 * @param {string} csv
...
export function parseReunionComplete(csv) {
  ...
  // Decalage des colonnes >= 57 : 0 pour le format standard (76 colonnes),
  // 1 pour la variante "journee" (77 colonnes, colonne "Pedigree Faible"
  // supplementaire juste avant "Cote Calc"). Calcule une seule fois a
  // partir de la premiere ligne de donnees (toutes les lignes d'un meme
  // fichier partagent le meme format).
  const decalage = Math.max(0, rows[0].length - 76);
  const dec = (colonne) => colonne + decalage; // pour les colonnes >= 57 uniquement

  const parReunionCourse = new Map();
  const ordre = [];
  for (const row of rows) {
    const numReunion = intField(row, dec(58));
    const numCourse = intField(row, dec(59));
  ...
  for (const cle of ordre) {
    ...
    const disciplineBrute = field(first, dec(62));
    const context = {
      lieu: field(first, dec(60)),
      discipline: disciplineFromRaw(disciplineBrute),
      disciplineBrute,
      distanceJour: doubleField(first, dec(66)),
      allocation: doubleField(first, dec(65)),
      nbPartants: intField(first, dec(67)),
      numeroCourse: intField(first, dec(59)),
      numeroReunion: intField(first, dec(58)),
      heureDepart: field(first, dec(61))
    };
    ...
    const horses = rowsCourse.map((row) => ({
      ...
      cotePredictive: convertirCote(field(row, dec(57))),
      ...
      // Toutes ces colonnes sont < 57, donc jamais affectees par `decalage`.
    ...
    result.push({ context, horses, arriveeBrute: field(first, dec(70)) });
  }
  return result;
}
