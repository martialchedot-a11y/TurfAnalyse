...
const POSITIONS_REPLI = {
  ...
  sc: 46, p1: 15, p2: 16, ja: 13,
  reunion: 58, course: 59, lieuCourse: 60, heure: 61, discipline: 62,
  typeCourse: 64, allocation: 65, distance: 66, partants: 67, arrivee: 70
};
...
export function parseReunionComplete(csv) {
  ...
  // Noms bruts jockey/entraineur (distincts de reussiteJockey/reussiteEntraineur
  // ci-dessous, qui sont des SCORES numeriques de rubrique) - sept. 2026, a la
  // demande de l'utilisateur, pour matcher les fichiers "Statistiques Ecarts
  // Jockeys/Entraineurs" (turf.bzh) importes separement. Pas de repli position
  // fixe (colonnes non presentes dans POSITIONS_REPLI) : `undefined` si l'en-tete
  // est absent, `field()` renvoie alors '' proprement.
  const posDrivJock = col(['DrivJock'], undefined);
  const posEntraineurNom = col(['Entraineur'], undefined);
  ...
  // "JA" (Jours d'Absence) : nombre de jours ecoules depuis la derniere
  // course du cheval, deja calcule par turfbzh/PMU et fourni directement
  // dans le fichier _partants du jour (donc fiable, point-in-time, sans
  // dependre de l'historique _musiques dont on a decouvert - sept. 2026 -
  // qu'il peut contenir des performances FUTURES par rapport a la date de
  // la reunion sur une bonne partie de l'archive, cf. HEBERGEMENT.md).
  const posJA = col(['JA'], POSITIONS_REPLI.ja);
  ...
  const posTypeCourse = col(['TypeCourse'], POSITIONS_REPLI.typeCourse);
  ...
  for (const cle of ordre) {
    ...
    const context = {
      ...
      disciplineBrute,
      typeCourse: field(first, posTypeCourse).trim(),
    ...
    const horses = rowsCourse.map((row) => ({
      ...
      jockeyNom: posDrivJock != null ? field(row, posDrivJock).trim() : '',
      entraineurNom: posEntraineurNom != null ? field(row, posEntraineurNom).trim() : '',
      ...
      p1: field(row, posP1),
      p2: field(row, posP2),
      joursAbsence: field(row, posJA).trim() === '' ? null : intField(row, posJA)
    }));
 ...
 * la course qu'on est censé prédire).
 *
 * *** Elargissement (sept. 2026, à la demande de l'utilisateur) : exclusion
 * de TOUTE performance posterieure, pas seulement celle du meme jour ***
 * Constat (session sept. 2026, cf. HEBERGEMENT.md) : pour une bonne partie
 * de l'archive, le fichier musiques n'est PAS un instantane figé a la date
 * du dossier - il contient l'historique complet du cheval jusqu'a la date
 * ou l'archive a ete constituee (recemment), donc des performances
 * FUTURES par rapport a la date de la course etudiee (pas seulement le
 * meme jour). Le filtre par egalite stricte de date (`!==`) ne retirait que
 * la course du jour elle-meme, laissant passer toute course ULTERIEURE
 * presente dans le fichier. Le paramètre `dateCourseAExclure`
 * (format "AAAA-MM-JJ", optionnel) retire desormais de l'historique toute
 * performance dont la date est POSTERIEURE OU EGALE à celle de la course
 * étudiée (comparaison lexicographique de chaines "AAAA-MM-JJ", fiable car
 * format a largeur fixe). Sans effet en utilisation reelle/en direct
 * (aucune performance future n'existe jamais a ce moment-la) - corrige
 * uniquement la contamination des BACKTESTS sur l'archive.
 * @param {string} nomCheval
 * @param {Array<Object>} toutesPerfs
 * @param {string|null|undefined} [dateCourseAExclure] - date "AAAA-MM-JJ" de
 *   la course en cours d'analyse ; toute performance de CETTE date OU
 *   POSTERIEURE est exclue de l'historique retourné (fuite de données
 *   évitée, y compris les performances ulterieures presentes par erreur
 *   dans le fichier musiques d'archive).
 */
