...
// Nombre minimum de partants pour considerer une course jouable : identique
// au seuil utilise pour generer l'echantillon reel valide ci-dessus (moins
// de 6 partants n'a jamais ete teste en conditions reelles).
export const MIN_PARTANTS_CROISEMENT = 6;
 ...
 * @returns {{jouable:true, classement:Array, poolCouple:number[], poolTrio:number[]}
 *          |{jouable:false}}
 */
export function jeuCoupleTrioCroisement(chevaux) {
  const valides = (chevaux || []).filter((c) => c && c.entry && typeof c.entry.numero === 'number');
  if (valides.length < MIN_PARTANTS_CROISEMENT) return { jouable: false };
