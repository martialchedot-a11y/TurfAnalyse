...
// -------------------------------------------------------------------
// SCORE GLOBAL SANS COTE : a la demande de l'utilisateur ("un pourcentage
// de chance de gagner... sans tenir compte des cotes, purement
// intrinseque"), meme composantes que scoreGlobal ci-dessus SAUF la cote
// (scoreCote, qui reflete le jugement du marche - donc PAS intrinseque au
// cheval), avec les 4 poids restants renormalises pour sommer a 1 (au lieu
// de 0,90 une fois la cote retiree) :
//   forme 0,35/0,90 ; aptitude 0,25/0,90 ; conditions 0,15/0,90 ;
//   similaire 0,15/0,90.
// Volontairement SANS le bonus rubriques (ajoute a part dans raceAnalyzer.js,
// deja intrinseque - base sur des criteres techniques, pas la cote) pour
// rester strictement comparable a scoreGlobal une fois ce bonus reintegre
// de la meme facon par l'appelant. N'entre dans AUCUN calcul de Score
// Global/Value/classement existant : sert uniquement a alimenter une
// probabilite de victoire "intrinseque" separee (voir probabilityEngine.js,
// reutilise tel quel avec ce score en entree).
// -------------------------------------------------------------------
const POIDS_SANS_COTE = 1 - 0.1; // 0,90 : somme des 4 poids restants
 ...
 * @returns {number}
 */
export function scoreGlobalSansCote(forme, aptitude, conditions, similaire) {
  const brut = forme * 0.35 + aptitude * 0.25 + conditions * 0.15 + similaire * 0.15;
  return brut / POIDS_SANS_COTE;
}
