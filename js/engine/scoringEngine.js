...
// *** aout 2026 (a la demande de l'utilisateur) : normalisation des noms
// d'hippodromes doublonnes entre le fichier musiques (historique d'un
// cheval) et le nom de la reunion du jour ***
// Constat (fiche cheval HENRIQUE, VINCENNES 24/08/2026) : le sous-score
// "Lieu" de `scoreAptitude` compare `perf.lieu` a `lieuJour` par egalite de
// texte stricte. Or, pour un meme hippodrome, le fichier musiques utilise
// parfois une forme differente de celle de la reunion du jour ("PARIS
// VINCENNES" cote historique contre "VINCENNES" cote reunion du jour,
// "LONGCHAMP" contre "PARISLONGCHAMP") - verifie sur un echantillon large de
// l'archive (423 fichiers). Sans normalisation, une partie de l'historique
// pourtant bien a CET hippodrome etait ignoree a tort par le sous-score
// Lieu (ex. HENRIQUE : seulement 3 des 6 courses a Vincennes comptees, dont
// sa seule victoire manquee - scoreLieu et donc scoreAptitude sous-estimes).
const SYNONYMES_HIPPODROME = {
 ...
 * @returns {string} nom d'hippodrome normalise (majuscules, sans espaces
 *   superflus, formes doublonnes connues ramenees a une forme canonique
 *   commune - voir `SYNONYMES_HIPPODROME` ci-dessus).
 */
export function normaliserHippodrome(nom) {
  const u = (nom || '').trim().toUpperCase();
  return SYNONYMES_HIPPODROME[u] || u;
}
...
export function scoreAptitude(perfs, lieuJour, disciplineJour, distanceJour, disciplinePlat) {
  ...
  const lieuJourU = normaliserHippodrome(lieuJour);
  ...
  for (const perf of perfs) {
    ...
    if (normaliserHippodrome(perf.lieu) === lieuJourU) {
      coursesLieu += 1;
...
// Un cheval est considere "deferre" (aujourd'hui ou lors d'une course passee)
// si son code de ferrage contient un "D" (DF, DA, DP, D4, DAP/DPA, "PA DP"...).
// Meme convention cote parsePerformances (colonne "Deferre") que cote fichier
// du jour (colonne "Ferrure") - verifie sur les archives reelles.
function estFerrageDeferre(code) {
  return /D/i.test((code || '').trim());
}

function estDisciplineTrot(disciplineBrute) {
  const d = (disciplineBrute || '').toUpperCase();
  return d.includes('ATTELE') || d.includes('MONTE');
}

/**
 * Bonus/malus base sur la reussite HISTORIQUE PERSONNELLE du cheval quand il
 * a couru deferre (colonne "Deferre" de l'historique, trot uniquement).
 * Ajoute suite a un backtest reel (31 journees d'archives, 7145 courses
 * trot) : les chevaux deferres aujourd'hui avec >=50% de podiums dans leurs
 * courses passees en deferre finissent podium a 37,4% (contre 24,9% pour
 * ceux en-dessous de 50%, et 27,7% en moyenne tous chevaux deferres
 * confondus). Le simple fait d'etre deferre aujourd'hui (sans distinction
 * d'historique) etait deja capture par `scoreFerrage` ci-dessous - ceci
 * affine ce score avec l'historique propre au cheval. Seuil de 3 courses
 * deferre minimum pour eviter de statuer sur un echantillon trop petit.
 * Le backtest n'a pas montre de signal pour le "1er deferrage" (chevaux
 * deferres pour la 1ere fois vs deja habitues : 26,7% vs 28,5%, pas de
 * difference significative) - volontairement non implemente.
 *
 * *** Fenetre recente (sept 2026, a la demande de l'utilisateur) ***
 * Avant ce correctif, cette fonction parcourait TOUT `perfsHistorique` sans
 * limite - contrairement a `scoreForme` (6 courses les plus recentes) et
 * `scoreAptitude`/`scoreConditionsSimilaires` (10 courses les plus
 * recentes), qui se bornent volontairement a une fenetre recente fixe. Cas
 * releve (MINNESOTA, 31/08/2026) : l'historique cumule en base locale
 * comptait 11 courses connues alors que le fichier musiques du jour n'en
 * fournit que 10 (11e course plus ancienne, conservee d'un import
 * precedent - comportement voulu de l'historique cumulatif, pas un
 * doublon). Ce bonus etait la SEULE des 4 fonctions de score consommant
 * l'historique a rester sensible a cette 11e course (et plus generalement a
 * grandir sans limite au fil des mois d'accumulation), les 3 autres etant
 * deja bornees. Desormais bornee aux 10 courses les plus recentes, comme
 * `scoreAptitude`, pour un calcul coherent et stable dans le temps plutot
 * qu'un taux "toute carriere connue" qui derive avec la taille de la base.
 * @param {Array<Object>} perfsHistorique
 * @returns {number}
 */
function bonusReussiteHistoDeferre(perfsHistorique) {
  const perfsRecentes = (perfsHistorique || []).slice(0, 10);
  const perfsDeferreTrot = perfsRecentes.filter(
    (p) => estDisciplineTrot(p.discipline) && estFerrageDeferre(p.deferreOuIndiceValeur)
  );
  if (perfsDeferreTrot.length < 3) return 0;
  const nbPodiums = perfsDeferreTrot.filter((p) => p.place && p.place <= 3).length;
  const taux = nbPodiums / perfsDeferreTrot.length;
  return taux >= 0.5 ? 6 : -3;
}
 ...
 * @param {boolean} disciplinePlat
 * @param {Array<Object>} [perfsHistorique] - historique du cheval (pour le
 *   bonus de reussite personnelle en deferre, trot uniquement).
 * @returns {number}
 */
export function scoreConditions(reussiteJockey, reussiteEntraineur, ferrage, disciplinePlat, perfsHistorique = []) {
  ...
  const bonusHisto = (!disciplinePlat && estFerrageDeferre(ferrage)) ? bonusReussiteHistoDeferre(perfsHistorique) : 0;

  const score = scoreJockey + scoreEntraineur + scoreFerrage + bonusHisto;
  return Math.min(100, Math.max(0, score));
}
