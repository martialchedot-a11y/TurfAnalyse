 ...
 * une fois la course terminee et les rapports mis en paiement), sans
 * parametre `specialisation` : on recupere ainsi a la fois les types de pari
 * "point de vente" (COUPLE_GAGNANT, mise de base 2€) et "internet" (les
 * variantes E_..., mise de base 1€) — voir extraireRapportsCoupleGagnant,
 * qui privilegie le premier et se replie sur le second si absent.
 * @param {Date|string} date
 * @param {number} numReunion
 * @param {number} numCourse
 * @returns {string}
 */
export function buildRapportsUrl(date, numReunion, numCourse) {
  return `${BASE_URL}/${formatDatePmu(date)}/R${numReunion}/C${numCourse}/rapports-definitifs`;
}
 ...
 * .../rapports-definitifs, voir buildRapportsUrl), les dividendes officiels
 * du pari **Couplé Gagnant** — un rapport par combinaison gagnante (en
 * pratique une seule, sauf cas rarissime). Privilegie le type de pari
 * "point de vente" (`COUPLE_GAGNANT`, la reference habituellement citee) et
 * se replie sur la variante "internet" (`E_COUPLE_GAGNANT`) si la premiere
 * est absente (ex. course reservee aux paris internet). Renvoie un tableau
 * vide si aucun des deux types de pari n'est present dans la reponse (le
 * PMU n'ouvre pas toujours le Couplé Gagnant, notamment sur les tres
 * petits champs). Fonction pure, testable sans reseau.
 * @param {Array} json - reponse de l'endpoint rapports-definitifs (tableau
 *   d'objets `{typePari, rapports:[{combinaison, dividendePourUnEuro, ...}]}`).
...
export function extraireRapportsCoupleGagnant(json) {
  ...
  const bloc = json.find((p) => p?.typePari === 'COUPLE_GAGNANT') || json.find((p) => p?.typePari === 'E_COUPLE_GAGNANT');
 ...
 * Extrait, d'une reponse JSON "rapports-definitifs" de l'API PMU (endpoint
 * .../rapports-definitifs, voir buildRapportsUrl), les dividendes officiels
 * du pari **Trio** — un rapport par combinaison gagnante (en pratique une
 * seule, sauf cas rarissime). Privilegie le type de pari "point de vente"
 * (`TRIO`, la reference habituellement citee) et se replie sur la variante
 * "internet" (`E_TRIO`) si la premiere est absente. Renvoie un tableau vide
 * si aucun des deux types de pari n'est present dans la reponse (le PMU
 * n'ouvre pas toujours le Trio, notamment sur les tres petits champs).
 * Fonction pure, testable sans reseau - meme structure que
 * extraireRapportsCoupleGagnant ci-dessus, avec 3 numeros par combinaison
 * au lieu de 2.
 * @param {Array} json - reponse de l'endpoint rapports-definitifs.
 * @returns {Array<{numeros:number[], dividende:number}>} dividende exprime
 *   en euros pour une mise de 1€.
 */
export function extraireRapportsTrio(json) {
  if (!Array.isArray(json)) return [];
  const bloc = json.find((p) => p?.typePari === 'TRIO') || json.find((p) => p?.typePari === 'E_TRIO');
  if (!bloc || !Array.isArray(bloc.rapports)) return [];
...
export async function fetchRapportsPmu(date, numReunion, numCourse, { timeoutMs = TIMEOUT_PAR_DEFAUT_MS } = {}) {
  const url = buildRapportsUrl(date, numReunion, numCourse);

  for (const tentative of TENTATIVES_RAPPORTS) {
    try {
      const json = await fetchJson(tentative.build(url, date, numReunion, numCourse), timeoutMs);
