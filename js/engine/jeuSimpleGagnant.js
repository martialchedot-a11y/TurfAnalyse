// =============================================================================
// jeuSimpleGagnant.js
// "Jeu Simple Gagnant" (v11, aout 2026) : ne retient plus QUE le rang 1 du
// classement Score Global, via DEUX sources de cote possibles :
//
//   Cote DIRECTE > 3,8 ET ecart de Score Global (rang1 vs rang2) >= 10
//     -> "1er du classement seul" (rendement reel 190,3% sur l'archive
//        complete, 413 fichiers/14004 courses, n=2133 - voir HEBERGEMENT.md).
//   Cote 8H       > 3,8 ET ecart de Score Global (rang1 vs rang2) >= 15
//     -> "1er du classement 8h", UNIQUEMENT si le rang 1 ne qualifie PAS
//        deja en cote directe (seuil de cote identique, seuil d'ECART plus
//        strict depuis v11 - voir historique ci-dessous).
//
 ...
 *   mode8h:false`) si `entry.coteDirecte` depasse son seuil ET que l'ecart
 *   de Score Global avec le rang 2 est >= `SEUIL_ECART_SCORE_RANG1` (10) ;
 *   sinon, qualifie en cote 8h (`rang1Value:false, mode8h:true`) si
 *   `entry.cote8h` depasse le meme seuil de cote ET que l'ecart est >=
 *   `SEUIL_ECART_SCORE_RANG1_8H` (15, plus strict depuis v11 - voir
 *   commentaire d'en-tete). La cote directe est toujours prioritaire quand
 *   les deux qualifient. `principal` = "jouer le
 ...
 *   `multiplicateurMiseEcartRang1` - applicable aux deux modes), `null`
 *   sinon (rang 2 absent/score inconnu, ou jeu non rentable).
 */
export function jeuSimpleGagnant(chevaux) {
  ...
  const ecartOkDirecte = ecartScoreRang1 !== null && ecartScoreRang1 >= SEUIL_ECART_SCORE_RANG1;
  const ecartOk8h = ecartScoreRang1 !== null && ecartScoreRang1 >= SEUIL_ECART_SCORE_RANG1_8H;

  if (rang1 && ecartOkDirecte) {
