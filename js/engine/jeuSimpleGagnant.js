...
// jeuSimpleGagnant.js
//
// *** sept. 2026 (a la demande explicite de l'utilisateur) : retrait complet
// du gate "jockey+entraineur en forme" (v21) et du module Ecarts Jockeys/
// Entraineurs qui l'alimentait. Motif : le backtest v21 (voir mise a jour
// 39, HEBERGEMENT.md) reposait sur un snapshot "instant present" (fuite de
// donnees). Reteste ensuite avec des snapshots mensuels FIGES (aucune fuite
// possible, un fichier par mois date de la fin du mois precedent, fourni par
// l'utilisateur) sur 9 mois (janvier-septembre 2026, n=23131) : le rendement
// "enForme" (88,4%) n'est PAS meilleur que "insuffisant" (89,5%) ni
// "inconnu" (89,8%) - le signal ne se confirme pas, c'etait bien un artefact
// de la fuite de donnees. Le gate est donc retire, `jeuSimpleGagnant()`
// redevient a 1 seul parametre (comportement identique a v20 ci-dessous).
//
 ...
 *   `nbCandidats` = taille du pool complet (etape 1, cotes<3 incluses).
 *   `rang1Value`/`mode8h` sont conserves a `true`/`false` fixes uniquement
 *   pour compatibilite avec le filtre de mode et les bilans historiques de
 *   app.js (l'ancienne distinction cote directe/8h n'existe plus, mode
 *   unique desormais). `joursAbsenceRang1` = `entry.joursAbsence` du cheval
 *   retenu, sert toujours au multiplicateur de mise
 *   `multiplicateurMiseJoursAbsenceRang1` (independant de v12-v18, non
 *   affecte par cette refonte).
 */
