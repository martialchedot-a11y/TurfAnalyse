...
// Seuils du pool Value<=-30/cote>=3 (v19, sept. 2026) - voir commentaire
// d'en-tete pour le detail de toute l'exploration qui y a mene.
...
/**
 * Retrouve, parmi une liste de candidats deja filtres sur Value<=-30/
 * cote>=3, celui qui domine STRICTEMENT tous les autres sur MX (desc), OR
 * (desc) et MA (asc) simultanement. `null` si un candidat a une donnee
 * manquante sur l'une des 3 rubriques (course ignoree, pas assez fiable
 * pour trancher) ou si aucun candidat ne domine ainsi (pas de compromis).
...
/**
 * "Jeu Simple Gagnant" (v19, sept. 2026) - voir commentaire d'en-tete pour
 ...
 *   Pool = chevaux avec `value <= SEUIL_VALUE_SIMPLE_GAGNANT` (-30) ET
 *   `cotePourAffichage >= SEUIL_COTE_SIMPLE_GAGNANT` (3). Aucun candidat ->
 *   non rentable. Un seul candidat -> joue automatiquement. Plusieurs
 *   candidats -> departage par dominance stricte MX+OR+MA
 *   (`candidatDominantMxOrMa`) ; non rentable si aucun candidat ne domine
 *   ainsi ou si une donnee est manquante. `rang1Value`/`mode8h` sont
 *   conserves a `true`/`false` fixes uniquement pour compatibilite avec le
 *   filtre de mode et les bilans historiques de app.js (l'ancienne
 *   distinction cote directe/8h n'existe plus, mode unique desormais).
 *   `joursAbsenceRang1` = `entry.joursAbsence` du cheval retenu, sert
 *   toujours au multiplicateur de mise `multiplicateurMiseJoursAbsenceRang1`
 *   (independant de v12-v18, non affecte par cette refonte).
 */
export function jeuSimpleGagnant(chevaux) {
  const candidats = (chevaux || []).filter((c) => typeof c.value === 'number' && c.value <= SEUIL_VALUE_SIMPLE_GAGNANT && c.cotePourAffichage >= SEUIL_COTE_SIMPLE_GAGNANT);
  if (candidats.length === 0) return { rentable: false };

  const pick = candidats.length === 1 ? candidats[0] : candidatDominantMxOrMa(candidats);
