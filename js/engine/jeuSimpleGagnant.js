...
//
// *** v9 (aout 2026, a la demande de l'utilisateur) *** : condition
// SUPPLEMENTAIRE pour que "Cheval value seul (rang hors 1)" qualifie : le
// score Croisement du cheval retenu (0 a 4 - nombre de fois qu'il figure
// dans le top-3 de CHACUNE des 4 rubriques R10/TG/OR/IdC, voir
// classementCroisement dans jeuCoupleTrioCroisement.js) doit etre
// <= SEUIL_SCORE_CROISEMENT_CHEVAL_VALUE_SEUL (1). Backtest reel (8 mois) :
// gradient MONOTONE decroissant selon ce score (score 0 : 145,4% de
// rendement : score 1 : 129,8% ; score 2 : 96,1% ; score 3 : 74,3% ; score 4 :
// 60,5%) - contre-intuitif (moins ces rubriques confirment le choix du
// modele, mieux ca marche), mais net et regulier. En restreignant a
// score <= 1, le rendement global du mode passe de 118,0% a 138,1%
// (1617 courses sur 2614) et la robustesse s'ameliore (110,7% au lieu de
// 100,6% en retirant les 10 plus gros gains), sans toutefois eliminer
// entierement la fragilite de fond du mode (retombe a 96,8% en retirant les
// 20 plus gros gains). Meme signal teste sur le rang 1 (v8) mais NON retenu
// a la demande de l'utilisateur : le rang 1 est deja performant sous tous
// les angles testes (voir plus haut), restreindre par score Croisement y
// aurait surtout reduit le volume de courses jouables (1058 -> 420) sans
// justification suffisante.
//
// *** Mise par palier d'ecart (aout 2026, a la demande de l'utilisateur) ***
// : pour le mode "1er du classement seul" uniquement, mise optionnelle
// PONDEREE par l'ecart de Score Global (rang1 vs rang2) plutot que flat.
// Teste par l'utilisateur sur les 2133 courses de l'archive complete (mise
// de base = 1 unite) : x1 si ecart <20, x1,5 si 20<=ecart<30, x2 si
// ecart>=30 -> net total 2332,2 unites contre 1925,9 en mise flat, soit
// +21,1% de rendement, pour un risque quasiment identique (drawdown max
// 1,45% contre 1,5% en flat, capital min 996,80 contre 994,40 sur capital
// de depart 1000). A la difference des mises en % du capital (testees et
// ECARTEES - voir HEBERGEMENT.md : drawdown 15% a 56% pour un gain
// theorique non exploitable car explosif/non plafonne), cette mise reste a
// UNITE FIXE par palier (pas de reinjection des gains), donc sans risque de
// derive. Optionnelle (case a cocher), desactivee par defaut.
// =============================================================================
...
export const SEUIL_SCORE_CROISEMENT_CHEVAL_VALUE_SEUL = 1;
 ...
 *          |{rentable:true, rang1Value:boolean, ecartScoreRang1:number|null,
 ...
 *   `principal`. `ecartScoreRang1` = ecart de Score Global rang1 vs rang2
 *   quand `rang1Value` est vrai (sert au multiplicateur de mise par palier
 *   d'ecart, voir `multiplicateurMiseEcartRang1`), `null` sinon.
 */
export function jeuSimpleGagnant(chevaux) {
  ...
  if (rang1CoteValue) {
    ...
    if (rang2 && typeof rang1.scoreGlobal === 'number' && typeof rang2.scoreGlobal === 'number') {
      ecartScoreRang1 = rang1.scoreGlobal - rang2.scoreGlobal;
      rang1Value = ecartScoreRang1 >= SEUIL_ECART_SCORE_RANG1;
    }
  }

  if (rang1Value) {
    const principal = poolDutching([rang1]);
    return { rentable: true, rang1Value: true, ecartScoreRang1, principal, alternative: null, recommande: principal };
  }
  ...
  // *** v9 *** : le cheval retenu doit aussi avoir un score Croisement
  // (R10/TG/OR/IdC) faible - voir le commentaire d'en-tete pour le backtest.
  const candidat = autresValue[0];
  const classement = classementCroisement(chevaux);
  const entree = classement.find((x) => x.numero === candidat.entry.numero);
  const scoreCroisement = entree ? entree.score : null;
  if (scoreCroisement === null || scoreCroisement > SEUIL_SCORE_CROISEMENT_CHEVAL_VALUE_SEUL) return { rentable: false };

  const principal = poolDutching(autresValue);
  return { rentable: true, rang1Value: false, ecartScoreRang1: null, principal, alternative: null, recommande: principal };
}
