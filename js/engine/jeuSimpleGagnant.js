// =============================================================================
// jeuSimpleGagnant.js
// "Jeu Simple Gagnant" (v7, aout 2026) : compare la cote REELLE du cheval,
...
//
// *** v7 (aout 2026, a la demande de l'utilisateur) *** : le rang 1 reste
// prioritaire des qu'il depasse son seuil ("1er du classement seul"). S'il
// ne depasse PAS son seuil mais qu'un SEUL des autres rangs (2, 4, 5 - le
// rang 3 reste exclu, jamais rentable seul) depasse le sien, ce cheval est
// joue seul ("Cheval value seul (rang hors 1)") - equivalent a un Dutching
// degenere a 1 cheval, rendement = sa cote, chacun de ces rangs pris
// isolement etant rentable par backtest (103,2% a 112,5%).
//
// Le Dutching combinant PLUSIEURS chevaux (rang 1 + autres, ou plusieurs
// autres rangs entre eux) reste EXCLU de cette page : un backtest cible
// avait montre que ce cas precis (2+ rangs parmi 2/4/5, rang 1 non
// qualifie) etait fragile (102,8% sur 2137 courses, negatif des qu'on
// retire 5 a 10 gros gains - voir HEBERGEMENT.md). Si 2 rangs ou plus
// parmi {2, 4, 5} depassent leur seuil en meme temps (sans que le rang 1
// qualifie), le jeu n'est donc PAS jouable (ambigu, pas de regle fiable
// pour departager).
// =============================================================================
 ...
 *   qualifie pas (Dutching multi-chevaux exclu, trop fragile). `principal`
 *   = "jouer ce cheval seul" (le rang 1 s'il qualifie, sinon l'unique
 *   autre rang qui qualifie). `alternative` reste `null` (aucun Dutching
 *   multi-chevaux propose sur cette page). `recommande` = `principal`.
 */
