...
// filtre effectif des a present") : le badge "jockey+entraineur en forme"
// (voir `js/engine/ecartsJockeyEntraineurParser.js` et HEBERGEMENT.md,
// section "badge jockey/entraineur en forme", pour le detail complet du
// constat empirique et de sa reserve methodologique - fuite de donnees
// inherente a la photo instantanee 90 jours) devient un vrai GATE sur la
// selection, et non plus un simple affichage indicatif. Concretement : une
// fois le cheval retenu par les etapes 1-3 (pool Value<=-30, dominance
// MX+OR+MA, seuil cote>=3), s'il existe des donnees Ecarts Jockeys ET/OU
// Entraineurs importees (au moins l'une des deux), le jeu n'est plus
// rentable A MOINS QUE ce cheval soit "enForme"
// (`statutFormeCheval(...) === 'enForme'`, voir ecartsJockeyEntraineurParser.js :
// jockey ET entraineur avec >=15 courses et >=10% de victoires sur les 90
// derniers jours). Si le cheval est "inconnu" (jockey/entraineur absent des
// fichiers importes) ou "insuffisant" (au moins un des deux critere non
// rempli), le jeu devient non rentable - meme s'il domine legitimement le
// reste du champ sur MX+OR+MA. **Si aucun fichier Ecarts n'a ete importe du
// tout** (cas par defaut, fonctionnalite optionnelle), ce filtre est
// desactive d'office et le comportement reste identique a v20 (compatibilite
// ascendante totale pour qui n'utilise pas cette fonctionnalite).
// Implementation : `jeuSimpleGagnant(chevaux, ecartsJockeyEntraineur)` recoit
// desormais un 2e parametre optionnel `{jockeys, entraineurs}` (meme forme
// que retourne `chargerEcartsJockeyEntraineur()` dans app.js) ; absent/null
// ou aucune des deux sous-cles renseignee -> aucun gate applique.
// **Important - pas encore re-verifie par un vrai backtest apres integration
// au moteur** : le constat empirique (n=1341 "enForme" a 111,3% de rendement
// contre n=2061 "reste" a 78,7%) a ete mesure en CROISANT le pool Value<=-30
// brut (sans dominance MX+OR+MA v19/v20) avec le statut jockey/entraineur -
// PAS en appliquant ce gate APRES le departage MX+OR+MA comme c'est
// desormais fait ici. L'effet combine des deux filtres n'a pas ete
// specifiquement mesure (le fichier Ecarts n'existant, au moment de la
// mesure, que sur une fenetre de 90 jours glissants - impossible de
// reconstituer un vrai backtest date par date, voir la reserve
// methodologique complete dans ecartsJockeyEntraineurParser.js). A
// re-examiner avec Marty une fois quelques semaines de suivi reel
// disponibles.
//
// *** v20 (sept. 2026, a la demande de l'utilisateur) : le seuil cote>=3 ne
// filtre plus le POOL avant departage, il ne filtre plus que la DECISION DE
// JOUER, apres departage ***
// Question de l'utilisateur : est-il judicieux d'exclure les cotes <3 du
// departage MX+OR+MA (v19) ? Verifie sur l'archive complete (25386 courses
// avec >=1 candidat Value<=-30, toutes cotes) :
//   - Pool v19 (cote>=3 AVANT departage, methode ci-dessous en v19) :
//     rendement PLAT 98,6%/98,8% sur un split jours pairs/impairs (n=3908/
//     4005), z-score proche de 0 (PAS significativement different de 100%) -
//     alors que le chiffre de 130,1%/124,3% cite en v19 provenait d'un test
//     limite aux courses a PLUSIEURS candidats (hors auto-jeu a candidat
//     unique) : une fois les auto-jeux a candidat unique reintegres (comme en
...
//     - alors qu'un outsider cote<3 avec un meilleur profil MX/OR/MA existe
//     bel et bien dans la course et aurait du casser cette dominance.
//   - Alternative testee : calculer la dominance MX+OR+MA parmi TOUS les
//     candidats Value<=-30 (cote<3 incluses dans la COMPETITION), puis ne
//     jouer que si le cheval qui ressort dominant a lui-meme cote>=3 (donc
//     cote<3 reste toujours interdite AU JEU, seule la faculte de "casser" la
//     dominance d'un autre change). Resultat : rendement 127,2%/120,6% sur le
//     meme split pair/impair (n=1190/1252), IC95% nettement au-dessus de
//     100% dans les deux moities (z=4,34 et z=3,52) - signal net et robuste,
//     contrairement au pool v19. Volume divise par ~3,2 (n=2442 contre 7913
//     sur l'archive complete) en contrepartie.
// Retenu : le pool de depart (etape 1) redevient Value<=-30 SANS plancher de
// cote ; le seuil `SEUIL_COTE_SIMPLE_GAGNANT` (3) n'intervient plus qu'a la
// toute fin, sur le cheval RETENU par le departage (auto-jeu a candidat
// unique inclus) - voir la nouvelle logique ci-dessous.
//
// *** v19 (sept. 2026, a la demande de l'utilisateur) : refonte complete -
// desactivation des filtres v12 a v18, remplaces par le filtre MX+OR+MA sur
// le pool Value<=-30/cote>=3 ***
// Suite a une recherche systematique de rentabilite (Gagnant/Place, cote-
// brackets, criteres d'elimination, Dutching, OR+GENES, CJE, puis recherche
// combinatoire exhaustive de 816 combinaisons de 2/3 rubriques - voir
// HEBERGEMENT.md pour le detail complet de toute cette exploration), le
// filtre MX+OR+MA est ressorti comme le seul a la fois statistiquement
// significatif ET valide par un vrai test de generalisation hors echantillon
// (split eparse jours pairs/impairs, pas un simple split chronologique qui
// confond periode et echantillon) : rendement entrainement 136,4%, TEST
// (jamais vu pendant la recherche) 124,3% (n=1186, IC95% [111,0%, 137,7%],
// statistiquement different de 100%). Son edge s'est neanmoins nettement
// erode avec le temps (150-250% de rendement en 2024, ~103% sur les 12
// derniers mois de l'archive) - voir HEBERGEMENT.md pour la discussion
// complete de cette erosion (pas un artefact de niveau/echelle - la baisse
// touche aussi bien les picks "favori du groupe" que "non-favori", signe
// d'une vraie perte de pouvoir predictif dans le temps, pas d'un decalage
// statistique).
//
// A la demande explicite de l'utilisateur, les filtres v12 a v18 (top2 du
// classement Score Global, seuils scoreAptitude/scoreForme, exclusion
// d'hippodromes, exclusion TypeCourse F en PLAT, score Croisement<=3, mise
// ponderee par scoreAptitude v17) sont TOUS desactives - remplaces
// entierement par la logique ci-dessous. Le detail de chacun de ces filtres
// (backtests, seuils, justifications) reste documente plus bas pour memoire
// historique, mais AUCUN n'est plus applique en pratique.
//
// Nouvelle logique (v19) - ETAPE 1 (pool) SUPERSEDEE PAR v20 ci-dessus,
// conservee ici pour memoire :
//   1. Pool = tous les chevaux avec `value <= -30` ET `cotePourAffichage >= 3`
//      (memes seuils que toute l'exploration recente, voir HEBERGEMENT.md).
//      Aucun candidat -> jeu non rentable.
//   2. Un seul candidat -> il est joue automatiquement (pas de departage a
//      faire).
//   3. Plusieurs candidats -> departage par DOMINANCE STRICTE sur 3
//      rubriques du fichier partants (deja exposees via `entry.rubriques`,
//      voir rubriques.js) : MX (plus haut = meilleur), OR (plus haut =
//      meilleur), MA (plus bas = meilleur). Le candidat retenu doit battre
//      STRICTEMENT tous les autres candidats sur les 3 rubriques
//      SIMULTANEMENT. Si aucun candidat ne domine ainsi (ou si l'un des
//      candidats a une donnee manquante sur l'une des 3 rubriques), la
//      course n'est pas jouee - pas de repli/compromis.
// Le paiement/gain reel reste toujours base sur `cotePourAffichage` (priorite
// a la cote directe), comme dans toutes les versions precedentes.
//
// Logique EFFECTIVE depuis v20 : etapes 2 et 3 inchangees (auto-jeu a
// candidat unique / dominance MX+OR+MA), mais appliquees au pool Value<=-30
// SANS plancher de cote (etape 1) ; le seuil `SEUIL_COTE_SIMPLE_GAGNANT` (3)
// est verifie EN DERNIER, sur le cheval retenu par ces etapes - voir
// `jeuSimpleGagnant()` plus bas.
//
// Historique (v12 a v18, DESACTIVE depuis v19 ci-dessus mais conserve pour
// memoire) : "Jeu Simple Gagnant" (v18, aout 2026) : parmi les 2 PREMIERS du
// classement Score Global (top2, reduit du top3 depuis v15 - voir plus bas),
// on retient celui qui a le meilleur `scoreAptitude`, via DEUX sources de
// cote possibles :
//
...
import { statutFormeCheval } from './ecartsJockeyEntraineurParser.js';

// ***********************************************************************
// Constantes ci-dessous CONSERVEES bien que les filtres v12-v18 qui les
// utilisaient soient desactives (v19) : `SEUIL_SCORE_APTITUDE_RANG1` et
// `SEUIL_SCORE_FORME_RANG1` restent utilisees ailleurs dans l'appli
// (filtre "Courses du jour"/"Liste des courses" dans app.js, fonctionnalite
// INDEPENDANTE de `jeuSimpleGagnant()`) - ne pas supprimer.
// ***********************************************************************
export const SEUIL_SCORE_APTITUDE_RANG1 = 85;
export const SEUIL_SCORE_FORME_RANG1 = 70;

// Seuils Value<=-30 (pool de depart) et cote>=3 (v19/v20, sept. 2026) - voir
// commentaire d'en-tete pour le detail de toute l'exploration qui y a mene.
// Depuis v20, SEUIL_COTE_SIMPLE_GAGNANT ne filtre plus le POOL (etape 1) mais
// seulement le cheval RETENU en toute fin (voir `jeuSimpleGagnant()`).
export const SEUIL_VALUE_SIMPLE_GAGNANT = -30;
export const SEUIL_COTE_SIMPLE_GAGNANT = 3;

// Index dans RUBRIQUES (rubriques.js) des 3 rubriques utilisees pour le
// departage par dominance stricte (v19) : MX (desc, plus haut = meilleur),
// OR (desc, plus haut = meilleur), MA (asc, plus bas = meilleur - deja son
// sens natif dans RUBRIQUES).
...
// *** sept. 2026 (a la demande de l'utilisateur) : signal "JA" (Jours
// d'Absence, colonne du fichier _partants du jour - donc fiable et
// point-in-time, contrairement a l'historique _musiques dont on a decouvert
// qu'il contient des performances FUTURES par rapport a la date de la
// reunion sur une bonne partie de l'archive, cf. HEBERGEMENT.md) ***
// Backtest sur l'archive complete (1338 selections v18 rentables, hors
// septembre 2026) : plus le cheval retenu revient vite apres sa derniere
// course, mieux le pari se comporte -
//   JA<=3j  : n=75,  reussite 44,0%, rendement 187,3%
//   JA<=5j  : n=86,  reussite 40,7%, rendement 177,4%
//   JA<=7j  : n=103, reussite 40,8%, rendement 175,0%
//   JA<=10j : n=159, reussite 34,0%, rendement 139,7%
//   JA<=14j : n=320, reussite 30,3%, rendement 130,0%
//   JA<=20j : n=547, reussite 26,3%, rendement 113,6%
//   > 30j   : n=444, reussite 25,9%, rendement 123,8% (proche de la moyenne)
// Seuil retenu : 7 jours (n=103, meilleur compromis volume/rendement).
// Robustesse verifiee (retrait des 5 plus gros gains individuels du sous-
// ensemble JA<=7j : rendement 175,0% -> 135,6%, toujours nettement au-dessus
// de la moyenne generale ~118%), effet confirme A L'INTERIEUR de chaque
// discipline (ATTELE seul : 27,3%/117,9% -> 46,8%/197,9% ; PLAT seul :
// 22,3%/116,8% -> 37,5%/183,1%), et etale sur toute la periode (aout 2024 a
// aout 2026) et une quinzaine d'hippodromes differents (pas un artefact de
// discipline/hippodrome/periode). Voir HEBERGEMENT.md pour le detail complet.
export const SEUIL_JOURS_ABSENCE_MISE_RENFORCEE = 7;
export const MULTIPLICATEUR_MISE_JOURS_ABSENCE_ELEVEE = 2;

/**
 * Multiplicateur de mise applicable au cheval retenu par `jeuSimpleGagnant`
 * selon son propre nombre de jours d'absence avant cette course
 * (`jeu.joursAbsenceRang1`, colonne "JA" du fichier _partants du jour) -
 * voir le commentaire ci-dessus. Independant de
 * `multiplicateurMiseAptitudeRang1` : si les deux critères sont reunis, ne
 * PAS multiplier les deux facteurs entre eux (l'effet combine des deux n'a
 * pas ete backteste specifiquement) - prendre le plus grand des deux, voir
 * l'usage dans app.js.
 * @param {number|null|undefined} joursAbsence
 * @returns {number} 1 ou `MULTIPLICATEUR_MISE_JOURS_ABSENCE_ELEVEE` (2).
 */
export function multiplicateurMiseJoursAbsenceRang1(joursAbsence) {
  if (typeof joursAbsence !== 'number' || Number.isNaN(joursAbsence) || joursAbsence < 0) return 1;
  return joursAbsence <= SEUIL_JOURS_ABSENCE_MISE_RENFORCEE ? MULTIPLICATEUR_MISE_JOURS_ABSENCE_ELEVEE : 1;
}
...
/**
 * Retrouve, parmi une liste de candidats deja filtres sur Value<=-30 (SANS
 * plancher de cote depuis v20 - cote<3 incluses dans la competition, voir
 * commentaire d'en-tete), celui qui domine STRICTEMENT tous les autres sur
 * MX (desc), OR (desc) et MA (asc) simultanement. `null` si un candidat a
 * une donnee manquante sur l'une des 3 rubriques (course ignoree, pas assez
 * fiable pour trancher) ou si aucun candidat ne domine ainsi (pas de
 * compromis). Le seuil de cote pour jouer effectivement le candidat retenu
 * est verifie APRES cet appel, dans `jeuSimpleGagnant()`.
 * @param {Array} candidats
 * @returns {Object|null}
 */
function candidatDominantMxOrMa(candidats) {
  const valeur = (c, idx) => {
    const v = c.entry.rubriques ? c.entry.rubriques[idx] : null;
    return typeof v === 'number' && !Number.isNaN(v) ? v : null;
  };
  const exploitables = candidats.filter((c) => valeur(c, IDX_MX) !== null && valeur(c, IDX_OR) !== null && valeur(c, IDX_MA) !== null);
  if (exploitables.length < candidats.length) return null;

  for (const c of exploitables) {
    const autres = exploitables.filter((x) => x !== c);
    const domine = autres.every((a) =>
      valeur(c, IDX_MX) > valeur(a, IDX_MX) &&
      valeur(c, IDX_OR) > valeur(a, IDX_OR) &&
      valeur(c, IDX_MA) < valeur(a, IDX_MA)
    );
    if (domine) return c;
  }
  return null;
...
/**
 * "Jeu Simple Gagnant" (v21, sept. 2026) - voir commentaire d'en-tete pour
 * le detail complet de la methode et de l'exploration qui y a mene.
 * @param {Array} chevaux - result.chevaux (RaceAnalyzer.analyser), chaque
 *   element avec `.value`, `.cotePourAffichage`, `.entry.numero`,
 *   `.entry.rubriques` (MX/OR/MA), `.entry.joursAbsence`,
 *   `.entry.jockeyNom`/`.entry.entraineurNom`.
 * @param {{jockeys:Object|null, entraineurs:Object|null}|null|undefined} [ecartsJockeyEntraineur]
 *   Optionnel (v21) - meme forme que `chargerEcartsJockeyEntraineur()`
 *   (app.js). Si fourni avec au moins une des deux sous-cles renseignee, le
 *   cheval retenu doit etre "enForme" (voir `statutFormeCheval`,
 *   ecartsJockeyEntraineurParser.js) pour que le jeu reste rentable - voir
 *   commentaire d'en-tete (v21) pour le detail et la reserve methodologique.
 *   Absent/null/aucune sous-cle -> aucun gate (comportement identique a v20).
 * @returns {{rentable:false}
 *          |{rentable:true, rang1Value:true, mode8h:false, numero:number,
 *            cote:number, value:number, nbCandidats:number,
 *            joursAbsenceRang1:number|null,
 *            statutFormeJockeyEntraineur:('enForme'|null),
 *            principal:{chevaux,n,s,rendement},
 *            alternative:null,
 *            recommande:{chevaux,n,s,rendement}}}
 *   Pool (etape 1) = chevaux avec `value <= SEUIL_VALUE_SIMPLE_GAGNANT` (-30),
 *   SANS plancher de cote (depuis v20 - voir commentaire d'en-tete : les
 *   cotes<3 participent au departage, seule la decision finale de jouer les
 *   exclut). Aucun candidat -> non rentable. Un seul candidat -> retenu
 *   automatiquement (pas de departage). Plusieurs candidats -> departage par
 *   dominance stricte MX+OR+MA (`candidatDominantMxOrMa`), calculee sur TOUT
 *   le pool (cotes<3 incluses) ; non rentable si aucun candidat ne domine
 *   ainsi ou si une donnee est manquante. Puis, QUEL QUE SOIT le nombre de
 *   candidats, le cheval retenu par ces etapes n'est effectivement joue
 *   (`rentable:true`) que si sa cote >= `SEUIL_COTE_SIMPLE_GAGNANT` (3) -
 *   sinon non rentable, meme s'il domine legitimement le reste du champ.
 *   Enfin (v21), si `ecartsJockeyEntraineur` est fourni et non vide, le
 *   cheval doit en plus etre "enForme" - sinon non rentable (voir
 *   commentaire d'en-tete v21). `nbCandidats` = taille du pool complet
 *   (etape 1, cotes<3 incluses). `rang1Value`/`mode8h` sont conserves a
 *   `true`/`false` fixes uniquement pour compatibilite avec le filtre de
 *   mode et les bilans historiques de app.js (l'ancienne distinction cote
 *   directe/8h n'existe plus, mode unique desormais). `joursAbsenceRang1` =
 *   `entry.joursAbsence` du cheval retenu, sert toujours au multiplicateur
 *   de mise `multiplicateurMiseJoursAbsenceRang1` (independant de v12-v18,
 *   non affecte par cette refonte).
 */
export function jeuSimpleGagnant(chevaux, ecartsJockeyEntraineur) {
  const pool = (chevaux || []).filter((c) => typeof c.value === 'number' && c.value <= SEUIL_VALUE_SIMPLE_GAGNANT);
  if (pool.length === 0) return { rentable: false };

  const pick = pool.length === 1 ? pool[0] : candidatDominantMxOrMa(pool);
  if (!pick) return { rentable: false };
  if (typeof pick.cotePourAffichage !== 'number' || pick.cotePourAffichage < SEUIL_COTE_SIMPLE_GAGNANT) return { rentable: false };

  // *** v21 (sept. 2026, a la demande de l'utilisateur) : gate "jockey+
  // entraineur en forme", voir commentaire d'en-tete pour le detail complet.
  // Desactive d'office si aucun fichier Ecarts n'a ete importe.
  const donneesEcartsDisponibles = !!(ecartsJockeyEntraineur && (ecartsJockeyEntraineur.jockeys || ecartsJockeyEntraineur.entraineurs));
  const statutForme = donneesEcartsDisponibles
    ? statutFormeCheval(pick, ecartsJockeyEntraineur.jockeys, ecartsJockeyEntraineur.entraineurs)
    : null;
  if (donneesEcartsDisponibles && statutForme !== 'enForme') return { rentable: false };
  ...
  return {
    ...
    rang1Value: true,
    mode8h: false,
    numero: pick.entry.numero,
    cote: pick.cotePourAffichage,
    value: pick.value,
    nbCandidats: pool.length,
    joursAbsenceRang1: pick.entry.joursAbsence,
    statutFormeJockeyEntraineur: statutForme,
    principal,
    alternative: null,
    recommande: principal
  };
}
