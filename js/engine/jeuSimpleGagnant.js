 ...
 *   multiplicateur par palier d'ecart - voir plus bas). `joursAbsenceRang1`
 *   (sept. 2026) = `entry.joursAbsence` (colonne "JA" du fichier _partants)
 *   du cheval retenu, `null` si absent du fichier - sert au multiplicateur de
 *   mise `multiplicateurMiseJoursAbsenceRang1`.
 */
export function jeuSimpleGagnant(chevaux, hippodrome, disciplineCanonique, typeCourse) {
  ...
  if (candidatsDirecte.length > 0) {
    ...
    if (typeof pick.scoreAptitude === 'number' && pick.scoreAptitude >= SEUIL_SCORE_APTITUDE_RANG1 && qualifieForme(pick)) {
      ...
      return { rentable: true, rang1Value: true, mode8h: false, rangChoisi: pick.classement, ecartScoreRang1: ecartVsRivaux(pick), scoreAptitudeRang1: pick.scoreAptitude, joursAbsenceRang1: pick.entry.joursAbsence, principal, alternative: null, recommande: principal };
    }
  }
  ...
  if (candidats8h.length > 0) {
    ...
    if (typeof pick.scoreAptitude === 'number' && pick.scoreAptitude >= SEUIL_SCORE_APTITUDE_RANG1 && qualifieForme(pick) && ecart !== null && ecart >= SEUIL_ECART_SCORE_RANG1_8H) {
      if (!qualifieCroisement(pick)) return { rentable: false };
      const principal = poolDutching([pick]);
      return { rentable: true, rang1Value: false, mode8h: true, rangChoisi: pick.classement, ecartScoreRang1: ecart, scoreAptitudeRang1: pick.scoreAptitude, joursAbsenceRang1: pick.entry.joursAbsence, principal, alternative: null, recommande: principal };
    }
  }
