...
//
// *** sept. 2026 : le garde-fou MIN_PARTANTS_SIMPLE_PLACE (<8 partants =
// Simple Place non joue) a ete teste sur un backtest reel (01-21/09/2026,
// n=44 selections croisement, source : rapports officiels turfbzh) a la
// demande de l'utilisateur avant decision. Resultat : gain marginal
// (+0,8 pt de rendement sur les courses >=8 partants, mais le sous-ensemble
// exclu <8 partants restait lui-meme rentable, rendement 104,0% sur 21
// selections). A la demande explicite de l'utilisateur ("je garde la
// version avant V106"), le garde-fou est retire : le Simple Place est a
// nouveau joue sur TOUTES les courses du croisement, quel que soit le
// nombre de partants.
// =============================================================================
...
import { trouverSupplementation } from './supplementationParser.js';

export { SEUIL_VALUE_SIMPLE_GAGNANT, misesJeuSimpleGagnant };

/**
 * Seuil de Note IA (/20) du fichier "Supplementation_PMU_AAAAMMJJ.xlsx" a
 * partir duquel une selection Value<=-30 est jouee (voir jeuSupplementation).
 * Valide par backtest reel (01-26/09/2026, n=16) : reussite Gagnant 43,8%/
 * rendement 148,1%, reussite Place 75,0%/rendement 121,3%. Confirme par
 * l'utilisateur en reponse a la question de clarification ("Garder Note IA
 * >= 15,9 (Recommande)").
 */
export const SEUIL_NOTE_SUPPLEMENTATION = 15.9;
...
export function jeuSimpleGP(chevaux, topIAPlace, lieu, numeroCourse) {
  ...
  if (!place && !gagnant) return { rentable: false };
...
/**
 * "Supplementation" (sept. 2026, a la demande explicite de l'utilisateur) :
 * croise le pool Value<=-30 (meme seuil que le "Simple G/P") avec le
 * fichier "Supplementation_PMU_AAAAMMJJ.xlsx" du jour importe, en ne
 * retenant que les chevaux dont la Note IA (/20) est >= SEUIL_NOTE_SUPPLEMENTATION.
 * Contrairement au "Simple G/P" (base sur le Top 5 IA Place, ou seul le
 * rang IA 1 est jouable en Gagnant), ici Place ET Gagnant sont joues sur
 * EXACTEMENT le meme pool qualifie (aucune restriction de rang, la notion
 * de "rang" n'existant pas dans le fichier de Supplementation) - le
 * matching se fait par NOM DE CHEVAL (voir supplementationParser.js).
 * @param {Array} chevaux - result.chevaux (RaceAnalyzer.analyser).
 * @param {Array|null} supplementationRows - lignes Supplementation du jour
 *   (chargerSupplementation() cote app.js, ou directement
 *   parseSupplementation(...).rows).
 * @returns {{rentable:false}
 *          |{rentable:true,
 *             place:{chevaux:Array,n:number,s:number,rendement:number},
 *             gagnant:{chevaux:Array,n:number,s:number,rendement:number},
 *             notes:Array<{numero:number, note:number}>}}
 *   `rentable:false` si aucun cheval Value<=-30 de la course ne figure dans
 *   la Supplementation du jour avec Note>=15,9 (ou si la Supplementation
 *   n'a pas ete importee). Sinon, `place` et `gagnant` sont TOUJOURS tous
 *   les deux presents (Dutching sur le meme pool qualifie).
 */
export function jeuSupplementation(chevaux, supplementationRows) {
  const poolValue = (chevaux || []).filter((c) => typeof c.value === 'number' && c.value <= SEUIL_VALUE_SIMPLE_GAGNANT);
  if (poolValue.length === 0 || !Array.isArray(supplementationRows) || supplementationRows.length === 0) return { rentable: false };

  const avecNote = poolValue
    .map((c) => ({ cheval: c, supp: trouverSupplementation(supplementationRows, c.entry.nom) }))
    .filter((x) => x.supp && typeof x.supp.note === 'number' && x.supp.note >= SEUIL_NOTE_SUPPLEMENTATION)
    .sort((a, b) => b.supp.note - a.supp.note);
  if (avecNote.length === 0) return { rentable: false };

  const chevauxQualifies = avecNote.map((x) => x.cheval);
  const place = poolDutching(chevauxQualifies);
  const gagnant = poolDutching(chevauxQualifies);

  return {
    rentable: true,
    place,
    gagnant,
    notes: avecNote.map((x) => ({ numero: x.cheval.entry.numero, note: x.supp.note }))
  };
}
