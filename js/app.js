...
function trioValueAvecBaseHtml(bd, chevaux, cotesCibles) {
  ...
  return `<div class="card" style="margin-bottom:10px;">
      <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:6px;">
        ...
        ${baseConfirmee ? `<div style="display:flex; gap:6px; flex-wrap:wrap; justify-content:flex-end;">${baseConfirmee}</div>` : ''}
      </div>
...
function coupleValueHtml(bd, chevaux, cotesCibles) {
  ...
  const numerosConfirmes = confirmationsCotesCibles(candidats, cotesCibles);
  return `<div class="card" style="margin-bottom:10px;">
      <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:6px;">
...
async function renderCourseFeuVert() {
  ...
  for (const meeting of meetings) {
    ...
    for (const race of races) {
      ...
      const cotesCibles = calculerCotesCibles(result.chevaux, context.nbPartants);
      const numerosConfirmes = suggestion ? confirmationsCotesCibles(suggestion.candidats, cotesCibles) : null;
      candidates.push({ meeting, race, score, suggestion, numerosConfirmes, nbPartants: horses.length });
    }
  }
  ...
  appEl.innerHTML = `
    ...
    <div class="list-group">${candidates.map(({ meeting, race, score, suggestion, numerosConfirmes, nbPartants }) => {
      const niveau = SCORE_CONFIGURATION_NIVEAUX[score];
      return `
