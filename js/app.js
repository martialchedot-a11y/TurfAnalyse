...
async function renderRaceDetail(raceId, useCote8h = false) {
  ...
  appEl.innerHTML = `
    ...
    ${resumeHtml(result.resume, basesEtDangers, cotesCibles)}
...
function combinaisonsSuggereesHtml(bd, cotesCibles, r) {
  ...
  return `
      ...
      ${r?.coupleGagnant1030 ? `<div class="resume-line"><div class="label">Couple Gagnant (10-30&euro;)</div><div>${r.coupleGagnant1030.candidats.length > 0
        ? r.coupleGagnant1030.candidats.map((c) => `N&deg;${r.coupleGagnant1030.baseNumero}-N&deg;${c.numero} (~${c.valeur.toFixed(2)}&euro;)`).join(', ')
        : '&mdash; (aucune combinaison dans la fourchette)'}</div></div>` : ''}
  `;
}

function resumeHtml(r, bd, cotesCibles) {
  return `
    <div class="card">
      ...
      ${combinaisonsSuggereesHtml(bd, cotesCibles, r)}
      ...
      ${r.coupleGagnant1030 ? '<p class="muted small">Couple Gagnant (10-30&euro;) : associe la Base au(x) cheval(aux) dont (cote Base &times; cote autre) / 2 tombe entre 10 et 30 &mdash; methode terrain simple et independante du modele de score, a titre indicatif.</p>' : ''}
    </div>
  `;
}
