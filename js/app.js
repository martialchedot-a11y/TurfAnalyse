...
import { jeuSimpleGagnant, misesJeuSimpleGagnant, MISES_PRESETS_JEU_SIMPLE_GAGNANT, bilanJeuSimpleGagnant, rendementBilan, cumulerBilansJournaliers, SEUIL_SCORE_FORME_RANG1, SEUIL_SCORE_APTITUDE_RANG1, SEUIL_VALUE_SIMPLE_GAGNANT, multiplicateurMiseJoursAbsenceRang1, SEUIL_JOURS_ABSENCE_MISE_RENFORCEE } from './engine/jeuSimpleGagnant.js';
import { estDansFenetreAvantDepart, estAujourdHui } from './engine/surveillance.js';
import { parseTopIAPlace, trouverRangIAPlace } from './engine/topIAPlaceParser.js';
...
async function chargerTopIAPlace() {
  ...
  return enregistrement && Array.isArray(enregistrement.rows) && enregistrement.rows.length > 0 ? enregistrement.rows : null;
}
...
/**
 * Carte "Top IA Plac&eacute; x Value <= -30" (sept. 2026, a la demande
 * explicite de l'utilisateur) : parmi les chevaux du pool Value<=-30 (meme
 * seuil que jeuSimpleGagnant.js, `SEUIL_VALUE_SIMPLE_GAGNANT`), affiche
 * ceux qui figurent AUSSI dans le Top 5 IA Place Turf BZH du jour importe
 * (tous rangs 1 a 5 confondus - pas seulement le rang 1), avec leur rang IA
 * respectif. Distinct de `jeuSimpleGagnant()` : ce dernier ne retient QU'UN
 * seul cheval (dominance stricte MX+OR+MA) tandis que ce croisement peut
 * en afficher plusieurs (aucune dominance requise, juste l'appartenance aux
 * 2 pools). Backtest (crossIA.mjs, 01/01-21/09/2026, 214 jours) : "Tous
 * rangs ET dans notre pool Value<=-30" -> n=500, r&eacute;ussite Place
 * 75,0% / rendement 101,5%, r&eacute;ussite Gagnant 45,8% / rendement
 * 95,4% - voir HEBERGEMENT.md pour le detail complet.
 * @param {Array} chevaux - result.chevaux
 ...
 * @returns {string}
 */
function candidatsIAPlaceValueHtml(chevaux, topIAPlace, lieu, numeroCourse) {
  if (!topIAPlace) return '';
  const poolValue = (chevaux || []).filter((c) => typeof c.value === 'number' && c.value <= SEUIL_VALUE_SIMPLE_GAGNANT);
  if (poolValue.length === 0) return '';
  const matches = poolValue
    .map((c) => ({ cheval: c, rang: rangIAPlacePour(topIAPlace, lieu, numeroCourse, c.entry.numero) }))
    .filter((m) => m.rang != null)
    .sort((a, b) => a.rang - b.rang);
  if (matches.length === 0) return '';
  return `<div class="card" style="margin-bottom:10px;">
      <p class="bold" style="margin:0 0 4px;">Top IA Plac&eacute; &times; Value &le; -30</p>
      <p class="small" style="margin:0 0 4px;">${matches.map((m) => `N&deg;${m.cheval.entry.numero} (cote ${fmt1(m.cheval.cotePourAffichage)})${badgeTopIAPlaceHtml(m.rang)}`).join(' - ')}</p>
      <p class="muted small" style="margin-top:4px;">Crois&eacute;e s&eacute;lections IA turfbzh (Top 5 IA Plac&eacute;, tous rangs) &times; notre pool Value&le;-30 : sur 214 jours (n=500), r&eacute;ussite Place 75,0% / rendement 101,5% &middot; r&eacute;ussite Gagnant 45,8% / rendement 95,4% (voir HEBERGEMENT.md).</p>
    </div>`;
}

function jeuSimpleGagnantHtml(chevaux, topIAPlace, lieu, numeroCourse) {
  const jeu = jeuSimpleGagnant(chevaux);
  const candidatsIACard = candidatsIAPlaceValueHtml(chevaux, topIAPlace, lieu, numeroCourse);
  if (!jeu.rentable) {
    return `<div class="card" style="margin-bottom:10px;">
        ...
        <p class="muted small" style="margin-top:4px;">Aucun cheval Value &le; -30 avec cote &ge; 3, ou aucun candidat ne domine strictement les autres sur MX+OR+MA (voir HEBERGEMENT.md).</p>
      </div>${candidatsIACard}`;
  }
  ...
  // Badge "Top IA Place" (sept. 2026, remplace l'ancien badge jockey/
  // entraineur) : purement indicatif, sur le cheval retenu si le Top IA
  // Place du jour a ete importe et que ce cheval y figure.
  const rangIAPrincipal = rangIAPlacePour(topIAPlace, lieu, numeroCourse, jeu.principal.chevaux[0].entry.numero);
  const badgeIAHtml = badgeTopIAPlaceHtml(rangIAPrincipal);

  return `<div class="card" style="margin-bottom:10px;">
      <p class="bold" style="margin:0 0 4px;">Jeu Simple Gagnant</p>
      <p class="small" style="margin:0 0 4px;"><span class="tag-green bold">${libelleModeSimpleGagnant(jeu)}</span> : ${jeu.principal.chevaux.map((c) => `N&deg;${c.entry.numero} (cote ${fmt1(c.cotePourAffichage)})`).join(' - ')}${badgeIAHtml}</p>
...
function basesEtDangersHtml(bd, cotesCibles, r, chevaux, disciplineCanonique, hippodrome, typeCourse, topIAPlace, numeroCourse) {
  ...
  const jeuSimpleGagnantCard = jeuSimpleGagnantHtml(chevaux, topIAPlace, hippodrome, numeroCourse);
...
async function renderListeCourses() {
  ...
  for (const meeting of meetings) {
    ...
    for (const race of races) {
      ...
      // *** sept. 2026 (a la demande explicite de l'utilisateur : "l'onglet
      // course du jour devra contenir les courses des 5 top IA place du
      // jour") : une course couverte par le Top 5 IA Place du jour importe
      // (n'importe lequel des 5 rangs) figure desormais TOUJOURS dans cette
      // liste, meme si elle est hors fourchette 8-16 partants ou sans
      // cheval passant les seuils scoreForme/scoreAptitude du Jeu Simple
      // Gagnant - ces 2 filtres ne s'appliquent plus qu'aux courses NON
      // couvertes par le Top 5 IA Place. Calcule sur horseRecords (pas sur
      // result.chevaux) pour ne pas dependre du filtre partants ci-dessous. ***
      const matchesIA = horseRecords
        .map((h) => ({ numero: h.numero, rang: rangIAPlacePour(topIAPlace, race.lieu, race.numeroCourse, h.numero) }))
        .filter((m) => m.rang != null)
        .sort((a, b) => a.rang - b.rang);
      const couverteParIA = matchesIA.length > 0;

      if (!nbPartantsAcceptableFeuVert(horseRecords.length) && !couverteParIA) continue;
      ...
      if (chevauxEligibles.length === 0 && !couverteParIA) continue;

      const numerosEligibles = chevauxEligibles.map((c) => c.entry.numero);
      const jeu = jeuSimpleGagnant(result.chevaux);
      const badgeFormeHtml = jeu.rentable ? badgeTopIAPlaceHtml(rangIAPlacePour(topIAPlace, race.lieu, race.numeroCourse, jeu.principal.chevaux[0].entry.numero)) : '';
      coursesToutesDates.push({ meeting, race, nbPartants: horseRecords.length, numerosEligibles, badgeFormeHtml, matchesIA });
    }
  }
  ...
  if (courses.length === 0) {
    appEl.innerHTML = `
      ...
      <div class="empty-state">
        ...
        <p class="muted">Cette liste retient soit les courses de ${MIN_PARTANTS_FEU_VERT} &agrave; ${MAX_PARTANTS_FEU_VERT} partants (m&ecirc;me fourchette que "Top base") comptant au moins un cheval avec scoreForme &ge; ${SEUIL_SCORE_FORME_RANG1} ET scoreAptitude &ge; ${SEUIL_SCORE_APTITUDE_RANG1} (seuils du Jeu Simple Gagnant), soit les courses couvertes par le Top 5 IA Plac&eacute; Turf BZH du jour import&eacute; (onglet Importer), quels que soient leurs nombre de partants ou scores.</p>
      </div>`;
  ...
  appEl.innerHTML = `
    ...
    <div class="list-group">${courses.map(({ meeting, race, nbPartants, numerosEligibles, badgeFormeHtml, matchesIA }, i) => {
      const dateVal = new Date(meeting.date).toISOString().slice(0, 10);
      ...
      const iaHtml = matchesIA && matchesIA.length > 0
        ? `<div class="small tag-blue bold" style="margin-top:4px;" title="Chevaux de cette course figurant dans le Top 5 IA Plac&eacute; Turf BZH du jour - informatif">${matchesIA.map((m) => `Top ${m.rang} IA : N&deg;${m.numero}`).join(' - ')}</div>`
        : '';
      return `
      <div class="list-item clickable" data-goto="race/${race.id}">
        <div>
          ...
          <div class="muted small">${escapeHtml(race.discipline)} - ${nbPartants} partants</div>
          ${numerosHtml ? `<div class="small tag-green bold" style="margin-top:4px; display:inline-block;">${numerosHtml}</div>` : ''}
