...
async function renderListeCourses() {
  ...
  for (const meeting of meetings) {
    ...
    for (const race of races) {
      ...
      const jeu = jeuSimpleGagnant(result.chevaux);
      // *** sept. 2026 (a la demande explicite de l'utilisateur) : "Courses
      // du jour" indique desormais, pour chaque course, si elle est
      // couverte par le Top 5 IA Place Turf BZH du jour importe (n'importe
      // lequel des 5 rangs) - purement informatif, cf. topIAPlaceParser.js. ***
      const matchesIA = result.chevaux
        .map((c) => ({ numero: c.entry.numero, rang: rangIAPlacePour(topIAPlace, race.lieu, race.numeroCourse, c.entry.numero) }))
        .filter((m) => m.rang != null)
        .sort((a, b) => a.rang - b.rang);
