...
import { jeuSimpleGagnant, misesJeuSimpleGagnant, MISES_PRESETS_JEU_SIMPLE_GAGNANT, trancheConfiance, bilanJeuSimpleGagnant, rendementBilan, cumulerBilansJournaliers } from './engine/jeuSimpleGagnant.js';
import { jeuCoupleTrioCroisement, combinaisonsDuPool, bilanCoupleCroisement, bilanTrioCroisement, MISES_PRESETS_CROISEMENT, TAILLE_POOL_COUPLE, TAILLE_POOL_TRIO, MIN_PARTANTS_CROISEMENT, MAX_PARTANTS_CROISEMENT, CONFIANCE_COUPLE_RECOMMANDEE } from './engine/jeuCoupleTrioCroisement.js';
...
async function verifierEtNotifierCourse(meeting, race, toutesPerfs) {
  try {
    ...
    const scoreConfiance = scoreConfigurationCoupleValue(bd, result.chevaux);
    const jeu = jeuSimpleGagnant(result.chevaux, scoreConfiance);
...
function envoyerNotificationJsg(meeting, race, jeu) {
  const rendementPct = Math.round(jeu.rendement * 1000) / 10;
  const seuilPct = Math.round(jeu.seuil * 1000) / 10;
  const titre = `Jeu Simple Gagnant possible : ${meeting.hippodrome} C${race.numeroCourse}`;
  const corps = `Top ${jeu.n} (n° ${jeu.chevaux.map((c) => c.entry.numero).join('-')}) - rendement ${rendementPct}% > seuil ${seuilPct}% - depart ${race.heureDepart || ''}`;
  try {
...
async function renderRaceDetail(raceId, useCote8h = false) {
  ...
  bindJeuSimpleGagnant(result.chevaux, scoreConfigurationCoupleValue(basesEtDangers, result.chevaux));
 ...
 * detail de la methode Dutching et l'origine des seuils de rentabilite).
 * Affichee dans le prolongement du "Conseil de jeu" ci-dessus, mais
 * independante de sa cascade (ancre/Base tres solide) : s'applique a
 * n'importe quelle course des lors qu'au moins 2 chevaux ont une cote
 * reelle connue.
 */
const TRANCHE_CONFIANCE_LABEL = { faible: 'confiance faible', moyenne: 'confiance moyenne', forte: 'confiance forte' };
const TRANCHE_CONFIANCE_CLASSE = { faible: 'tag-orange', moyenne: 'tag-blue', forte: 'tag-green' };

/**
 * "(confiance faible/moyenne/forte)" colore, a afficher a cote du titre
 * "Jeu Simple Gagnant" pour rendre la tranche de confiance utilisee plus
 * visible d'un coup d'oeil (faible=orange, moyenne=bleu, forte=vert - memes
 * couleurs que le reste de l'appli, cf. css/style.css .tag-*).
 * @param {'faible'|'moyenne'|'forte'} tranche
 */
function trancheConfianceBadgeHtml(tranche) {
  return ` <span class="${TRANCHE_CONFIANCE_CLASSE[tranche]} bold">(${TRANCHE_CONFIANCE_LABEL[tranche]})</span>`;
}

function jeuSimpleGagnantHtml(chevaux, scoreConfiance) {
  const jeu = jeuSimpleGagnant(chevaux, scoreConfiance);
  if (!jeu.rentable) {
    const tranche = trancheConfiance(scoreConfiance);
    return `<div class="card" style="margin-bottom:10px;">
        <p class="bold" style="margin:0 0 4px;">Jeu Simple Gagnant${trancheConfianceBadgeHtml(tranche)}</p>
        <p class="small tag-red bold" style="margin:0;">Jeu simple gagnant non rentable</p>
        <p class="muted small" style="margin-top:4px;">Aucune combinaison de ${tranche === 'faible' ? '4' : '2'} &agrave; 8 chevaux (class&eacute;s par Score Global, cote r&eacute;elle connue) ne d&eacute;passe le seuil de rentabilit&eacute; de sa tranche de confiance (${TRANCHE_CONFIANCE_LABEL[tranche]}) sur cette course${tranche === 'faible' ? ' (minimum 4 chevaux impos&eacute; en confiance faible)' : ''}. Indicatif, n'entre dans aucun calcul de Score Global/Value/classement.</p>
      </div>`;
  }
  const optionsMise = MISES_PRESETS_JEU_SIMPLE_GAGNANT.map((m) => `<option value="${m}">${m}&euro;</option>`).join('');
  return `<div class="card" style="margin-bottom:10px;">
      <p class="bold" style="margin:0 0 4px;">Jeu Simple Gagnant${trancheConfianceBadgeHtml(jeu.tranche)}</p>
      <p class="small" style="margin:0 0 4px;">${jeu.n} chevaux retenus (m&eacute;thode Dutching) : ${jeu.chevaux.map((c) => `N&deg;${c.entry.numero} (cote ${fmt1(c.cotePourAffichage)})`).join(' - ')}</p>
      <p class="small" style="margin:0 0 8px;">Rendement esp&eacute;r&eacute; : <span class="tag-green bold">${(jeu.rendement * 100).toFixed(1)}%</span> &middot; minimum requis pour ${jeu.n} chevaux en ${TRANCHE_CONFIANCE_LABEL[jeu.tranche]} : ${(jeu.seuil * 100).toFixed(0)}%</p>
      <div class="field" style="margin-bottom:8px;">
        <label for="jsg-mise-select">Mise totale souhait&eacute;e</label>
        <select id="jsg-mise-select">${optionsMise}</select>
      </div>
      <button class="btn btn-secondary btn-block" id="jsg-calculer-btn">Calculer mises</button>
      <div id="jsg-resultat"></div>
      <p class="muted small" style="margin-top:8px;">M&eacute;thode Dutching : la mise de chaque cheval est proportionnelle &agrave; 1/cote, pour un gain quasi identique quel que soit celui des ${jeu.n} chevaux qui gagne (mises arrondies &agrave; l'euro pour rester jouables au guichet, d'o&ugrave; un l&eacute;ger &eacute;cart de gain d'un cheval &agrave; l'autre). Nombre de chevaux retenus (N) d&eacute;termin&eacute; dynamiquement pour CETTE course (le plus grand N de 2 &agrave; 8, selon les cotes r&eacute;elles du jour, dont le rendement d&eacute;passe le seuil de rentabilit&eacute; mesur&eacute; sur 8 mois d'archives r&eacute;elles POUR SA TRANCHE DE CONFIANCE - voir HEBERGEMENT.md). Indicatif, n'entre dans aucun calcul de Score Global/Value/classement.</p>
    </div>`;
}

/**
 * A appeler apres avoir injecte jeuSimpleGagnantHtml dans le DOM (voir
 * renderRaceDetail) : attache le clic du bouton "Calculer mises" au calcul
 * des mises Dutching pour la mise totale choisie dans le menu deroulant.
 * @param {number|null|undefined} scoreConfiance - meme valeur que celle
 *   utilisee pour generer jeuSimpleGagnantHtml (evite tout ecart entre la
 *   carte affichee et le calcul des mises).
 */
function bindJeuSimpleGagnant(chevaux, scoreConfiance) {
  ...
  btn.addEventListener('click', () => {
    ...
    resultEl.innerHTML = `
      ...
      `).join('')}</div>
      <p class="small" style="margin-top:6px;">Mise totale : ${fmt0(miseTotaleReelle)}&euro; &middot; gain si l'un de ces ${jeu.n} chevaux gagne : de ${fmt0(Math.min(...mises.map((m) => m.gain)))}&euro; &agrave; ${fmt0(Math.max(...mises.map((m) => m.gain)))}&euro; (mises arrondies &agrave; l'euro, principe du Dutching)</p>
    `;
  });
}
 ...
 * pour le detail de la methode et l'origine des tailles de pool K=3/K=4).
 * Contrairement au Jeu Simple Gagnant, ce jeu utilise une mise FLAT (pas de
 * Dutching) : le meme montant est joue sur chaque combinaison du pool.
 */
function jeuCroisementHtml(chevaux) {
  ...
  if (!jeu.jouable) {
    return `<div class="card" style="margin-bottom:10px;">
        ...
        <p class="muted small" style="margin-top:4px;">R&eacute;serv&eacute; aux courses de ${MIN_PARTANTS_CROISEMENT} &agrave; ${MAX_PARTANTS_CROISEMENT} partants (en dessous de ${MIN_PARTANTS_CROISEMENT} partants, le PMU ne propose souvent que le Coupl&eacute;/Trio &laquo;&nbsp;Ordre&nbsp;&raquo;, incompatible avec ce jeu). Indicatif, n'entre dans aucun calcul de Score Global/Value/classement.</p>
      </div>`;
  }

  const combosCouple = combinaisonsDuPool(jeu.poolCouple, 2);
  const combosTrio = combinaisonsDuPool(jeu.poolTrio, 3);
  const optionsMise = MISES_PRESETS_CROISEMENT.map((m) => `<option value="${m}">${m}&euro;</option>`).join('');
  const coupleConfianceOk = jeu.confianceCouple >= CONFIANCE_COUPLE_RECOMMANDEE;

  return `<div class="card" style="margin-bottom:10px;">
      <p class="bold" style="margin:0 0 4px;">Jeu Croisement Couple/Trio</p>
      <p class="muted small" style="margin:0 0 8px;">Pool obtenu en croisant les rubriques R10/TG/OR/IdC (top-3 de chacune). Valide sur un &eacute;chantillon r&eacute;el de 51 &agrave; 59 courses (voir HEBERGEMENT.md) : Couple 148,5% de rendement r&eacute;el (r&eacute;sultat robuste), Trio 128,7% (r&eacute;sultat plus fragile, port&eacute; en partie par un seul gros gain) &mdash; &eacute;chantillon encore petit, prudence recommand&eacute;e.</p>
      <div class="field" style="margin-bottom:8px;">
        <label for="croisement-mise-select">Mise par combinaison</label>
        <select id="croisement-mise-select">${optionsMise}</select>
      </div>
      <p class="small" style="margin:0 0 4px;">Couple Gagnant (${TAILLE_POOL_COUPLE} chevaux, ${combosCouple.length} combinaisons) : ${jeu.poolCouple.map((n) => `N&deg;${n}`).join(' - ')} <span class="small ${coupleConfianceOk ? 'tag-green' : 'tag-orange'} bold">confiance ${jeu.confianceCouple}/4</span></p>
      <p class="muted small" style="margin:0 0 8px;" id="croisement-couple-combos">${combosCouple.map((c) => c.join('-')).join(', ')}</p>
      <p class="small" style="margin:0 0 4px;">Trio (${TAILLE_POOL_TRIO} chevaux, ${combosTrio.length} combinaisons) : ${jeu.poolTrio.map((n) => `N&deg;${n}`).join(' - ')} <span class="small tag-gray bold">confiance ${jeu.confianceTrio}/4</span></p>
      <p class="muted small" style="margin:0 0 8px;" id="croisement-trio-combos">${combosTrio.map((c) => c.join('-')).join(', ')}</p>
      <div id="croisement-resultat"></div>
      <p class="muted small" style="margin-top:8px;">Confiance = score (0-4) du membre le plus faible du pool (nombre de rubriques ou il est top-3). Sur l'archive compl&egrave;te (8-16 partants), confiance Couple &ge; ${CONFIANCE_COUPLE_RECOMMANDEE} fait passer la r&eacute;ussite de 17,0% &agrave; 22,8% (n=952) - la confiance Trio n'apporte pas d'am&eacute;lioration nette, affich&eacute;e &agrave; titre indicatif uniquement. Mise flat (pas de Dutching) : le m&ecirc;me montant est jou&eacute; sur chaque combinaison list&eacute;e ci-dessus. Voir la page "Couple/Trio" pour suivre le bilan r&eacute;el au fil des jours. Indicatif, n'entre dans aucun calcul de Score Global/Value/classement.</p>
    </div>`;
}
...
function bindJeuCroisement(chevaux) {
  ...
  const majResultat = () => {
    ...
    resultEl.innerHTML = `
      <div class="stat-grid">
        <div class="stat-cell"><div class="v">Couple</div><div class="l">mise totale ${fmt0(miseParCombinaison * nbCombosCouple)}&euro; (${nbCombosCouple} x ${fmt0(miseParCombinaison)}&euro;)</div></div>
        <div class="stat-cell"><div class="v">Trio</div><div class="l">mise totale ${fmt0(miseParCombinaison * nbCombosTrio)}&euro; (${nbCombosTrio} x ${fmt0(miseParCombinaison)}&euro;)</div></div>
      </div>`;
  };
...
function basesEtDangersHtml(bd, cotesCibles, r, chevaux, disciplineCanonique, predictionExterne) {
  ...
  const jeuSimpleGagnantCard = jeuSimpleGagnantHtml(chevaux, scoreConfigurationCoupleValue(bd, chevaux));
 ...
 *   race, nbPartants, jeu, scoreConfiance }.
 */
async function collecterCandidatesSimpleGagnant() {
  ...
  for (const meeting of meetings) {
    ...
    for (const race of races) {
      ...
      candidates.push({ meeting, race, nbPartants: horses.length, jeu, scoreConfiance });
    }
  }
  return candidates;
}
...
async function renderBilanSimpleGagnant() {
  ...
  const analysees = candidates.map(({ meeting, race, nbPartants, jeu, scoreConfiance }) => {
    const ordreArrivee = CSVImporter.parseOrdreArrivee(race.arriveeBrute || '');
    if (ordreArrivee.length === 0) {
      return { meeting, race, nbPartants, jeu, scoreConfiance, connu: false, gagne: null, vrai1: null, rapportStatus: 'en_attente', bilan: null };
    }
    const vrai1 = ordreArrivee[0];
    const gagne = jeu.chevaux.some((c) => c.entry.numero === vrai1);

    if (!gagne) {
      const bilan = bilanJeuSimpleGagnant(jeu, MISE_STANDARD_BILAN_SIMPLE_GAGNANT, undefined, vrai1);
      return { meeting, race, nbPartants, jeu, scoreConfiance, connu: true, gagne, vrai1, rapportStatus: 'ok', bilan };
    }
    if (race.rapportSimpleGagnant === undefined) {
      return { meeting, race, nbPartants, jeu, scoreConfiance, connu: true, gagne, vrai1, rapportStatus: 'a_recuperer', bilan: null };
    }
    const bilan = bilanJeuSimpleGagnant(jeu, MISE_STANDARD_BILAN_SIMPLE_GAGNANT, race.rapportSimpleGagnant, vrai1);
    if (!bilan.dividendeConnu) {
      return { meeting, race, nbPartants, jeu, scoreConfiance, connu: true, gagne, vrai1, rapportStatus: 'indisponible', bilan: null };
    }
    return { meeting, race, nbPartants, jeu, scoreConfiance, connu: true, gagne, vrai1, rapportStatus: 'ok', bilan };
  });
  ...
  const rowHtml = (a) => `
    <div class="list-item clickable" data-goto="race/${a.race.id}">
      <div>
        ...
        <div class="muted small">${TRANCHE_CONFIANCE_LABEL[a.jeu.tranche]} - Top ${a.jeu.n} (n&deg;${a.jeu.chevaux.map((c) => c.entry.numero).join('-')})${a.connu ? ` - Vainqueur n&deg;${a.vrai1}` : ' - En attente de resultat'}</div>
      </div>
      <div style="display:flex; flex-direction:column; align-items:flex-end; gap:2px;">
        ${a.connu
...
// Filtre "confiance minimale (Couple)" pour la page Croisement, choisi par
// l'utilisateur et memorise dans ce navigateur (localStorage) - meme
// principe que le filtre Top base (getFiltreFeuVert). Cumulatif uniquement
// (confiance >= N) : voir confiancePool (js/engine/jeuCoupleTrioCroisement.js)
// pour l'origine du chiffre recommande par defaut (CONFIANCE_COUPLE_RECOMMANDEE).
// Ce filtre ne porte QUE sur la confiance du pool Couple : la confiance Trio
// n'a pas montre d'amelioration nette sur backtest (voir HEBERGEMENT.md),
// elle reste affichee a titre indicatif sur la fiche course mais n'est pas
// filtrable ici.
const FILTRE_CONFIANCE_CROISEMENT_CLE_STOCKAGE = 'turf-filtre-confiance-croisement';

function getFiltreConfianceCroisement() {
  const brut = Number(localStorage.getItem(FILTRE_CONFIANCE_CROISEMENT_CLE_STOCKAGE));
  return Number.isInteger(brut) && brut >= 0 && brut <= 4 ? brut : 0;
}

function setFiltreConfianceCroisement(valeur) {
  const n = Number(valeur);
  if (!Number.isInteger(n) || n < 0 || n > 4) return;
  localStorage.setItem(FILTRE_CONFIANCE_CROISEMENT_CLE_STOCKAGE, String(n));
}

function filtreConfianceCroisementSelectorHtml() {
  const actuel = getFiltreConfianceCroisement();
  const options = [0, 1, 2, 3, 4]
    .map((v) => `<option value="${v}" ${actuel === v ? 'selected' : ''}>${v === 0 ? 'Toutes' : `${v}/4 et plus${v === CONFIANCE_COUPLE_RECOMMANDEE ? ' (recommandé)' : ''}`}</option>`)
    .join('');
  return `
    <div class="card" style="margin-bottom:10px;">
      <div class="field" style="margin-bottom:0;">
        <label for="confiance-croisement-select">Confiance minimale (Couple)</label>
        <select id="confiance-croisement-select">${options}</select>
      </div>
      <p class="muted small" style="margin-top:6px; margin-bottom:0;">Sur l'archive compl&egrave;te (8-16 partants), confiance &ge; ${CONFIANCE_COUPLE_RECOMMANDEE} fait passer la r&eacute;ussite Couple de 17,0% &agrave; 22,8% (n=952/6720) - voir HEBERGEMENT.md. N'affecte pas le Trio (aucun gain net observ&eacute;).</p>
    </div>`;
}

function bindFiltreConfianceCroisementSelector(onChange) {
  const sel = document.getElementById('confiance-croisement-select');
  if (!sel) return;
  sel.addEventListener('change', () => {
    setFiltreConfianceCroisement(sel.value);
    onChange();
  });
}
...
async function renderBilanCroisement() {
  ...
  const filtreConfiance = getFiltreConfianceCroisement();
  const toutesCandidates = await collecterCandidatesCroisement();
  const candidates = toutesCandidates.filter((c) => c.jeu.confianceCouple >= filtreConfiance);

  const introHtml = `<div class="card" style="margin-bottom:10px;">
      <p class="small muted" style="margin:0;">Pool obtenu en croisant les rubriques R10/TG/OR/IdC (voir la fiche course pour le detail), sur les courses de ${MIN_PARTANTS_CROISEMENT} &agrave; ${MAX_PARTANTS_CROISEMENT} partants. Valide sur un &eacute;chantillon r&eacute;el de 51 &agrave; 59 courses PMU (Couple 148,5% de rendement, Trio 128,7% - voir HEBERGEMENT.md) : &eacute;chantillon encore petit, cette page sert &agrave; en accumuler un plus grand sans avoir besoin de miser en argent r&eacute;el.</p>
    </div>`;

  if (candidates.length === 0) {
    appEl.innerHTML = `
      ${bilanGlobalLinkHtml}
      ${filtreConfianceCroisementSelectorHtml()}
      ${introHtml}
      <div class="empty-state">
        <div class="icon">\u{1F500}</div>
        <p class="bold">Aucune course jouable pour l'instant</p>
        <p class="muted">${toutesCandidates.length > 0
          ? `${toutesCandidates.length} course(s) jouable(s) mais aucune n'atteint la confiance minimale choisie ci-dessus. Essayez un seuil plus bas.`
          : `Une course appara&icirc;t ici d&egrave;s qu'elle compte entre ${MIN_PARTANTS_CROISEMENT} et ${MAX_PARTANTS_CROISEMENT} partants avec des rubriques exploitables.`}</p>
      </div>`;
    bindGoto();
    bindFiltreConfianceCroisementSelector(renderBilanCroisement);
    return;
  }
  ...
  const avecResultat = analysees.filter((a) => a.connu);
  const enAttente = analysees.filter((a) => !a.connu);
  const reussiesCouple = avecResultat.filter((a) => a.bilanCouple.gagne);
  const reussiesTrio = avecResultat.filter((a) => a.bilanTrio.gagne);
  const aRecuperer = avecResultat.filter((a) => a.coupleStatus === 'a_recuperer' || a.trioStatus === 'a_recuperer');
  const avecBilanCouple = avecResultat.filter((a) => a.coupleStatus === 'ok');
  const avecBilanTrio = avecResultat.filter((a) => a.trioStatus === 'ok');

  const tauxHtml = avecResultat.length > 0
    ? `<div class="card">
        <div style="display:flex; justify-content:space-around; text-align:center; flex-wrap:wrap; gap:12px;">
          <div>
            <p class="bold" style="font-size:1.5em; margin:0;">${reussiesCouple.length}/${avecResultat.length}</p>
            <p class="muted small" style="margin-top:2px;">Couple captur&eacute; (${Math.round((reussiesCouple.length / avecResultat.length) * 100)}%)</p>
          </div>
          <div>
            <p class="bold" style="font-size:1.5em; margin:0;">${reussiesTrio.length}/${avecResultat.length}</p>
            <p class="muted small" style="margin-top:2px;">Trio captur&eacute; (${Math.round((reussiesTrio.length / avecResultat.length) * 100)}%)</p>
          </div>
        </div>
      </div>`
    : `<div class="card"><p class="muted small">Aucun resultat connu pour l'instant. Utilisez la mise a jour des cotes (course ou reunion complete) pour recuperer automatiquement les arrivees officielles.</p></div>`;
  ...
  const bilanHtml = avecResultat.length === 0 ? '' : `
    <div class="card" style="margin-top:10px;">
      ...
      <p class="muted small" style="margin-top:8px;">Hypoth&eacute;tique : suppose ${MISE_STANDARD_BILAN_CROISEMENT}&euro; jou&eacute; sur CHAQUE combinaison du pool (Couple 3 combinaisons, Trio 4 combinaisons), sur toutes les courses jouables du jour &mdash; ce n'est pas un historique de vos mises reelles. Ne remplace pas votre propre jugement.</p>
    </div>`;
  ...
  const rowHtml = (a) => `
    <div class="list-item clickable" data-goto="race/${a.race.id}">
      <div>
        <div class="bold">${a.race.heureDepart ? `${escapeHtml(a.race.heureDepart)} - ` : ''}${escapeHtml(a.meeting.hippodrome)} - Course ${a.race.numeroCourse}</div>
        <div class="muted small">Couple n&deg;${a.jeu.poolCouple.join('-')} (confiance ${a.jeu.confianceCouple}/4) &middot; Trio n&deg;${a.jeu.poolTrio.join('-')}${a.connu ? ` - Arriv&eacute;e ${a.ordreArrivee.slice(0, 3).join('-')}` : ' - En attente de resultat'}</div>
      </div>
      <div style="display:flex; flex-direction:column; align-items:flex-end; gap:2px;">
        ${a.connu
          ? `<span class="small ${a.bilanCouple.gagne ? 'tag-green' : 'tag-red'} bold">Couple ${a.bilanCouple.gagne ? '✓' : '✗'}</span><span class="small ${a.bilanTrio.gagne ? 'tag-green' : 'tag-red'} bold">Trio ${a.bilanTrio.gagne ? '✓' : '✗'}</span>`
          : '<span class="small tag-gray bold">En attente</span>'}
        ${a.coupleStatus === 'ok' && a.connu ? `<span class="small ${a.bilanCouple.net >= 0 ? 'tag-green' : 'tag-red'} bold">C ${a.bilanCouple.net >= 0 ? '+' : ''}${a.bilanCouple.net.toFixed(2)}&euro;</span>` : ''}
        ${a.trioStatus === 'ok' && a.connu ? `<span class="small ${a.bilanTrio.net >= 0 ? 'tag-green' : 'tag-red'} bold">T ${a.bilanTrio.net >= 0 ? '+' : ''}${a.bilanTrio.net.toFixed(2)}&euro;</span>` : ''}
      </div>
    </div>`;
  ...
  if (btnTransfert && dateTransfert) {
    btnTransfert.addEventListener('click', async () => {
      ...
      const avecResultatDuJour = avecResultat.filter((a) => new Date(a.meeting.date).toISOString().slice(0, 10) === dateTransfert);
      const reussiesCoupleDuJour = avecResultatDuJour.filter((a) => a.bilanCouple.gagne);
      const reussiesTrioDuJour = avecResultatDuJour.filter((a) => a.bilanTrio.gagne);
      const record = {
        id: dateTransfert,
        date: dateTransfert,
        nbCourses: avecResultatDuJour.length,
...
async function renderBilanGlobalCroisement() {
    ...
    <div class="card" style="margin-bottom:10px;">
      ...
      <div style="display:flex; justify-content:space-around; text-align:center; flex-wrap:wrap; gap:12px;">
        <div>
          ...
          <p class="muted small" style="margin:0;">rendement ${rendementCoupleTotal == null ? '-' : (rendementCoupleTotal * 100).toFixed(1) + '%'} &middot; r&eacute;ussite ${totalNbCourses > 0 ? `${totalCoupleReussies}/${totalNbCourses}` : '-'}</p>
        </div>
        <div>
          <p class="muted small" style="margin:0;">Trio</p>
          ...
          <p class="muted small" style="margin:0;">rendement ${rendementTrioTotal == null ? '-' : (rendementTrioTotal * 100).toFixed(1) + '%'} &middot; r&eacute;ussite ${totalNbCourses > 0 ? `${totalTrioReussies}/${totalNbCourses}` : '-'}</p>
        </div>
      </div>
    </div>`;
  ...
  const rowHtml = (b) => {
    ...
    <div class="list-item">
      <div>
        <div class="bold">${escapeHtml(b.date)}</div>
        <div class="muted small">Couple : ${b.nbCourses > 0 ? `${b.coupleReussies}/${b.nbCourses}` : '-'} (${rendementJourCouple == null ? '-' : (rendementJourCouple * 100).toFixed(1) + '%'}) &middot; cumul <span class="${b.cumulCoupleNet >= 0 ? 'tag-green' : 'tag-red'}">${b.cumulCoupleNet >= 0 ? '+' : ''}${b.cumulCoupleNet.toFixed(2)}&euro;</span></div>
        <div class="muted small">Trio : ${b.nbCourses > 0 ? `${b.trioReussies}/${b.nbCourses}` : '-'} (${rendementJourTrio == null ? '-' : (rendementJourTrio * 100).toFixed(1) + '%'}) &middot; cumul <span class="${b.cumulTrioNet >= 0 ? 'tag-green' : 'tag-red'}">${b.cumulTrioNet >= 0 ? '+' : ''}${b.cumulTrioNet.toFixed(2)}&euro;</span></div>
      </div>
      <div style="display:flex; flex-direction:column; align-items:flex-end; gap:2px;">
