...
// Value (cf. scoreConfigurationCoupleValue) est >= 4/5 (soit uniquement les
// indices de confiance 4/5 et 5/5, a la demande de l'utilisateur - seuil
// releve depuis 3/5 initialement), ET dont le nombre de partants est
// compris entre 8 et 16 (a la demande de l'utilisateur - les champs en
// dehors de cette plage sont ecartes de la liste, meme avec un bon score).
// Remplace l'ancienne page "Courses sures" (base solide + Score Global
// >= 80), a la demande de l'utilisateur, par un filtre directement aligne
// sur l'indicateur de confiance du Coupl&eacute; Value.
// -------------------------------------------------------------------
const SEUIL_FEU_VERT = 4;
  ...
  if (candidates.length === 0) {
    appEl.innerHTML = `
      <div class="empty-state">
        ...
        <p class="muted">Une course apparait ici des que son score de configuration du Coupl&eacute; Value atteint ${SEUIL_FEU_VERT}/5, avec entre ${MIN_PARTANTS_FEU_VERT} et ${MAX_PARTANTS_FEU_VERT} partants et au moins la moitie du champ avec un historique de performances retrouve.</p>
      </div>`;
    return;
  }
  ...
  if (analysees.length === 0) {
    appEl.innerHTML = `
      <div class="empty-state">
        <div class="icon">\u{1F3AF}</div>
        <p class="bold">Aucune course feu vert pour l'instant</p>
        <p class="muted">Cette page suit la reussite des courses dont le score de configuration Coupl&eacute; Value est >= ${SEUIL_FEU_VERT}/5, avec entre ${MIN_PARTANTS_FEU_VERT} et ${MAX_PARTANTS_FEU_VERT} partants et au moins la moitie du champ avec un historique de performances retrouve.</p>
      </div>`;
    return;
  }
