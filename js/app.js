...
function reportPlaceHtml(resultat, miseDepart) {
  ...
  const legLabel = (a) => `${escapeHtml(a.meeting.hippodrome)} C${a.race.numeroCourse}${a.race.heureDepart ? ` (${escapeHtml(a.race.heureDepart)})` : ''} N&deg;${a.check.base.entry.numero}`;
  const rows = combos.map((c) => `
    <div class="list-item">
      <div>
        <div class="small">${c.legs.map(legLabel).join(' &rarr; ')}</div>
      </div>
      <div style="display:flex; flex-direction:column; align-items:flex-end; gap:2px;">
        ${c.reussi ? '<span class="small tag-green bold">Reussi</span>' : '<span class="small tag-red bold">Rate</span>'}
        <span class="small ${c.net >= 0 ? 'tag-green' : 'tag-red'} bold">${c.net >= 0 ? '+' : ''}${c.net.toFixed(2)} &euro;</span>
      </div>
    </div>`).join('');
  ...
  return `
    <div class="card" style="margin-top:10px;">
      <p class="bold" style="margin:0;">Simulation report Place (3 courses)</p>
      <p class="muted small" style="margin-top:4px;">${miseDepart}&euro; mis&eacute; sur la Base (Simple Place) d'une course, gain int&eacute;gralement rejou&eacute; sur la Base d'une course suivante puis d'une 3e, dans l'ordre chronologique - toutes les combinaisons possibles parmi les ${nbCourses} courses feu vert au rapport connu ce jour (${combos.length} combinaison(s)). ${reussis.length}/${combos.length} r&eacute;ussie(s).</p>
      <div class="list-group" style="margin-top:8px;">${rows}</div>
    </div>`;
}
