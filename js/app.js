...
import { jeuSimpleGagnant, misesJeuSimpleGagnant, MISES_PRESETS_JEU_SIMPLE_GAGNANT, bilanJeuSimpleGagnant, rendementBilan, cumulerBilansJournaliers, multiplicateurMiseEcartRang1, estHippodromeNonRentable8h } from './engine/jeuSimpleGagnant.js';
 ...
 *
 * *** Warning hippodrome non rentable en cote 8h (aout 2026) *** : quand le
 * jeu qualifie en mode 8h ET que l'hippodrome de la course fait partie de
 * `HIPPODROMES_NON_RENTABLES_8H` (jeuSimpleGagnant.js), un warning s'affiche
 * ("Attention hippodrome non rentable en cote 8h"). Purement informatif,
 * n'empeche pas de jouer. Le mode cote directe n'est pas concerne.
 */
function libelleModeSimpleGagnant(jeu) {
  // Depuis v13, le cheval retenu peut etre le rang 2 ou 3 du classement
  // (meilleur scoreAptitude du top3), pas seulement le rang 1 - voir
  // jeuSimpleGagnant.js.
  const base = jeu.rangChoisi === 1 ? '1er du classement' : `Top3 (rang ${jeu.rangChoisi})`;
...
function jeuSimpleGagnantHtml(chevaux, hippodrome) {
  ...
  if (!jeu.rentable) {
    return `<div class="card" style="margin-bottom:10px;">
        ...
        <p class="muted small" style="margin-top:4px;">Aucun des 3 premiers du classement Score Global ne qualifie ni en cote directe (seuil 3,8, aptitude &ge;85) ni en cote 8h (seuil 3,8, &eacute;cart &ge;15, aptitude &ge;85). Indicatif, n'entre dans aucun calcul de Score Global/Value/classement.</p>
      </div>`;
  }
  ...
  const warningHippodromeHtml = (jeu.mode8h && estHippodromeNonRentable8h(hippodrome))
    ? `<p class="small tag-red bold" style="margin:0 0 8px;">&#9888; Attention hippodrome non rentable en cote 8h</p>`
    : '';
