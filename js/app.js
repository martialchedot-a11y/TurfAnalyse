...
import { jeuSimpleGagnant, misesJeuSimpleGagnant, MISES_PRESETS_JEU_SIMPLE_GAGNANT, bilanJeuSimpleGagnant, rendementBilan, cumulerBilansJournaliers, SEUIL_SCORE_FORME_RANG1, SEUIL_SCORE_APTITUDE_RANG1, multiplicateurMiseJoursAbsenceRang1, SEUIL_JOURS_ABSENCE_MISE_RENFORCEE } from './engine/jeuSimpleGagnant.js';
import { estDansFenetreAvantDepart, estAujourdHui } from './engine/surveillance.js';
import { parseEcarts, statutFormeCheval } from './engine/ecartsJockeyEntraineurParser.js';
...
/**
 * Charge les 2 snapshots "Ecarts Jockeys/Entraineurs" importes (cf.
 * js/engine/ecartsJockeyEntraineurParser.js), s'ils existent. Utilise par la
 * fiche course (carte "Jeu Simple Gagnant"), par "Liste des courses" et par
 * la surveillance automatique pour (1) afficher le badge "jockey+entraineur
 * en forme" et (2) depuis v21 (sept. 2026, a la demande explicite de
 * l'utilisateur), transmis a `jeuSimpleGagnant()` comme GATE effectif sur la
 * selection : si des donnees ont ete importees, seul un cheval "enForme" est
 * jouable - voir le commentaire d'en-tete (v21) de jeuSimpleGagnant.js pour
 * le detail et la reserve methodologique (fuite de donnees inherente a la
 * photo instantanee 90 jours). Si rien n'a ete importe (retour
 * `{jockeys:null, entraineurs:null}`), aucun gate n'est applique.
 * @returns {Promise<{jockeys:Object|null, entraineurs:Object|null}>}
 */
async function chargerEcartsJockeyEntraineur() {
  const [j, e] = await Promise.all([
    DB.getEcartsJockeyEntraineur('jockeys'),
    DB.getEcartsJockeyEntraineur('entraineurs')
  ]);
  return { jockeys: j ? j.data : null, entraineurs: e ? e.data : null };
}

/**
 * HTML du badge "jockey+entraineur en forme" pour un cheval (cf.
 * `statutFormeCheval`). `null` si aucune donnee n'a ete importee (pas
 * d'affichage plutot qu'un badge "inconnu" systematique dans ce cas).
 * Depuis v21, ce badge ne fait qu'AFFICHER le statut deja utilise comme gate
 * par `jeuSimpleGagnant()` (voir chargerEcartsJockeyEntraineur ci-dessus) :
 * si des donnees sont importees, un cheval affiche ici n'a pu etre retenu
 * (jeu.rentable) que s'il est "enForme" - les statuts "insuffisant"/
 * "inconnu" ne s'affichent donc plus que dans de rares contextes ou le badge
 * est calcule independamment du gate.
 * @param {Object} cheval - element de result.chevaux.
 * @param {{jockeys:Object|null, entraineurs:Object|null}} ecarts
 * @returns {string}
 */
function badgeFormeJockeyEntraineurHtml(cheval, ecarts) {
  if (!ecarts || (!ecarts.jockeys && !ecarts.entraineurs)) return '';
  const statut = statutFormeCheval(cheval, ecarts.jockeys, ecarts.entraineurs);
  if (statut === 'enForme') return ' <span class="small tag-green bold" title="Jockey et entraineur ont chacun >=10% de victoires sur les 90 derniers jours (>=15 courses chacun) - condition requise pour que ce jeu soit propose (v21)">Jockey+entra&icirc;neur en forme</span>';
  if (statut === 'insuffisant') return ' <span class="small tag-gray" title="Jockey et/ou entraineur en dessous du seuil de forme (90 derniers jours)">Jockey/entra&icirc;neur : forme insuffisante</span>';
  return ' <span class="small tag-gray" title="Jockey et/ou entraineur non trouve dans les fichiers Ecarts importes">Jockey/entra&icirc;neur : donn&eacute;e inconnue</span>';
}
...
async function verifierCoursesAVenir() {
  ...
  try {
    ...
    const ecartsJockeyEntraineur = await chargerEcartsJockeyEntraineur();
    for (const meeting of meetings) {
      ...
      for (const race of races) {
        ...
        await verifierEtNotifierCourse(meeting, race, toutesPerfs, ecartsJockeyEntraineur);
      }
    }
  } catch (err) {
...
async function verifierEtNotifierCourse(meeting, race, toutesPerfs, ecartsJockeyEntraineur) {
  try {
    ...
    const jeu = jeuSimpleGagnant(result.chevaux, ecartsJockeyEntraineur);
...
async function renderRaceDetail(raceId, useCote8h = false) {
  ...
  appEl.innerHTML = `
    ...
    ${basesEtDangersHtml(basesEtDangers, cotesCibles, result.resume, result.chevaux, context.discipline.canonical, predictionExterne, context.lieu, context.typeCourse, ecartsJockeyEntraineur)}
  ...
  bindJeuSimpleGagnant(result.chevaux, ecartsJockeyEntraineur);
 ...
 *
 * *** v19 (sept. 2026, a la demande de l'utilisateur) : refonte complete -
 * les filtres v10 a v18 (classement Score Global, seuils scoreAptitude/
 * scoreForme, exclusion d'hippodromes, exclusion TypeCourse F en PLAT,
 * score Croisement, mise ponderee scoreAptitude) sont TOUS DESACTIVES ***
 * Remplaces par le filtre MX+OR+MA sur le pool Value&le;-30/cote&ge;3 : un
 * seul candidat -> joue automatiquement ; plusieurs candidats -> depart
 * par dominance stricte sur 3 rubriques du fichier partants (MX et OR, plus
 * haut = meilleur ; MA, plus bas = meilleur) - non jouable si aucun
 * candidat ne domine ainsi. Voir jeuSimpleGagnant.js et HEBERGEMENT.md pour
 * le detail complet de l'exploration qui a mene a ce filtre (le seul,
 * parmi tout ce qui a ete teste, a passer un vrai test de generalisation
 * hors echantillon) et de son edge, qui s'est nettement erode dans le
 * temps (150-250% de rendement en 2024, ~103% sur les 12 derniers mois).
 */
function libelleModeSimpleGagnant(jeu) {
  return `Value &le; -30 (MX+OR+MA)`;
}

function jeuSimpleGagnantHtml(chevaux, ecartsJockeyEntraineur) {
  const jeu = jeuSimpleGagnant(chevaux, ecartsJockeyEntraineur);
  if (!jeu.rentable) {
    const donneesEcartsDisponibles = !!(ecartsJockeyEntraineur && (ecartsJockeyEntraineur.jockeys || ecartsJockeyEntraineur.entraineurs));
    return `<div class="card" style="margin-bottom:10px;">
        ...
        <p class="muted small" style="margin-top:4px;">Aucun cheval Value &le; -30 avec cote &ge; 3, aucun candidat ne domine strictement les autres sur MX+OR+MA, ou (v21) le cheval retenu n'est pas "jockey+entra&icirc;neur en forme"${donneesEcartsDisponibles ? ' (donn&eacute;es Ecarts import&eacute;es, filtre actif)' : ''} (voir HEBERGEMENT.md).</p>
      </div>`;
  }
  const optionsMise = MISES_PRESETS_JEU_SIMPLE_GAGNANT.map((m) => `<option value="${m}">${m}&euro;</option>`).join('');

  const miseJoursAbsenceHtml = jeu.joursAbsenceRang1 != null ? `
      <div class="field" style="margin-bottom:8px;">
        <label style="display:flex; align-items:center; gap:6px; cursor:pointer;">
          <input type="checkbox" id="jsg-mise-ja-check">
          <span>Mise renforc&eacute;e si retour rapide (JA actuel : ${jeu.joursAbsenceRang1}j, seuil &le;${SEUIL_JOURS_ABSENCE_MISE_RENFORCEE}j)</span>
        </label>
      </div>` : '';

  // *** v21 (sept. 2026, a la demande explicite de l'utilisateur : "rend le
  // filtre effectif des a present") : badge "jockey+entraineur en forme" (cf.
  // js/engine/ecartsJockeyEntraineurParser.js) sur le cheval retenu - reflete
  // desormais un GATE reel (jeuSimpleGagnant() ci-dessus), plus un simple
  // affichage indicatif. Voir le commentaire d'en-tete (v21) de
  // jeuSimpleGagnant.js pour le detail et la reserve methodologique. ***
  const badgeFormeHtml = badgeFormeJockeyEntraineurHtml(jeu.principal.chevaux[0], ecartsJockeyEntraineur);

  return `<div class="card" style="margin-bottom:10px;">
      <p class="bold" style="margin:0 0 4px;">Jeu Simple Gagnant</p>
      <p class="small" style="margin:0 0 4px;"><span class="tag-green bold">${libelleModeSimpleGagnant(jeu)}</span> : ${jeu.principal.chevaux.map((c) => `N&deg;${c.entry.numero} (cote ${fmt1(c.cotePourAffichage)})`).join(' - ')}${badgeFormeHtml}</p>
      ...
      ${miseJoursAbsenceHtml}
      <button class="btn btn-secondary btn-block" id="jsg-calculer-btn">Calculer mise</button>
      <div id="jsg-resultat"></div>
      <p class="muted small" style="margin-top:8px;">Mise flat sur le cheval retenu (Dutching &agrave; 1 cheval, rendement = sa cote). Seuils Value/cote calibr&eacute;s sur archives r&eacute;elles (voir HEBERGEMENT.md).${jeu.joursAbsenceRang1 != null ? ' Mise renforc&eacute;e retour rapide (optionnelle, sept. 2026) : x2 si JA &le;7j, x1 sinon &mdash; rendement mesur&eacute; 175,0% (n=103) contre ~118-124% sans ce filtre sur l\'archive compl&egrave;te &mdash; voir HEBERGEMENT.md.' : ''}</p>
    </div>`;
}
 ...
 * de la mise flat sur le cheval retenu pour la mise totale choisie.
 * `ecartsJockeyEntraineur` doit etre le MEME objet que celui passe a
 * `jeuSimpleGagnantHtml` pour cette course (v21 - gate identique, sinon le
 * bouton pourrait rester actif sur un rendu affichant "non jouable").
 */
function bindJeuSimpleGagnant(chevaux, ecartsJockeyEntraineur) {
  ...
  btn.addEventListener('click', () => {
    ...
    let multiplicateurJoursAbsence = 1;
    const jaCheck = appEl.querySelector('#jsg-mise-ja-check');
    if (jeu.joursAbsenceRang1 != null && jaCheck && jaCheck.checked) {
      multiplicateurJoursAbsence = multiplicateurMiseJoursAbsenceRang1(jeu.joursAbsenceRang1);
    }
    miseTotale = miseTotale * multiplicateurJoursAbsence;
    ...
    const miseAptitudeInfoHtml = multiplicateurJoursAbsence !== 1
      ? `<p class="small tag-blue bold" style="margin-top:4px;">Mise renforc&eacute;e appliqu&eacute;e : x${fmt1(multiplicateurJoursAbsence)} (JA ${jeu.joursAbsenceRang1}j)</p>`
      : '';
    resultEl.innerHTML = `
      ...
      ${miseAptitudeInfoHtml}
    `;
  });
}

function basesEtDangersHtml(bd, cotesCibles, r, chevaux, disciplineCanonique, predictionExterne, hippodrome, typeCourse, ecartsJockeyEntraineur) {
  const conseilJeuCard = conseilJeuHtml(bd, chevaux);
  const jeuSimpleGagnantCard = jeuSimpleGagnantHtml(chevaux, ecartsJockeyEntraineur);
...
function renderImport() {
  ...
  appEl.innerHTML = `
    ...
    <div class="card">
      <h3>Ecarts Jockeys/Entraineurs (optionnel)</h3>
      <p class="muted small">Fichiers "Statistiques Ecarts Jockeys/Entraineurs | Turf BZH", au format .xlsx (tel que t&eacute;l&eacute;charg&eacute;, aucune conversion necessaire) ou .csv. Photo des 90 derniers jours &agrave; la date d'export - chaque r&eacute;import REMPLACE enti&egrave;rement le pr&eacute;c&eacute;dent. Sert au badge "Jockey+entra&icirc;neur en forme" (Jeu Simple Gagnant/Liste des courses) : suivi PROSPECTIF, purement indicatif, n'entre dans aucun calcul de Score Global/Value/classement/dominance MX+OR+MA (voir HEBERGEMENT.md pour le d&eacute;tail et la r&eacute;serve m&eacute;thodologique).</p>
      <div class="field"><label>Fichier Jockeys</label><input type="file" id="file-ecarts-jockeys" accept=".xlsx,.csv,text/csv,text/plain,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"></div>
      <div class="field" style="margin-top:8px;"><label>Fichier Entraineurs</label><input type="file" id="file-ecarts-entraineurs" accept=".xlsx,.csv,text/csv,text/plain,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"></div>
    </div>
    ...
    <div class="card">
      ...
      <button class="btn btn-secondary btn-block" id="btn-reset-ecarts-jockey-entraineur" style="margin-top:8px;">Vider les ecarts jockeys/entraineurs importes</button>
    </div>
  ...
  document.getElementById('file-ecarts-jockeys').addEventListener('change', (e) => handleEcartsJockeyEntraineurImport(e.target.files[0], 'jockeys'));
  document.getElementById('file-ecarts-entraineurs').addEventListener('change', (e) => handleEcartsJockeyEntraineurImport(e.target.files[0], 'entraineurs'));
  ...
  document.getElementById('btn-reset-ecarts-jockey-entraineur').addEventListener('click', handleResetEcartsJockeyEntraineur);
}
...
// *** sept. 2026 (a la demande de l'utilisateur) : import des fichiers
// "Statistiques Ecarts Jockeys/Entraineurs | Turf BZH" (cf.
// js/engine/ecartsJockeyEntraineurParser.js). Chaque fichier remplace
// entierement le precedent de son type (photo de l'instant present, pas un
// historique cumulatif - contrairement aux performances). ***
async function handleEcartsJockeyEntraineurImport(file, type) {
  if (!file) return;
  try {
    const csv = await readEcartsFileAsCsv(file);
    const data = Object.fromEntries(parseEcarts(csv));
    const n = Object.keys(data).length;
    if (n === 0) { showImportMessage('Aucune statistique reconnue dans ce fichier.', true); return; }
    await DB.saveEcartsJockeyEntraineur(type, data);
    const label = type === 'jockeys' ? 'jockeys' : 'entraineurs';
    showImportMessage(`Ecarts ${label} importes (${n} ${label}).`, false);
  } catch (err) {
    showImportMessage(`Echec de l'import : ${err.message || err}`, true);
  }
}

async function handleResetEcartsJockeyEntraineur() {
  const ok = confirm('Vider les ecarts jockeys/entraineurs importes ?');
  if (!ok) return;
  await DB.resetEcartsJockeyEntraineur();
  showImportMessage('Ecarts jockeys/entraineurs vides.', false);
}
...
// *** sept. 2026 : les fichiers "Statistiques Ecarts Jockeys/Entraineurs |
// Turf BZH" sont exportes directement en .xlsx par le site. On detecte le
// format (extension, ou a defaut la signature binaire ZIP "PK" que tout
// fichier .xlsx possede) et on convertit la 1ere feuille en CSV via SheetJS
// (js/vendor/xlsx.core.min.js, charge globalement dans index.html) avant de
// reutiliser le parseur CSV existant (parseEcarts) sans le modifier. ***
async function readEcartsFileAsCsv(file) {
  const isXlsxName = /\.xlsx$/i.test(file.name || '');
  const buffer = await file.arrayBuffer();
  const bytes = new Uint8Array(buffer);
  const isZipSignature = bytes.length > 4 && bytes[0] === 0x50 && bytes[1] === 0x4b; // "PK"
  if (isXlsxName || (isZipSignature && typeof XLSX !== 'undefined')) {
    if (typeof XLSX === 'undefined') {
      throw new Error('Lecteur .xlsx indisponible (rechargez la page pour mettre a jour l\'application).');
    }
...
// vue d'ensemble des courses du jour a 8-16 partants (meme fourchette que
// "Top base" - cf. MIN_PARTANTS_FEU_VERT/MAX_PARTANTS_FEU_VERT plus haut),
// triees par heure de depart, avec un decompte en direct pour la prochaine
// course a partir. Contrairement a "Top base", ne filtre PAS sur le score de
// configuration / la confirmation externe : simple liste de consultation
// (pas un conseil de jeu).
//
// *** aout 2026 (a la demande de l'utilisateur) : filtre scoreForme/
// scoreAptitude, pour ne garder que les courses potentiellement eligibles
// au Jeu Simple Gagnant *** - une course n'est retenue que si AU MOINS UN
// cheval du champ a un `scoreForme >= SEUIL_SCORE_FORME_RANG1` (70) ET un
// `scoreAptitude >= SEUIL_SCORE_APTITUDE_RANG1` (85), les 2 memes seuils
// que ceux utilises par `jeuSimpleGagnant()` (jeuSimpleGagnant.js). C'est
// une condition NECESSAIRE mais pas suffisante (le moteur applique en plus
// le classement top2, le seuil de cote >3,8, l'ecart en mode 8h, le score
// Croisement, l'exclusion hippodrome/typeCourse) : cette liste sert donc a
// pre-trier rapidement les reunions du jour sur les courses qui MERITENT
// d'etre regardees en detail, pas a predire lesquelles seront rentables.
//
...
async function renderListeCourses() {
  ...
  for (const meeting of meetings) {
    ...
    for (const race of races) {
      ...
      const result = RaceAnalyzer.analyser(horses, context, false);
      const chevauxEligibles = result.chevaux.filter((c) =>
        typeof c.scoreForme === 'number' && c.scoreForme >= SEUIL_SCORE_FORME_RANG1 &&
        typeof c.scoreAptitude === 'number' && c.scoreAptitude >= SEUIL_SCORE_APTITUDE_RANG1
      );
      if (chevauxEligibles.length === 0) continue;

      const numerosEligibles = chevauxEligibles.map((c) => c.entry.numero);
      // *** v21 (sept. 2026, a la demande explicite de l'utilisateur) : le
      // statut "jockey+entraineur en forme" (cf. ecartsJockeyEntraineurParser.js)
      // est desormais un GATE reel sur le Jeu Simple Gagnant (v20, MX+OR+MA)
      // de cette course - voir jeuSimpleGagnant.js (commentaire d'en-tete v21)
      // pour le detail et la reserve methodologique. ***
      const jeu = jeuSimpleGagnant(result.chevaux, ecartsJockeyEntraineur);
      const badgeFormeHtml = jeu.rentable ? badgeFormeJockeyEntraineurHtml(jeu.principal.chevaux[0], ecartsJockeyEntraineur) : '';
      coursesToutesDates.push({ meeting, race, nbPartants: horseRecords.length, numerosEligibles, badgeFormeHtml });
    }
  }
  ...
  if (courses.length === 0) {
    appEl.innerHTML = `
      ...
      <div class="empty-state">
        ...
        <p class="muted">Cette liste ne retient que les courses de ${MIN_PARTANTS_FEU_VERT} &agrave; ${MAX_PARTANTS_FEU_VERT} partants (m&ecirc;me fourchette que "Top base") comptant au moins un cheval avec scoreForme &ge; ${SEUIL_SCORE_FORME_RANG1} ET scoreAptitude &ge; ${SEUIL_SCORE_APTITUDE_RANG1} (seuils du Jeu Simple Gagnant).</p>
      </div>`;
  ...
  appEl.innerHTML = `
    ...
    <button class="btn btn-secondary btn-block" data-refresh-cotes-liste style="margin-bottom:10px;">Rafra&icirc;chir les cotes (${reunionsAffichees.length} r&eacute;union${reunionsAffichees.length > 1 ? 's' : ''})</button>
    <div id="liste-courses-cotes-status"></div>
    <div class="list-group">${courses.map(({ meeting, race, nbPartants, numerosEligibles, badgeFormeHtml }, i) => {
      const dateVal = new Date(meeting.date).toISOString().slice(0, 10);
      ...
      const numerosHtml = numerosEligibles.map((n) => `N&deg;${n}`).join(' - ');
      return `
      <div class="list-item clickable" data-goto="race/${race.id}">
        <div>
          ...
          <div class="muted small">${escapeHtml(race.discipline)} - ${nbPartants} partants</div>
          <div class="small tag-green bold" style="margin-top:4px; display:inline-block;">${numerosHtml}</div>
          ${badgeFormeHtml ? `<div style="margin-top:4px;">${badgeFormeHtml}</div>` : ''}
  ...
  if (btnRefresh) {
    btnRefresh.addEventListener('click', async () => {
      ...
      for (let i = 0; i < courses.length; i++) {
        ...
        statusEl.innerHTML = `<p class="muted small" style="margin-top:8px;">Mise a jour cotes ${i + 1}/${courses.length} (${escapeHtml(meeting.hippodrome)} - Course ${race.numeroCourse})...</p>`;
        ...
        try {
          ...
          console.error('Rafraichissement cotes (Liste des courses) echoue pour', meeting.hippodrome, race.numeroCourse, err);
          fail++;
        }
        if (i < courses.length - 1) await new Promise((resolve) => setTimeout(resolve, 400));
      }
      statusEl.innerHTML = `<p class="muted small" style="margin-top:8px;">Mise a jour termin&eacute;e : ${ok} course(s) mise(s) &agrave; jour${fail > 0 ? `, ${fail} &eacute;chec(s)` : ''}.</p>`;
    });
  }
    ...
    <div class="card" style="margin-bottom:10px;">
      <div class="field" style="margin-bottom:0;">
        ...
        <select id="jsg-filtre-mode-select">
          ...
          <option value="rang1" ${actuel === 'rang1' ? 'selected' : ''}>Depuis v19 (Value&le;-30, MX+OR+MA)</option>
          <option value="value" ${actuel === 'value' ? 'selected' : ''}>Avant v19 (ancien classement Score Global)</option>
        </select>
      </div>
    </div>`;
}
 ...
 * `renderBilanSimpleGagnant()` pour le contexte complet (bug signale par
 ...
 * @param {Function} onDone - callback de re-rendu (ex. `renderBilanSimpleGagnant`).
 */
...
async function renderBilanSimpleGagnant() {
  ...
  const introHtml = `<div class="card" style="margin-bottom:10px;">
      <p class="small muted" style="margin:0;">Courses dont le Jeu Simple Gagnant est jouable (v20) : parmi tous les chevaux Value &le; -30 (departag&eacute;s par dominance stricte MX+OR+MA si plusieurs, cote&lt;3 incluses dans le d&eacute;partage), le cheval retenu doit avoir cote &ge; 3 pour &ecirc;tre effectivement jou&eacute; - voir la fiche course pour le d&eacute;tail, et HEBERGEMENT.md pour la m&eacute;thode.</p>
    </div>`;
  ...
  if (candidates.length === 0) {
    ...
    bindGoto();
    bindDateFiltreSimpleGagnantSelector(renderBilanSimpleGagnant);
    return;
  }
  ...
  // l'arrivee officielle SEULE (sans toucher aux cotes). Contexte du bug
  // signale : jusqu'ici, la seule facon d'obtenir automatiquement
  // l'arrivee d'une course etait la "mise a jour des cotes" (course ou
  // reunion complete, cf. lignes ~440-461/598-619 plus haut), qui ecrase
  // TOUJOURS `coteDirecte` de chaque cheval avec la cote PMU la plus
  // recente au passage. Une fois la course terminee, cette cote "la plus
  // recente" n'est plus la cote de depart mais une cote deja influencee par
  // le resultat (souvent tres resserree pour le vainqueur) - si elle
  // repasse a <= 3,8, le cheval sort du pool `candidatsDirecte` de
  // `jeuSimpleGagnant()` (jeuSimpleGagnant.js), et la course disparait de
  // cette page (recalculee EN DIRECT a partir des cotes actuelles en base,
  // pas d'apres un instantane fige au moment du pari) - rendant le
  // "Transfert bilan" impossible pour une course pourtant bel et bien
  // gagnee. `fetchResultatPmu` (js/engine/pmuApi.js) permet de recuperer
  // l'arrivee INDEPENDAMMENT des cotes (deja utilise en interne par la
  // mise a jour des cotes, mais jamais expose seul cote a cote) - ce
  // nouveau bouton l'appelle directement et n'ecrit que `race.arriveeBrute`,
  // sans jamais appeler `fetchCotesPmu`/`DB.updateHorses`.
  ...
  const arriveeHtml = enAttenteArrivee.length > 0 ? `
    <div class="card" style="margin-bottom:10px;">
      ...
      <p class="muted small" style="margin-top:6px;">R&eacute;cup&egrave;re uniquement l'arriv&eacute;e officielle de chaque course ci-dessous, SANS toucher aux cotes - &agrave; utiliser &agrave; la place de "mise &agrave; jour des cotes" une fois vos paris plac&eacute;s : mettre &agrave; jour les cotes apr&egrave;s la course peut faire dispara&icirc;tre une course gagn&eacute;e de cette page (la cote directe, d&eacute;j&agrave; influenc&eacute;e par le r&eacute;sultat, peut repasser sous le seuil de 3,8).</p>
  ...
  if (analyseesFiltrees.length === 0) {
    ...
    bindRecupererArriveesSimpleGagnant(enAttenteArrivee, renderBilanSimpleGagnant);
    return;
  }
    ...
    <div class="card" style="margin-top:10px;">
      ...
      ${avecBilan.length > 0
        ? `<p class="bold" style="font-size:1.3em; margin:8px 0 0;">${netGlobal >= 0 ? '+' : ''}${netGlobal.toFixed(2)} &euro;</p>
           ...
           <p class="muted small" style="margin-top:6px;">dont depuis v19 (Value&le;-30, MX+OR+MA) : <span class="${netRang1 >= 0 ? 'tag-green' : 'tag-red'} bold">${netRang1 >= 0 ? '+' : ''}${netRang1.toFixed(2)} &euro;</span>${avecBilanRang1.length > 0 ? ` (${avecBilanRang1.length} course${avecBilanRang1.length > 1 ? 's' : ''})` : ' (aucune course)'}</p>
           <p class="muted small" style="margin-top:2px;">dont avant v19 (ancien classement Score Global) : <span class="${net8h >= 0 ? 'tag-green' : 'tag-red'} bold">${net8h >= 0 ? '+' : ''}${net8h.toFixed(2)} &euro;</span>${avecBilan8h.length > 0 ? ` (${avecBilan8h.length} course${avecBilan8h.length > 1 ? 's' : ''})` : ' (aucune course)'}</p>`
  ...
  // *** sept 2026 *** : la date de la reunion est desormais toujours
  // affichee sur chaque ligne (uniquement quand plusieurs journees sont
  // melangees dans la liste, pour ne pas surcharger l'affichage habituel
  // ou une seule journee est presente) - cf. dateFiltreSimpleGagnantSelectorHtml
  ...
  const rowHtml = (a) => {
    ...
    <div class="list-item clickable" data-goto="race/${a.race.id}">
      <div>
        <div class="bold">${dateLabelHtml}${a.race.heureDepart ? `${escapeHtml(a.race.heureDepart)} - ` : ''}${escapeHtml(a.meeting.hippodrome)} - Course ${a.race.numeroCourse}</div>
...
async function renderBilanGlobalSimpleGagnant() {
  ...
  const modesHtml = `
    <div style="display:flex; gap:10px; flex-wrap:wrap; margin-bottom:${joursSansDetailMode > 0 ? '4px' : '10px'};">
      ${carteMode('Depuis v19 (Value&le;-30, MX+OR+MA)', totalRang1Seul, rendementRang1Seul)}
      ${carteMode('Avant v19 (ancien classement Score Global)', totalClassement8h, rendementClassement8h)}
    </div>
