...
/**
 * "Couplé champ total (Dangers)" : quand la course compte entre 2 et 5
 * Dangers (Value < -10%, cote jouable - cf. calculerBasesEtDangers),
 * propose de jouer TOUTES les combinaisons Couplé Gagnant parmi ces
 * chevaux (le "champ total" des Dangers).
 *
 * Valide sur le backtest reel (3 mois, 3037 courses, voir HEBERGEMENT.md) :
 * a la demande "tester la reussite du Couple Gagnant parmi les Dangers en
 * fonction du nb de partants", on constate que les 2 vrais chevaux du
 * Top2 se trouvent TOUS LES DEUX dans la liste des Dangers bien plus
 * souvent que le hasard ne le laisserait attendre a taille de liste egale
 * (ex. sur l'ensemble des courses : 41,0% observe contre 10,4% attendu au
 * hasard, +30,6 points d'ecart) - un signal robuste, independant du
 * signal Base (seulement 13,3% de recouvrement entre Danger et Base) et
 * qui ne depend pas de la presence d'une Base tres solide (ecart au
 * hasard similaire avec ou sans, ~+30 points dans les deux cas). Restreint
 * a 2-5 Dangers (au-dela, le nombre de combinaisons devient vite trop
 * eleve pour rester un pari raisonnable), le taux de reussite (les 2 vrais
 * Top2 dans le champ total des Dangers) augmente avec le nombre de
 * Dangers/combinaisons :
 *  - 2 Dangers (1 combinaison)   : 21,1% (n=436)
 *  - 3 Dangers (3 combinaisons)  : 33,2% (n=716)
 *  - 4 Dangers (6 combinaisons)  : 47,0% (n=755)
 *  - 5 Dangers (10 combinaisons) : 53,0% (n=572)
 * Purement indicatif : n'entre dans aucun calcul de Score Global/Value/
 * classement.
 * @returns {{dangers:Array, combos:number}|null}
 */
function coupleChampTotalDangers(bd, chevaux) {
  const numsDanger = new Set(bd?.danger || []);
  if (numsDanger.size < 2 || numsDanger.size >= 6) return null;
  const dangers = (chevaux || [])
    .filter((c) => numsDanger.has(c.entry.numero))
    .sort((a, b) => a.classement - b.classement);
  if (dangers.length < 2) return null;
  const combos = (dangers.length * (dangers.length - 1)) / 2;
  return { dangers, combos };
}

const COUPLE_CHAMP_TOTAL_STATS = {
  2: '21,1% de reussite (n=436) - backtest 3 mois, 3037 courses',
  3: '33,2% de reussite (n=716) - backtest 3 mois, 3037 courses',
  4: '47,0% de reussite (n=755) - backtest 3 mois, 3037 courses',
  5: '53,0% de reussite (n=572) - backtest 3 mois, 3037 courses'
};

function coupleChampTotalDangersHtml(bd, chevaux) {
  const suggestion = coupleChampTotalDangers(bd, chevaux);
  if (!suggestion) return '';
  const { dangers, combos } = suggestion;
  const stat = COUPLE_CHAMP_TOTAL_STATS[dangers.length] || '';
  return `<div class="card" style="margin-bottom:10px;">
      <p class="bold" style="margin:0 0 4px;">Coupl&eacute; champ total (Dangers)</p>
      <p class="small" style="margin:0 0 4px;">${dangers.length} Danger(s) : ${dangers.map((d) => `N&deg;${d.entry.numero}`).join(' - ')} (${combos} combinaison${combos > 1 ? 's' : ''} possible${combos > 1 ? 's' : ''})</p>
      <p class="muted small" style="margin-top:4px;">${escapeHtml(stat)}. Indicatif, n'entre dans aucun calcul de Score Global/Value/classement.</p>
    </div>`;
}

/**
 * "Couple Top5 x Dangers" : intersection entre les 5 premiers du
 * classement (Score Global) et la liste des Danger(s). Croisement teste
 * sur le backtest reel (3 mois, 3037 courses, voir HEBERGEMENT.md) a la
 * demande de l'utilisateur : "les 5 premiers du classement contiennent-ils
 * souvent le Couple Gagnant, et le croisement avec les Danger(s) est-il
 * benefique ?"
 *
 * Constat : le Top5 SEUL (10 combinaisons) capture deja les 2 vrais
 * chevaux du Top2 54,4% du temps (94,5% au moins 1) - un tres bon
 * signal a lui seul, sans condition. Restreindre a l'intersection avec
 * les Danger(s) (taille moyenne 3,59, disponible dans 97,5% des courses)
 * fait baisser le taux BRUT a 36,8% (les 2) / 88,5% (au moins 1) - on
 * perd des vrais gagnants qui ne sont pas assez "joues" par le marche
 * pour etre tagues Danger (souvent un favori tres largement domine).
 * MAIS avec deux fois moins de combinaisons en moyenne (5,37 contre 10),
 * le rendement PAR combinaison jouee est meilleur : 6,86%/combinaison
 * contre 5,44%/combinaison pour le Top5 seul (+26% environ). Interessant
 * si l'objectif est de limiter le nombre de tickets plutot que de
 * maximiser le taux de reussite brut - pas benefique si l'objectif est
 * l'inverse (dans ce cas, jouer le Top5 en entier reste meilleur).
 *
 * *** Limitation de combinaisons *** : l'intersection est par
 * construction un sous-ensemble du Top5 (5 chevaux maximum), donc jamais
 * plus de C(5,2) = 10 combinaisons - pas besoin d'un plafond distinct
 * comme pour "Couple champ total (Dangers)" ci-dessus (qui, lui, plafonne
 * a moins de 6 Danger(s) pour la meme raison de cout).
 *
 * @returns {{candidats:Array, combos:number}|null}
 */
function coupleTop5xDangers(bd, chevaux) {
  const numsDanger = new Set(bd?.danger || []);
  const candidats = (chevaux || [])
    .filter((c) => c.classement <= 5 && numsDanger.has(c.entry.numero))
    .sort((a, b) => a.classement - b.classement);
  if (candidats.length < 2) return null;
  const combos = (candidats.length * (candidats.length - 1)) / 2;
  return { candidats, combos };
}

const COUPLE_TOP5_DANGERS_STATS = '36,8% de reussite (les 2 chevaux), 88,5% au moins 1 (n=2962) - environ 5,4 combinaisons en moyenne au lieu de 10 pour le Top5 seul, pour un meilleur rendement par combinaison (6,86% contre 5,44%) - backtest 3 mois, 3037 courses';

function coupleTop5xDangersHtml(bd, chevaux) {
  const suggestion = coupleTop5xDangers(bd, chevaux);
  if (!suggestion) return '';
  const { candidats, combos } = suggestion;
  return `<div class="card" style="margin-bottom:10px;">
      <p class="bold" style="margin:0 0 4px;">Coupl&eacute; Top5 &times; Dangers</p>
      <p class="small" style="margin:0 0 4px;">${candidats.length} candidat(s) : ${candidats.map((c) => `N&deg;${c.entry.numero}`).join(' - ')} (${combos} combinaison${combos > 1 ? 's' : ''} possible${combos > 1 ? 's' : ''} - jamais plus de 10, l'intersection etant limitee au Top5)</p>
      <p class="muted small" style="margin-top:4px;">${escapeHtml(COUPLE_TOP5_DANGERS_STATS)}. Indicatif, n'entre dans aucun calcul de Score Global/Value/classement.</p>
    </div>`;
}

/**
 * Conseil de jeu global de la course : cascade a 3 niveaux, validee sur le
 * backtest reel (3 mois, 3037 courses, voir HEBERGEMENT.md), en reponse a
 * la question "que jouer sur cette course, si quelque chose ?" :
 *
 *  1. L'ancre existe (`chevalConfianceMaximale` : Base tres solide
 *     confirmee, classee n1) -> jouer Coupl&eacute; Gagnant et/ou Trio (cf.
 *     cartes ci-dessous). 39,6% de victoires / 67,9% de Top3 (n=623).
 ...
 *     jouer Simple gagnant/place sur la mieux classee d'entre elles. 42,3%
 *     de victoires / 71,2% de Top3 (n=378) - au moins aussi bon que le
 ...
 *     20,4%/49,1% (n=2036), et meme une "Base solide" confirmee (le cran
 *     en dessous) ne rattrape rien (17,8% de victoires sur n=297, pas
 *     mieux que ne rien filtrer du tout - 21,3%) : pas d'edge recuperable
 *     en dessous du seuil "tres solide", l'abstention est donc le choix
 *     statistiquement justifie, pas juste une prudence par defaut.
 *
 * @returns {{type:'couple_trio', pick:Object}|{type:'simple', pick:Object}|{type:'abstention'}}
 */
function conseilJeu(bd, chevaux) {
  const ancre = chevalConfianceMaximale(bd, chevaux);
  if (ancre) return { type: 'couple_trio', pick: ancre };
  ...
  if (numsTresSolides.size > 0) {
    const candidats = (chevaux || [])
      ...
      .sort((a, b) => a.classement - b.classement);
    if (candidats.length > 0) return { type: 'simple', pick: candidats[0] };
  }
...
const CONSEIL_JEU_STATS = {
  couple_trio: '39,6% de victoires, 67,9% de Top3 quand l\'ancre existe (n=623) - backtest 3 mois, 3037 courses',
  simple: '42,3% de victoires, 71,2% de Top3 pour ce type de pick (n=378) - backtest 3 mois, 3037 courses',
  abstention: 'Sans Base tres solide, le favori du modele tombe a 20,4%/49,1% (n=2036), et une simple Base solide ne rattrape rien (17,8% de victoires, n=297) - backtest 3 mois, 3037 courses'
};
...
function conseilJeuHtml(bd, chevaux) {
  ...
  return `<div class="card" style="margin-bottom:10px; border: 2px solid var(--border);">
      ...
      <p class="muted small" style="margin-top:4px;">${escapeHtml(CONSEIL_JEU_STATS[conseil.type])}. Indicatif, n'entre dans aucun calcul de Score Global/Value/classement.</p>
    </div>`;
}
...
function basesEtDangersHtml(bd, cotesCibles, r, chevaux, disciplineCanonique) {
  ...
  const coupleChampTotal = coupleChampTotalDangersHtml(bd, chevaux);
  const coupleTop5Dangers = coupleTop5xDangersHtml(bd, chevaux);
