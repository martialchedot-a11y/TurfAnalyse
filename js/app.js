...
async function renderCourseFeuVert() {
  ...
  for (const meeting of meetings) {
    ...
    for (const race of races) {
      ...
      candidates.push({ meeting, race, score, suggestion, numerosConfirmes, pool, nbPartants: horses.length, base: suggestionBase.base, predictionExterne });
    }
  }
  ...
  if (candidates.length === 0) {
    appEl.innerHTML = `
      ...
      <div class="empty-state">
        ...
        <p class="muted">Une course apparait ici des que son score de configuration du Coupl&eacute; Value est ${libelleFiltreFeuVert(filtre)}, avec entre ${MIN_PARTANTS_FEU_VERT} et ${MAX_PARTANTS_FEU_VERT} partants, au moins la moitie du champ avec un historique de performances retrouve, ET dont la Base (tres solide) est confirmee par un fichier de pr&eacute;dictions externe import&eacute; pour le bon jour/hippodrome/course (onglet Importer). Sans fichier de pr&eacute;dictions externe import&eacute;, aucune course ne peut appara&icirc;tre ici.</p>
      </div>`;
  ...
  appEl.innerHTML = `
    ...
    <div class="list-group">${candidates.map(({ meeting, race, score, suggestion, numerosConfirmes, pool, nbPartants, base, predictionExterne }) => {
      const niveau = SCORE_CONFIGURATION_NIVEAUX[score];
      return `
  ...
  if (analysees.length === 0) {
    appEl.innerHTML = `
      ...
      <div class="empty-state">
        <div class="icon">\u{1F3AF}</div>
        <p class="bold">Aucune course feu vert pour l'instant</p>
        <p class="muted">Cette page suit la reussite des courses dont le score de configuration Coupl&eacute; Value est ${libelleFiltreFeuVert(filtre)}, avec entre ${MIN_PARTANTS_FEU_VERT} et ${MAX_PARTANTS_FEU_VERT} partants, au moins la moitie du champ avec un historique de performances retrouve, ET dont la Base (tres solide) est confirmee par un fichier de pr&eacute;dictions externe import&eacute; pour le bon jour/hippodrome/course (onglet Importer). Sans fichier de pr&eacute;dictions externe import&eacute;, aucune course ne peut appara&icirc;tre ici.</p>
      </div>`;
