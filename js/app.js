 ...
 * Dangers" suite a un constat fait sur le backtest reel (3 mois, 3037
 * courses, voir HEBERGEMENT.md) : le classement du moteur trie d'abord
 ...
 *
 * Constat chiffre : classer par Value sur tout le champ fait mieux que
 * le Top5 classement, a cout EGAL (10 combinaisons) : 58,2% de reussite
 * (les 2 vrais chevaux du Top2) contre 54,4%. Et malgre l'abandon du
 * critere Danger explicite, rien n'est perdu de ce concept : ce Top5 par
 * Value capture deja naturellement 93,7% des vrais Danger(s) de la
 * course (3,74 sur 3,99 en moyenne) - les deux criteres reposant sur la
...
const COUPLE_VALUE_STATS = '58,2% de reussite (les 2 chevaux) pour 10 combinaisons, contre 54,4% pour le Top5 classement a cout egal - capture aussi 93,7% des vrais Danger(s) de la course - backtest 3 mois, 3037 courses';

function coupleValueHtml(chevaux) {
  const suggestion = coupleValue(chevaux);
  if (!suggestion) return '';
  const { candidats } = suggestion;
  const combos = (candidats.length * (candidats.length - 1)) / 2;
  return `<div class="card" style="margin-bottom:10px;">
      <p class="bold" style="margin:0 0 4px;">Coupl&eacute; Value</p>
      <p class="small" style="margin:0 0 4px;">${candidats.length} candidat(s) (Value croissante, tout le champ) : ${candidats.map((c) => `N&deg;${c.entry.numero}`).join(' - ')} (${combos} combinaison${combos > 1 ? 's' : ''} possible${combos > 1 ? 's' : ''} - jamais plus de 10, limite aux 5 premiers)</p>
...
function basesEtDangersHtml(bd, cotesCibles, r, chevaux, disciplineCanonique) {
  ...
  const coupleValueCard = coupleValueHtml(chevaux);
