  ...
  return `
    <div class="horse-row list-item clickable" data-horse="${c.entry.id}">
      ...
      <div class="horse-value">
        ...
        <div class="sub" title="${escapeHtml('Probabilite de victoire recalculee SANS la cote (forme/aptitude/conditions/similaire uniquement) - purement intrinseque au cheval. Indicatif, n\'entre dans aucun calcul de Score Global/Value/classement.')}">Sans cote ${fmt0(c.probVictoireIntrinseque)}%</div>
      </div>
    </div>
  `;
}
...
async function renderHorseDetail(raceId, horseId) {
  ...
  appEl.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="resume-line" title="${escapeHtml('Meme modele (Plackett-Luce) applique a un Score Global recalcule SANS la composante cote (forme/aptitude/conditions/similaire uniquement, poids renormalises) - purement intrinseque au cheval, sans le jugement du marche. Indicatif, n\'entre dans aucun calcul de Score Global/Value/classement.')}"><div class="label">Probabilite victoire (sans cote)</div><div>${fmt1(c.probVictoireIntrinseque)}%</div></div>
