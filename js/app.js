...
async function renderBilanSimpleGagnant() {
  ...
  const candidatesFiltres = candidates.filter((c) => matchFiltreModeSimpleGagnant(c.jeu.rang1Value, filtreActuel));

  if (candidatesFiltres.length === 0) {
    appEl.innerHTML = `
      ${bilanGlobalLinkHtml}
      ${introHtml}
      ${filtreHtml}
  ...
  candidatesFiltres.sort((a, b) => minutesDepart(a.race.heureDepart) - minutesDepart(b.race.heureDepart));

  const analysees = candidatesFiltres.map(({ meeting, race, nbPartants, jeu }) => {
    const pool = jeu.recommande;
    const ordreArrivee = CSVImporter.parseOrdreArrivee(race.arriveeBrute || '');
