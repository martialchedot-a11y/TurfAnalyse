...
// NP/4, NP/2, NP, NPx2 - plutot que sur la Value), compte combien des 4
...
// grimpe avec ce nombre de confirmations (gradient quasi lineaire, aussi
// net que celui du score de configuration). Purement indicatif et affiche
// separement : n'entre dans aucun calcul de Score Global/Value/score de
// configuration/classement (les seuils 2-5 deja verifies de Course feu
// vert/Resultat restent inchanges).
// -------------------------------------------------------------------
function nbConfirmationsCotesCibles(candidats, cotesCibles) {
  if (!Array.isArray(candidats) || !Array.isArray(cotesCibles)) return 0;
...
function confirmationCotesCiblesHtml(nbConfirm) {
  if (nbConfirm == null) return '';
  ...
  return `<span class="small ${niveau.cls} bold" title="${escapeHtml(titre)}">${escapeHtml(niveau.label)}</span>`;
}
...
function coupleValueHtml(bd, chevaux, cotesCibles) {
  ...
  const nbConfirm = nbConfirmationsCotesCibles(candidats, cotesCibles);
  return `<div class="card" style="margin-bottom:10px;">
      <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:6px;">
        ...
        <div style="display:flex; gap:6px; flex-wrap:wrap; justify-content:flex-end;">
          ...
          ${confirmationCotesCiblesHtml(nbConfirm)}
        </div>
      </div>
...
// Value (cf. scoreConfigurationCoupleValue) est >= au seuil choisi par
// l'utilisateur (cf. getSeuilFeuVert ci-dessous, 2 a 5, defaut 4/5), ET
// dont le nombre de partants est compris entre 8 et 16 (a la demande de
...
// Seuil de confiance minimum pour Course feu vert/Resultat, choisi par
// l'utilisateur (2 a 5) et memorise dans ce navigateur (localStorage) - a
// la demande de l'utilisateur, qui souhaite pouvoir elargir ou resserrer
// la liste depuis l'app, sans intervention sur le code. Les deux pages
// partagent le meme seuil pour rester coherentes entre elles.
const SEUIL_FEU_VERT_DEFAUT = 4;
const SEUIL_FEU_VERT_CLE_STOCKAGE = 'turf-seuil-feu-vert';

function getSeuilFeuVert() {
  const brut = Number(localStorage.getItem(SEUIL_FEU_VERT_CLE_STOCKAGE));
  if (Number.isInteger(brut) && brut >= 2 && brut <= 5) return brut;
  return SEUIL_FEU_VERT_DEFAUT;
}

function setSeuilFeuVert(valeur) {
  const v = Number(valeur);
  if (!Number.isInteger(v) || v < 2 || v > 5) return;
  localStorage.setItem(SEUIL_FEU_VERT_CLE_STOCKAGE, String(v));
}

function seuilFeuVertSelectorHtml() {
  const actuel = getSeuilFeuVert();
  const options = [2, 3, 4, 5]
    .map((v) => `<option value="${v}" ${v === actuel ? 'selected' : ''}>${v}/5 et plus</option>`)
    .join('');
  return `
    <div class="card" style="margin-bottom:10px;">
      <div class="field" style="margin-bottom:0;">
        <label for="seuil-feu-vert-select">Indice de confiance minimum</label>
        <select id="seuil-feu-vert-select">${options}</select>
      </div>
    </div>`;
}
...
async function renderCourseFeuVert() {
  ...
  for (const meeting of meetings) {
    ...
    for (const race of races) {
      ...
      const cotesCibles = calculerCotesCibles(result.chevaux, context.nbPartants);
      const nbConfirm = suggestion ? nbConfirmationsCotesCibles(suggestion.candidats, cotesCibles) : null;
      candidates.push({ meeting, race, score, suggestion, nbConfirm, nbPartants: horses.length });
    }
  }
  ...
  if (candidates.length === 0) {
    appEl.innerHTML = `
      ...
      <div class="empty-state">
        ...
        <p class="muted">Une course apparait ici des que son score de configuration du Coupl&eacute; Value atteint ${seuil}/5, avec entre ${MIN_PARTANTS_FEU_VERT} et ${MAX_PARTANTS_FEU_VERT} partants et au moins la moitie du champ avec un historique de performances retrouve.</p>
      </div>`;
  ...
  appEl.innerHTML = `
    ...
    <div class="list-group">${candidates.map(({ meeting, race, score, suggestion, nbConfirm, nbPartants }) => {
      const niveau = SCORE_CONFIGURATION_NIVEAUX[score];
      return `
...
// de configuration Coupl&eacute; Value >= au seuil choisi, cf. getSeuilFeuVert),
...
async function renderResultatJournee() {
  ...
  if (analysees.length === 0) {
    appEl.innerHTML = `
      ...
      <div class="empty-state">
        <div class="icon">\u{1F3AF}</div>
        <p class="bold">Aucune course feu vert pour l'instant</p>
        <p class="muted">Cette page suit la reussite des courses dont le score de configuration Coupl&eacute; Value est >= ${seuil}/5, avec entre ${MIN_PARTANTS_FEU_VERT} et ${MAX_PARTANTS_FEU_VERT} partants et au moins la moitie du champ avec un historique de performances retrouve.</p>
      </div>`;
  ...
  const tauxHtml = avecResultat.length > 0
    ? `<div class="card" style="text-align:center;">
        ...
        <p class="muted small" style="margin-top:2px;">Coupl&eacute; Value reussi (${Math.round((reussies.length / avecResultat.length) * 100)}%) sur les courses feu vert (score >= ${seuil}/5) avec resultat connu</p>
      </div>`
