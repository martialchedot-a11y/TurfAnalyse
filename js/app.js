...
function envoyerNotificationJsg(meeting, race, jeu) {
  ...
  const modeLabel = jeu.principal && pool === jeu.principal
    ? (jeu.rang1Value ? '1er du classement seul' : 'cheval value seul')
    : `Dutching ${pool.n} chevaux`;
 ...
 * n'importe quelle course des lors qu'au moins un cheval parmi les rangs
 * 1/2/4/5 du classement Score Global a une cote reelle au-dessus de son
 * seuil.
 *
 * *** v4 (aout 2026) *** : le 1er du classement (rang 1) est TOUJOURS
 * prioritaire des qu'il depasse son seuil (cote > 3,8) - c'est de loin le
 * signal le plus rentable pris seul (164,0% de rendement reel, backtest 8
 * mois). Si d'autres chevaux (rangs 2, 4, 5 - le rang 3 est exclu, jamais
 * rentable seul) depassent AUSSI leur propre seuil, un Dutching combinant
 * tous ces chevaux est propose EN PLUS, en alternative (120,2% de rendement
 * reel sur l'echantillon, mais marge plus fine que le rang 1 seul).
 */
function libelleModeSimpleGagnant(jeu, pool) {
  if (pool === jeu.principal) {
    return jeu.rang1Value ? '1er du classement seul' : 'Cheval value seul (rang hors 1)';
  }
  return `Dutching (${pool.n} chevaux)`;
}
...
function jeuSimpleGagnantHtml(chevaux) {
  ...
  if (!jeu.rentable) {
    return `<div class="card" style="margin-bottom:10px;">
        ...
        <p class="muted small" style="margin-top:4px;">Aucun cheval parmi les rangs 1, 2, 4 ou 5 du classement Score Global ne d&eacute;passe son seuil de cote (rang 1 : 3,8 &middot; rang 2 : 5,2 &middot; rang 4 : 9,4 &middot; rang 5 : 12,5 - le rang 3 est exclu, non rentable seul par backtest). Indicatif, n'entre dans aucun calcul de Score Global/Value/classement.</p>
      </div>`;
  }
  const optionsMise = MISES_PRESETS_JEU_SIMPLE_GAGNANT.map((m) => `<option value="${m}">${m}&euro;</option>`).join('');

  const blocPrincipal = jeu.principal ? `
      <p class="small" style="margin:0 0 4px;"><span class="tag-green bold">${libelleModeSimpleGagnant(jeu, jeu.principal)}</span> : ${jeu.principal.chevaux.map((c) => `N&deg;${c.entry.numero} (cote ${fmt1(c.cotePourAffichage)})`).join(' - ')}</p>
  ...
  const blocAlternative = jeu.alternative ? `
      <p class="small" style="margin:${jeu.principal ? '8px 0 4px' : '0 0 4px'};"><span class="${jeu.principal ? 'tag-blue' : 'tag-green'} bold">Alternative : Dutching ${jeu.alternative.n} chevaux</span> : ${jeu.alternative.chevaux.map((c) => `N&deg;${c.entry.numero} (cote ${fmt1(c.cotePourAffichage)})`).join(' - ')}</p>
      <p class="small" style="margin:0 0 8px;">Rendement esp&eacute;r&eacute; : <span class="${jeu.alternative.rendement >= 1 ? 'tag-green' : 'tag-orange'} bold">${(jeu.alternative.rendement * 100).toFixed(1)}%</span></p>` : '';

  const poolChoix = jeu.principal && jeu.alternative ? 'principal' : (jeu.principal ? 'principal' : 'alternative');

  return `<div class="card" style="margin-bottom:10px;">
      <p class="bold" style="margin:0 0 4px;">Jeu Simple Gagnant</p>
      ${blocPrincipal}
      ${blocAlternative}
      ${jeu.principal && jeu.alternative ? `
      <div class="field" style="margin-bottom:8px;">
        <label for="jsg-choix-select">Mise sur</label>
        <select id="jsg-choix-select">
          <option value="principal">${libelleModeSimpleGagnant(jeu, jeu.principal)}</option>
          <option value="alternative">Alternative (Dutching ${jeu.alternative.n} chevaux)</option>
        </select>
      </div>` : ''}
      <div class="field" style="margin-bottom:8px;">
      ...
      <button class="btn btn-secondary btn-block" id="jsg-calculer-btn" data-choix-defaut="${poolChoix}">Calculer mises</button>
      <div id="jsg-resultat"></div>
      <p class="muted small" style="margin-top:8px;">M&eacute;thode Dutching : la mise de chaque cheval est proportionnelle &agrave; 1/cote, pour un gain quasi identique quel que soit celui des chevaux retenus qui gagne (mises arrondies &agrave; l'euro pour rester jouables au guichet, d'o&ugrave; un l&eacute;ger &eacute;cart de gain d'un cheval &agrave; l'autre - avec un seul cheval, la mise est simplement flat sur ce cheval). Seuils de cote par rang calibr&eacute;s sur 8 mois d'archives r&eacute;elles (voir HEBERGEMENT.md). Indicatif, n'entre dans aucun calcul de Score Global/Value/classement.</p>
    </div>`;
}
 ...
 * des mises Dutching pour le pool (principal ou alternative) et la mise
 * totale choisis dans les menus deroulants.
 */
function bindJeuSimpleGagnant(chevaux) {
  ...
  btn.addEventListener('click', () => {
    const selectChoix = appEl.querySelector('#jsg-choix-select');
    const choix = selectChoix ? selectChoix.value : btn.dataset.choixDefaut;
    const pool = choix === 'alternative' ? jeu.alternative : jeu.principal;
  ...
  const rowHtml = (a) => `
    <div class="list-item clickable" data-goto="race/${a.race.id}">
      <div>
        ...
        <div class="muted small">${libelleModeSimpleGagnant(a.jeu, a.pool)} (n&deg;${a.pool.chevaux.map((c) => c.entry.numero).join('-')})${a.connu ? ` - Vainqueur n&deg;${a.vrai1}` : ' - En attente de resultat'}</div>
      </div>
      <div style="display:flex; flex-direction:column; align-items:flex-end; gap:2px;">
        ${a.connu
