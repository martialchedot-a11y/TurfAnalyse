...
import { fetchCotesPmu, fetchResultatPmu, fetchRapportsPmu, extraireRapportsSimpleGagnant, extraireRapportsSimplePlace, extraireRapportsCoupleGagnant } from './engine/pmuApi.js';
...
const TABS = [
  ...
  { id: 'couplerentable', label: 'Couplé rentable', icon: '\u{1F4B0}' },
...
function currentTab(parts) {
  const known = ['meetings', 'import', 'feuvert', 'couplerentable', 'race'];
  if (parts.length === 0) return 'meetings';
  if (parts[0] === 'race') return 'meetings';
  if (parts[0] === 'resultat') return 'feuvert';
  if (parts[0] === 'coupleresultat') return 'couplerentable';
...
async function render() {
  ...
  try {
    ...
    } else if (parts[0] === 'couplerentable') {
      await renderCoupleRentable();
    } else if (parts[0] === 'coupleresultat') {
      await renderCoupleRentableResultat();
    } else {
...
function trioValueAvecBaseHtml(bd, chevaux, cotesCibles, predictionExterne) {
  ...
  return `<div class="card" style="margin-bottom:10px;">
      ...
      <p class="small tag-red bold" style="margin:4px 0;">&#9888; ROI negatif mesure sur rapports PMU reels (juillet 2026, 446 courses) : -38% (mise 4436e / gain 2767e). Ce pari n'est plus recommande par le Conseil de jeu ci-dessus - carte laissee a titre informatif.</p>
 ...
 * @returns {{mise:number, gain:number, net:number}}
 */
function bilanSimpleBase(base, nbPartants, rapportsGagnant, rapportsPlace) {
  ...
  const mise = MISE_PAR_COMBINAISON_FEU_VERT + misePlace;
  const gagnant = (rapportsGagnant || []).find((r) => r.numero === base.entry.numero);
  const place = (rapportsPlace || []).find((r) => r.numero === base.entry.numero);
  let gain = 0;
  if (gagnant) gain += gagnant.dividende * MISE_PAR_COMBINAISON_FEU_VERT;
  if (place && misePlace > 0) gain += place.dividende * misePlace;
  return { mise, gain, net: gain - mise };
}

/**
 * *** Nouveau *** (a la demande de l'utilisateur : "simuler en plus le
 * report place sur 3 course (toutes combinaisons possibles)") : un
 * "report" consiste a miser sur le Simple Place de la Base d'une course,
 * puis, si elle est placee, a rejouer l'INTEGRALITE du gain sur la Base
 * d'une course suivante, et ainsi de suite sur 3 courses. Simule TOUTES
 * les combinaisons possibles de 3 courses distinctes parmi celles au
 * rapport connu (avecBilan), triees dans l'ordre chronologique de leur
 * heure de depart au sein de chaque combinaison (obligatoire : on ne peut
 * miser sur une course qu'apres connaitre le resultat de la precedente -
 * l'ordre de selection des 3 courses n'a donc pas d'importance, seul
 * l'ordre chronologique compte).
 * @param {Array} avecBilan - sous-ensemble de `analysees` avec
 *   rapportStatus === 'ok' (check et bilan connus).
 * @param {number} miseDepart
 * @returns {{nbCourses:number, combos:Array<{legs:Array, reussi:boolean, montantFinal:number|null, net:number}>}}
 *   combos tries par net decroissant (meilleur report en tete).
 */
function simulerReportPlace(avecBilan, miseDepart = MISE_PAR_COMBINAISON_FEU_VERT) {
  // Pour chaque course : la Base est-elle placee, et a quel dividende
  // Simple Place ? Exclut les rares cas ou le rapport Place attendu est
  // absent malgre une place reelle (donnee PMU incomplete).
  const eligibles = avecBilan
    .map((a) => {
      const numero = a.check.base.entry.numero;
      const trouve = (a.race.rapportSimplePlace || []).find((r) => r.numero === numero);
      const dividende = trouve ? trouve.dividende : null;
      if (a.check.place && dividende == null) return null;
      return { a, minutes: minutesDepart(a.race.heureDepart), placee: a.check.place, dividende };
    })
    .filter(Boolean);
  ...
  for (let i = 0; i < eligibles.length; i++) {
    for (let j = i + 1; j < eligibles.length; j++) {
      for (let k = j + 1; k < eligibles.length; k++) {
        const legs = [eligibles[i], eligibles[j], eligibles[k]].sort((x, y) => x.minutes - y.minutes);
        let montant = miseDepart;
        let reussi = true;
        for (const leg of legs) {
          if (leg.placee) {
            montant *= leg.dividende;
          } else {
            reussi = false;
            break;
          }
        }
        combos.push({ legs: legs.map((l) => l.a), reussi, montantFinal: reussi ? montant : null, net: (reussi ? montant : 0) - miseDepart });
      }
    }
  }

  combos.sort((x, y) => y.net - x.net);
  return { nbCourses: eligibles.length, combos };
}

/**
 * Affiche la simulation du report Place (cf. simulerReportPlace) : nombre
 * de combinaisons testees/reussies, puis chaque combinaison (les 3
 * courses dans l'ordre chronologique, reussie ou non, montant final).
 * @param {{nbCourses:number, combos:Array}} resultat
 * @param {number} miseDepart
 * @returns {string}
 */
function reportPlaceHtml(resultat, miseDepart) {
  const { nbCourses, combos } = resultat;
  if (nbCourses < 3) {
    return `<div class="card" style="margin-top:10px;">
      <p class="bold" style="margin:0;">Simulation report Place (3 courses)</p>
      <p class="muted small" style="margin-top:4px;">Pas assez de courses feu vert avec rapport connu pour simuler un report sur 3 courses (${nbCourses}/3 minimum). R&eacute;cup&eacute;rez d'abord les rapports ci-dessus.</p>
    </div>`;
  }

  const reussis = combos.filter((c) => c.reussi);
  // Format demande par l'utilisateur, plus lisible que la version
  // precedente (nom d'hippodrome + heure) : "R<reunion>-<course><numero du
  // cheval sur 2 chiffres>", ex. R1-112 = Reunion 1, Course 1, N°12.
  const legLabel = (a) => `R${a.meeting.numeroReunion}-${a.race.numeroCourse}${String(a.check.base.entry.numero).padStart(2, '0')}`;
  const rows = combos.map((c, i) => {
    const rapport = c.reussi ? `${c.montantFinal.toFixed(2)}&euro;` : 'perdu';
    return `<p class="small" style="margin:4px 0;"><span class="bold">report${i + 1}:</span> ${c.legs.map(legLabel).join('/')} &nbsp; rapport : <span class="${c.reussi ? 'tag-green' : 'tag-red'} bold">${rapport}</span></p>`;
  }).join('');

  return `
    <div class="card" style="margin-top:10px;">
      <p class="bold" style="margin:0;">Simulation report Place (3 courses)</p>
      <p class="muted small" style="margin-top:4px;">${miseDepart}&euro; mis&eacute; sur la Base (Simple Place) d'une course, gain int&eacute;gralement rejou&eacute; sur la Base d'une course suivante puis d'une 3e, dans l'ordre chronologique - toutes les combinaisons possibles parmi les ${nbCourses} courses feu vert au rapport connu ce jour (${combos.length} combinaison(s)). ${reussis.length}/${combos.length} r&eacute;ussie(s). Format d'une &eacute;tape : R(r&eacute;union)-(course)(num&eacute;ro du cheval).</p>
...
// *** Signal "jeu conseille" (aout 2026) *** : a la demande de l'utilisateur,
// suite au tableau ROI reel par profil confiance x confirmations (2 mois,
// janvier + juillet 2026, rapports PMU reels), 3 profils se sont demarques
// comme rentables :
//   - faible-moyenne : +61% de ROI (n=337 courses - echantillon le plus
//     large et le plus solide des 3)
//   - forte-moyenne  : +75% de ROI (n=41 courses - mais domine par un seul
//     rapport extreme a 299e ; sans lui, ROI -18%. Fragile.)
//   - moyenne-faible : +48% de ROI (n=12 courses seulement - trop peu pour
//     conclure statistiquement)
// *** Resultat exploratoire, pas encore valide *** : mesure sur seulement 2
// mois d'archives. Marty doit fournir d'autres mois pour confirmer avant de
// considerer ce signal fiable - voir HEBERGEMENT.md pour le detail complet
// et les reserves methodologiques (petit echantillon, outlier).
...
function jeuConseilleHtml(cle) {
  ...
  const detail = 'Profil identifie comme rentable sur un premier echantillonnage de rapports PMU reels (2 mois, janvier + juillet 2026) - resultat exploratoire, en cours de validation sur davantage de mois d\'archive. Voir HEBERGEMENT.md';
  return `<p class="small tag-green bold" style="margin:4px 0;" title="${escapeHtml(detail)}">&#9733; Jeu conseill&eacute; (profil rentable sur rapports r&eacute;els - exploratoire)</p>`;
}
...
async function renderResultatJournee() {
  ...
  const tauxHtml = avecResultat.length > 0
    ? `<div class="card" style="text-align:center;">
        ...
        <p class="muted small" style="margin-top:2px;">Base reussie (Gagnant ou Place, ${Math.round((reussies.length / avecResultat.length) * 100)}%) sur les courses feu vert (score ${libelleFiltreFeuVert(filtre)}) avec resultat connu</p>
      </div>`
  ...
  const bilanHtml = avecResultat.length === 0 ? '' : `
    <div class="card" style="margin-top:10px;">
      <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:6px;">
        <p class="bold" style="margin:0;">Bilan financier (hypoth&eacute;tique) Simple Gagnant/Place (Base)</p>
      ...
      ${avecBilan.length > 0
        ...
        : `<p class="muted small" style="margin-top:8px;">Cliquez sur "R&eacute;cup&eacute;rer les rapports" pour calculer le bilan financier &agrave; partir des dividendes PMU officiels (Simple Gagnant/Place).</p>`}
      <div id="rapports-status"></div>
      <p class="muted small" style="margin-top:8px;">Hypoth&eacute;tique : suppose ${MISE_PAR_COMBINAISON_FEU_VERT}&euro; jou&eacute; sur la Base en Simple Gagnant ET ${MISE_PAR_COMBINAISON_FEU_VERT}&euro; en Simple Place (2&euro; par course, 1&euro; seulement si le march&eacute; Place n'existe pas - moins de 4 partants), sur toutes les courses feu vert du jour &mdash; ce n'est pas un historique de vos mises reelles. Les courses sans aucun rapport disponible sont exclues du bilan. Ne remplace pas votre propre jugement.</p>
    </div>`;

  const reportPlace = simulerReportPlace(avecBilan, MISE_PAR_COMBINAISON_FEU_VERT);
  const reportHtml = avecResultat.length === 0 ? '' : reportPlaceHtml(reportPlace, MISE_PAR_COMBINAISON_FEU_VERT);
...
// -------------------------------------------------------------------
// COUPLE RENTABLE (aout 2026, a la demande de l'utilisateur) : regroupe
// les courses dont le profil confiance x confirmations du pool adaptatif
// Coupl&eacute; Value fait partie des 3 profils identifies comme rentables
// sur un premier echantillonnage de rapports PMU reels (2 mois, janvier +
// juillet 2026 - voir PROFILS_RENTABLES_COUPLE_VALUE et le badge "Jeu
// conseille" plus haut, sur la fiche course) : forte-moyenne, moyenne-
// faible, faible-moyenne. *** Resultat exploratoire ***, en cours de
// validation sur davantage de mois d'archive - voir HEBERGEMENT.md.
//
// Contrairement a Course feu vert (qui filtre en plus sur le nombre de
// partants, la couverture d'historique ET une confirmation externe sur la
// Base), ce nouveau filtre reste volontairement IDENTIQUE a la population
// mesuree par le backtest ROI (aucune de ces restrictions n'y etait
// appliquee) : seule condition, un pool Coupl&eacute; Value d'au moins 2
// candidats (>= 5 partants).
// -------------------------------------------------------------------

/**
 * Calcule, pour une course deja analysee (bd/chevaux/cotesCibles), le
 * profil du pool adaptatif Coupl&eacute; Value et le pool lui-meme.
 * Factorise entre renderCoupleRentable et renderCoupleRentableResultat.
 * @returns {{score:number, pool:Object, candidats:Array, combos:number}|null}
 *   null si le champ est trop petit pour un pool Value (< 2 candidats), ou
 *   si le profil de la course ne fait pas partie des 3 profils rentables.
 */
function profilCoupleRentable(bd, chevaux, cotesCibles) {
  const base5 = coupleValue(chevaux, 5);
  if (!base5) return null;
  const score = scoreConfigurationCoupleValue(bd, chevaux);
  const numerosConfirmes = confirmationsCotesCibles(base5.candidats, cotesCibles);
  const pool = poolAdaptatifCoupleValue(score, numerosConfirmes.length);
  if (pool.n == null || !PROFILS_RENTABLES_COUPLE_VALUE.has(pool.cle)) return null;
  const suggestion = pool.n === 5 ? base5 : coupleValue(chevaux, pool.n);
  const candidats = suggestion.candidats;
  const combos = (candidats.length * (candidats.length - 1)) / 2;
  return { score, pool, candidats, combos };
}

// Detail du ROI mesure par profil (cf. tableau complet dans HEBERGEMENT.md),
// affiche en info-bulle/texte sur la page liste.
const PROFILS_RENTABLES_ROI = {
  'forte-moyenne': '+75% de ROI mesure (n=41 courses - resultat fragile : domine par un seul rapport isole a 299e, sans lui -18%)',
  'moyenne-faible': '+48% de ROI mesure (n=12 courses seulement sur 2 mois - trop peu pour conclure statistiquement)',
  'faible-moyenne': '+61% de ROI mesure (n=337 courses - le resultat le plus solide des 3, plus large volume et plus large echantillon de rapports)'
};

/**
 * Reconstruit, pour toutes les courses importees, la liste des candidates
 * "Couple rentable" (profil dans PROFILS_RENTABLES_COUPLE_VALUE). Factorise
 * entre renderCoupleRentable (liste) et renderCoupleRentableResultat
 * (bilan), qui ont besoin exactement de la meme boucle/du meme filtre.
 * @returns {Promise<Array>} un element par course candidate : { meeting,
 *   race, score, pool, candidats, combos }.
 */
async function collecterCandidatesCoupleRentable() {
  const meetings = await DB.getAllMeetings();
  const toutesPerfs = await DB.getAllPerformances();
  const candidates = [];
  ...
  for (const meeting of meetings) {
    ...
    for (const race of races) {
      ...
      const bd = calculerBasesEtDangers(result.chevaux, context.discipline.canonical);
      const cotesCibles = calculerCotesCibles(result.chevaux, context.nbPartants);
      const profil = profilCoupleRentable(bd, result.chevaux, cotesCibles);
      if (!profil) continue;

      candidates.push({ meeting, race, nbPartants: horses.length, ...profil });
    }
  }
  return candidates;
}

async function renderCoupleRentable() {
  renderTopbar('Couplé rentable');
  const meetings = await DB.getAllMeetings();
  ...
  if (meetings.length === 0) {
    appEl.innerHTML = `
      <div class="empty-state">
        ...
        <p class="bold">Aucune reunion importee</p>
        <p class="muted">Importez une reunion depuis l'onglet "Importer" pour voir apparaitre ici les courses au profil rentable.</p>
      </div>`;
    return;
  }

  const candidates = await collecterCandidatesCoupleRentable();

  const introHtml = `<div class="card" style="margin-bottom:10px;">
      <p class="small muted" style="margin:0;">3 profils confiance/confirmations identifi&eacute;s comme rentables sur un premier &eacute;chantillonnage de rapports PMU r&eacute;els (2 mois, janvier + juillet 2026) : <strong>forte-moyenne</strong> (+75%), <strong>moyenne-faible</strong> (+48%), <strong>faible-moyenne</strong> (+61%). R&eacute;sultat exploratoire, en cours de validation sur davantage de mois d'archive - voir HEBERGEMENT.md.</p>
    </div>`;
  ...
  if (candidates.length === 0) {
    appEl.innerHTML = `
      ...
      <div class="empty-state">
        <div class="icon">\u{1F4B0}</div>
        <p class="bold">Aucune course au profil rentable pour l'instant</p>
        <p class="muted">Une course appara&icirc;t ici d&egrave;s que son profil confiance/confirmations (pool adaptatif Coupl&eacute; Value) est forte-moyenne, moyenne-faible ou faible-moyenne.</p>
      </div>`;
    return;
  }
  ...
  appEl.innerHTML = `
    ${introHtml}
    <button class="btn btn-secondary btn-block" data-goto="coupleresultat" style="margin-bottom:10px;">Voir la r&eacute;ussite du jour</button>
    <div class="list-group">${candidates.map(({ meeting, race, score, pool, candidats, combos, nbPartants }) => {
      const niveau = SCORE_CONFIGURATION_NIVEAUX[score];
      const roiDetail = PROFILS_RENTABLES_ROI[pool.cle] || '';
      return `
      <div class="list-item clickable" data-goto="race/${race.id}">
        <div>
          <div class="bold">${race.heureDepart ? `${escapeHtml(race.heureDepart)} - ` : ''}${escapeHtml(meeting.hippodrome)} - Course ${race.numeroCourse}</div>
          <div class="muted small">${escapeHtml(race.discipline)} - ${nbPartants} partants</div>
          <div style="margin-top:4px; display:flex; gap:6px; flex-wrap:wrap;">
            <span class="small ${niveau.cls} bold" title="${escapeHtml(niveau.label)} (${score}/5)">${escapeHtml(niveau.label)} (${score}/5)</span>
            <span class="small tag-green bold" title="${escapeHtml(roiDetail)}">Profil ${escapeHtml(pool.cle)}</span>
            <span class="small muted">${candidats.length} candidats (${combos} combinaison${combos > 1 ? 's' : ''})</span>
          </div>
        </div>
        <div class="muted">&rsaquo;</div>
      </div>`;
...
/**
 * Cherche, dans un tableau de rapports Coupl&eacute; Gagnant (cf.
 * extraireRapportsCoupleGagnant), l'entree correspondant exactement &agrave;
 * la paire gagnante {vrai1, vrai2} (ordre indifferent). Renvoie le
 * dividende (pour 1&euro;) si trouve, sinon `null` (course terminee mais
 * Coupl&eacute; Gagnant non propose par le PMU, ex. tres petit peloton -
 * seul le Coupl&eacute; Ordre est alors offert).
 * @param {Array<{numeros:number[], dividende:number}>|undefined} rapports
 * @param {number} vrai1
 * @param {number} vrai2
 * @returns {number|null}
 */
function rapportCoupleCorrespondant(rapports, vrai1, vrai2) {
  if (!Array.isArray(rapports)) return null;
  const cible = [vrai1, vrai2].sort((a, b) => a - b).join('-');
  const trouve = rapports.find((r) => Array.isArray(r.numeros) && [...r.numeros].sort((a, b) => a - b).join('-') === cible);
  return trouve ? trouve.dividende : null;
}

/**
 * Page "Reussite du jour" de Couple rentable : sur le meme principe que
 * renderResultatJournee (Course feu vert), mais le bilan financier porte
 * ici sur le pool Coupl&eacute; Value adapte au profil (mise = nombre de
 * combinaisons du pool, 1&euro; par combinaison), et le rapport recupere
 * est celui du pari Coupl&eacute; Gagnant (extraireRapportsCoupleGagnant),
 * pas Simple Gagnant/Place.
 *
 * *** Optimisation par rapport a Course feu vert *** : une course perdue
 * (le pool ne capture pas les 2 vrais chevaux) n'a JAMAIS besoin d'aller
 * chercher le rapport officiel pour connaitre son bilan (mise perdue quel
 * que soit le dividende du gagnant) - seules les courses GAGNEES declenchent
 * une requete "Recuperer les rapports", ce qui reduit le nombre d'appels
 * reseau par rapport a la page Course feu vert.
 */
async function renderCoupleRentableResultat() {
  renderTopbar('Réussite Couplé rentable', { back: () => navigate('couplerentable') });
  const meetings = await DB.getAllMeetings();
  ...
  if (meetings.length === 0) {
    appEl.innerHTML = `
      <div class="empty-state">
        <div class="icon">\u{1F4B0}</div>
        <p class="bold">Aucune reunion importee</p>
        <p class="muted">Importez une reunion depuis l'onglet "Importer" pour suivre ici la reussite du profil Couple rentable.</p>
      </div>`;
    return;
  }
  ...
  if (candidates.length === 0) {
    appEl.innerHTML = `
      <div class="empty-state">
        <div class="icon">\u{1F4B0}</div>
        <p class="bold">Aucune course au profil rentable pour l'instant</p>
        <p class="muted">Cette page suit la reussite des courses dont le profil confiance/confirmations (pool adaptatif Coupl&eacute; Value) est forte-moyenne, moyenne-faible ou faible-moyenne.</p>
      </div>`;
    return;
  }

  const analysees = candidates.map(({ meeting, race, score, pool, candidats, combos }) => {
    const ordreArrivee = CSVImporter.parseOrdreArrivee(race.arriveeBrute || '');
    if (ordreArrivee.length < 2) {
      return { meeting, race, score, pool, candidats, combos, connu: false, gagne: null, rapportStatus: 'en_attente', bilan: null };
    }
    const [vrai1, vrai2] = ordreArrivee;
    const numsPool = candidats.map((c) => c.entry.numero);
    const gagne = numsPool.includes(vrai1) && numsPool.includes(vrai2);

    if (!gagne) {
      return { meeting, race, score, pool, candidats, combos, connu: true, gagne, vrai1, vrai2, rapportStatus: 'ok', bilan: { mise: combos, gain: 0, net: -combos } };
    }
    if (race.rapportCoupleGagnant === undefined) {
      return { meeting, race, score, pool, candidats, combos, connu: true, gagne, vrai1, vrai2, rapportStatus: 'a_recuperer', bilan: null };
    }
    const dividende = rapportCoupleCorrespondant(race.rapportCoupleGagnant, vrai1, vrai2);
    if (dividende == null) {
      return { meeting, race, score, pool, candidats, combos, connu: true, gagne, vrai1, vrai2, rapportStatus: 'indisponible', bilan: null };
    }
    return { meeting, race, score, pool, candidats, combos, connu: true, gagne, vrai1, vrai2, rapportStatus: 'ok', bilan: { mise: combos, gain: dividende, net: dividende - combos } };
  });
  ...
  const introHtml = `<div class="card" style="margin-bottom:10px;">
      <p class="small muted" style="margin:0;">Suivi des courses au profil forte-moyenne, moyenne-faible ou faible-moyenne (Coupl&eacute; Value) - r&eacute;sultat exploratoire, 2 mois d'archives seulement. Voir HEBERGEMENT.md.</p>
    </div>`;

  const tauxHtml = avecResultat.length > 0
    ? `<div class="card" style="text-align:center;">
        <p class="bold" style="font-size:1.6em; margin:0;">${reussies.length}/${avecResultat.length}</p>
        <p class="muted small" style="margin-top:2px;">Pool ayant captur&eacute; les 2 vrais chevaux (${Math.round((reussies.length / avecResultat.length) * 100)}%) sur les courses Coupl&eacute; rentable avec r&eacute;sultat connu</p>
      </div>`
    : `<div class="card"><p class="muted small">Aucun resultat connu pour l'instant parmi les courses Couplé rentable. Utilisez la mise a jour des cotes (course ou reunion complete) pour recuperer automatiquement les arrivees officielles.</p></div>`;
  ...
  const bilanHtml = avecResultat.length === 0 ? '' : `
    <div class="card" style="margin-top:10px;">
      <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:6px;">
        <p class="bold" style="margin:0;">Bilan financier (hypoth&eacute;tique) Coupl&eacute; Value (pool adapt&eacute;)</p>
        <button class="btn btn-secondary" data-recuperer-rapports ${aRecuperer.length === 0 ? 'disabled' : ''}>R&eacute;cup&eacute;rer les rapports${aRecuperer.length > 0 ? ` (${aRecuperer.length})` : ''}</button>
      </div>
      ${avecBilan.length > 0
        ? `<p class="bold" style="font-size:1.3em; margin:8px 0 0;">${netGlobal >= 0 ? '+' : ''}${netGlobal.toFixed(2)} &euro;</p>
           <p class="muted small" style="margin-top:2px;">Mise ${bilanGlobal.mise.toFixed(2)} &euro; / Gains ${bilanGlobal.gain.toFixed(2)} &euro; sur ${avecBilan.length} course(s) avec bilan calculable</p>`
        : `<p class="muted small" style="margin-top:8px;">Cliquez sur "R&eacute;cup&eacute;rer les rapports" pour calculer le bilan financier &agrave; partir des dividendes PMU officiels (Coupl&eacute; Gagnant), sur les courses gagn&eacute;es uniquement (une course perdue n'a pas besoin de rapport pour conna&icirc;tre son bilan).</p>`}
      <div id="couple-rentable-rapports-status"></div>
      <p class="muted small" style="margin-top:8px;">Hypoth&eacute;tique : suppose 1&euro; jou&eacute; par combinaison du pool adapt&eacute; (nombre variable selon le profil, ${Object.entries(POOL_ADAPTATIF_NIVEAUX).map(([, v]) => v.n).filter((n) => n != null).join('-')} combinaisons max selon les profils), sur toutes les courses Coupl&eacute; rentable du jour &mdash; ce n'est pas un historique de vos mises reelles. Les courses gagn&eacute;es sans rapport disponible (Coupl&eacute; Gagnant non propos&eacute; par le PMU sur petit peloton) sont exclues du bilan. Ne remplace pas votre propre jugement.</p>
    </div>`;
  ...
  const rowHtml = (a) => `
    <div class="list-item clickable" data-goto="race/${a.race.id}">
      <div>
        <div class="bold">${escapeHtml(a.meeting.hippodrome)} - Course ${a.race.numeroCourse}</div>
        <div class="muted small">Profil ${escapeHtml(a.pool.cle)}${a.connu ? ` - Arriv&eacute;e ${a.vrai1}-${a.vrai2}` : ' - En attente de resultat'}</div>
      </div>
      <div style="display:flex; flex-direction:column; align-items:flex-end; gap:2px;">
        ${a.connu
  ...
  // Recuperation des rapports Coupl&eacute; Gagnant, course par course,
  // sequentielle (meme raison que Course feu vert : ne pas multiplier les
  // requetes simultanees) - uniquement pour les courses GAGNEES (voir note
  // au-dessus de renderCoupleRentableResultat).
  const btnRecuperer = appEl.querySelector('[data-recuperer-rapports]');
  if (btnRecuperer && !btnRecuperer.disabled) {
    btnRecuperer.addEventListener('click', async () => {
      const statusEl = appEl.querySelector('#couple-rentable-rapports-status');
      btnRecuperer.disabled = true;
      let ok = 0;
      let fail = 0;
      for (let i = 0; i < aRecuperer.length; i++) {
        ...
        try {
          ...
          console.error('Recuperation rapport Couple rentable echouee pour', a.meeting.hippodrome, a.race.numeroCourse, err);
          fail++;
        }
        if (i < aRecuperer.length - 1) await new Promise((resolve) => setTimeout(resolve, 400));
      }
