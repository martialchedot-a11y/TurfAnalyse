...
// (pas un conseil de jeu), limitee aux reunions du jour (`estAujourdHui`)
// puisque le decompte n'a de sens que pour des courses pas encore parties.
//
...
async function renderListeCourses() {
  ...
  for (const meeting of meetings) {
    ...
    for (const race of races) {
      ...
      courses.push({ meeting, race, nbPartants: horseRecords.length, numerosEligibles });
    }
  }
  ...
  if (courses.length === 0) {
    appEl.innerHTML = `
      <div class="empty-state">
        <div class="icon">\u{1F4C5}</div>
        <p class="bold">Aucune course potentiellement &eacute;ligible aujourd'hui</p>
