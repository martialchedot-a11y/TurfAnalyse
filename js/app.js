 ...
 * detail de la methode et l'origine des seuils de rentabilite par rang).
 * Affichee dans le prolongement du "Conseil de jeu" ci-dessus, mais
 * independante de sa cascade (ancre/Base tres solide).
 *
 * *** v7 (aout 2026) *** : le rang 1 reste prioritaire des qu'il depasse
 * son seuil (cote > 3,8, 164,0% de rendement reel, backtest 8 mois). S'il
 * ne qualifie pas mais qu'un SEUL des autres rangs (2, 4, 5 - le rang 3
 * reste exclu) depasse le sien, ce cheval est joue seul. Le Dutching
 * combinant plusieurs chevaux reste exclu de cette page (voir
 * HEBERGEMENT.md) : si 2 rangs ou plus parmi {2, 4, 5} qualifient sans que
 * le rang 1 qualifie, le jeu n'est pas jouable.
 *
 * *** v8 (aout 2026) *** : le rang 1 doit EN PLUS dominer le rang 2 d'au
 * moins 10 points de Score Global pour qualifier ("1er du classement
 * seul"). Cette condition d'ecart fait grimper la reussite de 27,4% a
 * 35,7% et le rendement de 164,0% a 201,1% (1058 courses sur 8 mois,
 * robuste - voir HEBERGEMENT.md). Ne s'applique qu'au rang 1.
 *
 * *** v9 (aout 2026) *** : le cheval retenu par "Cheval value seul" doit EN
 * PLUS avoir un score Croisement (R10/TG/OR/IdC, voir
 * jeuCoupleTrioCroisement.js) &le;1. Gradient contre-intuitif mais net et
 * monotone (backtest 8 mois) : moins ces 4 rubriques confirment le cheval,
 * mieux ce mode reussit. Rendement 118,0% -> 138,1% (1617 courses sur 2614),
 * robustesse amelioree (110,7% au lieu de 100,6% en retirant les 10 plus
 * gros gains) - voir HEBERGEMENT.md. Ne s'applique qu'a "Cheval value
 * seul" : teste aussi sur le rang 1 mais NON retenu (deja performant sous
 * tous les angles testes, restreindre y aurait surtout reduit le volume de
 * courses jouables sans justification suffisante).
 */
function libelleModeSimpleGagnant(jeu) {
  return jeu.rang1Value ? '1er du classement seul' : 'Cheval value seul (rang hors 1)';
}
...
function jeuSimpleGagnantHtml(chevaux) {
  ...
  if (!jeu.rentable) {
    return `<div class="card" style="margin-bottom:10px;">
        ...
        <p class="muted small" style="margin-top:4px;">Ni le 1er du classement Score Global (seuil de cote 3,8 ET &eacute;cart de Score Global &ge;10 sur le 2e), ni un seul autre rang parmi 2/4/5 (seuils 5,2/9,4/12,5, ET score Croisement R10/TG/OR/IdC &le;1) ne qualifie. Indicatif, n'entre dans aucun calcul de Score Global/Value/classement.</p>
      </div>`;
  }
  const optionsMise = MISES_PRESETS_JEU_SIMPLE_GAGNANT.map((m) => `<option value="${m}">${m}&euro;</option>`).join('');

  const palierEcartHtml = jeu.rang1Value ? `
  ...
  return `<div class="card" style="margin-bottom:10px;">
      ...
      <p class="muted small" style="margin-top:8px;">Mise flat sur le cheval retenu (Dutching &agrave; 1 cheval, rendement = sa cote). Seuils de cote par rang calibr&eacute;s sur 8 mois d'archives r&eacute;elles (voir HEBERGEMENT.md).${jeu.rang1Value ? ' Mise par palier d\'&eacute;cart (optionnelle, 1er du classement seul) : x1 si &eacute;cart &lt;20, x1,5 si 20&ndash;30, x2 si &ge;30 &mdash; +21% de rendement net mesur&eacute; sur l\'archive compl&egrave;te pour un risque quasi identique (voir HEBERGEMENT.md).' : ''} Indicatif, n'entre dans aucun calcul de Score Global/Value/classement.</p>
    </div>`;
}
...
function bindJeuSimpleGagnant(chevaux) {
  ...
  btn.addEventListener('click', () => {
    ...
    if (jeu.rang1Value && palierCheck && palierCheck.checked) {
    ...
    <div class="card" style="margin-bottom:10px;">
      <div class="field" style="margin-bottom:0;">
        ...
        <select id="jsg-filtre-mode-select">
          <option value="tous" ${actuel === 'tous' ? 'selected' : ''}>1er du classement et Cheval value seul</option>
          <option value="rang1" ${actuel === 'rang1' ? 'selected' : ''}>1er du classement seul</option>
          <option value="value" ${actuel === 'value' ? 'selected' : ''}>Cheval value seul</option>
        </select>
      </div>
    </div>`;
}
...
async function renderBilanSimpleGagnant() {
  ...
  const introHtml = `<div class="card" style="margin-bottom:10px;">
      <p class="small muted" style="margin:0;">Courses dont le Jeu Simple Gagnant est jouable : 1er du classement Score Global (cote &gt; 3,8), ou &agrave; d&eacute;faut l'unique autre rang (2, 4 ou 5) qui d&eacute;passe son propre seuil - voir la fiche course pour le d&eacute;tail, et HEBERGEMENT.md pour la m&eacute;thode.</p>
    </div>`;
  ...
  const avecBilanCheval = avecBilan.filter((a) => a.jeu.rang1Value === false);
  const bilanRang1 = avecBilanRang1.reduce((acc, a) => ({ mise: acc.mise + a.bilan.mise, gain: acc.gain + a.bilan.gain }), { mise: 0, gain: 0 });
  const bilanCheval = avecBilanCheval.reduce((acc, a) => ({ mise: acc.mise + a.bilan.mise, gain: acc.gain + a.bilan.gain }), { mise: 0, gain: 0 });
  const netRang1 = bilanRang1.gain - bilanRang1.mise;
  const netCheval = bilanCheval.gain - bilanCheval.mise;
    ...
    <div class="card" style="margin-top:10px;">
      ...
      ${avecBilan.length > 0
        ? `<p class="bold" style="font-size:1.3em; margin:8px 0 0;">${netGlobal >= 0 ? '+' : ''}${netGlobal.toFixed(2)} &euro;</p>
           ...
           <p class="muted small" style="margin-top:6px;">dont 1er du classement seul : <span class="${netRang1 >= 0 ? 'tag-green' : 'tag-red'} bold">${netRang1 >= 0 ? '+' : ''}${netRang1.toFixed(2)} &euro;</span>${avecBilanRang1.length > 0 ? ` (${avecBilanRang1.length} course${avecBilanRang1.length > 1 ? 's' : ''})` : ' (aucune course)'}</p>
           <p class="muted small" style="margin-top:2px;">dont Cheval value seul : <span class="${netCheval >= 0 ? 'tag-green' : 'tag-red'} bold">${netCheval >= 0 ? '+' : ''}${netCheval.toFixed(2)} &euro;</span>${avecBilanCheval.length > 0 ? ` (${avecBilanCheval.length} course${avecBilanCheval.length > 1 ? 's' : ''})` : ' (aucune course)'}</p>`
  ...
  // ("rang1Seul", "chevalValueSeul"), pour alimenter le cumul par mode de
  ...
  if (btnTransfert && dateTransfert) {
    btnTransfert.addEventListener('click', async () => {
      ...
      const chevalDuJour = avecResultatDuJour.filter((a) => a.jeu.rang1Value === false);
      const record = {
        id: dateTransfert,
        date: dateTransfert,
        ...
        chevalValueSeul: sousBilanJour(chevalDuJour)
      };
      try {
...
async function renderBilanGlobalSimpleGagnant() {
  ...
  // jeu ("1er du classement seul" / "Cheval value seul"), a partir des
  // sous-bilans `rang1Seul`/`chevalValueSeul` sauvegardes par "Transfert
  ...
  const cumulerMode = (champ) => bilans.reduce((acc, b) => {
    const m = b[champ] || modeVide();
  ...
  const totalChevalValueSeul = cumulerMode('chevalValueSeul');
  const rendementRang1Seul = rendementBilan({ mise: totalRang1Seul.mise, gain: totalRang1Seul.gain });
  const rendementChevalValueSeul = rendementBilan({ mise: totalChevalValueSeul.mise, gain: totalChevalValueSeul.gain });
  const joursSansDetailMode = bilans.filter((b) => !b.rang1Seul || !b.chevalValueSeul).length;
  ...
  const modesHtml = `
    <div style="display:flex; gap:10px; flex-wrap:wrap; margin-bottom:${joursSansDetailMode > 0 ? '4px' : '10px'};">
      ${carteMode('1er du classement seul', totalRang1Seul, rendementRang1Seul)}
      ${carteMode('Cheval value seul', totalChevalValueSeul, rendementChevalValueSeul)}
    </div>
