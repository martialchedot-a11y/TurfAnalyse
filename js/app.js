...
import { jeuCoupleTrioCroisement, combinaisonsDuPool, bilanCoupleCroisement, bilanTrioCroisement, MISES_PRESETS_CROISEMENT, TAILLE_POOL_COUPLE, TAILLE_POOL_TRIO, MIN_PARTANTS_CROISEMENT } from './engine/jeuCoupleTrioCroisement.js';
...
function jeuCroisementHtml(chevaux) {
  ...
  if (!jeu.jouable) {
    return `<div class="card" style="margin-bottom:10px;">
        ...
        <p class="muted small" style="margin-top:4px;">Il faut au moins ${MIN_PARTANTS_CROISEMENT} partants avec des rubriques exploitables pour former les pools. Indicatif, n'entre dans aucun calcul de Score Global/Value/classement.</p>
      </div>`;
  }
  ...
  return `<div class="card" style="margin-bottom:10px;">
      ...
      <p class="small" style="margin:0 0 4px;">Couple Gagnant (${TAILLE_POOL_COUPLE} chevaux, ${combosCouple.length} combinaisons) : ${jeu.poolCouple.map((n) => `N&deg;${n}`).join(' - ')}</p>
      <p class="muted small" style="margin:0 0 8px;" id="croisement-couple-combos">${combosCouple.map((c) => c.join('-')).join(', ')}</p>
      <p class="small" style="margin:0 0 4px;">Trio (${TAILLE_POOL_TRIO} chevaux, ${combosTrio.length} combinaisons) : ${jeu.poolTrio.map((n) => `N&deg;${n}`).join(' - ')}</p>
...
async function collecterCandidatesCroisement() {
  ...
  for (const meeting of meetings) {
    ...
    for (const race of races) {
      const horseRecords = await DB.getHorsesForRace(race.id);
      if (horseRecords.length < MIN_PARTANTS_CROISEMENT) continue;
...
async function renderBilanCroisement() {
  ...
  if (candidates.length === 0) {
    appEl.innerHTML = `
      ...
      <div class="empty-state">
        ...
        <p class="muted">Une course appara&icirc;t ici d&egrave;s qu'elle compte au moins ${MIN_PARTANTS_CROISEMENT} partants avec des rubriques exploitables.</p>
      </div>`;
    bindGoto();
    return;
  }
  ...
  const rowHtml = (a) => `
    <div class="list-item clickable" data-goto="race/${a.race.id}">
      <div>
        <div class="bold">${a.race.heureDepart ? `${escapeHtml(a.race.heureDepart)} - ` : ''}${escapeHtml(a.meeting.hippodrome)} - Course ${a.race.numeroCourse}</div>
        <div class="muted small">Couple n&deg;${a.jeu.poolCouple.join('-')} &middot; Trio n&deg;${a.jeu.poolTrio.join('-')}${a.connu ? ` - Arriv&eacute;e ${a.ordreArrivee.slice(0, 3).join('-')}` : ' - En attente de resultat'}</div>
      </div>
      <div style="display:flex; flex-direction:column; align-items:flex-end; gap:2px;">
        ${a.connu
