 ...
 *
 * *** v5 (aout 2026) *** : le 1er du classement (rang 1) est desormais
 * OBLIGATOIRE - jouable uniquement s'il depasse son seuil (cote > 3,8),
 * de loin le signal le plus rentable pris seul (164,0% de rendement reel,
 * backtest 8 mois). Un Dutching Rang 2/4/5 seul (sans le rang 1) a ete
 * retire : isole a ce cas precis, son rendement reel n'est que de 102,8%
 * (2137 courses) et devient negatif des qu'on retire les 5 a 10 plus gros
 * gains - trop fragile pour rester une recommandation. Quand le rang 1
 * qualifie et que d'autres rangs (2, 4, 5 - le rang 3 reste exclu, jamais
 * rentable seul) depassent AUSSI leur propre seuil, un Dutching combinant
 * tous ces chevaux (rang 1 inclus) reste propose EN PLUS, en alternative
 * facultative.
 */
function libelleModeSimpleGagnant(jeu, pool) {
  if (pool === jeu.principal) return '1er du classement seul';
...
function jeuSimpleGagnantHtml(chevaux) {
  ...
  if (!jeu.rentable) {
    return `<div class="card" style="margin-bottom:10px;">
        ...
        <p class="muted small" style="margin-top:4px;">Le 1er du classement Score Global ne d&eacute;passe pas son seuil de cote (3,8). C'est la seule condition de jouabilit&eacute; (backtest : Dutching sur les autres rangs sans le rang 1 trop fragile, retir&eacute; en aout 2026). Indicatif, n'entre dans aucun calcul de Score Global/Value/classement.</p>
      </div>`;
  }
