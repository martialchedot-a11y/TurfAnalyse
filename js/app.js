...
/**
 * Suggestion "Couplé Gagnant" (jusqu'à 5 chevaux) : quand l'ancre existe
 * (chevalConfianceMaximale - Base très solide confirmée, classée n°1),
 * propose les 5 AUTRES chevaux à associer pour un pari Couplé Gagnant
 * (l'ancre + un des 5, dans n'importe quel ordre).
 *
 ...
 * la plus négative en premier, hors ancre) qui capture le mieux le
 * partenaire réel du Top2 : 80,1% de réussite (n=346, courses où l'ancre
 * finit bien Top2) contre 75,7% avec le simple rang du modèle. Sur
 * l'ensemble des courses où l'ancre existe (n=623), cela donne 44,5% de
 * Couplés Gagnants réussis (277/623) avec seulement 5 tickets.
 * Purement indicatif : n'entre dans aucun calcul de Score Global/Value/
 * classement, et ne remplace pas la Valeur des couples (Base confirmée x
 * Trio) décrite plus haut, qui répond à une question différente (valeur
 * du pari, pas choix des chevaux).
 * @returns {{anchor:Object, partenaires:Array}|null}
 */
function suggestionCoupleGagnant(bd, chevaux) {
...
function suggestionCoupleGagnantHtml(bd, chevaux) {
  const suggestion = suggestionCoupleGagnant(bd, chevaux);
