...
import { misesJeuSimpleGagnant, MISES_PRESETS_JEU_SIMPLE_GAGNANT, rendementBilan, cumulerBilansJournaliers, SEUIL_SCORE_FORME_RANG1, SEUIL_SCORE_APTITUDE_RANG1, SEUIL_VALUE_SIMPLE_GAGNANT, multiplicateurMiseJoursAbsenceRang1, SEUIL_JOURS_ABSENCE_MISE_RENFORCEE } from './engine/jeuSimpleGagnant.js';
...
function envoyerNotificationJsg(meeting, race, jeu) {
  const placeNumeros = jeu.place.chevaux.map((c) => c.entry.numero).join('-');
  ...
  const corps = `Place N° ${placeNumeros}${gagnantTxt} - depart ${race.heureDepart || ''}`;
  try {
...
function bindJeuSimpleGP(chevaux, topIAPlace, lieu, numeroCourse) {
  ...
  btn.addEventListener('click', () => {
    ...
    const miseTotalePlace = Number(selectPlace.value) || 0;
...
async function renderBilanSimpleGagnant() {
  ...
  const rowHtml = (a) => {
    ...
    <div class="list-item clickable" data-goto="race/${a.race.id}">
      <div>
        ...
        <div class="muted small">Place n&deg;${a.jeu.place.chevaux.map((c) => c.entry.numero).join('-')}${gagnantTxt}${a.connu ? ` - Vainqueur n&deg;${a.vrai1}` : ' - En attente de resultat'}</div>
      </div>
      <div style="display:flex; flex-direction:column; align-items:flex-end; gap:2px;">
        ${a.connu
  ...
  const sousBilanJour = (liste) => {
    ...
    return { nbCourses: liste.length, reussies: liste.filter((a) => a.placeGagne).length, mise, gain, net: gain - mise };
  };
...
async function renderBilanGlobalSimpleGagnant() {
  ...
  const modesHtml = `
    <div style="display:flex; gap:10px; flex-wrap:wrap; margin-bottom:${joursSansDetailMode > 0 ? '4px' : '10px'};">
      ${carteMode('Avec Simple Gagnant (rang IA 1 dans le pool)', totalRang1Seul, rendementRang1Seul)}
      ${carteMode('Simple Place seul', totalClassement8h, rendementClassement8h)}
    </div>
