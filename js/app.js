...
// Value (cf. scoreConfigurationCoupleValue) est >= 3/5 (toutes reunions
// importees). Remplace l'ancienne page "Courses sures" (base solide + Score
// Global >= 80), a la demande de l'utilisateur, par un filtre directement
// aligne sur l'indicateur de confiance du Coupl&eacute; Value.
// -------------------------------------------------------------------
  ...
  if (candidates.length === 0) {
    appEl.innerHTML = `
      <div class="empty-state">
        ...
        <p class="muted">Une course apparait ici des que son score de configuration du Coupl&eacute; Value atteint ${SEUIL_FEU_VERT}/5.</p>
      </div>`;
    return;
  }
...
async function renderResultatJournee() {
  ...
  for (const meeting of meetings) {
    ...
    for (const race of races) {
      ...
      analysees.push({ meeting, race, score, check });
    }
  }
  ...
  if (analysees.length === 0) {
    appEl.innerHTML = `
      <div class="empty-state">
        <div class="icon">\u{1F3AF}</div>
        <p class="bold">Aucune course feu vert pour l'instant</p>
        <p class="muted">Cette page suit la reussite des courses dont le score de configuration Coupl&eacute; Value est >= ${SEUIL_FEU_VERT}/5.</p>
      </div>`;
    return;
  }
