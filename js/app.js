...
function basesEtDangersHtml(bd, cotesCibles, r, chevaux, disciplineCanonique) {
  const confianceMax = chevalConfianceMaximale(bd, chevaux);
  const confianceMaxHtml = confianceMax
    ? `<div class="card" style="border: 2px solid var(--tag-green, #2e7d32); margin-bottom:10px;">
        <p class="bold" style="margin:0;">&#127919; Confiance maximale : N&deg;${confianceMax.entry.numero}</p>
        <p class="muted small" style="margin-top:4px;">Base tres solide confirmee techniquement, ET classee n1 par le Score Global. Sur le backtest reel (1 mois, 1027 courses, voir HEBERGEMENT.md), cette combinaison precise gagne 41,7% du temps et finit dans le Top3 75,0% du temps (n=24 - rare mais nettement au-dessus du reste). Indicatif, n'entre dans aucun calcul.</p>
      </div>`
    : '';
...
async function renderCoursesSures() {
  ...
  for (const meeting of meetings) {
    ...
    for (const race of races) {
      ...
      if (estCourseSure(bd, result.chevaux, result.resume)) {
        ...
        candidates.push({ meeting, race, bd, r: result.resume, meilleurScore, petitChamp, nbPartants: horses.length });
      }
    }
  }
  ...
  appEl.innerHTML = `<div class="list-group">${candidates.map(({ meeting, race, bd, r, meilleurScore, petitChamp, nbPartants }) => `
