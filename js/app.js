...
function combosDansTrancheProbable(candidats, tranche) {
  ...
  for (let i = 0; i < avecCote.length; i++) {
    for (let j = i + 1; j < avecCote.length; j++) {
      const rapport = (avecCote[i].cotePourAffichage * avecCote[j].cotePourAffichage) / 2;
      if (rapport >= tranche.p25 && rapport <= tranche.p75) {
        combos.push({ numeros: [avecCote[i].entry.numero, avecCote[j].entry.numero], rapport });
      }
    }
  }
  return combos.sort((a, b) => a.rapport - b.rapport);
}
...
function combosDansTrancheHtml(candidats, table, cle) {
  ...
  const titre = 'Combinaison(s) du pool dont le rapport theorique ((cote1 x cote2)/2) tombe dans la tranche probable du profil (P25-P75, cf. Tranche probable ci-dessus) - purement informatif, n\'exclut aucune combinaison de la suggestion ni du calcul de la mise.';
  if (combos.length === 0) {
  ...
  const items = combos.map((c) => `N&deg;${c.numeros[0]}-N&deg;${c.numeros[1]} (${c.rapport.toFixed(1)}&euro;)`).join(', ');
  return `<p class="small" style="margin-top:2px;" title="${escapeHtml(titre)}">Coupl&eacute;(s) dans la tranche probable : ${items}</p>`;
}
...
async function renderCourseFeuVert() {
  ...
  for (const meeting of meetings) {
    ...
    for (const race of races) {
      ...
      const numerosConfirmes = base5 ? confirmationsCotesCibles(base5.candidats, cotesCibles) : [];
      const pool = poolAdaptatifCoupleValue(score, numerosConfirmes.length);
      const suggestion = pool.n == null ? null : (pool.n === 5 ? base5 : coupleValue(result.chevaux, pool.n));
      candidates.push({ meeting, race, score, suggestion, numerosConfirmes, pool, nbPartants: horses.length });
    }
  }
  ...
  if (candidates.length === 0) {
    appEl.innerHTML = `
      ...
      <div class="empty-state">
        ...
        <p class="muted">Une course apparait ici des que son score de configuration du Coupl&eacute; Value est ${libelleFiltreFeuVert(filtre)}, avec entre ${MIN_PARTANTS_FEU_VERT} et ${MAX_PARTANTS_FEU_VERT} partants et au moins la moitie du champ avec un historique de performances retrouve.</p>
      </div>`;
  ...
  appEl.innerHTML = `
    ...
    <div class="list-group">${candidates.map(({ meeting, race, score, suggestion, numerosConfirmes, pool, nbPartants }) => {
      const niveau = SCORE_CONFIGURATION_NIVEAUX[score];
      return `
      <div class="list-item clickable" data-goto="race/${race.id}">
        <div>
          ...
          <div style="margin-top:4px; display:flex; gap:6px; flex-wrap:wrap;">
            ...
            ${confianceRenforceeHtml(score, numerosConfirmes)}
            ${suggestion
              ? `<span class="small tag-blue bold" title="Pool adapte au profil ${escapeHtml(pool.label)} : ${pool.taux} (n=${pool.nCourses})">Coupl&eacute; Value (${pool.n}) : ${suggestion.candidats.map((c) => `N&deg;${c.entry.numero}`).join('-')}</span>`
              : `<span class="small tag-red bold" title="${escapeHtml('Profil ' + pool.label + ' : ' + pool.taux + ' (n=' + pool.nCourses + ')')}">Profil incertain : pas de suggestion</span>`}
...
// de configuration Coupl&eacute; Value filtre selon getFiltreFeuVert),
// quelle proportion a effectivement vu ses 2 vrais chevaux du Top2 dans les
// 5 candidats suggeres (cf. coupleValueReussi ci-dessus) - uniquement pour
// les courses dont l'arrivee officielle est deja connue. Les courses feu
// vert sans arrivee connue sont listees a part ("en attente de resultat").
// -------------------------------------------------------------------
async function renderResultatJournee() {
  ...
  for (const meeting of meetings) {
    ...
    for (const race of races) {
      ...
      const numerosConfirmes = base5 ? confirmationsCotesCibles(base5.candidats, cotesCibles) : [];
      const pool = poolAdaptatifCoupleValue(score, numerosConfirmes.length);
      const abstentionProfil = pool.n == null;

      const ordreArrivee = CSVImporter.parseOrdreArrivee(race.arriveeBrute || '');
      const check = abstentionProfil ? null : coupleValueReussi(result.chevaux, ordreArrivee, pool.n);
      ...
      // et 'ok' (rapport disponible, bilan calcule). Sans objet si
      // abstentionProfil (aucune suggestion faite pour cette course).
      ...
      analysees.push({ meeting, race, score, check, rapportStatus, bilan, abstentionProfil, pool });
    }
  }
  ...
  if (analysees.length === 0) {
    appEl.innerHTML = `
      ...
      <div class="empty-state">
        <div class="icon">\u{1F3AF}</div>
        <p class="bold">Aucune course feu vert pour l'instant</p>
        <p class="muted">Cette page suit la reussite des courses dont le score de configuration Coupl&eacute; Value est ${libelleFiltreFeuVert(filtre)}, avec entre ${MIN_PARTANTS_FEU_VERT} et ${MAX_PARTANTS_FEU_VERT} partants et au moins la moitie du champ avec un historique de performances retrouve.</p>
      </div>`;
  ...
  const enAttente = analysees.filter((a) => !a.abstentionProfil && a.check == null);
  const abstentions = analysees.filter((a) => a.abstentionProfil);
  ...
  const tauxHtml = avecResultat.length > 0
    ? `<div class="card" style="text-align:center;">
        ...
        <p class="muted small" style="margin-top:2px;">Coupl&eacute; Value reussi (${Math.round((reussies.length / avecResultat.length) * 100)}%) sur les courses feu vert (score ${libelleFiltreFeuVert(filtre)}) avec resultat connu</p>
      </div>`
  ...
  const bilanHtml = avecResultat.length === 0 ? '' : `
    <div class="card" style="margin-top:10px;">
      ...
      <p class="muted small" style="margin-top:8px;">Hypoth&eacute;tique : suppose ${MISE_PAR_COMBINAISON_FEU_VERT}&euro; jou&eacute; sur CHACUNE des combinaisons Coupl&eacute; Gagnant possibles entre les candidats du Coupl&eacute; Value (le nombre de candidats s'adapte au profil confiance/confirmations de chaque course, de 5 &agrave; 8), sur toutes les courses feu vert du jour &mdash; ce n'est pas un historique de vos mises reelles. Les courses o&ugrave; le PMU n'a pas propose le Coupl&eacute; Gagnant (champ trop reduit une fois les non-partants retires, etc.) sont exclues du bilan, faute de rapport officiel disponible. Ne remplace pas votre propre jugement.</p>
    </div>`;
  ...
  const rowHtml = (a) => `
    <div class="list-item clickable" data-goto="race/${a.race.id}">
      <div>
        ...
        <div class="muted small">Score ${a.score}/5${a.check ? ` - Arrivee ${a.check.top2.join('-')} (pool ${a.pool.n})` : ' - En attente de resultat'}</div>
      </div>
  ...
  const rowAbstentionHtml = (a) => `
    <div class="list-item clickable" data-goto="race/${a.race.id}">
      <div>
        <div class="bold">${escapeHtml(a.meeting.hippodrome)} - Course ${a.race.numeroCourse}</div>
        <div class="muted small" title="${escapeHtml(a.pool.taux + ' (n=' + a.pool.nCourses + ')')}">Score ${a.score}/5 - Profil ${escapeHtml(a.pool.label.toLowerCase())}</div>
      </div>
      <span class="small tag-red bold">Sans suggestion</span>
    </div>`;
  ...
  appEl.innerHTML = `
    ...
    ${abstentions.length > 0 ? `<h3 style="margin-top:16px;">Sans suggestion Coupl&eacute; Value (${abstentions.length})</h3><p class="muted small">Profil confiance/confirmations trop faible (voir HEBERGEMENT.md) : aucun pool de taille raisonnable ne capture fiablement les 2 vrais chevaux, exclues du taux de reussite et du bilan financier ci-dessus.</p><div class="list-group">${abstentions.map(rowAbstentionHtml).join('')}</div>` : ''}
  `;
