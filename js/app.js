...
function currentTab(parts) {
  const known = ['meetings', 'import', 'feuvert', 'simplegagnant', 'race'];
...
async function renderBilanSimpleGagnant() {
  ...
  if (meetings.length === 0) {
    appEl.innerHTML = `
      <div class="empty-state">
        ...
        <p class="bold">Aucune reunion importee</p>
        <p class="muted">Importez une reunion depuis l'onglet "Importer" pour voir apparaitre ici les courses au Jeu Simple Gagnant rentable.</p>
      </div>`;
    return;
  }
  ...
  const bilanHtml = avecResultat.length === 0 ? '' : `
    <div class="card" style="margin-top:10px;">
      ...
      ${avecBilan.length > 0 ? `
        <button class="btn btn-primary btn-block" data-transfert-bilan style="margin-top:10px;">Transfert bilan</button>
  ...
  // Date = aujourd'hui (jour ou le transfert est effectue) ; un nouveau
  // transfert le meme jour ecrase proprement le precedent (cf. db.js).
  ...
  if (btnTransfert) {
    btnTransfert.addEventListener('click', async () => {
      ...
      try {
        ...
        statusEl.innerHTML = `<p class="small tag-green bold" style="margin-top:6px;">Bilan du ${dateVal} transf&eacute;r&eacute; (${netGlobal >= 0 ? '+' : ''}${netGlobal.toFixed(2)} &euro;). <a href="#/bilanglobalsimplegagnant">Voir le bilan global</a></p>`;
      } catch (err) {
