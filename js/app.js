...
import { jeuSimpleGP, jeuSupplementation, SEUIL_NOTE_SUPPLEMENTATION, bilanJourSimpleGP } from './engine/jeuSimpleGP.js';
...
import { parseSupplementation, trouverSupplementation } from './engine/supplementationParser.js';
...
/**
 * Charge le snapshot "Supplementation" du jour importe (fichier
 * "Supplementation_PMU_AAAAMMJJ.xlsx", cf. js/engine/supplementationParser.js)
 * - a la demande explicite de l'utilisateur (sept. 2026). Utilise par
 * "Courses du jour" (courses couvertes par la Supplementation du jour) et
 * par la fiche course ("Simple G/P" : selections Supplementation x
 * Value<=-30 x Note IA>=15,9).
 * @returns {Promise<Array|null>} lignes de la Supplementation, ou `null` si
 *   rien n'a ete importe.
 */
async function chargerSupplementation() {
  const enregistrement = await DB.getSupplementation();
  return enregistrement && Array.isArray(enregistrement.rows) && enregistrement.rows.length > 0 ? enregistrement.rows : null;
}
...
async function renderRaceDetail(raceId, useCote8h = false) {
  ...
  const topIAPlace = await chargerTopIAPlace();
  const supplementation = await chargerSupplementation();
  ...
  appEl.innerHTML = `
    ...
    ${basesEtDangersHtml(basesEtDangers, cotesCibles, result.resume, result.chevaux, context.discipline.canonical, context.lieu, context.typeCourse, topIAPlace, race.numeroCourse, supplementation)}
  ...
  bindJeuSupplementation(result.chevaux, supplementation);
...
/**
 * Carte "Suppl&eacute;mentation" (sept. 2026, a la demande explicite de
 * l'utilisateur) : croisement du fichier "Supplementation_PMU_AAAAMMJJ.xlsx"
 * du jour import&eacute; (Note IA/20 &ge; SEUIL_NOTE_SUPPLEMENTATION) avec
 * notre pool Value&le;-30 (voir jeuSupplementation, jeuSimpleGP.js).
 * Contrairement au "Simple G/P" (bas&eacute; sur le Top 5 IA Place), Place
 * ET Gagnant sont jou&eacute;s sur EXACTEMENT le m&ecirc;me pool qualifi&eacute;
 * (Dutching, aucune restriction de rang).
 * @param {Array} chevaux - result.chevaux
 * @param {Array|null} supplementation - chargerSupplementation()
 * @returns {string}
 */
function jeuSupplementationHtml(chevaux, supplementation) {
  const jeu = jeuSupplementation(chevaux, supplementation);
  if (!jeu.rentable) {
    return `<div class="card" style="margin-bottom:10px;">
        <p class="bold" style="margin:0 0 4px;">Suppl&eacute;mentation</p>
        <p class="small tag-red bold" style="margin:0;">Suppl&eacute;mentation non jouable</p>
        <p class="muted small" style="margin-top:4px;">Aucun cheval Value &le; -30 ne figure dans la Suppl&eacute;mentation PMU du jour avec Note IA &ge; ${SEUIL_NOTE_SUPPLEMENTATION} (ou Suppl&eacute;mentation non import&eacute;e - onglet Importer).</p>
      </div>`;
  }
  const optionsMise = MISES_PRESETS_JEU_SIMPLE_GAGNANT.map((m) => `<option value="${m}">${m}&euro;</option>`).join('');
  const listeHtml = jeu.notes.map((n) => `N&deg;${n.numero} (${n.note}/20)`).join(' - ');

  return `<div class="card" style="margin-bottom:10px;">
      <p class="bold" style="margin:0 0 4px;">Suppl&eacute;mentation</p>
      <p class="small" style="margin:0 0 4px;"><span class="tag-orange bold">Place &amp; Gagnant (m&ecirc;me pool)</span> : ${listeHtml}</p>
      <div class="field" style="margin-bottom:8px;">
        <label for="jsupp-mise-place-select">Mise totale Place</label>
        <select id="jsupp-mise-place-select">${optionsMise}</select>
      </div>
      <div class="field" style="margin-bottom:8px;">
        <label for="jsupp-mise-gagnant-select">Mise totale Gagnant</label>
        <select id="jsupp-mise-gagnant-select">${optionsMise}</select>
      </div>
      <button class="btn btn-secondary btn-block" id="jsupp-calculer-btn">Calculer mise</button>
      <div id="jsupp-resultat"></div>
      <p class="muted small" style="margin-top:8px;">Backtest r&eacute;el (01-26/09/2026, n=16, Note IA&ge;${SEUIL_NOTE_SUPPLEMENTATION} ET Value&le;-30) : r&eacute;ussite Gagnant 43,8% / rendement 148,1% &middot; r&eacute;ussite Place 75,0% / rendement 121,3% (voir HEBERGEMENT.md) - &eacute;chantillon encore r&eacute;duit. Mises en Dutching (proportionnelles a 1/cote) sur le pool.</p>
    </div>`;
}

/**
 * A appeler apres avoir injecte jeuSupplementationHtml dans le DOM : attache
 * le clic du bouton "Calculer mise" au calcul des mises Dutching Place et
 * Gagnant, chacune sur le pool complet des chevaux qualifies.
 */
function bindJeuSupplementation(chevaux, supplementation) {
  const btn = appEl.querySelector('#jsupp-calculer-btn');
  if (!btn) return;
  const jeu = jeuSupplementation(chevaux, supplementation);
  if (!jeu.rentable) return;
  btn.addEventListener('click', () => {
    const resultEl = appEl.querySelector('#jsupp-resultat');
    if (!resultEl) return;

    const selectPlace = appEl.querySelector('#jsupp-mise-place-select');
    const miseTotalePlace = selectPlace ? (Number(selectPlace.value) || 0) : 0;
    const misesPlace = misesJeuSimpleGagnant(jeu.place, miseTotalePlace);

    const selectGagnant = appEl.querySelector('#jsupp-mise-gagnant-select');
    const miseTotaleGagnant = selectGagnant ? (Number(selectGagnant.value) || 0) : 0;
    const misesGagnant = misesJeuSimpleGagnant(jeu.gagnant, miseTotaleGagnant);

    const blocHtml = (titre, mises) => mises.length === 0 ? '' : `
      <p class="small bold" style="margin-top:6px;">${titre}</p>
    ...
    resultEl.innerHTML = `${blocHtml('Place', misesPlace)}${blocHtml('Gagnant', misesGagnant)}`;
  });
}

function basesEtDangersHtml(bd, cotesCibles, r, chevaux, disciplineCanonique, hippodrome, typeCourse, topIAPlace, numeroCourse, supplementation) {
  ...
  const jeuSupplementationCard = jeuSupplementationHtml(chevaux, supplementation);
...
function renderImport() {
  ...
  appEl.innerHTML = `
    ...
    <div class="card">
      <h3>Suppl&eacute;mentation PMU (optionnel)</h3>
      <p class="muted small">Fichier Excel "Supplementation_PMU_AAAAMMJJ.xlsx". Utilis&eacute; par "Courses du jour" (courses couvertes par la Suppl&eacute;mentation du jour) et par la fiche course ("Simple G/P" : chevaux Value&lt;=-30 avec Note IA&gt;=15,9). La date est lue dans le fichier ; chaque r&eacute;import REMPLACE enti&egrave;rement le pr&eacute;c&eacute;dent (voir HEBERGEMENT.md).</p>
      <div class="field"><input type="file" id="file-supplementation" accept=".xlsx"></div>
    </div>
    ...
    <div class="card">
      ...
      <button class="btn btn-secondary btn-block" id="btn-reset-supplementation" style="margin-top:8px;">Vider la Suppl&eacute;mentation import&eacute;e</button>
    </div>
  ...
  document.getElementById('file-supplementation').addEventListener('change', (e) => handleSupplementationImport(e.target.files[0]));
  ...
  document.getElementById('btn-reset-supplementation').addEventListener('click', handleResetSupplementation);
}
...
// *** sept. 2026 (a la demande explicite de l'utilisateur) : import du
// fichier quotidien "Supplementation_PMU_AAAAMMJJ.xlsx" (cf.
// js/engine/supplementationParser.js). Utilise la librairie XLSX deja
// embarquee (js/vendor/xlsx.core.min.js, chargee globalement via
// index.html - voir window.XLSX) ; chaque reimport REMPLACE entierement le
// precedent (photo du jour, pas un historique cumulatif).
async function handleSupplementationImport(file) {
  if (!file) return;
  try {
    ...
    if (rows.length === 0) { showImportMessage('Aucune ligne reconnue dans ce fichier de Supplementation.', true); return; }
    const dateFinale = dateVal || extraireDateReunionDepuisNomFichier(file.name) || new Date().toISOString();
    await DB.saveSupplementation(dateFinale, rows);
    showImportMessage(`Supplementation importee (${rows.length} ligne${rows.length > 1 ? 's' : ''}).`, false);
  } catch (err) {
    showImportMessage(`Echec de l'import : ${err.message || err}`, true);
  }
}

async function handleResetSupplementation() {
  const ok = confirm('Vider la Supplementation importee ?');
  if (!ok) return;
  await DB.resetSupplementation();
  showImportMessage('Supplementation videe.', false);
}
...
async function renderListeCourses() {
  ...
  for (const meeting of meetings) {
    ...
    for (const race of races) {
      ...
      // (n'importe lequel des 5 rangs) figure dans cette liste. Calcule sur
      // horseRecords (pas sur result.chevaux) pour rester independant de
      // tout filtre partants/forme. ***
      ...
      // *** sept. 2026 (a la demande explicite de l'utilisateur) : de meme,
      // une course couverte par la Supplementation PMU du jour importee
      // (n'importe quel cheval y figurant, matching par nom) figure dans
      // cette liste. Remplace ENTIEREMENT l'ancien filtre forme>=70/
      // aptitude>=85 (SEUIL_SCORE_FORME_RANG1/SEUIL_SCORE_APTITUDE_RANG1),
      // desormais retire de "Courses du jour" a la demande de l'utilisateur
      // ("il faudrait egalement supprimer de la page 'courses du jour' les
      // courses que l'on gardait avant pour forme>70 et aptitude>85").
      const matchesSupplementation = horseRecords
        .map((h) => ({ numero: h.numero, supp: trouverSupplementation(supplementation, h.nom) }))
        .filter((m) => m.supp);
      const couverteParSupplementation = matchesSupplementation.length > 0;

      if (!couverteParIA && !couverteParSupplementation) continue;
      ...
      const result = RaceAnalyzer.analyser(horses, context, false);
      // *** sept. 2026, a la demande de l'utilisateur : remplacement complet
      // de l'ancien moteur (Value<=-30 + dominance MX+OR+MA, juge non
      // rentable) par jeuSimpleGP() (croisement Top 5 IA Place x Value<=-30,
      // voir jeuSimpleGP.js) - le badge ci-dessous reflete desormais ce
      // nouveau signal (pool Place + eventuel Gagnant) plutot que l'ancien.
      const jeu = jeuSimpleGP(result.chevaux, topIAPlace, race.lieu, race.numeroCourse);
      // *** fix sept. 2026 *** : `jeu.place` n'existe que si `jeu.rentable`
      // est vrai (sinon `jeu` vaut `{rentable:false}` sans autre champ) -
      // calculer `placeTxtListe` doit donc rester a l'interieur du meme
      // garde que `badgeFormeHtml`, sous peine de "Cannot read properties
      // of undefined (reading 'chevaux')" sur toute course non couverte
      // par le croisement Simple G/P (bug signale par l'utilisateur sur
      // "Liste des courses").
      const badgeFormeHtml = jeu.rentable
        ? `<span class="small tag-blue bold">Simple G/P : Place N&deg;${jeu.place.chevaux.map((c) => c.entry.numero).join('-')}${jeu.gagnant ? ` / Gagnant N&deg;${jeu.gagnant.chevaux[0].entry.numero}` : ''}</span>`
        : '';
      const jeuSupp = jeuSupplementation(result.chevaux, supplementation);
      const badgeSupplementationHtml = jeuSupp.rentable
        ? `<span class="small tag-orange bold">Suppl&eacute;mentation : Place/Gagnant N&deg;${jeuSupp.place.chevaux.map((c) => c.entry.numero).join('-')}</span>`
        : '';
      coursesToutesDates.push({ meeting, race, nbPartants: horseRecords.length, badgeFormeHtml, badgeSupplementationHtml, matchesIA, matchesSupplementation });
    }
  }
  ...
  if (courses.length === 0) {
    appEl.innerHTML = `
      ...
      <div class="empty-state">
        ...
        <p class="muted">Cette liste retient les courses couvertes par le Top 5 IA Plac&eacute; Turf BZH du jour et/ou par la Suppl&eacute;mentation PMU du jour (onglet Importer), quels que soient leur nombre de partants ou leurs scores.</p>
      </div>`;
  ...
  appEl.innerHTML = `
    ...
    <div class="list-group">${courses.map(({ meeting, race, nbPartants, badgeFormeHtml, badgeSupplementationHtml, matchesIA, matchesSupplementation }, i) => {
      const dateVal = new Date(meeting.date).toISOString().slice(0, 10);
      ...
      const iaHtml = matchesIA && matchesIA.length > 0
        ...
        : '';
      const suppHtml = matchesSupplementation && matchesSupplementation.length > 0
        ? `<div class="small tag-orange bold" style="margin-top:4px;" title="Chevaux de cette course figurant dans la Suppl&eacute;mentation PMU du jour - informatif">${matchesSupplementation.map((m) => `Suppl. N&deg;${m.numero} (${m.supp.note ?? '-'}/20)`).join(' - ')}</div>`
        : '';
      return `
      <div class="list-item clickable" data-goto="race/${race.id}">
        <div>
          ...
          ${badgeSupplementationHtml ? `<div style="margin-top:4px;">${badgeSupplementationHtml}</div>` : ''}
          ${iaHtml}
          ${suppHtml}
...
async function renderBilanSimpleGagnant() {
  ...
  const introHtml = `<div class="card" style="margin-bottom:10px;">
      <p class="small muted" style="margin:0;">Courses dont le Simple G/P est jouable (sept. 2026) : croisement du Top 5 IA Plac&eacute; Turf BZH du jour import&eacute; avec notre pool Value &le; -30. Simple PLACE jou&eacute; sur tous les chevaux du croisement (prioritaire) ; Simple GAGNANT jou&eacute; uniquement si le cheval au rang IA 1 en fait partie - voir la fiche course pour le d&eacute;tail, et HEBERGEMENT.md pour la m&eacute;thode.</p>
    </div>`;
  ...
  if (candidates.length === 0) {
    appEl.innerHTML = `
      ...
      <div class="empty-state">
        ...
        <p class="muted">Une course appara&icirc;t ici d&egrave;s qu'au moins un cheval Value&le;-30 figure dans le Top 5 IA Plac&eacute; Turf BZH du jour import&eacute; (onglet Importer).</p>
      </div>`;
    bindGoto();
  ...
  const analysees = candidates.map(({ meeting, race, nbPartants, jeu }) => {
    ...
    if (ordreArrivee.length === 0) {
      return { meeting, race, nbPartants, jeu, connu: false, vrai1: null, placeGagne: null, gagnantGagne: null, rapportStatus: 'en_attente', bilan: null };
    }
    ...
    const gagnantGagne = jeu.gagnant ? jeu.gagnant.chevaux[0].entry.numero === vrai1 : false;
    ...
    if (besoinRapportPlace || besoinRapportGagnant) {
      return { meeting, race, nbPartants, jeu, connu: true, vrai1, placeGagne, gagnantGagne, rapportStatus: 'a_recuperer', bilan: null };
    }
    ...
    if (!dividendeConnu) {
      return { meeting, race, nbPartants, jeu, connu: true, vrai1, placeGagne, gagnantGagne, rapportStatus: 'indisponible', bilan: null };
    }
    return { meeting, race, nbPartants, jeu, connu: true, vrai1, placeGagne, gagnantGagne, rapportStatus: 'ok', bilan };
  });
  ...
  const rowHtml = (a) => {
    ...
    const dateLabelHtml = datesCandidates.length > 1
      ? `<span class="small tag-blue bold" style="margin-right:6px;">${new Date(dateVal).toLocaleDateString('fr-FR')}</span>`
      : '';
    const gagnantTxt = a.jeu.gagnant ? ` / Gagnant n&deg;${a.jeu.gagnant.chevaux[0].entry.numero}` : '';
...
async function renderBilanGlobalSimpleGagnant() {
  renderTopbar('Bilan Global Simple G/P', { back: () => navigate('simplegagnant') });
