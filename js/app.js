...
async function renderRaceDetail(raceId, useCote8h = false) {
  ...
  appEl.innerHTML = `
    ...
    ${convergenceHtml(RaceAnalyzer.calculerIndiceConvergence(result.chevaux))}
...
/**
 * "Confiance maximale" : sur le backtest reel (1 mois d'archives, 1027
 * courses, voir HEBERGEMENT.md), la combinaison la plus fiable trouvee est
 * un cheval a la fois "Base tres solide" confirmee techniquement
 * (Module 2, niveau `confirmee_forte`) ET classe n1 par le Score Global
 * (Module 1) : 41,7% de victoires, 75,0% de Top3 sur cette combinaison
 * (n=24 - rare mais tres au-dessus de tout le reste). Purement indicatif,
 ...
 * selon la discipline, mesure sur le meme backtest (1027 courses) : le
 * taux de victoire des bases confirmees varie fortement selon la
 * discipline (Attele 35,4%, Steeple 44,4% mais echantillon reduit, Haies
 * 28,6% echantillon reduit, contre Plat 21,7% et Monte 21,2%).
 * Purement indicatif (affichage), n'entre dans aucun calcul.
 */
function fiabiliteDiscipline(disciplineCanonique) {
  switch (disciplineCanonique) {
    case 'ATTELE':
      return { cls: 'tag-green', label: 'Fiabilite renforcee (Attele)', detail: '35,4% de victoires sur les bases confirmees, n=198' };
    case 'STEEPLE':
      return { cls: 'tag-green', label: 'Fiabilite renforcee (Steeple)', detail: '44,4% de victoires sur les bases confirmees, n=18 (echantillon reduit)' };
    case 'HAIES':
      return { cls: 'tag-gray', label: 'Fiabilite intermediaire (Haies)', detail: '28,6% de victoires sur les bases confirmees, n=14 (echantillon reduit)' };
    case 'PLAT':
      return { cls: 'tag-orange', label: 'Fiabilite plus moderee (Plat)', detail: '21,7% de victoires sur les bases confirmees, n=115' };
    case 'MONTE':
      return { cls: 'tag-orange', label: 'Fiabilite plus moderee (Monte)', detail: '21,2% de victoires sur les bases confirmees, n=33' };
    default:
      return null;
  }
}
 ...
 * le backtest reel (1 mois, 1027 courses, voir HEBERGEMENT.md) COURSE PAR
 * COURSE (un seul "pick" par course, pas cheval par cheval), pour repondre
 * a la question "quelle course a la plus grande probabilite de se gagner ?".
 * Cascade en 2 niveaux, du plus fiable au plus courant :
 *  1. "confiance_maximale" (cf. chevalConfianceMaximale) : base tres
 *     solide confirmee ET classee n1 -> 41,7% victoires / 75,0% Top3
 *     (n=24, 2,3% des courses).
 *  2. "base_confirmee_unique" : UNE SEULE base confirmee (forte ou
 *     simple) dans toute la course, classee 1ere ou 2e par le Score
 *     Global -> 31,6% victoires / 61,1% Top3 (n=285, 27,8% des courses).
 * Note testee et ECARTEE comme niveau de repli : utiliser seul le badge
 * "Course logique + Hierarchie claire" en l'absence de base confirmee ne
 * marche pas (18,2% victoires sur l'echantillon, n=22 - moins bien que le
 * simple favori du modele sans aucun filtre, 26,3%) : ce fallback n'a
 * PAS ete retenu ici.
 * @returns {{pick:Object, niveau:string}|null}
 */
function estCourseFiable(bd, chevaux) {
  const confMax = chevalConfianceMaximale(bd, chevaux);
  if (confMax) return { pick: confMax, niveau: 'confiance_maximale' };
  ...
  if (confirmees.length === 1) {
    ...
    if (cheval && (cheval.classement === 1 || cheval.classement === 2)) {
      return { pick: cheval, niveau: 'base_confirmee_unique' };
    }
  }
  return null;
}
 ...
 * meme backtest, un petit champ (<=10 partants) et les disciplines
 * Attele/Steeple sont associes a une bien meilleure fiabilite des bases
 * confirmees. Les deux bonus reunis montent a 51,6% victoires / 67,7%
 * Top3 (n=31, echantillon reduit) contre 32,4%/62,1% sans distinction.
 */
function bonusCourseFiable(nbPartants, disciplineCanonique) {
  return {
    petitChamp: nbPartants <= 10,
    disciplineFavorable: disciplineCanonique === 'ATTELE' || disciplineCanonique === 'STEEPLE'
  };
}

const LIBELLES_NIVEAU_COURSE_FIABLE = {
  confiance_maximale: { label: 'Confiance maximale', detail: '41,7% de victoires, 75,0% de Top3 sur le backtest (n=24)' },
  base_confirmee_unique: { label: 'Base confirmee unique', detail: '31,6% de victoires, 61,1% de Top3 sur le backtest (n=285)' }
};

function courseFiableHtml(bd, chevaux, disciplineCanonique) {
  const fiable = estCourseFiable(bd, chevaux);
  if (!fiable) return '';
  const { pick, niveau } = fiable;
  const info = LIBELLES_NIVEAU_COURSE_FIABLE[niveau];
  ...
  const bonusNote = (petitChamp && disciplineFavorable)
    ? ' Les deux bonus reunis (petit champ + discipline favorable) montent a 51,6% de victoires / 67,7% de Top3 sur le backtest (n=31, echantillon reduit).'
    : '';
  ...
  return `<div class="card" style="border: 2px solid var(--tag-green, #2e7d32); margin-bottom:10px;">
      ...
      <p class="muted small" style="margin-top:4px;">${escapeHtml(info.label)} : ${escapeHtml(info.detail)}.${bonusNote} Base sur le backtest reel (1 mois, 1027 courses, voir HEBERGEMENT.md). Indicatif, n'entre dans aucun calcul de Score Global/Value/classement.</p>
    </div>`;
}
...
// Seuils d'affichage de l'Indice de convergence (RaceAnalyzer.calculerIndiceConvergence,
// 5 signaux au total). >= 4/5 = convergence forte, 3/5 = convergence moderee.
const SEUIL_CONVERGENCE_FORTE = 4;
const SEUIL_CONVERGENCE_MODEREE = 3;

function signauxActifsLabel(signaux) {
  return Object.entries(signaux)
    .filter(([, actif]) => actif)
    .map(([cle]) => RaceAnalyzer.LIBELLES_SIGNAUX_CONVERGENCE[cle] || cle)
    .join(', ');
}

function convergenceLigneHtml(c) {
  return `<div class="list-item"><div><div class="bold small">N&deg;${c.numero} - ${escapeHtml(c.nom)}</div><div class="muted small">${c.nbSignaux}/5 signaux : ${signauxActifsLabel(c.signaux)}</div></div></div>`;
}

function convergenceHtml(convergence) {
  const forte = (convergence || []).filter((c) => c.nbSignaux >= SEUIL_CONVERGENCE_FORTE);
  const moderee = (convergence || []).filter((c) => c.nbSignaux === SEUIL_CONVERGENCE_MODEREE);

  return `
    <div class="card">
      <h3 style="margin:0;">Indice de convergence</h3>
      <p class="muted small" style="margin-top:4px;">Compte, pour chaque cheval, combien de signaux independants (cote qui se resserre, forme, aptitude et conditions au-dessus de la moyenne du champ, bonus rubriques positif) vont dans le meme sens favorable. Regle de bon sens transparente, PAS une correlation statistique prouvee : a prendre comme un element de plus, pas comme une prediction a elle seule.</p>
      <p class="bold small" style="margin-bottom:4px;">Convergence forte (4-5 signaux sur 5)</p>
      ${forte.length > 0
        ? `<div class="list-group">${forte.map(convergenceLigneHtml).join('')}</div>`
        : '<p class="muted small">Aucun cheval avec convergence forte sur cette course.</p>'}
      ${moderee.length > 0 ? `
      <div style="margin: 10px 0; border-top: 1px solid var(--border);"></div>
      <p class="bold small" style="margin-bottom:4px;">Convergence moderee (3 signaux sur 5)</p>
      <div class="list-group">${moderee.map(convergenceLigneHtml).join('')}</div>` : ''}
    </div>
  `;
}
  ...
  for (const meeting of meetings) {
    ...
    for (const race of races) {
      ...
      if (estCourseSure(bd, result.chevaux, result.resume)) {
        ...
        // *** Petit champ *** : sur le backtest reel (1 mois, 1027 courses,
        // voir HEBERGEMENT.md), les bases confirmees dans un champ de <=10
        // partants sont nettement plus fiables (35,2% victoire / 73,0%
        // Top3, n=122) que dans un champ de >=14 partants (25,2%/57,7%,
        // n=123). Purement indicatif : ne retire aucune course de la
    ...
    <div class="list-item clickable" data-goto="race/${race.id}">
      <div>
        ...
        <div style="margin-top:4px; display:flex; gap:6px; flex-wrap:wrap;">
          ...
          ${fiable ? `<span class="small tag-green bold" title="${escapeHtml(LIBELLES_NIVEAU_COURSE_FIABLE[fiable.niveau].detail)}">&#9989; Course fiable : N&deg;${fiable.pick.entry.numero}</span>` : ''}
        </div>
      </div>
      <div class="muted">&rsaquo;</div>
    </div>
