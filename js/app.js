...
function fourchetteRapportHtml(fourchette, detail) {
  if (!fourchette) return '';
  return `<p class="muted small" style="margin-top:2px;" title="${escapeHtml(detail)}">Fourchette th&eacute;orique du rapport : ${fourchette.basse.toFixed(2)}&euro; &agrave; ${fourchette.haute.toFixed(2)}&euro;</p>`;
}

function trioValueAvecBaseHtml(bd, chevaux, cotesCibles) {
  ...
  return `<div class="card" style="margin-bottom:10px;">
      ...
      ${fourchetteRapportHtml(fourchette, detailFourchette)}
    </div>`;
}
...
function basesEtDangersHtml(bd, cotesCibles, r, chevaux, disciplineCanonique) {
  ...
  const trioValue = trioValueAvecBaseHtml(bd, chevaux, cotesCibles);
