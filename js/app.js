...
import { jeuSimpleGagnant, misesJeuSimpleGagnant, MISES_PRESETS_JEU_SIMPLE_GAGNANT, bilanJeuSimpleGagnant, rendementBilan, cumulerBilansJournaliers, multiplicateurMiseAptitudeRang1, SEUIL_SCORE_FORME_RANG1, SEUIL_SCORE_APTITUDE_RANG1, multiplicateurMiseJoursAbsenceRang1, SEUIL_JOURS_ABSENCE_MISE_RENFORCEE } from './engine/jeuSimpleGagnant.js';
...
async function verifierEtNotifierCourse(meeting, race, toutesPerfs) {
  try {
    ...
    const jeu = jeuSimpleGagnant(result.chevaux, race.lieu, context.discipline.canonical, context.typeCourse);
...
async function renderRaceDetail(raceId, useCote8h = false) {
  ...
  appEl.innerHTML = `
    ...
    ${basesEtDangersHtml(basesEtDangers, cotesCibles, result.resume, result.chevaux, context.discipline.canonical, predictionExterne, context.lieu, context.typeCourse)}
  ...
  bindJeuSimpleGagnant(result.chevaux, race.lieu, context.discipline.canonical, context.typeCourse);
 ...
 *
 * *** v17 (aout 2026, a la demande de l'utilisateur) *** : la mise
 * ponderee optionnelle n'est plus basee sur l'ecart de Score Global mais
 * sur le scoreAptitude du cheval retenu (x2 si >=100, retestee plus
 * rentable et plus robuste sur le pool v16 - voir jeuSimpleGagnant.js).
 *
 * *** v18 (aout 2026, a la demande de l'utilisateur) *** : les courses de
 * categorie F (TypeCourse) en discipline PLAT sont exclues d'office
 * (`jeu.typeCourseExclu === true`, jeu non jouable) - voir jeuSimpleGagnant.js
 * pour le detail du backtest (rendement quasi a l'equilibre et fragile sur
 * cette categorie en PLAT, effet absent en ATTELE/MONTE/8h).
 */
...
function jeuSimpleGagnantHtml(chevaux, hippodrome, disciplineCanonique, typeCourse) {
  const jeu = jeuSimpleGagnant(chevaux, hippodrome, disciplineCanonique, typeCourse);
  if (!jeu.rentable) {
    const raison = jeu.hippodromeExclu
      ...
      : jeu.typeCourseExclu
      ? `Course de cat&eacute;gorie F en discipline Plat, exclue d'office (non rentable sur l'archive compl&egrave;te - voir HEBERGEMENT.md).`
  ...
  const miseAptitudeHtml = jeu.scoreAptitudeRang1 != null ? `
      <div class="field" style="margin-bottom:8px;">
        <label style="display:flex; align-items:center; gap:6px; cursor:pointer;">
          <input type="checkbox" id="jsg-mise-aptitude-check">
          <span>Mise renforc&eacute;e si scoreAptitude &eacute;lev&eacute; (actuel : ${fmt1(jeu.scoreAptitudeRang1)})</span>
        </label>
      </div>` : '';
  ...
  return `<div class="card" style="margin-bottom:10px;">
      ...
      <p class="muted small" style="margin-top:8px;">Mise flat sur le rang 1 (Dutching &agrave; 1 cheval, rendement = sa cote). Seuil de cote calibr&eacute; sur archives r&eacute;elles (voir HEBERGEMENT.md).${jeu.scoreAptitudeRang1 != null ? ' Mise renforc&eacute;e scoreAptitude (optionnelle, v17) : x2 si scoreAptitude &ge;100, x1 sinon &mdash; rendement pond&eacute;r&eacute; mesur&eacute; 131,7% &rarr; 137,1% sur l\'archive compl&egrave;te, remplace l\'ancienne mise par palier d\'&eacute;cart (retest&eacute;e contre-productive sur le pool actuel, voir HEBERGEMENT.md).' : ''}${jeu.joursAbsenceRang1 != null ? ' Mise renforc&eacute;e retour rapide (optionnelle, sept. 2026) : x2 si JA &le;7j, x1 sinon &mdash; rendement mesur&eacute; 175,0% (n=103) contre ~118-124% sans ce filtre sur l\'archive compl&egrave;te ; si les deux cases sont coch&eacute;es, le plus grand des deux multiplicateurs est appliqu&eacute; (pas cumulatif, effet combin&eacute; non backtest&eacute; s&eacute;par&eacute;ment) &mdash; voir HEBERGEMENT.md.' : ''} Indicatif, n'entre dans aucun calcul de Score Global/Value/classement.</p>
    </div>`;
}
 ...
 */
function bindJeuSimpleGagnant(chevaux, hippodrome, disciplineCanonique, typeCourse) {
  ...
  btn.addEventListener('click', () => {
    ...
    let multiplicateurAptitude = 1;
    const aptitudeCheck = appEl.querySelector('#jsg-mise-aptitude-check');
    if (jeu.scoreAptitudeRang1 != null && aptitudeCheck && aptitudeCheck.checked) {
      multiplicateurAptitude = multiplicateurMiseAptitudeRang1(jeu.scoreAptitudeRang1);
    }
    ...
    // Si les deux cases sont cochees et qualifient, on prend le PLUS GRAND des
    // deux multiplicateurs (pas de cumul multiplicatif) : l'effet combine des
    // deux signaux n'a pas ete backteste separement, voir jeuSimpleGagnant.js.
    const multiplicateurApplique = Math.max(multiplicateurAptitude, multiplicateurJoursAbsence);
    ...
    const raisons = [];
    if (multiplicateurAptitude === multiplicateurApplique && multiplicateurAptitude !== 1) raisons.push(`scoreAptitude ${fmt1(jeu.scoreAptitudeRang1)}`);
    if (multiplicateurJoursAbsence === multiplicateurApplique && multiplicateurJoursAbsence !== 1) raisons.push(`JA ${jeu.joursAbsenceRang1}j`);
    const miseAptitudeInfoHtml = multiplicateurApplique !== 1
      ? `<p class="small tag-blue bold" style="margin-top:4px;">Mise renforc&eacute;e appliqu&eacute;e : x${fmt1(multiplicateurApplique)} (${raisons.join(' et ')})</p>`
      : '';
    resultEl.innerHTML = `
...
function basesEtDangersHtml(bd, cotesCibles, r, chevaux, disciplineCanonique, predictionExterne, hippodrome, typeCourse) {
  const conseilJeuCard = conseilJeuHtml(bd, chevaux);
  const jeuSimpleGagnantCard = jeuSimpleGagnantHtml(chevaux, hippodrome, disciplineCanonique, typeCourse);
...
async function renderListeCourses() {
  ...
  for (const meeting of meetings) {
    ...
    for (const race of races) {
      ...
      coursesToutesDates.push({ meeting, race, nbPartants: horseRecords.length, numerosEligibles });
    }
  }
  ...
  appEl.innerHTML = `
    ...
    <div class="list-group">${courses.map(({ meeting, race, nbPartants, numerosEligibles }, i) => {
      const dateVal = new Date(meeting.date).toISOString().slice(0, 10);
