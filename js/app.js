...
async function renderListeCourses() {
  ...
  for (const meeting of meetings) {
    ...
    for (const race of races) {
      ...
      const jeu = jeuSimpleGP(result.chevaux, topIAPlace, race.lieu, race.numeroCourse);
      const placeTxtListe = `Place N&deg;${jeu.place.chevaux.map((c) => c.entry.numero).join('-')}`;
      const badgeFormeHtml = jeu.rentable
        ? `<span class="small tag-blue bold">Simple G/P : ${placeTxtListe}${jeu.gagnant ? ` / Gagnant N&deg;${jeu.gagnant.chevaux[0].entry.numero}` : ''}</span>`
        : '';
