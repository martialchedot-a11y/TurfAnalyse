...
**Methode retenue, validee sur le backtest reel (3 mois, 3037 courses)** :
plusieurs criteres ont ete testes pour choisir ces 5 chevaux parmi le
reste du champ (rang du modele, Danger en priorite, ProbTop3/ProbVictoire
decroissante, Indice de convergence) :

| Critere | Reussite (l'ancre finit Top2, n=346) |
|---|---|
...
sur les courses a 13 partants ou plus (n=127), l'ecart persiste (73,2%
pour la Value contre 68,5% pour le rang du modele). Sur l'ensemble des
courses ou l'ancre existe (n=623, avant meme de savoir si elle finira
Top2), cette methode aboutit a **44,5% de Couples Gagnants reussis
(277/623)** avec seulement 5 tickets.

Compromis nombre de chevaux / reussite (methode Value, parmi les 346
courses ou l'ancre finit Top2) : 3 chevaux -> 61,0%, 4 -> 71,7%, 5 ->
80,1%, 6 -> 85,0%, 7 -> 89,9%. Le choix de 5 (demande par l'utilisateur)
...
course. Fonctions concernees : `candidatsCombinaison` (calcul partage avec
la Suggestion Trio ci-dessous) et `suggestionCoupleGagnantHtml` dans
`js/app.js` (comme les autres indicateurs bases sur
`chevalConfianceMaximale`, ces fonctions ont besoin du DOM et ne peuvent
donc pas etre couvertes par les tests automatises Node — verification de
coherence faite manuellement sur les 3037 courses du backtest).

## Suggestion Trio (2 des 5 chevaux + l'ancre)

Meme principe et memes 5 candidats que la Suggestion Couple Gagnant
ci-dessus (meme ancre, meme tri par Value croissante), mais adaptee au
pari Trio (il faut cette fois deviner les 2 chevaux qui completent le
Top3 avec l'ancre, pas juste 1 pour le Top2) : nouvelle carte affichant
l'ancre + les 5 candidats, avec la consigne de combiner 2 des 5 avec
l'ancre (10 combinaisons possibles au total).

**Methode validee sur le meme backtest (3 mois, 3037 courses)** : l'ancre
finit Top3 67,9% du temps (423/623). Sur ces 423 courses, comparaison des
criteres pour choisir les 5 candidats (il faut cette fois capturer les 2
VRAIS partenaires, pas un seul) :

| Critere | Reussite (les 2 partenaires dans le top 5 candidats, n=423) |
|---|---|
| Rang du modele (classement croissant) | 55,6% |
| Danger d'abord puis rang | 55,6% |
| ProbTop3 decroissante | 44,0% |
| Convergence (nbSignaux) decroissante | 48,9% |
| **Value croissante** | **59,6%** |

La Value croissante reste la meilleure methode, comme pour le Couple
Gagnant. Compromis nombre de candidats / reussite (methode Value) : 2
candidats (1 combinaison) -> 10,4%, 3 (3 combinaisons) -> 27,9%, 4 (6
combinaisons) -> 45,2%, 5 (10 combinaisons) -> 59,6%.

**Important** : purement indicative, n'entre dans aucun calcul de Score
Global/Value/classement. Fonction concernee : `suggestionTrioHtml` dans
`js/app.js` (meme limitation de test que ci-dessus - fonctions liees au
DOM, verification de coherence faite manuellement sur le backtest).

## Conseil de jeu (cascade Couple/Trio -> Simple -> Abstention)
...
envisageable ?", une nouvelle carte **"Conseil de jeu"** a ete ajoutee en
tete de la page Course (avant "Course fiable"), qui applique une cascade
a 3 niveaux valide sur le backtest reel (3 mois, 3037 courses) :

| Niveau | Condition | Conseil affiche | Victoires | Top3 | n |
|---|---|---|---|---|---|
| 1 | Ancre existe (Base tres solide confirmee, classee n1) | Jouer Couple Gagnant et/ou Trio | 39,6% | 67,9% | 623 |
| 2 | Sinon, une Base tres solide existe encore (confirmee a un autre rang, ou non confirmee) | Jouer Simple gagnant/place sur la mieux classee | 42,3% | 71,2% | 378 |
| 3 | Sinon (aucune Base tres solide) | S'abstenir de jouer | - | - | 2036 |

**Le niveau 2 est au moins aussi bon que le niveau 1** (42,3% contre
39,6% de victoires), ce qui valide pleinement son usage comme repli : ce
n'est pas un pis-aller, c'est un signal a part entiere.

**Sur la justification du niveau 3 (abstention)** : sans aucune Base tres
solide, le simple favori du modele (rang 1, sans filtre) tombe a
20,4%/49,1% (n=2036) - grosso modo moitie moins bien que les niveaux 1 et
2. Point important verifie avant de conclure a l'abstention : une "Base
solide" confirmee (le cran juste en dessous de "tres solide") ne rattrape
rien - 17,8% de victoires sur les 297 courses ou elle existe sans Base
tres solide, ce qui n'est PAS mieux que les 21,3% obtenus sur les 1739
courses restantes sans aucune base confirmee du tout. Il n'y a donc pas
d'edge recuperable juste en dessous du seuil "tres solide" : l'abstention
est le choix statistiquement justifie a ce niveau, pas une simple
prudence par defaut.
...
au DOM, verification de coherence faite manuellement sur le backtest).
Purement indicatif : n'entre dans aucun calcul de Score Global/Value/
classement, et ne remplace pas votre propre jugement sur la course.
...
## Suggestion "Couple champ total (Dangers)" (2 a 5 Danger(s))

A la demande d'exploiter ce constat, une nouvelle carte a ete ajoutee sur
la page Course : quand la course compte entre 2 et 5 Danger(s), elle
propose de jouer TOUTES les combinaisons Couple Gagnant parmi ces
chevaux (le "champ total" des Danger(s)). Au-dela de 5 Danger(s), le
nombre de combinaisons devient vite trop eleve (15+) pour rester un pari
raisonnable, la suggestion ne s'affiche donc pas.

**Taux de reussite mesure (les 2 vrais chevaux du Top2 dans le champ
total des Danger(s))**, qui augmente logiquement avec le nombre de
Danger(s)/combinaisons :

| Danger(s) | Combinaisons | Reussite | n |
|---|---|---|---|
| 2 | 1 | 21,1% | 436 |
| 3 | 3 | 33,2% | 716 |
| 4 | 6 | 47,0% | 755 |
...
Sur l'ensemble des courses a 2-5 Danger(s) (81,6% de l'echantillon,
n=2479), la reussite moyenne est de 39,9%. Contrairement au Couple
Gagnant et au Trio bases sur l'ancre (qui ne s'affichent que dans les
~20% de courses avec une Base tres solide confirmee classee n1), cette
suggestion s'applique a la quasi-totalite des courses (des qu'il y a 2 a
5 Danger(s), independamment de toute Base).

Fonctions concernees : `coupleChampTotalDangers` et
`coupleChampTotalDangersHtml` dans `js/app.js` (meme limitation de test
que les sections precedentes). Purement indicatif : n'entre dans aucun
calcul de Score Global/Value/classement, et ne remplace pas votre propre
jugement sur la course.

## Suggestion "Couple Top5 x Dangers" (intersection Top5 classement / Dangers)
...
donne une reponse en deux temps, avec un resultat nuance sur le
croisement :

**Le Top5 seul est deja un tres bon signal, sans aucune condition** : les
2 vrais chevaux du Top2 s'y trouvent 54,4% du temps (94,5% au moins un),
sur l'ensemble des courses - meilleur que la liste des Danger(s) seule
(41,0%/90,5%, voir plus haut).

**Le croisement (intersection Top5 x Danger(s))** fait baisser le taux
BRUT de reussite : 36,8% (les 2) / 88,5% (au moins 1) sur les 97,5% de
courses ou l'intersection contient au moins 2 chevaux (taille moyenne
3,59) - on perd les vrais gagnants qui ne sont pas assez "joues" par le
marche pour porter le tag Danger (typiquement un favori tres largement
domine). **Mais avec deux fois moins de combinaisons en moyenne** (5,37
contre 10 pour le Top5 entier), **le rendement par combinaison jouee est
meilleur** : 6,86% de reussite par combinaison contre 5,44% pour le Top5
seul, soit environ +26%.

**Conclusion** : le croisement n'est pas benefique si l'objectif est de
maximiser le taux de reussite brut (dans ce cas, jouer le Top5 entier,
10 combinaisons, reste meilleur) - il l'est si l'objectif est de limiter
le nombre de tickets pour un meilleur rendement par mise.

Nouvelle carte **"Coupl&eacute; Top5 &times; Dangers"** ajoutee sur la
page Course (visible des que l'intersection contient au moins 2
chevaux), affichant les candidats et le nombre de combinaisons.
**Limitation de combinaisons** : l'intersection est par construction un
sous-ensemble du Top5 (5 chevaux maximum), donc jamais plus de
C(5,2) = 10 combinaisons - contrairement au "Couple champ total
(Dangers)" ci-dessus, cette suggestion n'a pas besoin d'un plafond
distinct sur le nombre de Danger(s) pour rester raisonnable.

Fonctions concernees : `coupleTop5xDangers` et `coupleTop5xDangersHtml`
dans `js/app.js` (meme limitation de test que les sections precedentes).
Purement indicatif : n'entre dans aucun calcul de Score Global/Value/
classement, et ne remplace pas votre propre jugement sur la course.
