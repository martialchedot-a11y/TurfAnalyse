...
- **Filtre** : toutes les courses, toutes reunions importees confondues,
  dont le **score de configuration du Coupl&eacute; Value correspond au
  filtre choisi** (cumulatif ou exact, 2 a 5, par defaut "4/5 et plus" —
  cf. "Filtre de confiance ajustable" ci-dessous pour le detail du
  selecteur et les taux de reussite mesures par palier) **ET** dont
  le nombre de partants est compris entre **8 et
  16** (a la demande de l'utilisateur — les champs en dehors de cette plage
  sont ecartes de la liste, meme avec un bon score) **ET** dont **au plus
  50% des chevaux sont sans historique de performances retrouve**
  ...
  Value et le score de configuration peu fiables). Triees par score
  decroissant.
- **Affichage** : pour chaque course, le niveau de confiance (ex.
  "Confiance elevee (4/5)", meme badge/couleur que sur la carte Coupl&eacute;
...
## Rapports officiels PMU (Coupl&eacute; Gagnant) et bilan financier

A la demande de l'utilisateur, la page **Resultat** (voir section
precedente) affiche desormais un **bilan financier hypoth&eacute;tique** du
Coupl&eacute; Value, calcul&eacute; a partir des **rapports officiels PMU**
(dividendes reellement pay&eacute;s) plut&ocirc;t que d'une simple
r&eacute;ussite/&eacute;chec :
...
- **Nouvel endpoint PMU** : `.../rapports-definitifs`, qui renvoie, une fois
  ...
  CORS), via la nouvelle fonction `fetchRapportsPmu` (jamais d'exception,
  renvoie `null` en cas d'&eacute;chec reseau).
- **Extraction du Coupl&eacute; Gagnant** (`extraireRapportsCoupleGagnant`,
  `js/engine/pmuApi.js`) : privil&eacute;gie le type de pari
  "point de vente" (`COUPLE_GAGNANT`, la r&eacute;f&eacute;rence
  habituellement cit&eacute;e), et se replie sur la variante "internet"
  (`E_COUPLE_GAGNANT`) si absente. Renvoie un tableau vide si le PMU n'a
  ouvert aucun des deux (arrive encore sur certains tres petits champs,
  m&ecirc;me apres filtrage 8-16 partants, si des non-partants ram&egrave;nent
  le champ effectif sous le seuil requis par le PMU pour ce pari).
- **Bilan (`bilanCoupleValue`, `js/app.js`)** : suppose **1&euro; jou&eacute;
  sur CHACUNE des combinaisons possibles** entre les candidats du
  Coupl&eacute; Value (mise 1€/combinaison choisie par l'utilisateur) - le
  nombre de candidats (et donc de combinaisons) suit d&eacute;sormais le
  **pool adaptatif** (cf. section pr&eacute;c&eacute;dente) : C(5,2) = 10
  paires (10&euro;) sur le profil confiance forte/confirmations fortes,
  jusqu'&agrave; C(8,2) = 28 paires (28&euro;) sur les profils les plus
  incertains, et aucune mise sur la cellule d'abstention. Chaque
  combinaison est compar&eacute;e **sans tenir compte de l'ordre** des 2
  chevaux au rapport officiel (r&egrave;gle r&eacute;elle du Coupl&eacute;
  Gagnant PMU, contrairement au Coupl&eacute; Ordre). Le bilan (mise, gains,
...
- **R&eacute;cup&eacute;ration manuelle** : un bouton "R&eacute;cup&eacute;rer
  ...
  propos&eacute; le Coupl&eacute; Gagnant) est **persist&eacute;** sur la
  course (`race.rapportCoupleGagnant`) pour ne plus avoir a le
  re-demander a chaque ouverture de la page.
- **Affichage** : une carte "Bilan financier (hypoth&eacute;tique) Coupl&eacute;
  Value" en haut de la page R&eacute;sultat (net global en gros, d&eacute;tail
  mise/gains, nombre de courses prises en compte), et un petit indicateur
  (net +/- en &euro;, ou "Coupl&eacute; Gagnant indisponible"/"Rapport non
  r&eacute;cup&eacute;r&eacute;") sur chaque ligne de course.
- *** Mise a jour *** : le rapport officiel (combinaison gagnante reelle +
  dividende pour 1&euro;), une fois connu, s'affiche aussi directement sur la
  page **Course feu vert** elle-m&ecirc;me, juste a c&ocirc;t&eacute; du tag
  "Coupl&eacute; Value" de chaque course (ex. "Rapport officiel : N&deg;6-N&deg;1
  &rarr; 40,70&euro;") — pas besoin d'aller sur la page R&eacute;sultat pour
  ...
  Fonction concernee : `rapportCoupleGagnantHtml` (`js/app.js`).
...
- C'est un bilan **hypoth&eacute;tique**, pas un historique de vos mises
  ...
  les 10 combinaisons du Coupl&eacute; Value sur TOUTES les courses feu vert
  du jour, avec 1&euro; par combinaison. Si vous jouez diff&eacute;remment
  (montant different, sous-ensemble des combinaisons, pas toutes les
  courses feu vert...), ce chiffre ne refl&egrave;te pas votre resultat reel.
- Les courses o&ugrave; le PMU n'a pas propos&eacute; de Coupl&eacute;
  Gagnant sont **exclues** du bilan (mise ET gain), puisqu'il aurait
  &eacute;t&eacute; impossible de placer ce pari dans la r&eacute;alit&eacute;
  — elles ne comptent donc ni comme une perte ni comme un gain.
...
(`tests/engine.test.js`), executes reellement avec Node.js : 72 tests, tous
