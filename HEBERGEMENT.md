...
**Date manquante sur "Bilan Simple Gagnant" - courses de plusieurs jours
melangees sans distinction (sept 2026, signale par l'utilisateur)** :
l'utilisateur a signale qu'en testant successivement deux journees
d'archive differentes (01/08/2026 puis 08/08/2026), une meme course
(ENGHIEN R3C6) semblait "changer de date" entre l'appli et le fichier Excel
de reference. Verification faite avec le moteur reel directement sur les
fichiers d'archive : le calcul lui-meme est correct et coincide exactement
avec l'Excel pour les deux dates (01/08 -> R1C6 + R3C6/NOB HILL ; 08/08 ->
R8C1 uniquement ; la course ENGHIEN R3C6 du 08/08 est un champ totalement
different de celle du 01/08 - meme numero de reunion/course reutilise un
autre jour, hippodrome tres actif - et n'est pas rentable au Jeu Simple
Gagnant ce jour-la).

La racine du probleme est ailleurs : `collecterCandidatesSimpleGagnant()`
(qui alimente la page "Bilan Simple Gagnant", la ou l'utilisateur consulte
les "courses a jouer") parcourt TOUTES les reunions actuellement stockees
(`DB.getAllMeetings()`, sans filtre de date) - contrairement a "Liste des
courses"/"Courses du jour" qui se limitent a `estAujourdHui()`. C'est
volontaire (l'utilisateur doit pouvoir tester une journee d'archive
independamment de la date du jour reel), mais tant que "Vider toutes les
reunions importees" n'a pas ete cliquee entre deux imports de jours
differents, les courses des deux journees se retrouvaient melangees dans
une seule liste SANS AUCUNE indication de date sur chaque ligne - risque
concret d'attribuer une course a la mauvaise journee en la lisant.

Deux correctifs dans `js/app.js` (`renderBilanSimpleGagnant`) :
1. La date de la reunion (`meeting.date`) est desormais affichee en badge
   sur chaque ligne de la liste, mais uniquement quand plusieurs journees
   sont effectivement melangees (pour ne pas surcharger l'affichage dans le
   cas normal d'une seule journee en cours de test).
2. Des que plusieurs journees sont detectees parmi les candidats
   (`datesCandidates.length > 1`), un selecteur "Filtrer par date"
   apparait (meme emplacement que le filtre de mode existant), permettant
   d'isoler une seule journee et de retrouver exactement le meme
   comportement qu'avant (sans etre oblige de vider les reunions entre
   deux tests). Le filtre de date s'applique en amont (sur `candidates`),
   donc le bilan financier, le taux de reussite et le "Transfert bilan"
   redeviennent coherents pour la journee isolee.

`APP_VERSION` passe a `v86` (et `CACHE_NAME` dans `sw.js` a
`turf-analyse-v86`). Aucun changement moteur (uniquement `js/app.js`) - 177
tests au total, tous toujours passants.

**Suppression du filtre "confirmation externe" sur Top base/Resultat (sept
2026, a la demande de l'utilisateur : "je n'utilise plus ce fichier")** :
`renderCourseFeuVert` (page "Top base") et `renderResultatJournee` (page
"Resultat") exigeaient chacune, en plus du score de configuration et de la
Base tres solide, que cette Base soit CONFIRMEE (simple ou double) par un
fichier de predictions externe importe pour le jour/hippodrome/course
concerne (onglet Importer) - sans un tel fichier, aucune course ne pouvait
jamais apparaitre sur ces 2 pages. L'utilisateur n'utilisant plus ce
fichier, ce filtre est retire dans les deux fonctions : seule l'existence
d'une Base tres solide (`trioValueAvecBase` non nul) reste requise, en plus
du score de configuration (seuil choisi par l'utilisateur, `ge4` par
defaut) et des filtres deja en place (8 a 16 partants, historique retrouve
pour au moins la moitie du champ). Le texte des etats vides des 2 pages est
mis a jour en consequence. Le badge "Confirmation externe" sur chaque ligne
(`confirmationExterneHtml`) reste affiche a titre purement informatif si
l'utilisateur importe malgre tout un fichier de predictions un jour donne,
mais ne conditionne plus l'apparition d'aucune course - le reste de la
fonctionnalite (import, stockage, badge sur la fiche course) est inchange.

Backtest de verification (sur tout l'archive, score de configuration >=4/5,
sans le filtre retire) : 2032 courses desormais eligibles (contre un volume
quasi nul auparavant, faute de fichiers de predictions importes sur
l'archive), 71,6% de reussite de la Base (Gagnant OU Place). Un filtre
supplementaire sur la colonne SC (voir plus haut, section correspondante) a
ete teste sur ce meme perimetre et ecarte : SC>0 est deja verifie par 91%
des Bases tres solides retenues (les 2 logiques reperent le meme profil de
cheval), et le reste (SC<=0, 181 courses) n'est pas mesurablement moins bon
(66,9% de reussite, ecart non significatif vu le petit echantillon) - SC
n'est donc PAS ajoute comme filtre supplementaire ici non plus.

`APP_VERSION` passe a `v87` (et `CACHE_NAME` dans `sw.js` a
`turf-analyse-v87`). Aucun changement moteur (uniquement `js/app.js`) - 177
tests au total, tous toujours passants.

**Recuperation de l'arrivee independante des cotes sur "Bilan Simple
Gagnant" (sept 2026, bug signale par l'utilisateur)** : cas reel rapporte -
un cheval joue a quelques secondes du depart (selection Jeu Simple Gagnant)
a gagne, mais une fois les cotes mises a jour pour recuperer l'arrivee et
les rapports, la course avait disparu de "Bilan Simple Gagnant",
empechant le "Transfert bilan".

Cause : `collecterCandidatesSimpleGagnant()` recalcule `jeuSimpleGagnant()`
EN DIRECT a partir des cotes actuellement en base (pas d'apres un instantane
fige au moment du pari). Or la seule facon d'obtenir automatiquement
l'arrivee officielle d'une course etait jusqu'ici la "mise a jour des
cotes" (course ou reunion complete), qui ecrase TOUJOURS `coteDirecte` de
chaque cheval avec la cote PMU la plus recente au passage - y compris apres
la course, ou cette cote n'est plus la cote de depart mais une cote deja
influencee par le resultat (souvent tres resserree pour le vainqueur). Si
elle repasse a 3,8 ou moins, le cheval sort du pool `candidatsDirecte` de
`jeuSimpleGagnant()` (jeuSimpleGagnant.js) et la course disparait purement
et simplement de la page, malgre une victoire reelle.

Correctif (uniquement `js/app.js`, `renderBilanSimpleGagnant`) : nouveau
bouton "Recuperer les arrivees" sur cette page, qui appelle directement
`fetchResultatPmu` (deja utilise en interne par la mise a jour des cotes,
mais jamais expose seul) et n'ecrit QUE `race.arriveeBrute` - sans jamais
toucher aux cotes des chevaux (`fetchCotesPmu`/`DB.updateHorses` ne sont pas
appeles). A utiliser desormais A LA PLACE de "mise a jour des cotes" une
fois les paris places, pour ne jamais risquer de perdre l'eligibilite
Simple Gagnant d'une course deja jouee. Les textes d'aide de la page
("Aucun resultat connu...") sont mis a jour pour pointer vers ce nouveau
bouton plutot que vers la mise a jour des cotes.

Limite assumee : ce correctif evite le probleme pour les prochaines
courses, mais ne peut pas "reparer" une course dont la cote directe a deja
ete ecrasee avant ce correctif (l'ancienne cote de depart n'est plus
recuperable a posteriori) - dans ce cas, seul un ajout manuel du resultat
dans le suivi personnel de l'utilisateur reste possible pour cette course
precise.

`APP_VERSION` passe a `v88` (et `CACHE_NAME` dans `sw.js` a
`turf-analyse-v88`). Aucun changement moteur (uniquement `js/app.js`) - 177
tests au total, tous toujours passants.

**"Liste des courses" : bouton de rafraichissement des cotes + deverrouillage
de la date reelle du jour (sept 2026, a la demande de l'utilisateur)** :
deux changements demandes suite a une question sur la stabilite des scores
Forme/Aptitude (confirmee independante des cotes, voir plus haut) :

1. La page etait jusqu'ici limitee aux reunions dont la date correspond
   EXACTEMENT a la date reelle du jour (`estAujourdHui`). Elle reprend
   desormais TOUTES les reunions actuellement importees (meme logique que
   "Bilan Simple Gagnant"/`collecterCandidatesSimpleGagnant()`), pour
   pouvoir aussi l'utiliser sur une journee d'archive. Comme pour le
   correctif ENGHIEN R3C6 (v86), un badge de date par ligne + un selecteur
   "Filtrer par date" apparaissent des que plusieurs journees sont
   melangees parmi les reunions importees (etat/fonctions dedies a cette
   page, independants de ceux de Bilan Simple Gagnant). Le decompte de la
   prochaine course reste calcule par rapport a l'heure REELLE actuelle et
   a la date de CHAQUE reunion (et non plus toujours "aujourd'hui") : sur
   une journee d'archive, le depart calcule tombe donc naturellement dans
   le passe ("Depart passe"), sans decompte actif ni faux-positif.
2. Nouveau bouton "Rafraichir les cotes" en haut de la page, qui reprend le
   mecanisme du bouton "Mettre a jour les cotes de toute la reunion" de la
   fiche reunion (`fetchCotesPmu` -> `apparierCotesZeturf` ->
   `DB.updateHorses`), course par course et de facon sequentielle, pour
   toutes les reunions actuellement affichees (apres filtre de date
   eventuel). Volontairement limite aux cotes seules, sans recuperation
   d'arrivee (cette page sert a preparer/consulter des courses a venir, pas
   a suivre des resultats) - et sans le risque identifie en v88 sur "Bilan
   Simple Gagnant" : le filtre d'eligibilite de cette page (scoreForme/
   scoreAptitude) ne depend jamais de la cote, donc rafraichir les cotes
   ici ne peut jamais faire disparaitre une course de la liste.

`APP_VERSION` passe a `v89` (et `CACHE_NAME` dans `sw.js` a
`turf-analyse-v89`). Aucun changement moteur (uniquement `js/app.js`) - 177
tests au total, tous toujours passants.

**Alarme sonore 2 minutes avant chaque depart sur "Liste des courses" (sept
2026, a la demande de l'utilisateur, pour iPhone)** : deux approches
possibles ont ete presentees a l'utilisateur, qui a choisi la premiere :

1. **Alerte de premier plan** (retenue) : un son se declenche automatiquement
   2 minutes avant le depart de CHAQUE course affichee, tant que la page
   reste ouverte et l'ecran allume. Simple, s'appuie sur le decompte deja
   existant.
2. Vraie notification meme telephone verrouille/appli fermee : aurait
   necessite un systeme de notifications Push (infrastructure serveur
   programmee, du meme ordre que la fonction externe deja utilisee pour les
   cotes PMU) - projet nettement plus lourd, ecarte.

Implementation (uniquement `js/app.js`, `renderListeCourses`) : nouveau
bouton "Activer l'alarme sonore" en haut de la page. Contrainte technique
iOS/Safari incontournable : AUCUNE lecture audio automatique n'est permise
sans geste utilisateur prealable dans la page - le clic sur ce bouton sert
donc a la fois a activer l'alarme et a "deverrouiller" l'audio (AudioContext)
pour le reste de la session (redevient necessaire a chaque rechargement de
l'appli, l'etat n'est pas persiste). Le son (3 bips a 880 Hz) est genere en
memoire via l'API Web Audio (oscillateur), sans fichier audio externe -
coherent avec le fonctionnement hors-ligne de l'appli. Le decompte existant
(`listeCoursesCountdownIntervalId`, deja calcule course par course, une a la
fois) declenche l'alarme une seule fois par course des que le temps restant
passe sous 2 minutes (`SEUIL_ALARME_LISTE_COURSES_MS`), y compris si la page
est ouverte alors qu'on est deja sous ce seuil. Un `Set` en memoire
(`racesAlarmeListeCoursesDeclenchees`) evite tout redeclenchement pour une
meme course.

`APP_VERSION` passe a `v90` (et `CACHE_NAME` dans `sw.js` a
`turf-analyse-v90`). Aucun changement moteur (uniquement `js/app.js`) - 177
tests au total, tous toujours passants.

**Changement du mot de passe d'acces (sept 2026, a la demande de
l'utilisateur)** : nouvelle valeur de `HASH_ATTENDU` dans
`js/passwordGate.js` (le mot de passe en clair n'est jamais ecrit dans le
code ni dans cette documentation - voir section "Ecran de mot de passe"
plus haut pour le mecanisme et la procedure pour le changer soi-meme la
prochaine fois).

`APP_VERSION` passe a `v91` (et `CACHE_NAME` dans `sw.js` a
`turf-analyse-v91`). Aucun changement moteur (uniquement
`js/passwordGate.js`) - 177 tests au total, tous toujours passants.

**Correction (v92)** : dans la v91, la cle de memorisation `localStorage`
(`turfanalyse_acces_ok`) etait fixe, donc un appareil deja deverrouille
avant le changement de mot de passe ne redemandait rien - le nouveau hash
n'etait compare qu'aux appareils pas encore deverrouilles. Corrige en
derivant la cle de memorisation du hash attendu lui-meme
(`turfanalyse_acces_ok_` + les 8 premiers caracteres de `HASH_ATTENDU`,
voir `js/passwordGate.js`) : desormais, tout changement futur de mot de
passe invalide automatiquement le deverrouillage memorise sur TOUS les
appareils, qui redemanderont le mot de passe une seule fois au prochain
chargement.

`APP_VERSION` passe a `v92` (et `CACHE_NAME` dans `sw.js` a
`turf-analyse-v92`). Aucun changement moteur (uniquement
`js/passwordGate.js`) - 177 tests au total, tous toujours passants.

## Mise a jour septembre 2026 (31) : mise renforcee sur retour rapide (signal "JA", jours d'absence)

**Contexte : l'archive a ete etendue a 26 mois (juillet 2024 - aout 2026) et
un bug de fuite de donnees a ete decouvert dans les fichiers musiques.**
Suite a l'ajout de 6 mois d'archive (juillet-decembre 2024), l'utilisateur a
juge les resultats "pas convaincants". Investigation complete : la baisse
provenait d'une periode juillet-octobre 2024 plus faible, mais sans lien
avec la saison/le mois (les memes mois en 2025-2026 restent bons), ni avec
l'hippodrome ou la discipline (verifie sur l'archive complete) - conclusion :
simple variance statistique sur une periode plus ancienne, pas un defaut de
la methode v18.

A la demande de l'utilisateur, plusieurs pistes de signal de confirmation
supplementaire (turfbzh : Note_IA, Value BZH, ELO_Cheval, Sigma_Horse, ELO
Jockey/Entraineur, Synergie JCh, TPch90 ; "value positive" classique, seule
puis filtree par score Aptitude/Forme) ont ete testees et **toutes ecartees**
(aucun signal propre, monotone et robuste - voir details dans les scripts de
backtest ad-hoc, non repris dans ce document). Au cours de cette recherche,
un **bug de fuite de donnees majeur** a ete identifie dans les fichiers
`_musiques.csv` de l'archive : contrairement a ce qu'on pourrait attendre
d'archives datees, ces fichiers ne sont PAS des instantanes figes a la date
du dossier - ils contiennent l'historique COMPLET de chaque cheval jusqu'a
la date ou l'archive a ete constituee (recemment, selon l'utilisateur), donc
des performances **futures** par rapport a la date de la reunion pour une
bonne partie de l'archive (jusqu'a 94% du contenu utilise par
`scoreForme`/`scoreAptitude` pour les mois les plus anciens). **Ce bug
n'affecte QUE les backtests sur archive** (aucune donnee future n'existe en
utilisation reelle/en direct, puisqu'on ne peut pas connaitre l'avenir) -
mais il fausse en partie le calibrage historique de `scoreForme`/
`scoreAptitude` sur les mois anciens de l'archive. Ce point reste a traiter
separement si l'utilisateur souhaite fiabiliser davantage les futurs
backtests ; aucune action n'a ete demandee dans l'immediat au-dela de la
piste ci-dessous.

**Solution demandee par l'utilisateur : la colonne "JA" (Jours d'Absence).**
L'utilisateur a signale que le fichier `_partants` du jour (pas le fichier
`_musiques` contamine) contient directement une colonne "JA" (jours ecoules
depuis la derniere course du cheval), deja calculee par turfbzh/PMU **au
moment de la course elle-meme** - donc fiable et point-in-time, sans aucune
dependance a l'historique contamine.

**Backtest complet (786 fichiers, 1338 selections v18 rentables, hors le
mois "septembre 2026" en cours) sur le JA du cheval retenu par v18 :**

- JA&le;3j : n=75, reussite 44,0%, rendement 187,3%
- JA&le;5j : n=86, reussite 40,7%, rendement 177,4%
- **JA&le;7j : n=103, reussite 40,8%, rendement 175,0%**
- JA&le;10j : n=159, reussite 34,0%, rendement 139,7%
- JA&le;14j : n=320, reussite 30,3%, rendement 130,0%
- JA&le;20j : n=547, reussite 26,3%, rendement 113,6%
- JA&gt;30j : n=444, reussite 25,9%, rendement 123,8% (proche de la
  moyenne generale)

Seuil retenu : **7 jours** (n=103, meilleur compromis volume/rendement -
descendre a 3-5j ameliore encore legerement le rendement mais sur un
echantillon trop reduit). Robustesse verifiee par retrait des 5 plus gros
gains individuels du sous-ensemble JA&le;7j : rendement 175,0% -> 135,6%,
toujours nettement au-dessus de la moyenne generale (~118-124%). Effet
confirme A L'INTERIEUR de chaque discipline prise separement (Attele seul :
27,3%/117,9% -> 46,8%/197,9% ; Plat seul : 22,3%/116,8% -> 37,5%/183,1%), et
etale sur toute la periode (aout 2024 a aout 2026) et une quinzaine
d'hippodromes differents - pas un artefact d'une seule discipline,
periode ou hippodrome.

**Decision retenue :** comme pour le signal scoreAptitude (v17), ce signal
ameliore la MISE (pas un nouveau filtre - le volume de selections v18 reste
inchange) : mise renforcee optionnelle (case a cocher, desactivee par
defaut) x2 si JA&le;7j, x1 sinon.

**Implementation (`js/engine/jeuSimpleGagnant.js`) :** nouvelles constantes
exportees `SEUIL_JOURS_ABSENCE_MISE_RENFORCEE = 7` et
`MULTIPLICATEUR_MISE_JOURS_ABSENCE_ELEVEE = 2`, nouvelle fonction
`multiplicateurMiseJoursAbsenceRang1(joursAbsence)` (meme forme que
`multiplicateurMiseAptitudeRang1`). `jeuSimpleGagnant()` expose desormais
`joursAbsenceRang1` (le `entry.joursAbsence` du cheval retenu, `null` si
absent du fichier) dans les DEUX modes (cote directe et cote 8h), en plus de
`scoreAptitudeRang1` (v17, inchange). Prealable necessaire
(`js/engine/csvImporter.js`) : la colonne "JA" du fichier `_partants` n'etait
pas encore extraite - ajout de la resolution par nom (`col(['JA'], ...)`,
position de repli 13 pour les rares fichiers sans en-tete exploitable) et du
champ `entry.joursAbsence` (entier, `null` si case vide) sur chaque cheval
importe.

**`js/app.js` :** deuxieme case a cocher "Mise renforcee si retour rapide
(JA&le;7j)" affichee sous la case scoreAptitude existante (memes mecanique
et style). **Si les DEUX cases sont cochees et qualifient simultanement**, le
plus grand des deux multiplicateurs est applique (`Math.max`, PAS de cumul
multiplicatif x2*x2=x4) : l'effet combine des deux signaux n'a pas ete
backteste separement, donc pas de raison de supposer qu'il se cumule
lineairement - prudence deliberee. Le message informatif sous le bouton
"Calculer mise" precise desormais lequel des deux criteres (ou les deux) a
declenche la mise renforcee appliquee.

`APP_VERSION` passe a `v93` (et `CACHE_NAME` dans `sw.js` a
`turf-analyse-v93`). 5 nouveaux tests dans `tests/engine.test.js`
(`multiplicateurMiseJoursAbsenceRang1` bornes x1/x2 et valeurs invalides,
exposition de `joursAbsenceRang1` par `jeuSimpleGagnant()` dans les deux
modes et lorsqu'absent du fichier, parsing de la colonne "JA" par
`parseReunionComplete()` sans valeur - le cas avec valeur etant ajoute a un
test existant) - 182 tests au total, tous passants.

## Mise a jour septembre 2026 (32) : historiquePour exclut desormais TOUTE performance posterieure a la course (pas seulement le meme jour) - corrige la contamination musiques.csv pour les backtests

**Question de l'utilisateur** : "pour les backtests, est-ce trop complique
de ne tenir compte que des perfs ayant une date anterieure a la course ?" -
en reponse directe a la contamination des fichiers `_musiques.csv`
decouverte dans la mise a jour precedente (31).

**Reponse : non, changement simple et cible.** Le fix lookahead existant
(aout 2026, mise a jour (21)) excluait de `historiquePour()` uniquement la
performance dont la date correspond EXACTEMENT a celle de la course etudiee
(`String(p.datePerf).slice(0,10) !== dateCourseAExclure`) - ce qui bloquait
la fuite la plus evidente (le cheval "connaissant" le resultat de la course
qu'on analyse) mais laissait passer toute autre performance ULTERIEURE
presente dans le fichier musiques (le vrai probleme identifie en (31) :
pour une bonne partie de l'archive, ce fichier contient l'historique
complet du cheval jusqu'a la date d'export de l'archive, donc de nombreuses
courses posterieures a la date etudiee, pas seulement celle du jour meme).

**Correctif implemente (`js/engine/csvImporter.js`, `historiquePour()`)** :
le filtre passe d'une egalite stricte (`!==`) a une comparaison
d'ordre (`<`) : `String(p.datePerf).slice(0,10) < dateCourseAExclure`.
Consequence : toute performance datee DU JOUR MEME de la course OU
POSTERIEURE est desormais exclue de l'historique utilise pour calculer les
scores (`scoreForme`, `scoreAptitude`, etc.), au lieu de la seule
performance du jour meme. La comparaison reste une simple comparaison de
chaines "AAAA-MM-JJ" (format a largeur fixe, donc fiable lexicographiquement,
meme methode que l'ancien filtre - pas de nouvelle dependance).

**Portee du changement : backtests uniquement, aucun effet en direct.** En
utilisation reelle (analyse d'une course du jour), le fichier musiques ne
peut par definition jamais contenir de performance posterieure a
aujourd'hui - le nouveau filtre `<` ne retire donc rien de plus que l'ancien
`!==` dans ce contexte. Le changement corrige exclusivement les scripts de
backtest ad-hoc qui rejouent l'archive (ils appellent la meme fonction
`historiquePour()` avec la date de la course archivee comme
`dateCourseAExclure`) : desormais, `scoreForme`/`scoreAptitude` recalcules
sur l'archive ne voient plus JAMAIS le futur du cheval, quelle que soit
l'anciennete du mois archive.

**Consequence attendue sur les chiffres deja cites dans ce document** :
les backtests realises AVANT ce correctif (tous ceux cites plus haut,
notamment ceux qui s'appuient sur `scoreForme`/`scoreAptitude` - v12 et
suivantes du Jeu Simple Gagnant) ont pu, dans une mesure variable selon
l'anciennete du mois d'archive concerne, beneficier d'une fuite de donnees
partielle. Aucun nouveau backtest global n'a ete relance a ce jour pour
requantifier l'ecart exact - a faire si l'utilisateur souhaite revalider les
seuils actuels (`SEUIL_SCORE_APTITUDE_RANG1`, `SEUIL_SCORE_FORME_RANG1`,
etc.) sur des donnees desormais garanties point-in-time.

`APP_VERSION` passe a `v94` (et `CACHE_NAME` dans `sw.js` a
`turf-analyse-v94`). 1 nouveau test dans `tests/engine.test.js`
(`historiquePour` exclut les performances posterieures ET celle du jour
meme, ne conserve que les performances strictement anterieures) - 183 tests
au total, tous passants.

## Mise a jour septembre 2026 (33) : revalidation complete post-fix (32) - tous les filtres v12-v18 invalides, nouveau seuil de classement Value<=-30

**Contexte.** Suite au fix (32) (`historiquePour` exclut desormais toute
performance posterieure, pas seulement le jour meme), l'utilisateur a
demande de rejouer le backtest complet pour etre sur des chiffres. Resultat :
la contamination `_musiques.csv` corrigee en (32) avait un impact bien plus
severe que prevu - le rendement global du pool top2+cote>3,8 (base de tout
le Jeu Simple Gagnant v12-v18) passe de 119,3% (avant fix) a **89,9%** (apres
fix, n=1411) sur l'archive complete. C'est une PERTE, pas un gain.

**Revalidation systematique de tous les filtres v12-v18 sur donnees
desormais debiaisees** (script `backtestRevalidationV18.mjs`, pool brut
top2+cote>3,8+meilleurAptitude, sans aucun filtre v12-v18 applique, pour
pouvoir retester chaque seuil isolement) :

- Signal JA (jours d'absence <=7, mise a jour (31)) : 175,0% -> **91,5%**
  rendement post-fix, NON robuste (tombe a 59,2% apres retrait des 5 plus
  gros gains). Signal invalide, mais conserve dans le moteur/l'UI (v94) en
  l'absence de demande explicite de retrait de l'utilisateur.
- Filtre scoreAptitude>=85 / scoreForme>=70 (production) : 86,0% rendement
  (une perte), NON robuste (tombe a 71,5% apres retrait des 50 plus gros
  gains).
- Score Croisement<=3 (v16) : sens INVERSE sur donnees debiaisees (score=4
  a 82,0% de rendement contre 94,4% pour score=2 - aucun gradient propre).
- Exclusion TypeCourse=F en PLAT (v18) : sens INVERSE (sous-ensemble exclu a
  91,1% de rendement contre 85,9% pour le sous-ensemble conserve).
- Ecart8h>=15 (v11) : tient encore faiblement (88,8% contre 78,5%) mais les
  deux sous-ensembles restent des pertes.
- Liste d'exclusion hippodromes (v14, 18 hippodromes) : tient partiellement
  en moyenne (86,0% -> 89,6% applique) mais la liste precise est obsolete
  (plusieurs hippodromes precedemment exclus semblent desormais corrects ;
  plusieurs non-exclus sont catastrophiques, ex. Chartres 0%, Cabourg 38,4%,
  Toulouse 56,6%).

**Recherche systematique d'un nouveau signal** (script
`backtestRechercheSignal.mjs`, tous les candidats qualifies du top2, pas de
pre-selection par aptitude) : comparaison de profil gagnants/perdants sur 14
sous-scores (scoreGlobal, scoreAptitude, scoreForme, scoreConditions,
scoreCote, scoreSimilaire, scoreRubriques, nbCourses, value, probVictoire,
ecart, scoreCroisement, joursAbsence, cote) - separation quasi nulle sauf sur
les mesures derivees de la cote (tautologiques). 13 variantes de departage a
2 candidats testees - toutes regroupees entre 84,8% et 87,3% de rendement,
statistiquement indiscernables. Aucun nouveau signal robuste trouve dans ce
cadre (pool top2+cote>3,8).

**Association cheval x jockey/entraineur performant** (a la demande de
l'utilisateur, script `backtestSignalJockeyEntraineur.mjs`, jointure avec
`turfbzh_database.csv` - verifie point-in-time, contrairement aux fichiers
"Statistiques Ecarts Jockeys/Entraineurs Turf BZH.xlsx" fournis par
l'utilisateur qui sont des instantanes du jour, inutilisables en backtest) :
ELO_Jockey, Synergie JCh et Rang J. ne montrent aucune separation
gagnant/perdant. ELO_Entraineur montre un gradient reel (84,7% -> 129,5%
rendement selon le seuil) mais NON robuste (s'effondre apres retrait des
plus gros gains) et confondu avec la cote (ELO_Entraineur eleve correle
avec une cote moyenne plus courte : 6,33 sous 1400 ELO contre 4,95 au-dessus
de 1550 ELO) - capte probablement juste le statut de favori deja integre au
classement, pas une information nouvelle.

**Decouverte cle : Value<=-30, SANS aucune restriction de classement ni de
cote, sur l'ensemble de l'archive** (tous chevaux, toutes courses, script
`backtestValueBrute.mjs`, n=307836 chevaux) :

| Groupe | n | victoires | reussite | rendement |
|---|---|---|---|---|
| Value <= -30 | 71181 | 15058 | **21,2%** | 87,6% |
| Value > -30 | 236655 | 10415 | 4,4% | - |

Value<=-30 concentre **59,1% (~60%) de TOUS les vainqueurs de l'archive**
(15058 sur 25473 victoires totales) alors qu'il ne represente qu'environ 23%
des chevaux au depart. C'est le signal le plus net de toute l'investigation.

**Redesign du TRI (classement) - `js/engine/raceAnalyzer.js`.** A la suite
de ce constat, l'utilisateur a propose de reviser le seuil de groupe du TRI
(`Value<0` -> `Value<=-30`). Deux variantes de tri secondaire au sein de
chaque groupe testees par backtest complet (`backtestClassementV2.mjs` puis
`V3.mjs`, 307836 chevaux, seuil de groupe -30 dans les deux cas) :

| Variante | Reussite rang1 | Rendement rang1 | % vainqueurs captes (rang1) | % vainqueurs captes (top2) |
|---|---|---|---|---|
| Ancien (seuil Value<0, tri ProbVictoire+ProbTop3) | 25,5% | 85,6% | 25,5% | 44,8% |
| V2 (seuil -30, tri "proximite a -30") | 19,8% | 88,0% | 19,8% | 41,8% |
| **V3 (seuil -30, tri ProbVictoire+ProbTop3 INCHANGE) - RETENU** | **28,6%** | **86,1%** | **28,6%** | **48,3%** |

La variante V2 (tri par proximite a -30) a ete rejetee : elle perd
l'information du modele Plackett-Luce (ProbVictoire+ProbTop3, qui integre
deja la cote) et degrade la concentration des vainqueurs en tete de
classement. La variante V3 (seul le SEUIL de groupe change, le tri
secondaire ProbVictoire+ProbTop3 decroissant est conserve a l'identique) est
retenue : +3,1 points de reussite au rang1 par rapport a l'ancien systeme,
rendement stable, et robuste - stable sur les deux moities chronologiques de
l'archive (29,0%/28,3% de reussite), stable apres retrait des 50 plus gros
gains (28,5%), et cote moyenne du rang1 plus basse (3,72 contre 4,48) donc
le gain n'est pas un artefact de quelques gros coups.

**Implemente dans `js/engine/raceAnalyzer.js`** (fonction `analyser()`, TRI
final) : le seuil de groupe passe de `value < 0` a `value <= -30` ; le tri
secondaire (`probVictoire + probTop3` decroissant au sein de chaque groupe)
est inchange. Ce changement se propage automatiquement a tout ce qui
consomme le champ `classement` (Jeu Simple Gagnant, Bases/Dangers, Cotes
cibles, etc.) sans modification de leur code.

**A faire / non fait a ce jour :** les filtres v12-v18 desormais invalides
sur donnees debiaisees (scoreAptitude>=85/scoreForme>=70, exclusion
hippodromes, score Croisement<=3, exclusion TypeCourse=F, signal JA<=7)
restent presents dans `jeuSimpleGagnant.js`/`app.js` (v95) - ils n'ont pas
ete retires, l'utilisateur n'ayant pas encore demande leur suppression.
Impact du nouveau classement V3 sur le Jeu Simple Gagnant (qui repose sur
`classement<=2`) non encore rebacktestee a jour avec le nouveau TRI - a
faire si l'utilisateur souhaite requantifier v18 sur le nouveau classement.

1 nouveau test dans `tests/engine.test.js` (verifie qu'un cheval Value<=-30
est classe avant un cheval Value>-30 meme avec un moins bon
ProbVictoire+ProbTop3) - 184 tests au total, tous passants. `APP_VERSION`
passe a `v95` (et `CACHE_NAME` dans `sw.js` a `turf-analyse-v95`).

## Mise a jour septembre 2026 (34) : au sein du groupe Value<=-30, departage par ELIMINATION (cote la plus basse) plutot que par score

**Question de l'utilisateur.** Sachant que le choix se fait desormais parmi
les chevaux Value<=-30 (mise a jour 33), et en oubliant provisoirement la
restriction top2/cote>3,8 : plutot que de choisir un cheval par un score,
ne serait-il pas plus adapte d'ELIMINER selon certains criteres pour n'en
garder qu'un seul ?

**Analyse (script `backtestValueMoins30Elimination.mjs`, archive complete
debiaisee, 25386 courses ayant au moins 1 cheval Value<=-30 - soit 99,7% de
toutes les courses de l'archive, SANS restriction top2/cote) :**

- **13,0% des courses (3299) n'ont qu'UN SEUL candidat Value<=-30** : la
  selection est deja faite par construction. Ce candidat unique gagne
  **44,3%** du temps (rendement 90,6%).
- **87,0% des courses (22087) ont PLUSIEURS candidats Value<=-30** en
  concurrence (de 2 a 9 dans un meme champ). Dans ce cas, UN des candidats
  Value<=-30 gagne 61,6% du temps - le bon cheval est presque toujours dans
  le lot, il faut juste le departager correctement.

**Criteres de departage testes** (choisir le "meilleur" candidat Value<=-30
selon chaque critere, parmi les 22087 courses a candidats multiples) :

| Critere | Reussite | Rendement |
|---|---|---|
| **Cote la plus basse - RETENU** | **30,1%** | 85,1% |
| Value la plus negative (extreme) | 28,2% | 86,5% |
| Meilleur scoreRubriques | 26,5% | 88,9% |
| Meilleur scoreGlobal / ProbVictoire (ancien critere) | 26,3% | 85,3-85,4% |
| Meilleur scoreAptitude / scoreForme | 25,1% | 85,1-85,4% |
| Moins de jours d'absence | 22,3% | 88,7% |
| Value la plus proche de -30 | 16,1% | 87,7% |
| Moins bon score Croisement | 16,9% | 80,7% |

La "cote la plus basse parmi les candidats Value<=-30" domine nettement.
Robuste : stable sur les deux moities chronologiques (29,5%/30,7%), stable
apres retrait des 50 plus gros gains (29,9%). Des cascades a 2 etapes
(eliminer par score Croisement ou scoreRubriques puis departager par cote)
ont aussi ete testees mais n'ameliorent pas le resultat (21,3% et 29,7%
respectivement - moins bien que la cote seule).

**Strategie complete retenue** (candidat unique si n=1 ; cote la plus basse
si n>=2 candidats Value<=-30) sur les 25386 courses : **31,8% de reussite,
85,3% de rendement** - contre 28,6%/86,1% avec l'ancien tri secondaire
(ProbVictoire+ProbTop3, mise a jour 33). Robuste : 31,5%/32,0% sur les deux
moities chronologiques, 31,6% apres retrait des 50 plus gros gains, cote
moyenne du pick 3,02 (favoris nets, pas des longshots).

**Implemente dans `js/engine/raceAnalyzer.js`** (fonction `analyser()`,
TRI final) : au sein du groupe Value<=-30, le tri secondaire passe de
"ProbVictoire+ProbTop3 decroissant" a "cote la plus basse" (cote
croissante) - ce qui revient exactement a la logique d'elimination
souhaitee : quand plusieurs candidats Value<=-30 existent dans une course,
celui dont le marche le juge le plus favori (cote la plus basse) est
retenu en priorite. Le groupe Value>-30 ("le reste", moins de 0,3% des
courses n'ont AUCUN candidat Value<=-30) garde le tri ProbVictoire+ProbTop3
decroissant, inchange. Ce changement se propage automatiquement partout ou
`classement` est consomme (Jeu Simple Gagnant, Bases/Dangers, Cotes
cibles).

1 nouveau test dans `tests/engine.test.js` (verifie qu'entre 2 chevaux tous
deux Value<=-30, celui a la cote la plus basse est classe avant l'autre
meme si ce dernier a un meilleur ProbVictoire+ProbTop3) - 185 tests au
total, tous passants. `APP_VERSION` passe a `v96` (et `CACHE_NAME` dans
`sw.js` a `turf-analyse-v96`).

**A faire / non fait a ce jour :** meme remarque que la mise a jour (33) -
les filtres v12-v18 desormais invalides restent presents dans
`jeuSimpleGagnant.js`/`app.js`, et l'impact de ce nouveau tri sur le Jeu
Simple Gagnant (`classement<=2`) n'a pas encore ete rebacktestee a jour.

## Mise a jour septembre 2026 (35) : recherche exhaustive de rentabilite (Gagnant ET Place) - aucun angle robuste trouve

Suite a la remarque de l'utilisateur "le probleme, ce n'est pas rentable !" -
la reussite (taux de bons pronostics) s'ameliorait au fil des mises a jour
(33)-(34), mais le rendement (ROI reel apres mise) restait sous 90%
(seuil de rentabilite = 100%) depuis le debut. Deux recherches completes
ont ete menees pour verifier s'il existe QUELQUE PART un angle rentable :

**1) Balayage exhaustif Gagnant (Value x cote), sur l'archive complete
(n=307836 lignes, fichier `value_brute_all.json`) :** 13 tranches de Value
(de <=-80 a >200) testees individuellement, puis une grille 2D
Value x cote. Aucune tranche simple ne depasse 90% de rendement. La grille
2D a fait apparaitre 3 cellules apparemment positives (108,3% / 120,9% /
101,5%), mais les 3 se sont effondrees sous le controle de robustesse
habituel de ce projet (retrait des plus gros gains) : 98,2% / 91,3% / 69%
apres retrait des 2%-72 plus gros gains selon la cellule. Conclusion :
**aucune poche robuste de rendement Gagnant > 100% n'existe** dans le
spectre Value/cote - le plafond observe (~85-90%) correspond au taux de
prelevement pari-mutuel, ce qui signifie que le moteur reproduit
l'efficience du marche plutot que de la battre.

**2) Test du Place (Placé) sur la strategie d'elimination Value<=-30 de
la mise a jour (34)** (candidat unique retenu si n=1 ; cote la plus basse
retenue si n>=2 - logique de production v96) :

- **Reussite Place** (calculee sur l'archive complete, sans API, n=25385
  picks eligibles - courses avec au moins 4 partants donc marche Place
  ouvert) : **58,9%** (63,8% sur les courses a 4-7 partants ou 2 places
  sont payees, 58,4% sur les courses a 8+ partants ou 3 places sont
  payees). Cote moyenne du pick place : 2,85.
- **Rendement Place** (calcule sur un echantillon reel de 78 courses
  interrogees via l'API PMU historique `rapports-definitifs`,
  echantillonnage stratifie 3 courses/mois sur les 26 mois d'archive) :
  dividende moyen reel du Simple Place pour les picks places = 1,50€ pour
  1€ mise (moyenne sur les 78 echantillons, deviation-controlee : 1,48€
  apres retrait du plus gros gain de l'echantillon). En ponderant par la
  proportion reelle de picks qui se placent effectivement
  (14940/25385 = 58,9%), le **rendement Place estime est d'environ
  87-88%** - dans la meme fourchette que le rendement Gagnant, et toujours
  sous le seuil de rentabilite de 100%.

**Conclusion honnete :** ni le Gagnant ni le Place ne presentent d'angle
rentable robuste avec la strategie Value<=-30 actuelle. Le rendement
converge systematiquement vers le taux de prelevement du pari-mutuel
(~85-90%), que ce soit en Gagnant sec ou en Place - ce qui est coherent
avec le fait que le moteur reproduit l'efficience du marche des cotes
plutot que de trouver une inefficience exploitable. Ameliorer la reussite
seule (comme les mises a jour (33)-(34) l'ont fait) ne resout pas ce
probleme structurel : reussite et rendement ne sont pas la meme chose, et
seul le second determine si le jeu est gagnant sur la duree.

**Limite de la methode :** l'estimation du rendement Place repose sur un
echantillon de 78 courses (pas un recensement complet, car l'API PMU
n'est interrogeable qu'une course a la fois et un recensement des ~15000
picks places serait trop long) ; 1 des 78 echantillons presentait une
incoherence (le numero attendu comme place selon l'archive CSV n'apparait
pas dans le rapport Simple Place officiel PMU pour cette course) - a
retenir comme rappel que les donnees d'archive et les donnees officielles
PMU peuvent occasionnellement diverger (cf. mise a jour anterieure sur la
discrepance de dates R3C6 ENGHIEN), mais cela ne remet pas en cause la
conclusion generale (retirer cet echantillon ne change pas l'ordre de
grandeur du rendement estime).

**Aucun changement de code pour cette mise a jour** (recherche pure,
`APP_VERSION` reste `v96`) - la question de savoir si l'app doit
continuer a afficher ces strategies (Jeu Simple Gagnant, "Base tres
solide"...) malgre l'absence d'edge demontre, ou si une refonte plus
profonde est necessaire, reste ouverte et sera tranchee avec
l'utilisateur.

## Mise a jour septembre 2026 (36) : Jeu Simple Gagnant v19 - departage par dominance stricte MX+OR+MA, tous les filtres v12-v18 desactives

Suite a la conclusion negative de la mise a jour (35) ("aucun angle robuste
trouve"), une recherche combinatoire systematique a ete menee sur les 17
rubriques disponibles (CJE, JA, MP, PtH, MN, RC, RX, MX, CX, IdC, CFP, OR,
PC, MA, TG, SC, R10) pour tenter de departager de maniere fiable/rentable
les candidats Value<=-30/cote>=3 (pool retenu depuis la mise a jour (33)),
au lieu de la simple elimination par cote la plus basse de la mise a jour
(34).

**Methode :** pour chaque combinaison de 2 ou 3 rubriques (816 combinaisons
au total, C(17,2)+C(17,3)), departage par **dominance stricte** : parmi les
candidats d'une course, on ne retient que celui qui bat STRICTEMENT tous
les autres candidats simultanement sur chaque rubrique du combo (sens
"meilleur" propre a chaque rubrique - desc ou asc). Si aucun candidat ne
domine ainsi, ou si une donnee est manquante, la course est ignoree pour ce
combo. Recherche menee sur l'archive complete (n=16457 courses
multi-candidats).

**Resultat principal :** la combinaison **MX+OR+MA** ressort nettement en
tete (n=2284, rendement brut 130,1%). Elle a ete soumise a une validation
poussee avant d'etre retenue :

- **Split chronologique (1re/2e moitie de l'archive)** : rendement
  158,5% sur la 1re moitie contre 101,7% sur la 2e - a premiere vue
  inquietant (semble s'effondrer avec le temps).
- **Split eparse (jour du mois pair = entrainement, impair = test)**,
  methode plus rigoureuse suggeree par l'utilisateur pour separer
  veritable generalisation et surajustement pur (un split chronologique
  confond periode temporelle et echantillon d'entrainement) : recherche
  des 816 combos sur l'ECHANTILLON D'ENTRAINEMENT (jours pairs, n=8117)
  seul, puis verification du TOP15 sur l'echantillon TEST jamais vu
  (jours impairs, n=8340). MX+OR+MA confirme un edge reel hors-echantillon :
  entrainement 136,4% (n=1098), test 124,3% (n=1186), IC95%=[111,0% ;
  137,7%], z=3,57 (statistiquement significatif), tient toujours a 106,5%
  apres retrait des 2% plus gros gains.
- **Analyse mensuelle** : l'edge s'erode progressivement au fil du temps
  (150-256% sur 2024, ~103% sur les 12 derniers mois, n=1001) - decroissance
  reelle et graduelle, pas un saut brutal.
- **Verification que l'erosion est un vrai signal et non un artefact
  d'echelle** : la meme decroissance est observee separement chez les picks
  "favori" (cote la plus basse du groupe de candidats) et "non-favori" -
  ce qui exclut un simple decalage de moyenne sur des donnees ordinales
  (le departage se fait par hierarchie relative, pas par seuil absolu).

**Conclusion :** MX+OR+MA est le seul angle de ce projet a avoir passe la
validation hors-echantillon avec une significativite statistique claire,
meme si son edge s'amenuise avec le temps (rendement actuel proche de
100-110% sur les mois les plus recents plutot que les 130% de la moyenne
historique). C'est sur cette base qu'il remplace desormais la selection
en production.

**Implementation (`js/engine/jeuSimpleGagnant.js`, v19) :** le pool de
candidats reste Value<=-30 (`SEUIL_VALUE_SIMPLE_GAGNANT`) et cote>=3
(`SEUIL_COTE_SIMPLE_GAGNANT`, inchanges). Avec un seul candidat, il est
joue automatiquement. Avec plusieurs candidats, on ne joue que si l'un
d'eux domine strictement tous les autres simultanement sur MX (plus haut
= meilleur), OR (plus haut = meilleur) et MA (plus bas = meilleur) ; sinon
(pas de dominant unique, egalite sur une rubrique, ou donnee MX/OR/MA
manquante chez un candidat) -> non rentable, aucune mise n'est proposee.

**Tous les filtres v12 a v18 sont desactives** (ils ne servent plus, la
selection ne repose plus sur le classement Score Global) :
scoreAptitude>=85/scoreForme>=70 (v12/v14), pool top2/top3 du classement
(v13/v15), exclusion de 18 hippodromes non rentables (v14), score
Croisement<=3 (v16), mise ponderee par scoreAptitude x2 (v17), exclusion
des courses de categorie F en Plat (v18) - toutes ces constantes/fonctions
ont ete retirees du moteur (`SEUILS_VALUE_RANG_SIMPLE_GAGNANT`,
`SEUIL_ECART_SCORE_RANG1`, `SEUIL_ECART_SCORE_RANG1_8H`,
`HIPPODROMES_EXCLUS_SIMPLE_GAGNANT`, `estHippodromeExclu`,
`TYPE_COURSE_EXCLU_SIMPLE_GAGNANT`, `estCourseExclueTypeCourse`,
`SEUIL_SCORE_CROISEMENT_MAX`, `multiplicateurMiseAptitudeRang1` et ses 2
constantes associees). `SEUIL_SCORE_APTITUDE_RANG1` (85) et
`SEUIL_SCORE_FORME_RANG1` (70) restent exportes uniquement parce qu'ils
sont encore utilises par le filtre independant "Courses du jour"/"Liste
des courses" (mise a jour aout 2026 (29)) - ils n'interviennent plus dans
`jeuSimpleGagnant()` lui-meme. Le multiplicateur de mise sur retour rapide
(signal JA<=7, mise a jour septembre 2026 (31)) est inchange et continue
de s'appliquer normalement.

**Compatibilite ascendante (bilans historiques) :** pour ne pas casser les
bilans deja enregistres dans IndexedDB (`bilansJournaliersSimpleGagnant`,
avec leurs sous-cumuls par mode `rang1Seul`/`classement8h`), chaque
resultat rentable de la nouvelle logique v19 continue de porter
`rang1Value: true, mode8h: false` en dur - ce qui le fait tomber
naturellement dans le seau "Depuis v19" du filtre de mode et des cumuls
Bilan Global, tandis que les entrees anterieures a v19 restent dans leur
segmentation d'origine ("Avant v19"). Seuls les libelles affiches ont ete
mis a jour dans l'interface (selecteur de filtre, page "Bilan Simple
Gagnant", page "Bilan Global Simple Gagnant") - les valeurs internes
(`'tous'`/`'rang1'`/`'value'`) et les noms de champs ne changent pas.

**Tests unitaires** (`tests/engine.test.js`) entierement reecrits pour la
nouvelle logique v19 : pool vide, valeur/cote hors et sur les bornes
(-30 et 3 inclusifs), candidat unique (jeu automatique), dominance stricte
a 2 et 3 candidats (y compris absence de dominant, egalite sur une
rubrique, donnee manquante), exposition de `joursAbsenceRang1`, et
conservation des tests inchanges pour `misesJeuSimpleGagnant` (Dutching),
`bilanJeuSimpleGagnant`, `rendementBilan` et `cumulerBilansJournaliers`
(fonctions non modifiees par ce changement). Suite complete : 160 tests,
tous passants.

`APP_VERSION` et `CACHE_NAME` passent de `v96` a **`v97`**.

**Limite a garder en tete pour l'usage courant :** l'edge de MX+OR+MA,
bien que statistiquement valide hors-echantillon sur l'ensemble de
l'archive, s'est erode au fil du temps (proche de 100-110% sur les
periodes les plus recentes). Rien ne garantit qu'il se maintienne a ce
niveau dans les mois a venir - un suivi du rendement reel en conditions
de jeu reste recommande.

## Mise a jour septembre 2026 (37) : Jeu Simple Gagnant v20 - le seuil cote>=3 ne filtre plus le pool avant departage, seulement la decision de jouer

Question de l'utilisateur ("par curiosite") : le pool v19 (Value<=-30 ET
cote>=3 des l'etape 1) exclut-il a tort les cotes<3 du departage MX+OR+MA ?

**Verification sur l'archive complete** (25386 courses avec >=1 candidat
Value<=-30, toutes cotes) :

- **Pool v19** (cote>=3 filtre AVANT le departage, auto-jeux a candidat
  unique inclus comme en production reelle) : rendement **98,6%/98,8%** sur
  un split jours pairs/impairs (n=3908/4005), z-score proche de 0 (PAS
  significativement different de 100%). Le chiffre de 130,1%/124,3% cite en
  v19 provenait d'un test limite aux courses a PLUSIEURS candidats (hors
  auto-jeux a candidat unique) - une fois ceux-ci reintegres comme en
  production reelle, l'edge du pool v19 s'evapore.
- **Cause identifiee** : exclure les cotes<3 du pool AVANT le departage fait
  qu'un cheval cote>=3 devient parfois "dominant par defaut" simplement
  parce qu'aucun autre cheval cote>=3 n'est present pour le concurrencer -
  alors qu'un outsider cote<3 avec un meilleur profil MX/OR/MA existe bel et
  bien dans la course et aurait du casser cette dominance.
- **Alternative testee** : calculer la dominance MX+OR+MA parmi TOUS les
  candidats Value<=-30 (cote<3 incluses dans la COMPETITION), puis ne jouer
  que si le cheval qui ressort dominant a lui-meme cote>=3 (la cote<3 reste
  donc toujours interdite AU JEU, seule sa faculte de "casser" la
  dominance d'un autre candidat change). Resultat : rendement
  **127,2%/120,6%** sur le meme split pair/impair (n=1190/1252), IC95%
  nettement au-dessus de 100% dans les deux moities (z=4,34 et z=3,52) -
  signal net et robuste, contrairement au pool v19. Volume divise par ~3,2
  (n=2442 contre 7913 sur l'archive complete) en contrepartie.

**Conclusion :** ce n'est pas la restriction cote>=3 elle-meme qui posait
probleme (l'idee de ne jamais JOUER une cote<3 reste logique cote gestion),
c'est de l'appliquer AVANT le calcul de dominance plutot qu'APRES.

**Implementation (`js/engine/jeuSimpleGagnant.js`, v20) :** le pool de
depart (etape 1) redevient Value<=-30 SANS plancher de cote. Les etapes 2
et 3 sont inchangees (candidat unique -> joue automatiquement ; plusieurs
candidats -> dominance stricte MX+OR+MA calculee sur tout le pool, cotes<3
incluses). Le seuil `SEUIL_COTE_SIMPLE_GAGNANT` (3, valeur inchangee)
n'intervient plus qu'EN DERNIER, sur le cheval retenu par ces etapes : s'il
a cote<3, le jeu est non rentable, meme s'il domine legitimement le reste
du champ. `nbCandidats` refere desormais a la taille du pool complet
(cotes<3 incluses), pas seulement aux candidats jouables.

**Tests unitaires** (`tests/engine.test.js`) : ajout de 3 tests dedies -
un candidat cote<3 qui casse la dominance d'un candidat cote>=3 (-> non
rentable, alors qu'il aurait ete joue en v19), un candidat dominant qui a
lui-meme cote<3 (-> non rentable), et un candidat cote<3 present mais qui
ne casse rien (-> le candidat cote>=3 est quand meme joue normalement).
Suite complete : 163 tests, tous passants.

`APP_VERSION` et `CACHE_NAME` passent de `v97` a **`v98`**.

**Limite a garder en tete :** comme pour v19, cette validation repose sur
l'archive historique complete (pas un test prospectif en conditions
reelles) ; le volume de jeu attendu est nettement plus faible qu'avec le
pool v19 (~3,2x moins de courses jouables), en echange d'un edge
statistiquement plus solide.

## Mise a jour septembre 2026 (38) : badge "Jockey/Entraineur en forme" (informatif, ne modifie pas la selection)

A la demande de l'utilisateur : croiser le pool Value<=-30 avec des fichiers
tiers optionnels "Statistiques Ecarts Jockeys/Entraineurs | Turf BZH"
(export turf.bzh, converti en CSV avant import) pour voir si le
pourcentage de victoires recent du jockey et de l'entraineur ajoute un
signal.

**Constat empirique** (candidats Value<=-30, 90 derniers jours d'archive,
la meme fenetre que celle couverte par les fichiers Ecarts) : quand le
jockey ET l'entraineur ont chacun au moins 15 courses et au moins 10% de
victoires sur cette fenetre :

- Jockey ET entraineur "en forme" (n=1341) : reussite **30,8%**, rendement
  **111,3%**.
- Le reste (au moins un des deux < 10% ou < 15 courses, n=2061) : reussite
  **17,1%**, rendement **78,7%**.
- Ecart tres significatif (z≈9,3) et robuste sur un sous-decoupage
  recent/ancien de la fenetre (fuite de donnees minimale sur la partie
  recente) : 106,8%/84,7% (quinzaine recente) vs 112,3%/77,4% (reste de la
  fenetre) — l'effet ne s'effondre pas quand on reduit la fuite, ce qui est
  rassurant.

**Reserve methodologique importante (a garder en tete) :** les fichiers
Ecarts sont une **photo instantanee** des 90 derniers jours glissants a la
date de l'export, pas un historique date course par course. Tester cette
photo contre l'archive des 90 derniers jours implique necessairement qu'une
partie de l'information (les stats "aujourd'hui" du jockey/entraineur)
est posterieure a certaines des courses testees dans cette meme fenetre —
ce n'est donc **pas un backtest propre au sens strict** (fuite de donnees /
look-ahead bias inherente a l'absence d'un historique date). Le fait que le
signal tienne aussi bien sur la portion recente (fuite minimale) que sur le
reste est encourageant mais ne l'elimine pas completement.

**Decision (a cause de cette reserve) :** le signal est integre en mode
**purement informatif et prospectif**, sans modifier en rien le moteur de
selection (`jeuSimpleGagnant()`, dominance MX+OR+MA v20 inchangee). Il
s'affiche comme un badge indicatif a cote du cheval retenu, pour que Marty
puisse suivre en conditions reelles (sans fuite, puisque les prochaines
courses n'existent pas encore dans les fichiers Ecarts au moment ou elles
sont jouees) si l'ecart se maintient, avant toute decision de le faire
influencer le calcul.

**Implementation :**

- `js/engine/ecartsJockeyEntraineurParser.js` (nouveau fichier) :
  `parseEcarts(csv)` (parse le CSV export turf.bzh — titre + en-tete reel
  detecte par la presence de "Vict", colonnes Courses/Victoires/% Vict.
  etc. par position), `normaliserNomJockeyEntraineur(nom)` (normalisation
  pour faire correspondre le format des fichiers Ecarts, ex. "M.
  Guarnieri", parfois pollue par un compteur "3 courses" concatene par le
  site source, avec celui du fichier partants du jour, ex. "M.GUARNIERI"),
  `statutFormeJockeyEntraineur(statsJockey, statsEntraineur)` et
  `statutFormeCheval(c, dataJockeys, dataEntraineurs)` (`'enForme'` si les
  deux criteres sont remplis, `'insuffisant'` si les deux stats sont
  connues mais au moins un critere manque, `'inconnu'` si jockey et/ou
  entraineur non trouves dans les fichiers importes). Seuils retenus :
  `SEUIL_COURSES_FIABILITE = 15`, `SEUIL_PCT_VICT_FORME = 10`.
- `js/engine/csvImporter.js` : le fichier partants expose desormais aussi
  le nom brut du jockey (`jockeyNom`, colonne `DrivJock`) et de
  l'entraineur (`entraineurNom`, colonne `Entraineur`) sur chaque cheval,
  en plus des scores numeriques `reussiteJockey`/`reussiteEntraineur`
  (rubriques) deja existants — necessaires pour faire le lien avec les
  fichiers Ecarts.
- `js/db.js` (`DB_VERSION` 4 -> **5**) : nouveau store IndexedDB
  `ecartsJockeyEntraineur` (deux entrees, `jockeys` et `entraineurs`,
  chacune remplacee integralement a chaque import — comme
  `predictionsExternes`, ce sont des instantanes, pas un cumul), avec
  `saveEcartsJockeyEntraineur`, `getEcartsJockeyEntraineur`,
  `getAllEcartsJockeyEntraineur`, `resetEcartsJockeyEntraineur`, inclus
  dans l'export/import complet de la base.
- `js/app.js` : nouvelle carte "Ecarts Jockeys/Entraineurs (optionnel)"
  dans Reglages (deux fichiers a importer separement — jockeys et
  entraineurs — plus un bouton de reinitialisation ; le fichier doit etre
  converti en CSV avant import, comme pour les Predictions externes). Le
  badge (vert "Jockey+Entraineur en forme" si `'enForme'`, gris discret
  sinon ou si `'inconnu'`) s'affiche sur la carte "Jeu Simple Gagnant" de
  la fiche course et sur chaque ligne rentable de la page "Liste des
  courses", uniquement pour le cheval retenu par le moteur — il ne
  modifie ni le calcul, ni le classement, ni la decision "rentable/non
  rentable".

**Tests unitaires** (`tests/engine.test.js`) : parsing CSV (titre + vrai
en-tete, colonnes par position, CSV vide), normalisation de nom (majuscules,
espace/tiret/apostrophe retires, prefixe "Mme" retire, artefact "N course(s)"
concatene retire), les 3 statuts de `statutFormeJockeyEntraineur` (y
compris le cas "pourcentage eleve mais pas assez de courses pour etre
fiable"), `statutFormeCheval` (lookup via `entry.jockeyNom`/`entraineurNom`,
noms manquants ou fichiers non importes), et l'exposition de
`jockeyNom`/`entraineurNom` par `csvImporter.js`. Suite complete : 168
tests, tous passants.

`APP_VERSION` et `CACHE_NAME` passent de `v98` a **`v99`**.

**A confirmer avec Marty :** ce choix (badge informatif, aucun impact sur
la selection) a ete fait par prudence a cause de la fuite de donnees
inherente au fichier "photo instantanee" — si le suivi prospectif (sans
fuite) confirme l'ecart dans les semaines a venir, integrer le critere au
calcul lui-meme (ex. en exigeant "enForme" pour jouer, ou en l'ajoutant a
la dominance MX+OR+MA) sera la suite logique.

## Mise a jour septembre 2026 (39) : le badge "Jockey/Entraineur en forme" devient un vrai GATE (Jeu Simple Gagnant v21)

Suite a la mise a jour precedente (38, badge purement informatif), demande
explicite et immediate de l'utilisateur : **"rend le filtre effectif des a
present"**.

**Changement :** `jeuSimpleGagnant(chevaux, ecartsJockeyEntraineur)` (v21,
`js/engine/jeuSimpleGagnant.js`) accepte desormais un 2e parametre optionnel
`{jockeys, entraineurs}` (meme forme que `chargerEcartsJockeyEntraineur()`
dans app.js). Une fois le cheval retenu par les etapes habituelles (pool
Value&le;-30, dominance stricte MX+OR+MA, seuil cote&ge;3 — v19/v20
inchanges) :

- **Si des fichiers Ecarts (jockeys et/ou entraineurs) ont ete importes**, le
  jeu ne reste rentable QUE si ce cheval est "enForme"
  (`statutFormeCheval(...) === 'enForme'` : jockey ET entraineur avec &ge;15
  courses et &ge;10% de victoires sur les 90 derniers jours — voir mise a
  jour 38 ci-dessus pour le detail du constat empirique et sa reserve
  methodologique). Un cheval "insuffisant" (au moins un des deux crit&egrave;res
  non rempli) ou "inconnu" (jockey/entraineur absent des fichiers importes)
  rend desormais le jeu **non rentable** — meme s'il domine legitimement le
  reste du champ sur MX+OR+MA.
- **Si aucun fichier Ecarts n'a ete importe** (fonctionnalite restee
  optionnelle), le gate est desactive d'office et le comportement reste
  identique a v20 — compatibilite ascendante totale pour qui n'utilise pas
  cette fonctionnalite.

**Implementation :** tous les points d'appel de `jeuSimpleGagnant()` dans
`js/app.js` chargent desormais `ecartsJockeyEntraineur` (via
`chargerEcartsJockeyEntraineur()`) et le transmettent en 2e argument : la
fiche course (carte "Jeu Simple Gagnant" + son bouton "Calculer mise"),
"Liste des courses", la surveillance automatique (notifications), et le
recensement des candidates pour "Bilan Simple Gagnant". Le badge affiche
(`badgeFormeJockeyEntraineurHtml`) n'est pas modifie dans sa logique interne
— il se contente desormais d'AFFICHER le statut deja utilise comme gate,
donc un cheval affiche avec le badge montre systematiquement "en forme" des
lors que des fichiers sont importes (puisque sinon le jeu n'aurait pas ete
propose). Les libelles "Indicatif, n'entre dans aucun calcul de Score
Global/Value/classement" ont ete retires des ecrans concernes, remplaces par
une mention explicite du gate dans le message "non jouable" quand des
donnees Ecarts sont importees.

**Reserve methodologique importante, non encore levee :** le constat
empirique de la mise a jour 38 (n=1341 "enForme" a 111,3% de rendement
contre n=2061 "reste" a 78,7%) a ete mesure en croisant le pool Value&le;-30
BRUT (sans dominance MX+OR+MA v19/v20) avec le statut jockey/entraineur —
PAS en appliquant ce gate APRES le departage MX+OR+MA comme c'est desormais
fait en production. L'effet COMBINE des deux filtres n'a pas ete
specifiquement mesure : le fichier Ecarts n'existant qu'en photo instantanee
90 jours glissants (pas d'historique date par date), il est impossible de
reconstituer un vrai backtest retroactif de la combinaison exacte
maintenant utilisee. De plus, la reserve de fuite de donnees deja documentee
en mise a jour 38 (photo instantanee incluant necessairement une part
d'information posterieure a certaines courses de la meme fenetre) s'applique
toujours, et s'applique desormais A LA DECISION DE JOUER elle-meme, pas
seulement a un affichage indicatif. **A recroiser avec Marty une fois
quelques semaines de suivi reel disponibles** (le volume de jeu quotidien va
mecaniquement baisser tant que la couverture des fichiers Ecarts reste
partielle ou que le critere "enForme" n'est pas rempli, ce qui est attendu
et pas un bug).

**Tests unitaires** (`tests/engine.test.js`) : 6 nouveaux tests dedies au
gate — comportement inchange si `ecartsJockeyEntraineur` est omis ou vide
(compatibilite ascendante v20), cheval "enForme" -> rentable (avec
`jeu.statutFormeJockeyEntraineur` expose), cheval "insuffisant" -> non
rentable, cheval "inconnu" (jockey/entraineur absents des fichiers) -> non
rentable, et le cas ou un seul des deux fichiers (jockeys OU entraineurs) a
ete importe -> gate quand meme actif. Suite complete : 174 tests, tous
passants.

`APP_VERSION` et `CACHE_NAME` passent de `v99` a **`v100`**.
...
## Mise a jour septembre 2026 (40) : import direct des fichiers .xlsx (Ecarts Jockeys/Entraineurs)

Demande explicite de l'utilisateur : les fichiers "Statistiques Ecarts
Jockeys/Entraineurs | Turf BZH" sont desormais telecharges directement au
format .xlsx par turf.bzh, et l'appli n'acceptait jusqu'ici que du CSV -
obligeant a une conversion manuelle prealable (Excel > Enregistrer sous >
CSV UTF-8) avant chaque import.

**Changement :** la carte "Ecarts Jockeys/Entraineurs" (page Reglages)
accepte maintenant directement les fichiers `.xlsx` en plus des `.csv` -
plus aucune conversion manuelle necessaire. La lecture du fichier passe par
une nouvelle fonction `readEcartsFileAsCsv(file)` (`js/app.js`) qui detecte
le format (extension `.xlsx`, ou a defaut la signature binaire ZIP "PK" que
tout fichier .xlsx possede), convertit la 1ere feuille du classeur en texte
CSV via la librairie SheetJS, puis reutilise sans aucune modification le
parseur existant `parseEcarts()` (`js/engine/ecartsJockeyEntraineurParser.js`).
Un fichier `.csv` classique continue de suivre exactement l'ancien chemin
(detection d'encodage UTF-8/Windows-1252 inchangee).

**Librairie ajoutee :** `js/vendor/xlsx.core.min.js` (SheetJS `xlsx`
v0.18.5, "core build" - environ 430 Ko). Ce build core, sans les tables de
correspondance de caracteres CJK/proprietaires de la version "full", est
largement suffisant pour des fichiers .xlsx standards (format ouvert
XML/ZIP) en francais - il n'est utile que pour les anciens formats binaires
`.xls` de certaines langues asiatiques, hors sujet ici. Charge globalement
via une balise `<script>` classique dans `index.html` (avant `app.js`),
expose l'objet global `XLSX` ; ajoute a la liste `ASSETS` de `sw.js` pour
rester disponible hors-ligne comme le reste de l'application.

**Tests unitaires** (`tests/engine.test.js`) : 1 nouveau test qui genere un
classeur .xlsx en memoire avec SheetJS (memes donnees que le test CSV
existant), le convertit en CSV exactement comme le fait `readEcartsFileAsCsv()`,
puis verifie que `parseEcarts()` retourne un resultat identique au chemin
CSV direct. Suite complete : 175 tests, tous passants. Verifie egalement
manuellement (script Node autonome) que la conversion .xlsx -> CSV -> objet
JS aboutit aux memes valeurs numeriques (courses, victoires, % Vict., etc.)
qu'un import CSV classique, y compris pour les colonnes en pourcentage.

`APP_VERSION` et `CACHE_NAME` passent de `v100` a **`v101`**.

## Mise a jour septembre 2026 (22) : suppression de "Predictions externes" et "Ecarts Jockeys/Entraineurs", remplaces par "Top IA Places"

A la demande explicite de l'utilisateur ("il faudrait supprimer ce qui ne sert plus a savoir"), deux modules d'import devenus obsoletes sont retires integralement de l'application, et remplaces par un nouveau module unique aligne sur le fichier Turf BZH Premium que l'utilisateur utilise desormais au quotidien.

### Ce qui est supprime

- **"Predictions externes"** (onglet Importer, fichier `Predictions_JJMMAAAA_HHMM.csv`) : UI d'import, parseur (`js/engine/predictionsExternesParser.js`, supprime), store IndexedDB `predictionsExternes`, badge "Confirmation externe" (fiche course et page "Top base").
- **"Ecarts Jockeys/Entraineurs"** (onglet Importer, fichiers "Statistiques Ecarts Jockeys/Entraineurs | Turf BZH", xlsx ou csv) : UI d'import, parseur (`js/engine/ecartsJockeyEntraineurParser.js`, supprime), store IndexedDB `ecartsJockeyEntraineur`, badge "Jockey+entraineur en forme" (Jeu Simple Gagnant/Liste des courses/Courses du jour).
- **Le gate v21** de `jeuSimpleGagnant()` (statut "enForme" obligatoire pour jouer) est retire du meme coup : `jeuSimpleGagnant(chevaux)` redevient a 1 seul parametre, comportement identique a v20 (dominance stricte MX+OR+MA sur le pool Value<=-30, cote>=3 verifiee en dernier). Ce retrait complete la reevaluation deja documentee dans la mise a jour precedente (backtest debiaise sur snapshots mensuels figes, 9 mois, n=23131 : rendement "enForme" 88,4% statistiquement indiscernable de "insuffisant" 89,5% et "inconnu" 89,8% - le gate n'apportait pas de signal reel).
- Le lecteur .xlsx integre (SheetJS, `js/vendor/xlsx.core.min.js`) reste present dans le projet mais n'est plus utilise par aucun module (le fichier Top IA Places est un CSV) - conserve pour une eventuelle reutilisation future plutot que retire.

### Ce qui est ajoute : "Top IA Places"

Nouveau module (`js/engine/topIAPlaceParser.js`) alimente par le fichier quotidien **`export-premium-turfbzh-AAAAMMJJ.csv`** (Turf BZH Premium, telecharge chaque jour par l'utilisateur), dont seule la section **"TOP 5 IA PLACE"** est retenue (les 3 autres sections du fichier - Top 5 IA Gagnant, Top 5 chance combine, Selection du Renifleur - ne sont pas exploitees). Chaque reimport REMPLACE entierement le precedent (photo du jour, pas un historique cumulatif) - store IndexedDB `topIAPlace` (`DB_VERSION` passe a **6**, migration `deleteObjectStore` pour les 2 anciens stores).

Purement informatif : n'entre dans aucun calcul de Score Global/Value/classement/dominance MX+OR+MA, exactement comme les modules qu'il remplace.

**Decodage du format "Course" du fichier quotidien** (different du fichier historique cumulatif deja utilise pour le backtest de croisement, mise a jour precedente) : `R{reunion}C{course}{numero}` concatenes sans separateur (ex. `R3C1204` = reunion 3, course 12, cheval numero 04). Comme pour le fichier historique, le numero de reunion turfbzh n'est PAS fiable pour le matching (numerotation nationale+internationale, differente de la numerotation locale de notre archive PMU) - le matching se fait par **hippodrome normalise (accents/tirets retires, table de synonymes VINCENNES/PARIS VINCENNES, LONGCHAMP/PARIS LONGCHAMP, LA TESTE/LA TESTE DE BUCH, CLAIREFONTAINE/CLAIREFONTAINE DEAUVILLE) + numero de course + numero de cheval**, la meme logique deja validee sur le fichier historique.

### Ou c'est utilise dans l'application

- **Onglet Importer** : nouvelle carte "Top IA Places (optionnel)", remplace les 2 anciennes cartes "Predictions externes" et "Ecarts Jockeys/Entraineurs". Bouton de reinitialisation associe.
- **"Courses du jour"** (page `Liste des courses`) : chaque course affiche desormais, si le Top 5 IA Place du jour est importe et que des chevaux de cette course y figurent, la liste "Top N IA : N°X" (tous rangs 1 a 5 confondus, tries par rang) - repond directement a la demande "l'onglet course du jour devra contenir les courses des 5 top IA place du jour".
- **Fiche course, carte "Jeu Simple Gagnant"** : le cheval retenu par le moteur (dominance MX+OR+MA) affiche desormais un badge "Top N IA Place" si son rang est retrouve dans le Top 5 IA Place du jour (remplace l'ancien badge "Jockey+entraineur en forme").
- **Fiche course, nouvelle carte "Top IA Place x Value <= -30"** : a la demande explicite de l'utilisateur ("mettre le ou les courses repondant a Tous rangs ET dans notre pool Value<=-30... en precisant le rang top IA pour chaque cheval"), cette carte liste TOUS les chevaux du pool Value<=-30 de la course qui figurent AUSSI dans le Top 5 IA Place du jour (n'importe lequel des 5 rangs, sans condition de dominance MX+OR+MA - distincte du pick unique de "Jeu Simple Gagnant"), avec le rang IA de chacun. Rappel du backtest qui motive cette carte (crossIA.mjs, fichier historique "Selection IA Place" turfbzh croise avec l'archive PMU, 01/01-21/09/2026, 214 jours) : profil "Tous rangs ET dans le pool Value<=-30" -> n=500, reussite Place 75,0% / rendement 101,5%, reussite Gagnant 45,8% / rendement 95,4% - environ 3,6x le volume du rang 1 seul, pour une rentabilite equivalente.

### Fichiers modifies

`js/engine/topIAPlaceParser.js` (nouveau), `js/engine/predictionsExternesParser.js` et `js/engine/ecartsJockeyEntraineurParser.js` (supprimes), `js/engine/jeuSimpleGagnant.js` (retrait du parametre/gate v21), `js/db.js` (`DB_VERSION` 6, nouveau store `topIAPlace`, suppression des 2 anciens), `js/app.js`, `tests/engine.test.js` (164 tests, tous verts).

`APP_VERSION` et `CACHE_NAME` passent de `v101` a **`v102`**.

## Mise a jour septembre 2026 (23) : correction "Courses du jour" ne montrait pas les courses couvertes par le Top 5 IA Place

Bug signale par l'utilisateur juste apres la mise a jour precedente : sur le fichier `export-premium-turfbzh-20260923.csv` reellement importe, aucune course de "Courses du jour" n'affichait de badge Top IA Place alors que le Top 5 IA Place du jour couvrait bien des courses aux memes hippodromes (Argentan, Chantilly).

**Diagnostic** : `renderListeCourses()` ecartait une course de la liste (`continue`) des qu'elle etait hors de la fourchette 8-16 partants OU qu'aucun cheval n'y passait les seuils scoreForme/scoreAptitude - AVANT meme de calculer si cette course etait couverte par le Top 5 IA Place. Concretement, la course Argentan (18 partants, au-dela de la fourchette 8-16) et la course Chantilly (8 partants mais sans cheval scoreForme/scoreAptitude suffisant) couvertes par le Top 5 IA Place du jour etaient donc systematiquement exclues de la liste avant meme d'y etre confrontees - jamais l'occasion d'afficher le badge. Le decodage lui-meme du format `R{reunion}C{course}{numero}` (verifie manuellement sur le fichier reel : `R1C113`/`R1C108` -> Argentan course 1, chevaux n&deg;13 et n&deg;08 ; `R3C107` -> Chantilly course 1, cheval n&deg;07) et le matching hippodrome+course+numero se sont averes corrects - le probleme etait uniquement l'ordre des filtres.

**Correction** : le calcul de couverture Top 5 IA Place (`matchesIA`, sur `horseRecords` bruts, independant du filtre partants) est desormais effectue AVANT les 2 filtres existants (fourchette 8-16 partants, seuils scoreForme/scoreAptitude) - une course couverte par le Top 5 IA Place (n'importe lequel des 5 rangs) est desormais TOUJOURS incluse dans "Courses du jour", meme hors fourchette de partants ou sans cheval scoreForme/scoreAptitude suffisant. Les 2 filtres existants continuent de s'appliquer normalement aux courses NON couvertes par le Top 5 IA Place. Message d'etat vide et affichage adaptes en consequence (pas de badge vide "N°" quand la course n'a aucun cheval eligible scoreForme/scoreAptitude mais est couverte par l'IA).

`APP_VERSION` et `CACHE_NAME` passent de `v102` a **`v103`**.

## Mise a jour septembre 2026 (24) : correction erreur bloquante "SEUIL_VALUE_SIMPLE_GAGNANT is not defined"

Bug signale par l'utilisateur ("aucune course ne peut etre affichee dans l'onglet reunion") : la fonction `candidatsIAPlaceValueHtml()` (carte "Top IA Place x Value <= -30", ajoutee mise a jour 22) utilisait la constante `SEUIL_VALUE_SIMPLE_GAGNANT` sans qu'elle soit importee depuis `js/engine/jeuSimpleGagnant.js` dans `js/app.js`. Toute ouverture de la fiche d'une course levait une `ReferenceError` non rattrapee, qui interrompait le rendu de la page entiere (d'ou l'ecran "Erreur" et l'impossibilite d'afficher la moindre course dans l'onglet reunion).

**Correction** : `SEUIL_VALUE_SIMPLE_GAGNANT` ajoutee a l'import de `jeuSimpleGagnant.js` en tete de `js/app.js`. Verifie par une passe ESLint `no-undef` sur tout le fichier (aucune autre variable non definie detectee) en plus du `node --check` et des 164 tests habituels.

Pour memoire : l'absence de recoupement entre les courses du "Bilan Simple Gagnant" (Value<=-30 + dominance MX+OR+MA, signal V20) et celles du Top 5 IA Place turfbzh n'est PAS un bug - ce sont deux signaux independants, rien ne garantit qu'ils portent sur les memes courses un jour donne. Le croisement des deux (carte "Top IA Place x Value <= -30") s'affiche sur la fiche de CHAQUE course individuellement, quand il existe.

`APP_VERSION` et `CACHE_NAME` passent de `v103` a **`v104`**.

## Mise a jour septembre 2026 (25)

**A la demande explicite de l'utilisateur** : remplacement COMPLET de l'ancien
"Jeu Simple Gagnant" (Value<=-30 + dominance stricte MX+OR+MA, `jeuSimpleGagnant()`
dans `js/engine/jeuSimpleGagnant.js`) par un nouveau moteur "Simple G/P"
(`jeuSimpleGP()` dans le nouveau fichier `js/engine/jeuSimpleGP.js`). Motif
invoque par l'utilisateur : l'edge de l'ancien signal MX+OR+MA s'est nettement
erode dans le temps (150-250% de rendement en 2024, ~103% sur les 12 derniers
mois - voir commentaire d'en-tete de jeuSimpleGagnant.js), et l'utilisateur
juge le signal "n'a plus grand interet puisque non rentable".

**Nouveau signal** : croisement du Top 5 IA Place Turf BZH du jour importe
(tous rangs 1 a 5, `js/engine/topIAPlaceParser.js`) avec notre pool Value<=-30
(meme seuil `SEUIL_VALUE_SIMPLE_GAGNANT` que l'ancien moteur, reutilise tel
quel). Deja explore et documente en amont sous le nom "Top IA Place x
Value<=-30" (crossIA.mjs, 01/01-21/09/2026, 214 jours) : n=500, reussite Place
75,0% / rendement 101,5%, reussite Gagnant 45,8% / rendement 95,4%.

**Methode de mise**, specifiee explicitement par l'utilisateur : *"le simple
place est a privilegier, le simple Gagnant ne pouvant etre envisage que pour
le top 1 IA place"* :
- **Simple PLACE** (prioritaire) : joue sur TOUS les chevaux du croisement
  (Top 5 IA Place, tous rangs, ET Value<=-30), Dutching (mise proportionnelle
  a 1/cote, reutilise `misesJeuSimpleGagnant` de jeuSimpleGagnant.js).
- **Simple GAGNANT** : joue UNIQUEMENT si, parmi ce meme croisement, le cheval
  au rang IA 1 y figure (il ne peut y avoir qu'un seul cheval au rang IA 1 par
  course) - sinon aucun Simple Gagnant n'est propose cette course-la (Place
  seule).

**Fichiers/fonctions ajoutes** (`js/engine/jeuSimpleGP.js`) :
- `jeuSimpleGP(chevaux, topIAPlace, lieu, numeroCourse)` : retourne
  `{rentable:false}` ou `{rentable:true, place, gagnant, rangsIA}` -
  `place` = pool Dutching de tous les chevaux du croisement, `gagnant` = pool
  Dutching a 1 cheval (rang IA 1) ou `null`.
- `bilanPlaceSimpleGP(poolPlace, miseTotale, rapportsPlace, numerosPlaces)` :
  bilan financier du pari Place - contrairement au Gagnant (1 seul vainqueur
  possible), PLUSIEURS chevaux du pool peuvent se placer simultanement,
  chacun rapportant son propre dividende officiel sur sa propre mise.
- `bilanJourSimpleGP(jeu, misePlaceTotale, miseGagnantTotale, rapportsGagnant,
  rapportsPlace, numerosPlaces, vrai1)` : combine les 2 bilans (Place toujours,
  Gagnant si `jeu.gagnant` existe), reutilise `bilanJeuSimpleGagnant()` de
  jeuSimpleGagnant.js pour le sous-bilan Gagnant.
- `jeuSimpleGagnant.js` **n'est pas supprime** : `SEUIL_VALUE_SIMPLE_GAGNANT`,
  `misesJeuSimpleGagnant`, `bilanJeuSimpleGagnant`, `rendementBilan`,
  `cumulerBilansJournaliers` restent utilises. Seul le moteur de SELECTION
  (`jeuSimpleGagnant()`, dominance MX+OR+MA) n'est plus appele nulle part
  dans l'UI/les bilans - le code reste en place pour memoire historique.

**app.js, changements** :
- Tabbar/pages renommees "Simple Gagnant" -> **"Simple G/P"** (libelle
  utilisateur uniquement - noms de fonctions/variables internes inchanges
  pour limiter le risque).
- Fiche course : carte "Jeu Simple Gagnant" + carte separee "Top IA Place x
  Value<=-30" fusionnees en une seule carte "Simple G/P"
  (`jeuSimpleGPHtml`/`bindJeuSimpleGP`), avec 2 selecteurs de mise
  independants (Place/Gagnant) et calcul Dutching sur chacun des 2 pools.
- "Courses du jour" (`renderListeCourses`) : le badge base sur l'ancien
  moteur est remplace par un badge "Simple G/P : Place N°... / Gagnant N°..."
  issu de `jeuSimpleGP()`.
- Surveillance automatique (notification H-3min) : charge desormais
  `topIAPlace` et utilise `jeuSimpleGP()` ; la notification indique les
  numeros Place et, le cas echeant, le numero Gagnant.
- "Bilan Simple G/P" (ex-"Bilan Simple Gagnant") : `collecterCandidatesSimpleGagnant()`
  utilise `jeuSimpleGP()`. Le filtre par mode ("Filtrer par mode") ne
  distingue plus les anciens "rang1"/"value" mais si un Simple Gagnant est
  propose en plus du Simple Place (`jeu.gagnant !== null`). Le bilan
  financier journalier combine desormais 2 paris par course (Place + Gagnant
  conditionnel). Le "Transfert bilan" reutilise TELS QUELS les champs
  `rang1Seul`/`classement8h` de l'enregistrement sauvegarde (compatibilite
  avec `cumulerBilansJournaliers` et l'historique deja transfere) - ils
  portent desormais respectivement le sous-bilan "avec Simple Gagnant" et
  "Simple Place seul".
- "Bilan Global Simple G/P" : libelles des 2 cartes par mode mis a jour
  ("Avec Simple Gagnant (rang IA 1 dans le pool)" / "Simple Place seul").

**Tests** : 9 nouveaux tests unitaires (`jeuSimpleGP`, `bilanPlaceSimpleGP`,
`bilanJourSimpleGP`) dans `tests/engine.test.js` - suite complete 173/173 OK.
Verification `node --check` sur tous les fichiers modifies + verification
ESLint `no-undef` (technique introduite lors du correctif v104) sur `app.js`
et `jeuSimpleGP.js` - aucune erreur.

Bump `APP_VERSION`/`CACHE_NAME` -> **v105** (`sw.js` ASSETS inclut desormais
`js/engine/jeuSimpleGP.js`).

## Mise a jour septembre 2026 (26)

**A la demande explicite de l'utilisateur** : le Simple PLACE de "Simple G/P"
(`jeuSimpleGP()`, `js/engine/jeuSimpleGP.js`) n'est plus jou&eacute; sur les
courses de moins de 8 partants. Nouvelle constante export&eacute;e
`MIN_PARTANTS_SIMPLE_PLACE = 8` - le nombre de partants est deduit de
`chevaux.length` (RaceAnalyzer.analyser ne filtre jamais un cheval, donc
`chevaux.length` = nbPartants exact), pas besoin de parametre supplementaire.

Le Simple GAGNANT (cheval au rang IA 1) **n'est pas concerne** par ce seuil :
une course a moins de 8 partants peut toujours proposer un Simple Gagnant
seul si le cheval au rang IA 1 fait partie du croisement Value&le;-30 - dans
ce cas `jeu.place` vaut `null` et `jeu.gagnant` reste renseigne.
`jeu.rentable` devient `false` uniquement si NI le Place NI le Gagnant ne sont
jouables (aucun croisement, ou croisement existant mais &lt;8 partants ET
rang IA 1 hors du pool Value).

**app.js, changements** (tous les endroits accedant a `jeu.place.chevaux`
sont desormais gardes par `jeu.place ? ... : ...`) :
- Fiche course ("Simple G/P") : si `jeu.place` est `null`, la carte affiche la
  liste du croisement a titre informatif avec un message "Simple Place non
  jou&eacute; : moins de 8 partants sur cette course", sans selecteur de mise
  Place ni bouton associe (le Simple Gagnant, s'il existe, reste calculable).
- "Courses du jour" : le badge affiche "Place non jou&eacute;e (&lt;8
  partants)" a la place des numeros quand `jeu.place` est `null`.
- Surveillance automatique (notification) : idem, "Place non jouee (&lt;8
  partants)" dans le corps de la notification.
- "Bilan Simple G/P" : `placeGagne` vaut `false` quand `jeu.place` est `null`
  (aucune mise Place n'a ete engagee, donc rien a "capturer" sur ce pari -
  le suivi du Simple Gagnant, independant, n'est pas affecte) ; la ligne de
  liste affiche "Place non jou&eacute;e (&lt;8 partants)" a la place des
  numeros. `bilanPlaceSimpleGP(null, ...)` retourne deja un bilan nul
  (`mise:0, gain:0, net:0, dividendeConnu:true`), donc le bilan financier
  combine (Place+Gagnant) reste correct sans modification supplementaire.

**Tests** : 3 nouveaux tests unitaires dans `tests/engine.test.js`
(Simple Place desactive &lt;8 partants avec Gagnant preserve ; &lt;8 partants
sans rang IA 1 dans le pool -&gt; non rentable ; exactement 8 partants -&gt;
Place jou&eacute;) + adaptation de 4 tests existants (chevaux "de
remplissage" ajoutes pour atteindre 8 partants et continuer a exercer le pari
Place tel qu'ils le testaient avant ce changement) - suite complete 176/176
OK. Verification `node --check` + ESLint `no-undef` sans erreur.

Bump `APP_VERSION`/`CACHE_NAME` -> **v106**.

## Mise a jour septembre 2026 (27)

Avant d'activer definitivement le garde-fou `MIN_PARTANTS_SIMPLE_PLACE`
(v106 ci-dessus), l'utilisateur a demande un backtest reel comparant AVEC et
SANS ce seuil, sur les courses de septembre 2026 disposant de resultats
officiels (01/09 au 21/09 - fichier historique-selections-ia turfbzh, les
journees du 22 et 23/09 n'y figurent pas encore). Resultat, sur le
croisement Rang IA Place x Value&le;-30 (n=44 selections) :
- **Sans seuil** (tout le croisement) : reussite Place 79,5%, rendement
  104,9%.
- **Avec seuil** (courses &ge;8 partants seulement, n=23) : reussite Place
  82,6%, rendement 105,7%.
- **Sous-ensemble exclu par le seuil** (&lt;8 partants, n=21) : reussite
  Place 76,2%, rendement 104,0% - deja rentable, pas de perte a l'exclusion
  mais pas de gain notable non plus a l'inclusion.

Le gain apporte par le seuil est marginal (+0,8 point de rendement) sur cet
echantillon reduit, et le sous-ensemble exclu restait lui-meme rentable.
A la demande explicite de l'utilisateur ("je garde la version avant V106"),
**le garde-fou est retire** : le Simple Place est de nouveau joue sur
TOUTES les courses du croisement Rang IA Place x Value&le;-30, quel que
soit le nombre de partants (retour au comportement de la v105).

**jeuSimpleGP.js** : suppression de la constante `MIN_PARTANTS_SIMPLE_PLACE`
et de la variable `placeJouable` - `jeu.place` est de nouveau toujours
construit (`poolDutching(...)`) dès que le croisement existe, sans jamais
valoir `null`.

**app.js** : suppression de l'import `MIN_PARTANTS_SIMPLE_PLACE` et de tous
les branchements conditionnels `jeu.place ? ... : ...` ajoutes en v106
(fiche course "Simple G/P", "Courses du jour", notification de surveillance
automatique, "Bilan Simple G/P") - `jeu.place.chevaux` est de nouveau acce
de directement partout, comme avant la v106.

**Tests** : suppression des 3 tests unitaires specifiques au seuil (Simple
Place desactive &lt;8 partants, non rentable si &lt;8 partants sans rang IA
1, exactement 8 partants) - suite complete revenue a 173/173 OK (les 4
tests precedemment "remplis" a 8 chevaux pour la v106 restent inchanges,
sans impact puisqu'ils continuent d'exercer correctement le pari Place).
Verification `node --check` + ESLint `no-undef` sans erreur.

Bump `APP_VERSION`/`CACHE_NAME` -> **v107**.

## Mise a jour septembre 2026 (28)

Bug de libelle signale par l'utilisateur sur la page "Bilan Global Simple
G/P" : le compteur "r&eacute;ussite X/Y" affich&eacute; sur la carte "Avec
Simple Gagnant (rang IA 1 dans le pool)" mesurait en realite la capture du
Simple PLACE (`a.placeGagne`) et NON le taux de victoire reel du Simple
GAGNANT (`a.gagnantGagne`) - un "10/10" sur cette carte signifiait donc "le
Simple Place a reussi 10 fois sur 10 courses ou un Simple Gagnant etait
aussi joue", pas "le Simple Gagnant a gagne 10 fois sur 10". Les montants
financiers affich&eacute;s (net &euro;, rendement) etaient corrects (ils
combinent d&eacute;j&agrave; les gains reels Place+Gagnant) - seul le
compteur de r&eacute;ussite pr&ecirc;tait &agrave; confusion.

**jeuSimpleGP.js** : inchang&eacute; (le bug etait cot&eacute; affichage
uniquement).

**app.js** :
- `sousBilanJour()` (page "Bilan Simple G/P", "Transfert bilan") calcule
  d&eacute;sormais aussi `reussiesGagnant` (`liste.filter((a) =>
  a.gagnantGagne).length`), en plus de `reussies` (capture Place, conserv&eacute;
  tel quel - reste pertinent pour la carte "Simple Place seul", o&ugrave; seul
  ce pari existe). Ce nouveau champ est enregistr&eacute; dans le sous-bilan
  `rang1Seul` transf&eacute;r&eacute; (store `bilansJournaliersSimpleGagnant`,
  pas de bump de version IndexedDB n&eacute;cessaire - simple ajout de champ).
- Page "Bilan Global Simple G/P" (`renderBilanGlobalSimpleGagnant`) :
  `modeVide()`/`cumulerMode()` cumulent d&eacute;sormais aussi
  `reussiesGagnant` ; `carteMode()` prend un 4e param&egrave;tre optionnel
  `avecGagnant` et affiche deux lignes distinctes - "R&eacute;ussite Place"
  (toujours) et "R&eacute;ussite Gagnant" (uniquement sur la carte "Avec
  Simple Gagnant", o&ugrave; ce param&egrave;tre vaut `true`) - au lieu d'un
  unique compteur ambigu.
- Compatibilit&eacute; : les jours transf&eacute;r&eacute;s AVANT cette mise
  &agrave; jour n'ont pas de champ `reussiesGagnant` dans leur sous-bilan
  `rang1Seul` - `cumulerMode()` traite l'absence de ce champ comme `0` (`m.reussiesGagnant
  || 0`), donc "R&eacute;ussite Gagnant" sous-compte l&eacute;g&egrave;rement
  pour ces jours-l&agrave; (pas de correction retroactive possible, les
  donn&eacute;es d&eacute;taill&eacute;es par pari n'existaient pas &agrave;
  l'&eacute;poque) - m&ecirc;me limitation d&eacute;j&agrave; document&eacute;e
  pour `joursSansDetailMode`.

**Tests** : aucun changement moteur (`jeuSimpleGP.js`/`jeuSimpleGagnant.js`
inchang&eacute;s) - suite compl&egrave;te 173/173 toujours OK. V&eacute;rification
`node --check` + ESLint `no-undef` sans erreur.

Bump `APP_VERSION`/`CACHE_NAME` -> **v108**.

## Mise a jour septembre 2026 (29)

Bug signale par l'utilisateur : la page "Liste des courses" affichait
"Erreur - Cannot read properties of undefined (reading 'chevaux')" et ne
s'affichait plus du tout. Regression introduite par la mise a jour (27)
(retrait du garde-fou `MIN_PARTANTS_SIMPLE_PLACE`, v107) : le retrait des
gardes `jeu.place ? ... : ...` avait ete fait partout SAUF dans
`renderListeCourses()`, ou `placeTxtListe` deref&eacute;renc&ccedil;ait
`jeu.place.chevaux` de fa&ccedil;on inconditionnelle - or `jeu.place`
n'existe que si `jeu.rentable` est vrai (`jeuSimpleGP()` renvoie
`{rentable:false}` sans autre champ sinon), donc toute course NON couverte
par le croisement Top 5 IA Plac&eacute; x Value&le;-30 (largement majoritaire
sur cette page, qui liste aussi les courses 8-16 partants hors croisement)
faisait planter tout l'affichage de la page.

**app.js** : dans `renderListeCourses()`, le calcul de `placeTxtListe` est
d&eacute;sormais fait &agrave; l'INT&Eacute;RIEUR du ternaire `jeu.rentable
? ... : ''` (au lieu d'avant, hors du ternaire) - m&ecirc;me structure que
partout ailleurs dans le fichier (fiche course, surveillance, bilans), qui
n'&eacute;taient PAS affect&eacute;s par cette regression.

**Tests** : aucun changement moteur - suite compl&egrave;te 173/173 toujours
OK. V&eacute;rification `node --check` + ESLint `no-undef` sans erreur.

Bump `APP_VERSION`/`CACHE_NAME` -> **v109**.

## Mise a jour septembre 2026 (30) : Suppl&eacute;mentation PMU

A la demande explicite de l'utilisateur, ajout complet de la
"Suppl&eacute;mentation PMU" (fichier quotidien "Supplementation_PMU_AAAAMMJJ.xlsx" :
une note IA sur 20 sur une s&eacute;lection restreinte de chevaux, avec cote
et arriv&eacute;e officielle). Backtest r&eacute;el pr&eacute;alable
(01-26/09/2026, n=16, chevaux Note IA&ge;15,9 ET Value&le;-30, rapports
officiels r&eacute;cup&eacute;r&eacute;s manuellement par l'utilisateur) :
r&eacute;ussite Gagnant 43,8% / rendement 148,1% &middot; r&eacute;ussite
Place 75,0% / rendement 121,3% - tr&egrave;s prometteur mais &eacute;chantillon
encore r&eacute;duit (25/26 fichiers journaliers disponibles sur la p&eacute;riode,
1 manquant le 10/09).

**Nouveau fichier `js/engine/supplementationParser.js`** : `parseSupplementation(lignesBrutes)`
(prend directement le tableau `XLSX.utils.sheet_to_json(sheet, {header:1})`,
extrait la date depuis le titre "..._AAAAMMJJ" et les lignes Cheval/Note/Heure/
Discipline/Cote/Arrivee) et `trouverSupplementation(rows, nomCheval)` (matching
par nom de cheval normalis&eacute; - accents, casse, apostrophes - le fichier
ne contenant pas de colonne hippodrome exploitable pour un matching par
num&eacute;ro de course).

**`jeuSimpleGP.js`** : nouvelle fonction `jeuSupplementation(chevaux, supplementationRows)`
et constante `SEUIL_NOTE_SUPPLEMENTATION = 15.9` (seuil valid&eacute; par le
backtest, confirm&eacute; par l'utilisateur en r&eacute;ponse &agrave; la
question de clarification "Garder Note IA &ge;15,9 (Recommand&eacute;)").
Croise le pool Value&le;-30 (m&ecirc;me seuil que le Simple G/P) avec la
Suppl&eacute;mentation du jour (Note&ge;15,9) - contrairement au Simple G/P
(bas&eacute; sur le rang IA Place), ici Place ET Gagnant sont jou&eacute;s sur
EXACTEMENT le m&ecirc;me pool qualifi&eacute; (Dutching, aucune restriction
de rang, la notion de rang n'existant pas dans ce fichier).

**`db.js`** : nouveau store IndexedDB `supplementation` (m&ecirc;me pattern
"photo du jour" que `topIAPlace` - un seul enregistrement `id:'jour'`,
remplac&eacute; enti&egrave;rement &agrave; chaque r&eacute;import) - `DB_VERSION`
6&rarr;7. Nouvelles fonctions `saveSupplementation`, `getSupplementation`,
`resetSupplementation`, prises en compte dans `exportAll`/`importAll`.

**`app.js`** :
- Onglet Importer : nouvelle carte "Suppl&eacute;mentation PMU" (`<input
  type="file" accept=".xlsx">`) + bouton de r&eacute;initialisation. Utilise
  la librairie `window.XLSX` d&eacute;j&agrave; embarqu&eacute;e (`js/vendor/xlsx.core.min.js`,
  charg&eacute;e globalement via `index.html`, jusqu'ici inutilis&eacute;e
  dans le code applicatif). `handleSupplementationImport`/`handleResetSupplementation`
  suivent le m&ecirc;me pattern que le Top IA Plac&eacute;.
- Nouvelle fonction `chargerSupplementation()` (mirroir de `chargerTopIAPlace()`).
- **"Courses du jour" (`renderListeCourses`)** : l'ancien filtre forme&ge;70/
  aptitude&ge;85 (`SEUIL_SCORE_FORME_RANG1`/`SEUIL_SCORE_APTITUDE_RANG1`,
  seuils de l'ancien Jeu Simple Gagnant &agrave; dominance MX+OR+MA) est
  **ENTI&Egrave;REMENT RETIR&Eacute;** de cette page, &agrave; la demande
  explicite de l'utilisateur. D&eacute;sormais, une course figure dans la
  liste si elle est couverte par le Top 5 IA Plac&eacute; du jour ET/OU par
  la Suppl&eacute;mentation du jour (n'importe quel cheval du champ y
  figurant, matching par nom) - quels que soient le nombre de partants ou
  les scores. Un badge informatif ("Suppl. N&deg;X (note/20)") est
  affich&eacute; pour chaque cheval couvert par la Suppl&eacute;mentation, et
  une carte "Suppl&eacute;mentation : Place/Gagnant N&deg;..." si le
  croisement Value&le;-30&times;Suppl&eacute;mentation est jouable.
- **Fiche course (Simple G/P)** : nouvelle carte "Suppl&eacute;mentation"
  (`jeuSupplementationHtml`/`bindJeuSupplementation`), affich&eacute;e sous
  la carte "Simple G/P" existante. Deux s&eacute;lecteurs de mise
  ind&eacute;pendants (Place et Gagnant), tous deux Dutching sur le m&ecirc;me
  pool qualifi&eacute; (contrairement au Simple G/P, o&ugrave; le Gagnant ne
  porte que sur le rang IA 1).

**Tests** (`tests/engine.test.js`) : 7 nouveaux tests - parsing (extraction
date + lignes, tableau vide), matching par nom normalis&eacute;,
`jeuSupplementation` (non import&eacute;e, aucun match, note &lt; seuil
exclue, Place+Gagnant sur le m&ecirc;me pool). Suite compl&egrave;te
**180/180 OK**. V&eacute;rification `node --check` + ESLint `no-undef` sans
erreur (hors faux positifs `location`/`crypto` pr&eacute;-existants, non
li&eacute;s &agrave; cette mise &agrave; jour).

Bump `APP_VERSION`/`CACHE_NAME` -> **v110**.
