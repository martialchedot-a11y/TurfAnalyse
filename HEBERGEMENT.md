...
- **Danger(s)** : les chevaux tres joues par le marche (Value < -10%, cote
  jouable) mais non retenus comme base — a surveiller comme trouble-fete
  potentiel.
...
## Couple Gagnant (10-30€) : methode terrain a la Base

Le bloc "Combinaisons suggerees" affiche une ligne **"Couple Gagnant
(10-30€)"** : elle associe la Base (le cheval le mieux classe par l'app,
`chevaux[0]`) a chacun des autres chevaux du champ, et calcule pour chaque
paire une valeur approximative de couple avec la formule classique du
terrain :
...
Seules les paires dont cette valeur tombe entre 10 et 30€ sont retenues et
affichees. C'est une methode empirique simple et repandue chez les joueurs
de couple (mise a l'unite, sans tenir compte de la commission/TRJ reel du
PMU), qui vise a reperer des combinaisons avec un compromis risque/gain
raisonnable — ni trop favorites (rapport trop faible), ni trop longshot
(rapport trop eleve). Elle est independante du modele de score/probabilites
(Plackett-Luce) utilise ailleurs dans l'app.
...
Si aucune paire ne tombe dans la fourchette 10-30€, ou si la cote de la Base
est inconnue, la ligne l'indique clairement (aucune combinaison proposee).
...
- Pas de calcul bayesien complet de combinaisons de paris (module "Analyse
  ...
  methode Couple Gagnant (10-30€) decrite plus haut, qui reste volontairement
  simple (formule directe cote×cote/2, fourchette fixe) et n'a pas vocation a
  remplacer un vrai module de calcul de combinaisons.
...
(Victoire, Top2, Top3), la methode Couple Gagnant (10-30€) (`coupleGagnant1030`,
voir plus haut), le tri/classement, l'import CSV, le module "Base(s)
