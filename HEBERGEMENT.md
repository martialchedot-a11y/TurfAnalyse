...
## Marche vs modele : verification sur donnees reelles (backtest 1 mois, 1027 courses)

A la demande de l'utilisateur, une premiere verification avait ete
effectuee sur 6 reunions (51 courses, 624 chevaux — voir historique). Elle
a ensuite ete elargie a un mois complet d'archives reelles fournies par
l'utilisateur (30 journees, fin fevrier a fin mars, formats "journee"
multi-reunions), soit **1027 courses et 12 534 chevaux**, en comparant les
indices du moteur a l'arrivee officielle reellement enregistree dans les
CSV. L'historique de performances utilise pour chaque journee est celui
disponible la veille (aucune donnee posterieure a la course analysee
n'entre dans le calcul, pour eviter tout biais retrospectif). Ce n'est
toujours pas un backtest automatise integre a l'app (aucun code de
backtest n'a ete ajoute au moteur) : c'est une verification faite
manuellement pour repondre a la question "quels indices sont fiables ?".

**Constat principal (confirme et renforce sur ce plus grand echantillon) :
le marche (la cote et son evolution) reste plus fiable que l'ecart Value
(Score Global vs marche).**

- **Base(s) confirmee(s)** (Module 2) reste de loin le signal le plus
  fort : 30,4% de victoires, 61,4% de Top3 (n=381) contre 7,5%/23,4% pour
  les chevaux non confirmes (n=12 153).
- Chevaux **"tres joues"** (Value ≤ -30%) : 25,7% de victoires, 53,9% de
  Top3 (n=2073) — tres largement devant les chevaux **"delaisses par le
  marche"** (Value ≥ +20%, la "value bet" que le modele est cense
  reperer) : seulement 4,1% de victoires, 19,3% de Top3 (n=1651), plus
  faible que la moyenne du champ. Confirme le constat initial : les
  libelles actuels ("Delaisse par le marche" / "Tres joue (confiance
  marche)") restent justifies.
- **Danger(s)** (Value < -10%, non retenus comme Base) : 18,7% de
  victoires, 44,1% de Top3 (n=4130) contre 3,0%/15,0% pour le reste du
  champ (n=8404) — signal tres fort, confirme a grande echelle.
- **Cote qui se resserre** entre le matin (8h) et la cote directe, prise
  isolement (>10%) : 13,0% de victoires, 33,0% de Top3 (n=5394) contre
  4,6%/18,2% sans resserrement (n=7140) — signal plus net que sur
  l'echantillon initial, confirme robuste sur un grand volume.
- **Indice de convergence** : toujours pas de valeur demontree au-dela de
  Base(s) confirmee(s), et pas meme monotone sur ce volume (convergence
  "moderee" a 16,1% de victoires fait legerement mieux que "forte" a
  13,3%). Les chevaux en convergence forte mais SANS etre une Base
  confirmee performent a 10,4%/33,2% (n=479), au-dessus du bruit de fond
  (5,5%/19,6%) mais loin derriere une Base confirmee seule. Confirme que
  c'est un signal d'appoint, pas un signal autonome.
- Au niveau course (pour choisir QUELLE course jouer) : sur ce grand
  echantillon, le cheval n°1 du modele gagne quasiment aussi souvent dans
  les courses jugees **"Course logique"** (25,6%, n=121) que dans les
  **"Course aleatoire"** (26,4%, n=906) — l'ecart observe precedemment
  (27,3% vs 17,5%, n=11/40) ne se confirme pas sur un plus grand volume.
  Seul le Top3 progresse un peu (61,2% vs 54,0%). La **"Lisibilite"** ne
  predit toujours pas mieux le n°1 (Ouverte 27,5% n=710, Tres ouverte
  23,0% n=256, Lisible 26,2% n=61) : confirme, ne pas s'y fier comme
  filtre de choix de course.
- **Nouveau constat** : le badge **"Top2 fiable"** (deja affiche dans
  l'appli) se revele etre un signal tres fort mais rare — quand il est
  declenche, la meilleure base gagne 60,0% du temps et finit dans le Top2
  76,7% du temps (n=30) contre 31,3%/48,6% sinon (n=463). Echantillon
  encore petit pour ce cas precis, a confirmer sur davantage de donnees,
  mais tendance prometteuse.

**Consequence appliquee** : aucun changement supplementaire aux libelles
Value/Danger (deja corriges suite au premier constat) — ce plus grand
echantillon les confirme. **Aucune modification du Score Global, de la
Value, ni des formules du moteur** n'a ete faite suite a ce constat, ici
comme precedemment — uniquement des constats d'interpretation.

**Limites de cette verification, a garder en tete** :
- Periode limitee (fin fevrier a fin mars 2026) : les proportions
  pourraient varier sur d'autres periodes de l'annee ou d'autres
  hippodromes non representes dans cet echantillon.
...
- Le cas "Top2 fiable" reste a petit effectif (n=30) malgre l'elargissement
  a 1027 courses : a confirmer sur davantage de donnees avant d'en faire
  une regle definitive.
...
## Ameliorations issues du backtest 1 mois

A la suite du backtest ci-dessus, plusieurs affichages/priorisations ont
ete ajoutes dans l'appli pour aider a mieux exploiter le signal Base(s)
confirmee(s) et reperer les courses les plus fiables. **Aucun d'entre eux
ne modifie le Score Global, la Value, ni le classement (tri) calcules par
le moteur** : ce sont uniquement des indicateurs/priorisations
d'affichage, bases sur des constats empiriques.
...
- **Badge "Course fiable"** (page Course, en tete du bloc "Base(s)
  ...
  course avec la plus grande probabilite de gagner, en cascade sur deux
  niveaux (un seul niveau retenu par course, du plus fiable au plus
  courant) :
  1. **Confiance maximale** : une base **"tres solide" confirmee
     techniquement** (Module 2) ET **classee n1** par le Score Global
     (Module 1). Sur le backtest (1027 courses), cette combinaison gagne
     41,7% du temps et finit dans le Top3 75,0% du temps (n=24 — rare,
     mais nettement au-dessus de toute autre categorie mesuree).
  2. Sinon, **base confirmee unique** : une seule base confirmee dans
     toute la course (forte ou simple), classee 1ere ou 2e par le Score
     Global : 31,6% de victoires, 61,1% de Top3 (n=285). Note testee et
     ECARTEE comme niveau de repli supplementaire : utiliser seul le
     badge "Course logique + Hierarchie claire" en l'absence de base
     confirmee ne marche pas (18,2% de victoires sur l'echantillon, n=22
     — moins bien que le simple favori du modele sans aucun filtre,
     26,3%).
  En cumulant ces deux niveaux, 30,1% des courses du backtest recoivent
  un badge "Course fiable", avec 32,4% de victoires / 62,1% de Top3 en
  moyenne sur ce groupe.
- **Bonus "Petit champ" et "Discipline favorable"** (affiches en tag a
  cote du badge "Course fiable", n'excluent aucune course) : sur le
  backtest, les bases confirmees dans un champ <=10 partants gagnent
  35,2% du temps (Top3 73,0%, n=122) contre 25,2%/57,7% dans un champ
  >=14 partants (n=123), et les disciplines Attele/Steeple sont bien plus
  fiables que Plat/Monte pour ce signal (voir plus bas). Quand un badge
  "Course fiable" cumule les deux bonus, le taux de victoire monte a
  51,6%/67,7% de Top3 (n=31, echantillon reduit — a confirmer).
...
- **Niveau de fiabilite par discipline** (page Course, sous "Base(s)
  ...
  presente) : indique si la discipline de la course est associee, sur le
  backtest, a une fiabilite plus forte ou plus moderee du signal Base
  confirmee. Attele (35,4% victoire, n=198) et Steeple (44,4%, n=18 —
  echantillon reduit) sont classes "fiabilite renforcee" ; Haies (28,6%,
  n=14 — echantillon reduit) "intermediaire" ; Plat (21,7%, n=115) et
  Monte (21,2%, n=33) "plus moderee".

**A propos du reglage Cote directe/Cote 8h** : le badge "Course fiable" et
le signal Base(s) confirmee(s) ont aussi ete verifies avec le reglage
"Cote 8h" (au lieu de "Cote directe") comme reference de marche pour la
Value/le tri. Resultat net : le signal se degrade nettement (Confiance
maximale tombe a 17,6% de victoires/29,4% de Top3, n=17, contre
41,7%/75,0% en cote directe ; Base(s) confirmee(s) generale tombe a
27,0%/51,1%, n=174, contre 30,4%/61,4% en cote directe) — et les deux
reglages ne pointent quasiment pas les memes chevaux (4 recoupements sur
~24). Le reglage par defaut "Cote directe" reste donc le bon choix pour
exploiter ces indicateurs ; "Cote 8h" les affaiblit plutot qu'il ne les
affine.

Verification de coherence : tous ces indicateurs ont ete re-calcules
directement sur les donnees du backtest (1027 courses) a partir de la
...
logique/aleatoire" et "Courses sures".
