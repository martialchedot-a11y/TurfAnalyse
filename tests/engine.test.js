...
import { jeuSimpleGagnant, misesJeuSimpleGagnant, SEUILS_VALUE_RANG_SIMPLE_GAGNANT, SEUIL_ECART_SCORE_RANG1, SEUIL_ECART_SCORE_RANG1_8H, SEUIL_SCORE_APTITUDE_RANG1, SEUIL_SCORE_FORME_RANG1, SEUIL_SCORE_CROISEMENT_MAX, bilanJeuSimpleGagnant, rendementBilan, cumulerBilansJournaliers, multiplicateurMiseEcartRang1, PALIERS_MISE_ECART_RANG1, HIPPODROMES_EXCLUS_SIMPLE_GAGNANT, estHippodromeExclu } from '../js/engine/jeuSimpleGagnant.js';
...
// -------------------------------------------------------------------
// Jeu Simple Gagnant v15 (aout 2026) : parmi les 2 PREMIERS du classement
// Score Global (top2, reduit du top3 v13 depuis v15 - voir jeuSimpleGagnant.js),
// retient celui au meilleur scoreAptitude, selon 2 sources de cote possibles
// (meme seuil de COTE > 3,8 dans les deux cas) :
//   - cote DIRECTE (`entry.coteDirecte`) : prioritaire, le candidat du
//     top2 au meilleur scoreAptitude qualifie si ce scoreAptitude >=
//     SEUIL_SCORE_APTITUDE_RANG1 (85) ET son scoreForme >=
//     SEUIL_SCORE_FORME_RANG1 (70) -> "1er du classement seul" ou
//     "Top2 (rang 2)" (rang1Value:true, mode8h:false).
//   - cote 8H (`entry.cote8h`) : uniquement si la cote directe ne qualifie
//     aucun candidat, le candidat du top2 au meilleur scoreAptitude
//     qualifie si scoreAptitude >= SEUIL_SCORE_APTITUDE_RANG1 ET
//     scoreForme >= SEUIL_SCORE_FORME_RANG1 ET que son ecart de Score
//     Global sur l'AUTRE candidat du top2 est >= SEUIL_ECART_SCORE_RANG1_8H
//     (15) -> "1er du classement 8h" ou "Top2 (rang 2) 8h"
//     (rang1Value:false, mode8h:true).
// `alternative` vaut toujours `null` (aucun Dutching multi-chevaux propose
// sur cette page). `rang1Value` reste le nom du champ distinguant le MODE
// (cote directe vs cote 8h), pas le rang du cheval retenu - voir
// `rangChoisi` pour ca.
// `scoreAptitude`/`scoreForme` par defaut (90) qualifient toujours, sauf
// tests dedies au seuil, pour ne pas polluer les tests axes sur la
// cote/l'ecart.
// -------------------------------------------------------------------
function chevalPourJsg(numero, classement, coteDirecte, scoreGlobal = 0, cote8h, scoreAptitude = 90, scoreForme = 90) {
  const cotePourAffichage = coteDirecte > 0 ? coteDirecte : (cote8h > 0 ? cote8h : null);
  return {
    entry: { numero, coteDirecte: coteDirecte ?? null, cote8h: cote8h ?? null },
    classement,
    cotePourAffichage,
    scoreGlobal,
    scoreAptitude,
...
// Construit un tableau de 18 rubriques (indices RUBRIQUES.js) avec des
// valeurs uniquement sur R10 (17), TG (16), OR (12), IdC (10) - les 4
// rubriques utilisees par classementCroisement() (v16). Omettre une cle
// laisse la rubrique correspondante a `null` (non comptabilisee -> ne
// contribue pas au score Croisement de ce cheval).
function rubriquesCroisement({ r10, tg, or, idc } = {}) {
...
test('jeuSimpleGagnant: non rentable si moins de 2 chevaux ont un classement 1-2 (champ trop petit)', () => {
  assert.deepEqual(jeuSimpleGagnant([]), { rentable: false });
  const chevaux = [
    chevalPourJsg(1, 1, 4.0, 70)
  ];
  assert.deepEqual(jeuSimpleGagnant(chevaux), { rentable: false }); // seulement 1 candidat
});

test('jeuSimpleGagnant: non rentable si aucun candidat du top2 ne qualifie ni en cote directe ni en cote 8h', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 3.5, 70, 3.6),  // directe ET 8h toutes deux <= 3,8
    chevalPourJsg(2, 2, 3.0, 50)
  ];
  assert.deepEqual(jeuSimpleGagnant(chevaux), { rentable: false });
});

test('jeuSimpleGagnant: rang 1 qualifie en cote DIRECTE (>3,8) ET scoreAptitude>=85 -> "1er du classement seul"', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 4.0, 70),  // > 3,8 en directe ; scoreAptitude par defaut (90) >= 85
    chevalPourJsg(2, 2, 3.0, 50)
  ];
  ...
  assert.equal(jeu.ecartScoreRang1, 20); // 70-50 (rang1 vs meilleur rival, rang2 ici)
});

test('jeuSimpleGagnant (v13/v15): rang 2 a un meilleur scoreAptitude que rang 1 (tous deux value en directe) -> rang 2 retenu', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 4.0, 70, undefined, 85),   // value, aptitude tout juste au seuil
    chevalPourJsg(2, 2, 4.5, 60, undefined, 95)    // value, meilleure aptitude
  ];
  const jeu = jeuSimpleGagnant(chevaux);
  assert.equal(jeu.rentable, true);
  assert.equal(jeu.rang1Value, true);
  assert.equal(jeu.mode8h, false);
  assert.equal(jeu.rangChoisi, 2);
  assert.deepEqual(jeu.principal.chevaux.map((c) => c.entry.numero), [2]);
  approx(jeu.principal.rendement, 4.5, 1e-9); // sa propre cote, pas celle du rang 1
});

test('jeuSimpleGagnant (v15): le rang 3 n\'est jamais candidat, meme s\'il serait seul value en cote directe', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 3.0, 70),                    // non value en directe, hors du pool en 8h (non fourni)
    chevalPourJsg(2, 2, 3.2, 60),                    // non value en directe
    chevalPourJsg(3, 3, 4.5, 55, undefined, 90)      // value en directe, mais hors du pool top2 depuis v15
  ];
  assert.deepEqual(jeuSimpleGagnant(chevaux), { rentable: false }); // rang3 jamais regarde, meme s'il qualifierait seul
});

test('jeuSimpleGagnant: rang 1 ne qualifie PAS en cote directe mais qualifie en cote 8H (memes criteres) -> "1er du classement 8h"', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 3.0, 70, 4.2),  // directe <= 3,8 (non value), cote8h 4,2 > 3,8 ; ecart 70-50=20 >= 15
    chevalPourJsg(2, 2, 3.0, 50)
  ];
  const jeu = jeuSimpleGagnant(chevaux);
  assert.equal(jeu.rentable, true);
  assert.equal(jeu.rang1Value, false);
  assert.equal(jeu.mode8h, true);
  assert.equal(jeu.rangChoisi, 1);
  assert.deepEqual(jeu.principal.chevaux.map((c) => c.entry.numero), [1]);
  assert.equal(jeu.ecartScoreRang1, 20); // toujours peuple (palier applicable aux 2 modes)
});

test('jeuSimpleGagnant: rang 1 sans cote directe connue (undefined) mais cote 8h qualifie -> "1er du classement 8h"', () => {
  const chevaux = [
    chevalPourJsg(1, 1, undefined, 70, 4.5),
    chevalPourJsg(2, 2, 3.0, 50)
  ];
  const jeu = jeuSimpleGagnant(chevaux);
  assert.equal(jeu.rentable, true);
...
test('jeuSimpleGagnant: cote directe ET cote 8h qualifient toutes les deux -> la cote directe reste prioritaire', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 4.0, 70, 5.5),  // les deux > 3,8
    chevalPourJsg(2, 2, 3.0, 50)
  ];
  const jeu = jeuSimpleGagnant(chevaux);
  assert.equal(jeu.rentable, true);
  assert.equal(jeu.rang1Value, true);
  assert.equal(jeu.mode8h, false);
  approx(jeu.principal.rendement, 4.0, 1e-9); // base sur cotePourAffichage (priorite a la cote directe)
});

test('multiplicateurMiseEcartRang1: paliers x1 (<20) / x1,5 (20-30) / x2 (>=30), bornes inclusives basses', () => {
  assert.equal(multiplicateurMiseEcartRang1(10), 1);
  assert.equal(multiplicateurMiseEcartRang1(19.9), 1);
  assert.equal(multiplicateurMiseEcartRang1(20), 1.5);
  assert.equal(multiplicateurMiseEcartRang1(25), 1.5);
  assert.equal(multiplicateurMiseEcartRang1(29.9), 1.5);
  assert.equal(multiplicateurMiseEcartRang1(30), 2);
  assert.equal(multiplicateurMiseEcartRang1(50), 2);
});

test('multiplicateurMiseEcartRang1: valeur invalide (null/NaN/undefined) -> x1 par defaut', () => {
  assert.equal(multiplicateurMiseEcartRang1(null), 1);
  assert.equal(multiplicateurMiseEcartRang1(undefined), 1);
  assert.equal(multiplicateurMiseEcartRang1(NaN), 1);
});

test('PALIERS_MISE_ECART_RANG1: 3 paliers, seuils et multiplicateurs attendus', () => {
  assert.deepEqual(PALIERS_MISE_ECART_RANG1, [
...
test('HIPPODROMES_EXCLUS_SIMPLE_GAGNANT: 18 hippodromes attendus (backtest archive complete, n>=15, v14)', () => {
  assert.deepEqual(HIPPODROMES_EXCLUS_SIMPLE_GAGNANT, [
...
test('estHippodromeExclu: reconnait un hippodrome de la liste (insensible a la casse/espaces)', () => {
  assert.equal(estHippodromeExclu('SAINT CLOUD'), true);
  assert.equal(estHippodromeExclu('saint cloud'), true);
  assert.equal(estHippodromeExclu('  MESLAY DU MAINE  '), true);
  assert.equal(estHippodromeExclu('BEAUMONT DE LOMAGNE'), true);
});

test('estHippodromeExclu: renvoie false pour un hippodrome hors liste, null/undefined/vide', () => {
  assert.equal(estHippodromeExclu('VINCENNES'), false);
  assert.equal(estHippodromeExclu('CLAIREFONTAINE DEAUVILLE'), false); // ancienne liste v11, non reconduite en v14
  assert.equal(estHippodromeExclu(null), false);
  assert.equal(estHippodromeExclu(undefined), false);
  assert.equal(estHippodromeExclu(''), false);
});

test('jeuSimpleGagnant: hippodrome exclu -> non rentable d\'office, meme avec des candidats valables', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 4.0, 70),
    chevalPourJsg(2, 2, 3.0, 50)
  ];
  assert.deepEqual(jeuSimpleGagnant(chevaux, 'SAINT CLOUD'), { rentable: false, hippodromeExclu: true });
  assert.deepEqual(jeuSimpleGagnant(chevaux, '  saint cloud  '), { rentable: false, hippodromeExclu: true });
});

test('jeuSimpleGagnant: hippodrome non exclu ou absent -> comportement inchange', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 4.0, 70),
    chevalPourJsg(2, 2, 3.0, 50)
  ];
  assert.equal(jeuSimpleGagnant(chevaux, 'VINCENNES').rentable, true);
  assert.equal(jeuSimpleGagnant(chevaux).rentable, true);
});

test('jeuSimpleGagnant: rang 1 depasse aptitude (>=85) MAIS scoreForme insuffisant (<70) -> le mode directe ne qualifie pas via lui', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 4.0, 70, undefined, 90, 69.9), // aptitude ok, forme < 70
    chevalPourJsg(2, 2, 3.0, 50)
  ];
  assert.deepEqual(jeuSimpleGagnant(chevaux), { rentable: false });
});

test('jeuSimpleGagnant: rang 1 scoreForme tout juste au seuil (70) -> qualifie', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 4.0, 70, undefined, 90, 70),
    chevalPourJsg(2, 2, 3.0, 50)
  ];
  assert.equal(jeuSimpleGagnant(chevaux).rentable, true);
});

test('jeuSimpleGagnant: rang 1 depasse son seuil (directe) MAIS scoreAptitude insuffisant (<85) -> le mode directe ne qualifie pas via lui', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 4.0, 70, undefined, 84.9),  // > 3,8 en directe, mais aptitude < 85
    chevalPourJsg(2, 2, 3.0, 50)                    // non value en directe -> pas candidat
  ];
  assert.deepEqual(jeuSimpleGagnant(chevaux), { rentable: false }); // aucun autre candidat directe/8h ne prend le relais ici
});

test('jeuSimpleGagnant: scoreAptitude insuffisant sur TOUS les candidats (directe et 8h) -> non rentable', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 4.0, 70, 5.0, 80),  // les deux cotes > 3,8, mais aptitude 80 < 85
    chevalPourJsg(2, 2, 3.0, 50)
  ];
  assert.deepEqual(jeuSimpleGagnant(chevaux), { rentable: false });
});

test('jeuSimpleGagnant: scoreAptitude tout juste au seuil (=85) -> qualifie (borne inclusive), cote directe', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 4.0, 70, undefined, 85),  // scoreAptitude == SEUIL_SCORE_APTITUDE_RANG1
    chevalPourJsg(2, 2, 3.0, 50)
  ];
  const jeu = jeuSimpleGagnant(chevaux);
...
test('jeuSimpleGagnant: scoreAptitude juste sous le seuil (84,9) -> ne qualifie pas, cote directe', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 4.0, 70, undefined, 84.9),
    chevalPourJsg(2, 2, 3.0, 50)
  ];
  assert.deepEqual(jeuSimpleGagnant(chevaux), { rentable: false });
});

test('jeuSimpleGagnant: l\'ecart de Score Global du candidat retenu vs son meilleur rival ne conditionne PAS le mode directe, seul scoreAptitude compte', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 4.0, 51),  // ecart vs rang2 (51-50=1), tres faible, mais aptitude par defaut (90) >= 85
    chevalPourJsg(2, 2, 3.0, 50)
  ];
  const jeu = jeuSimpleGagnant(chevaux);
  assert.equal(jeu.rentable, true);
  assert.equal(jeu.rang1Value, true);
  assert.equal(jeu.mode8h, false);
  assert.equal(jeu.ecartScoreRang1, 1); // toujours calcule/renvoye (sert au palier de mise), meme s'il ne conditionne plus le mode directe
});

test('jeuSimpleGagnant: ecart de Score Global tout juste au seuil (=15) -> qualifie (borne inclusive), cote 8h', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 3.0, 65, 4.0),  // directe non value, cote8h > 3,8, ecart == 15
    chevalPourJsg(2, 2, 3.0, 50)
  ];
  const jeu = jeuSimpleGagnant(chevaux);
  assert.equal(SEUIL_ECART_SCORE_RANG1_8H, 15);
  assert.equal(jeu.rentable, true);
  assert.equal(jeu.mode8h, true);
  assert.equal(jeu.ecartScoreRang1, 15);
});

test('jeuSimpleGagnant: ecart == 10 (< seuil 8h de 15) -> ne qualifie pas en cote 8h (rang1 non value en cote directe ici)', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 3.0, 60, 4.0),  // directe non value, cote8h > 3,8, ecart == 10 : sous le nouveau seuil 8h
    chevalPourJsg(2, 2, 3.0, 50)
  ];
  assert.deepEqual(jeuSimpleGagnant(chevaux), { rentable: false });
});

test('jeuSimpleGagnant: ecart juste sous le seuil 8h (14,9) -> ne qualifie pas en cote 8h', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 3.0, 64.9, 4.0),  // ecart == 14,9 < 15
    chevalPourJsg(2, 2, 3.0, 50)
  ];
  assert.deepEqual(jeuSimpleGagnant(chevaux), { rentable: false });
});

test('jeuSimpleGagnant: mode directe non concerne par le seuil d\'ecart 8h (ni par l\'ecart du tout)', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 4.0, 62),  // > 3,8 en directe ; ecart 62-50=12 (< 15, sans importance en directe)
    chevalPourJsg(2, 2, 3.0, 50)
  ];
  const jeu = jeuSimpleGagnant(chevaux);
  assert.equal(jeu.rentable, true);
  assert.equal(jeu.rang1Value, true);
  assert.equal(jeu.mode8h, false); // mode directe non concerne par le seuil 8h
});

test('jeuSimpleGagnant (v15): rang 1 sans cote connue (cotePourAffichage null) -> exclu, l\'autre candidat du top2 prend le relais', () => {
  const chevaux = [
    { entry: { numero: 1, coteDirecte: null, cote8h: null }, classement: 1, cotePourAffichage: null, scoreGlobal: 70, scoreAptitude: 95 },
    chevalPourJsg(2, 2, 4.0, 50)  // value en directe, aptitude par defaut (90) >= 85
  ];
  const jeu = jeuSimpleGagnant(chevaux);
  assert.equal(jeu.rentable, true);
  assert.equal(jeu.rangChoisi, 2); // le n°1 est exclu (pas de cote exploitable), le n°2 prend le relais
  assert.deepEqual(jeu.principal.chevaux.map((c) => c.entry.numero), [2]);
});

test('jeuSimpleGagnant: rang 1 sans cote connue ET aucun autre candidat exploitable -> non rentable', () => {
  const chevaux = [
    { entry: { numero: 1, coteDirecte: null, cote8h: null }, classement: 1, cotePourAffichage: null, scoreGlobal: 70 },
    chevalPourJsg(2, 2, 3.0, 50)  // non value dans aucun mode
  ];
  assert.deepEqual(jeuSimpleGagnant(chevaux), { rentable: false });
});

test('jeuSimpleGagnant (v15): les rangs 3, 4... n\'entrent jamais en jeu, seuls les 2 premiers du classement sont consideres', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 3.0, 70),                       // non value en directe
    chevalPourJsg(2, 2, 3.0, 20),                       // non value en directe
    chevalPourJsg(3, 3, 100, 90, 100, 99),              // rang 3 tres "value" sur les 2 cotes : jamais pris en compte depuis v15
    chevalPourJsg(4, 4, 100, 90, 100, 99)               // rang 4 tres "value" sur les 2 cotes : jamais pris en compte
  ];
  assert.deepEqual(jeuSimpleGagnant(chevaux), { rentable: false }); // seul le top2 est regarde, aucun de ses membres ne qualifie
});

test('jeuSimpleGagnant (v16): rang 1 qualifie en cote directe MAIS score Croisement = 4 (R10/TG/OR/IdC tous top3) -> non rentable, sans repli sur le mode 8h', () => {
  const rang1 = chevalPourJsg(1, 1, 4.0, 70, 4.0); // qualifierait aussi en 8h si on l'atteignait
  rang1.entry.rubriques = rubriquesCroisement({ r10: 10, tg: 10, or: 10, idc: 10 }); // score Croisement = 4
  const chevaux = [rang1, chevalPourJsg(2, 2, 3.0, 20)];
  assert.deepEqual(jeuSimpleGagnant(chevaux), { rentable: false });
});

test('jeuSimpleGagnant (v16): score Croisement exactement au seuil (=3, une rubrique sur 4 manquante) -> qualifie toujours (borne inclusive)', () => {
  const rang1 = chevalPourJsg(1, 1, 4.0, 70);
  rang1.entry.rubriques = rubriquesCroisement({ r10: 10, tg: 10, or: 10 }); // IdC manquant -> score Croisement = 3
  const chevaux = [rang1, chevalPourJsg(2, 2, 3.0, 20)];
  assert.equal(SEUIL_SCORE_CROISEMENT_MAX, 3);
  assert.equal(jeuSimpleGagnant(chevaux).rentable, true);
});

test('jeuSimpleGagnant (v16): cheval sans donnees rubriques (entry.rubriques absent) -> score Croisement par defaut 0, qualifie toujours', () => {
  const chevaux = [chevalPourJsg(1, 1, 4.0, 70), chevalPourJsg(2, 2, 3.0, 20)];
  assert.equal(chevaux[0].entry.rubriques, undefined);
  assert.equal(jeuSimpleGagnant(chevaux).rentable, true);
});

test('jeuSimpleGagnant (v16): rang 1 qualifie en cote 8h MAIS score Croisement = 4 -> non rentable', () => {
  const rang1 = chevalPourJsg(1, 1, 3.5, 90, 4.0); // directe <=3,8 (ne qualifie pas), cote8h >3,8
  rang1.entry.rubriques = rubriquesCroisement({ r10: 10, tg: 10, or: 10, idc: 10 }); // score Croisement = 4
  const rang2 = chevalPourJsg(2, 2, 3.0, 50); // ecart Score Global >=15 avec rang1 (90 vs 50)
  const chevaux = [rang1, rang2];
  assert.deepEqual(jeuSimpleGagnant(chevaux), { rentable: false });
});
...
test('bilanJeuSimpleGagnant: course ratee (vainqueur hors pool) -> mise perdue, sans besoin du rapport', () => {
  const chevaux = [chevalPourJsg(1, 1, 4.0, 70), chevalPourJsg(2, 2, 6.0, 40)]; // rang1 qualifie (meilleure aptitude, priorite au 1er en cas d'egalite)
