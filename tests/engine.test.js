...
test('RaceAnalyzer: resume.coupleGagnant1030 associe la Base a chaque autre cheval et ne retient que les valeurs dans la fourchette 10-30€', () => {
  const context = {
    lieu: 'VINCENNES',
    discipline: disciplineFromRaw('ATTELE'),
    disciplineBrute: 'ATTELE',
  ...
  const result = RaceAnalyzer.analyser(horses, context, false);
  const cg = result.resume.coupleGagnant1030;

  // ALPHA (cote directe 2.8) est la Base (cheval le mieux classe, cf. tests
  // ci-dessus confirmant l'ordre ALPHA/BETA/GAMMA).
  assert.ok(cg);
  assert.equal(cg.baseNumero, 1);
  assert.equal(cg.baseNom, 'ALPHA');
  approx(cg.baseCote, 2.8, 0.001);

  // BETA : (2.8 x 9.0) / 2 = 12.6 -> dans la fourchette 10-30 -> retenu.
  // GAMMA : (2.8 x 25.0) / 2 = 35 -> hors fourchette -> exclu.
  assert.equal(cg.candidats.length, 1);
  assert.equal(cg.candidats[0].numero, 2);
  assert.equal(cg.candidats[0].nom, 'BETA');
  approx(cg.candidats[0].valeur, 12.6, 0.01);
});

test('RaceAnalyzer: resume.coupleGagnant1030 est null si la cote de la Base est inconnue', () => {
  const context = {
    lieu: 'VINCENNES',
    discipline: disciplineFromRaw('ATTELE'),
    disciplineBrute: 'ATTELE',
  ...
  const horses = [
    { entry: { numero: 1, nom: 'ALPHA', ferrage: '', sexeAge: '', reussiteJockey: 0, reussiteEntraineur: 0, cote8h: 0, coteDirecte: 0, cotePredictive: 0 }, historique: [] },
    { entry: { numero: 2, nom: 'BETA', ferrage: '', sexeAge: '', reussiteJockey: 0, reussiteEntraineur: 0, cote8h: 0, coteDirecte: 0, cotePredictive: 0 }, historique: [] }
  ];
  const result = RaceAnalyzer.analyser(horses, context, false);
  // Aucun des deux chevaux n'a de cote connue (8h/directe) : quel que soit
  // celui retenu comme Base (rang 1), sa cote est inconnue -> coupleGagnant1030 null.
  assert.equal(result.resume.coupleGagnant1030, null);
});

test('RaceAnalyzer: resume.coupleGagnant1030 renvoie un tableau de candidats vide si aucune valeur ne tombe dans la fourchette 10-30€', () => {
  const context = {
    lieu: 'VINCENNES',
    discipline: disciplineFromRaw('ATTELE'),
    disciplineBrute: 'ATTELE',
  ...
  const horses = [
    { entry: { numero: 1, nom: 'ALPHA', ferrage: '', sexeAge: '', reussiteJockey: 0, reussiteEntraineur: 0, cote8h: 3.0, coteDirecte: 2.8, cotePredictive: 3.0 }, historique: [] },
    // (2.8 x 3.0) / 2 = 4.2 -> trop bas (< 10).
    { entry: { numero: 2, nom: 'BETA', ferrage: '', sexeAge: '', reussiteJockey: 0, reussiteEntraineur: 0, cote8h: 3.5, coteDirecte: 3.0, cotePredictive: 3.2 }, historique: [] },
    // (2.8 x 25.0) / 2 = 35 -> trop haut (> 30).
    { entry: { numero: 3, nom: 'GAMMA', ferrage: '', sexeAge: '', reussiteJockey: 0, reussiteEntraineur: 0, cote8h: 20.0, coteDirecte: 25.0, cotePredictive: 22.0 }, historique: [] }
  ];
  const result = RaceAnalyzer.analyser(horses, context, false);
  const cg = result.resume.coupleGagnant1030;
  assert.ok(cg, 'la Base a une cote connue, donc coupleGagnant1030 ne doit pas etre null');
  assert.deepEqual(cg.candidats, []);
});
