...
import { formatDatePmu, buildParticipantsUrl, buildCourseUrl, buildRapportsUrl, buildProxiedUrl, mapParticipantsPmu, extraireArriveePmu, extraireRapportsCoupleGagnant, fetchCotesPmu, fetchResultatPmu, fetchRapportsPmu, _setExternalFunctionUrlPourTests } from '../js/engine/pmuApi.js';
...
// -------------------------------------------------------------------
test('buildRapportsUrl: construit l\'URL de l\'endpoint rapports-definitifs', () => {
...
// R1/C5) : deux blocs Couple Gagnant, "point de vente" (COUPLE_GAGNANT,
// dividendePourUnEuro=4980 -> 49,80€ pour 1€) et "internet" (E_COUPLE_GAGNANT,
// dividendePourUnEuro=4980 -> 49,80€ egalement dans cet exemple precis, mais
// les deux pools sont en realite independants et peuvent differer).
...
test('extraireRapportsCoupleGagnant: privilegie COUPLE_GAGNANT (point de vente) sur E_COUPLE_GAGNANT', () => {
  const rapports = extraireRapportsCoupleGagnant(RAPPORTS_JSON_REEL);
  assert.deepEqual(rapports, [{ numeros: [6, 1], dividende: 40.70 }]);
});

test('extraireRapportsCoupleGagnant: se replie sur E_COUPLE_GAGNANT si COUPLE_GAGNANT absent', () => {
  const json = [RAPPORTS_JSON_REEL[2]]; // uniquement le bloc E_COUPLE_GAGNANT
...
test('fetchRapportsPmu: utilise la fonction Netlify (type=rapports) et renvoie le JSON brut', async (t) => {
  const appelsRecus = [];
  const fetchOriginal = global.fetch;
  global.fetch = async (url) => {
    appelsRecus.push(url);
    return { ok: true, json: async () => RAPPORTS_JSON_REEL };
  };
  _setExternalFunctionUrlPourTests('');
  t.after(() => {
    global.fetch = fetchOriginal;
  ...
  assert.deepEqual(json, RAPPORTS_JSON_REEL);
  assert.equal(appelsRecus.length, 1);
  assert.ok(appelsRecus[0].startsWith('/.netlify/functions/pmu-cotes?'));
  assert.ok(appelsRecus[0].includes('type=rapports'));
});
