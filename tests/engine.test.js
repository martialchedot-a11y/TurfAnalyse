...
// -------------------------------------------------------------------
// Jeu Simple Gagnant v4 (aout 2026) : seuils fixes par RANG du classement
// Score Global (rang 1 : 3,8 / rang 2 : 5,2 / rang 4 : 9,4 / rang 5 : 12,5 -
// le rang 3 est exclu, non rentable seul par backtest reel). Le rang 1 est
// TOUJOURS prioritaire ("principal") des qu'il depasse son seuil ; un
// Dutching sur tous les rangs "value" est propose EN PLUS ("alternative")
// des que 2 rangs ou plus depassent leur seuil.
// -------------------------------------------------------------------
...
test('jeuSimpleGagnant: rang 1 value + un autre rang value -> principal = rang 1 seul, alternative = Dutching des deux', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 4.0),  // > 3,8 -> value
  ...
  assert.equal(jeu.rang1Value, true);
  assert.deepEqual(jeu.principal.chevaux.map((c) => c.entry.numero), [1]);
  assert.ok(jeu.alternative);
  assert.deepEqual(jeu.alternative.chevaux.map((c) => c.entry.numero), [1, 2]);
  assert.equal(jeu.alternative.n, 2);
  const sAttendu = 1 / 4.0 + 1 / 6.0;
  approx(jeu.alternative.s, sAttendu, 1e-9);
  approx(jeu.alternative.rendement, 1 / sAttendu, 1e-9);
  assert.equal(jeu.recommande, jeu.principal); // le rang 1 reste prioritaire
});
