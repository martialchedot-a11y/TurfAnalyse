...
import { jeuSimpleGagnant, misesJeuSimpleGagnant, SEUILS_VALUE_RANG_SIMPLE_GAGNANT, SEUIL_ECART_SCORE_RANG1, SEUIL_SCORE_CROISEMENT_CHEVAL_VALUE_SEUL, bilanJeuSimpleGagnant, rendementBilan, cumulerBilansJournaliers } from '../js/engine/jeuSimpleGagnant.js';
...
// -------------------------------------------------------------------
// Jeu Simple Gagnant v8 (aout 2026) : le rang 1 est prioritaire si sa cote
// depasse son seuil (3,8) ET que son ecart de Score Global avec le rang 2
// est >= SEUIL_ECART_SCORE_RANG1 (10). S'il ne qualifie pas (cote OU ecart
// insuffisant) mais qu'un SEUL autre rang parmi {2, 4, 5} depasse le sien,
// ce cheval est joue seul ("Cheval value seul (rang hors 1)"). Si 2 rangs
// ou plus parmi {2, 4, 5} qualifient sans que le rang 1 qualifie, non
// jouable (Dutching multi-chevaux exclu, trop fragile par backtest).
// `alternative` vaut toujours `null` (aucun Dutching multi-chevaux propose
// sur cette page).
// -------------------------------------------------------------------
function chevalPourJsg(numero, classement, cote, scoreGlobal = 0, rubriques) {
  return { entry: { numero, rubriques }, classement, cotePourAffichage: cote, scoreGlobal };
}

// Construit un tableau de rubriques (18 valeurs) ne renseignant QUE les 4
// rubriques croisees (R10=17, TG=16, OR=12, IdC=10 - voir
// jeuCoupleTrioCroisement.js), pour piloter le score Croisement (0-4) d'un
// cheval dans les tests v9 ci-dessous.
function rubriquesCroisement({ r10, tg, or_, idc } = {}) {
  const rubriques = new Array(18).fill(null);
  if (r10 !== undefined) rubriques[17] = r10;
  if (tg !== undefined) rubriques[16] = tg;
  if (or_ !== undefined) rubriques[12] = or_;
...
test('jeuSimpleGagnant: rang 1 depasse son seuil de cote ET son ecart de Score Global -> principal = rang 1 seul, pas d\'alternative', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 4.0, 70),  // > 3,8 -> value cote ; ecart vs rang2 (70-50=20) >= 10
    chevalPourJsg(2, 2, 5.0, 50),  // <= 5,2
    chevalPourJsg(4, 4, 9.0, 40),  // <= 9,4
    chevalPourJsg(5, 5, 12.0, 30)  // <= 12,5
  ];
...
test('jeuSimpleGagnant: rang 1 depasse son seuil de cote MAIS ecart de Score Global insuffisant (<10) -> ne qualifie pas', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 4.0, 55),  // > 3,8 -> value cote ; ecart vs rang2 (55-50=5) < 10
    chevalPourJsg(2, 2, 5.0, 50)   // <= 5,2, pas value
  ];
  assert.deepEqual(jeuSimpleGagnant(chevaux), { rentable: false });
});

test('jeuSimpleGagnant: ecart de Score Global tout juste au seuil (=10) -> qualifie (borne inclusive)', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 4.0, 60),  // ecart vs rang2 (60-50=10) == SEUIL_ECART_SCORE_RANG1
    chevalPourJsg(2, 2, 5.0, 50)
  ];
  const jeu = jeuSimpleGagnant(chevaux);
  assert.equal(SEUIL_ECART_SCORE_RANG1, 10);
  assert.equal(jeu.rentable, true);
  assert.equal(jeu.rang1Value, true);
  assert.deepEqual(jeu.principal.chevaux.map((c) => c.entry.numero), [1]);
});

test('jeuSimpleGagnant: rang 1 cote value mais pas de rang 2 dans la course -> ecart incalculable, ne qualifie pas', () => {
  const chevaux = [chevalPourJsg(1, 1, 4.0, 70)];
  assert.deepEqual(jeuSimpleGagnant(chevaux), { rentable: false });
});

test('jeuSimpleGagnant: rang 1 non value (cote) mais un seul autre rang value -> principal = ce cheval seul', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 3.0),  // <= 3,8
    chevalPourJsg(2, 2, 5.0),  // <= 5,2
    chevalPourJsg(4, 4, 10.0), // > 9,4 -> value, seul cheval value
    chevalPourJsg(5, 5, 12.0)  // <= 12,5
  ];
  const jeu = jeuSimpleGagnant(chevaux);
  assert.equal(jeu.rentable, true);
  ...
  assert.ok(jeu.principal);
  assert.deepEqual(jeu.principal.chevaux.map((c) => c.entry.numero), [4]);
  approx(jeu.principal.rendement, 10.0, 1e-9); // Dutching a 1 cheval = sa cote
  assert.equal(jeu.alternative, null);
  assert.equal(jeu.recommande, jeu.principal);
});

test('jeuSimpleGagnant: (v9) Cheval value seul avec score Croisement > 1 -> ne qualifie pas', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 3.0),  // <= 3,8, non value
    chevalPourJsg(2, 2, 5.0),  // <= 5,2, non value
    // rang 4 value (cote > 9,4), mais confirme par 2 rubriques croisees
    // (R10 et TG) -> score Croisement = 2 > SEUIL_SCORE_CROISEMENT_CHEVAL_VALUE_SEUL (1)
    chevalPourJsg(4, 4, 10.0, 0, rubriquesCroisement({ r10: 50, tg: 40 })),
    chevalPourJsg(5, 5, 12.0)  // <= 12,5
  ];
  assert.deepEqual(jeuSimpleGagnant(chevaux), { rentable: false });
});

test('jeuSimpleGagnant: (v9) Cheval value seul avec score Croisement = 1 -> qualifie (borne inclusive)', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 3.0),
    chevalPourJsg(2, 2, 5.0),
    // rang 4 value, confirme par une seule rubrique croisee (R10) -> score = 1
    chevalPourJsg(4, 4, 10.0, 0, rubriquesCroisement({ r10: 50 })),
    chevalPourJsg(5, 5, 12.0)
  ];
  const jeu = jeuSimpleGagnant(chevaux);
  assert.equal(SEUIL_SCORE_CROISEMENT_CHEVAL_VALUE_SEUL, 1);
  assert.equal(jeu.rentable, true);
  assert.equal(jeu.rang1Value, false);
  assert.deepEqual(jeu.principal.chevaux.map((c) => c.entry.numero), [4]);
});

test('jeuSimpleGagnant: (v9) Cheval value seul sans donnees de rubriques (score Croisement 0 par defaut) -> qualifie', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 3.0),
    chevalPourJsg(2, 2, 5.0),
    chevalPourJsg(4, 4, 10.0), // pas de rubriques -> score Croisement 0 <= 1
    chevalPourJsg(5, 5, 12.0)
  ];
  const jeu = jeuSimpleGagnant(chevaux);
  assert.equal(jeu.rentable, true);
...
test('jeuSimpleGagnant: (v9) le score Croisement ne s\'applique PAS au rang 1 (reste qualifie meme avec score = 4)', () => {
  const chevaux = [
    // rang1 value (cote + ecart) ET confirme par les 4 rubriques croisees
    // (score Croisement = 4) -> reste "1er du classement seul" (condition
    // reservee au mode "Cheval value seul", jamais appliquee au rang 1 - a
    // la demande explicite de l'utilisateur, le rang 1 etant deja performant
    // sous tous les angles testes).
    chevalPourJsg(1, 1, 4.0, 70, rubriquesCroisement({ r10: 50, tg: 40, or_: 30, idc: 20 })),
    chevalPourJsg(2, 2, 5.0, 50)
  ];
  const jeu = jeuSimpleGagnant(chevaux);
  assert.equal(jeu.rentable, true);
...
test('jeuSimpleGagnant: rang 1 value + un autre rang value -> principal = rang 1 seul, pas de Dutching (retire en v6)', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 4.0, 70),  // > 3,8 -> value cote, ecart 70-40=30 >= 10
    chevalPourJsg(2, 2, 6.0, 40),  // > 5,2 -> value, mais n'entre plus en jeu
    chevalPourJsg(4, 4, 9.0, 20),  // <= 9,4
    chevalPourJsg(5, 5, 12.0, 10)  // <= 12,5
  ];
  const jeu = jeuSimpleGagnant(chevaux);
  assert.equal(jeu.rentable, true);
  assert.equal(jeu.rang1Value, true);
...
test('jeuSimpleGagnant: le rang 3 est toujours exclu (jamais "value"), meme entoure de rangs qui, eux, qualifient', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 4.0, 70), // > 3,8 -> value ; ecart vs rang2 (70-20=50) >= 10
    chevalPourJsg(2, 2, 3.0, 20), // rang 2 non value (cote <= 5,2), sert juste a fournir un score pour l'ecart
...
test('jeuSimpleGagnant: rang 1 sans cote reelle connue (null) -> ignore, principal = l\'unique autre rang value', () => {
  const chevaux = [
    { entry: { numero: 1 }, classement: 1, cotePourAffichage: null }, // rang 1 sans cote -> non value
    chevalPourJsg(2, 2, 6.0) // > 5,2 -> value, seul cheval value
  ];
  const jeu = jeuSimpleGagnant(chevaux);
  assert.equal(jeu.rentable, true);
  assert.equal(jeu.rang1Value, false);
...
test('jeuSimpleGagnant: rang 1 non value et 2 autres rangs value -> non jouable (ambigu, Dutching exclu)', () => {
  const chevaux = [
    chevalPourJsg(1, 1, 3.0),  // <= 3,8
    chevalPourJsg(2, 2, 6.0),  // > 5,2 -> value
    chevalPourJsg(4, 4, 10.0), // > 9,4 -> value
    chevalPourJsg(5, 5, 12.0)  // <= 12,5
  ];
  assert.deepEqual(jeuSimpleGagnant(chevaux), { rentable: false });
});
...
test('bilanJeuSimpleGagnant: course ratee (vainqueur hors pool) -> mise perdue, sans besoin du rapport', () => {
  const chevaux = [chevalPourJsg(1, 1, 4.0, 70), chevalPourJsg(2, 2, 6.0, 40)]; // ecart Score Global 30 >= 10 -> rang1 qualifie
...
test('cumulerBilansJournaliers: cumule separement rang1Seul et chevalValueSeul (aout 2026)', () => {
  const bilans = [
    {
      date: '2026-08-16', mise: 40, gain: 60, net: 20,
      rang1Seul: { nbCourses: 2, reussies: 1, mise: 20, gain: 40, net: 20 },
      chevalValueSeul: { nbCourses: 3, reussies: 1, mise: 20, gain: 20, net: 0 }
    },
    {
      date: '2026-08-17', mise: 50, gain: 45, net: -5,
      rang1Seul: { nbCourses: 3, reussies: 1, mise: 30, gain: 30, net: 0 },
      chevalValueSeul: { nbCourses: 2, reussies: 0, mise: 20, gain: 15, net: -5 }
    }
  ];
  const cumul = cumulerBilansJournaliers(bilans);
  assert.deepEqual(cumul[0].cumulRang1Seul, { mise: 20, gain: 40, net: 20 });
  assert.deepEqual(cumul[0].cumulChevalValueSeul, { mise: 20, gain: 20, net: 0 });
  assert.deepEqual(cumul[1].cumulRang1Seul, { mise: 50, gain: 70, net: 20 });
  assert.deepEqual(cumul[1].cumulChevalValueSeul, { mise: 40, gain: 35, net: -5 });
});

test('cumulerBilansJournaliers: entrees sans rang1Seul/chevalValueSeul (anterieures a la mise a jour) traitees comme 0 dans les cumuls par mode, sans casser le cumul global', () => {
  const bilans = [
    { date: '2026-08-15', mise: 10, gain: 15, net: 5 }, // ancienne entree, pas de detail par mode
    {
      ...
      chevalValueSeul: { nbCourses: 1, reussies: 0, mise: 10, gain: 0, net: -10 }
    }
  ];
  const cumul = cumulerBilansJournaliers(bilans);
  assert.equal(cumul[0].cumulNet, 5);
  assert.deepEqual(cumul[0].cumulRang1Seul, { mise: 0, gain: 0, net: 0 });
  assert.deepEqual(cumul[0].cumulChevalValueSeul, { mise: 0, gain: 0, net: 0 });
  assert.equal(cumul[1].cumulNet, 25);
  assert.deepEqual(cumul[1].cumulRang1Seul, { mise: 10, gain: 30, net: 20 });
  assert.deepEqual(cumul[1].cumulChevalValueSeul, { mise: 10, gain: 0, net: -10 });
});
